...
- **Reuse established utilities**: Before adding common logic, search `src/shared/utils` and the relevant process, domain, or package utilities, then read their public contracts and existing callers. Use a suitable API through its supported import path instead of copying its implementation. Confirm behavior, runtime compatibility, and resource ownership; keep business policy in its owning module and respect dependency direction. Check library/framework docs for built-in options when no repository utility fits; write custom code only when neither provides a suitable API.
