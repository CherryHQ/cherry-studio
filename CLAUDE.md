...
**Downgrades must keep working.** The previous app version runs against the migrated DB, so a dropped column/table, a `NOT NULL` tightening or a new `NOT NULL` column without a DB default breaks it. Patch releases must stay downgrade-compatible; a break lands only in a minor and only with an entry in `migrations/downgrade-contract.json` (`pnpm db:downgrade:check`, enforced in CI). See [Database Construction § Downgrade compatibility](docs/references/data/database-construction.md#downgrade-compatibility).
