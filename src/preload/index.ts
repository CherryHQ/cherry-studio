...
const api = {
  ...
  mcp: {
    listServers: () => ipcRenderer.invoke(IpcChannel.Mcp_ListServers),
    addServer: (server: MCPServer) => ipcRenderer.invoke(IpcChannel.Mcp_AddServer, server),
    updateServer: (server: MCPServer) => ipcRenderer.invoke(IpcChannel.Mcp_UpdateServer, server),
    deleteServer: (serverName: string) => ipcRenderer.invoke(IpcChannel.Mcp_DeleteServer, serverName),
    setServerActive: (name: string, isActive: boolean) =>
      ipcRenderer.invoke(IpcChannel.Mcp_SetServerActive, { name, isActive }),
    listTools: (serverName?: string) => ipcRenderer.invoke(IpcChannel.Mcp_ListTools, serverName),
    callTool: (params: { client: string; name: string; args: any }) =>
      ipcRenderer.invoke(IpcChannel.Mcp_CallTool, params),
    cleanup: () => ipcRenderer.invoke(IpcChannel.Mcp_Cleanup)
  },
  shell: {
