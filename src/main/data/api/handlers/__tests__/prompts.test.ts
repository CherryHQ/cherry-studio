...
describe('promptHandlers', () => {
  ...
  describe('/prompts', () => {
    it('should delegate GET to promptService.list', async () => {
      ...
      await expect(promptHandlers['/prompts'].GET({})).resolves.toMatchObject([{ id: PROMPT_ID }])
    ...
    it('should reject POST with empty fields before calling the service', async () => {
      await expect(promptHandlers['/prompts'].POST({ body: { title: '', content: 'c' } })).rejects.toHaveProperty(
        'name',
        'ZodError'
      )
      await expect(promptHandlers['/prompts'].POST({ body: { title: 't', content: '' } })).rejects.toHaveProperty(
        'name',
        'ZodError'
      )
  ...
  describe('/prompts/:id', () => {
    it('should delegate GET with the parsed id', async () => {
      ...
      await expect(promptHandlers['/prompts/:id'].GET({ params: { id: PROMPT_ID } })).resolves.toMatchObject({
        id: PROMPT_ID
      })
    ...
    it('should delegate DELETE with the parsed id', async () => {
      ...
      await expect(promptHandlers['/prompts/:id'].DELETE({ params: { id: PROMPT_ID } })).resolves.toBeUndefined()
