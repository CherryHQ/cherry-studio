...
describe('MessageService', () => {
  ...
  describe('delete — virtual root guard', () => {
    ...
    it('keeps the surviving multi-model branch active when deleting the selected reply', async () => {
      await seedMultiModelTree()
      await dbh.db.update(topicTable).set({ activeNodeId: 'm-a2' }).where(eq(topicTable.id, 'topic-1'))
      ...
      expect(reloadedBranch.activeNodeId).toBe('m-follow')
      expect(reloadedBranch.items.map((item) => item.message.id)).toEqual(['m-root', 'm-a1', 'm-follow'])
    })
    ...
    it('does not infer snapshot provenance from an authoritative model ID', async () => {
    ...
    it('descends from the parent when no different-model successor remains', async () => {
