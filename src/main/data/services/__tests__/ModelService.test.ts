...
describe('ModelService.update', () => {
  ...
  it('keeps a token-only pricing override over a per-image preset baseline', async () => {
    await seedExistingModel()
    lookupModelMock.mockReturnValue({
      presetModel: {
        id: 'gpt-4o',
    ...
    const [row] = await dbh.db.select().from(userModelTable).where(eq(userModelTable.id, 'openai::gpt-4o'))
    const runtimePricing = modelService.getByKey('openai', 'gpt-4o').pricing
    expect(row.pricing).toEqual(pricing)
    expect(runtimePricing).toEqual(pricing)
    expect(runtimePricing).not.toHaveProperty('perImage')
  })
