...
- **Single-tuple keyset only**: covers `(key, id)` pagination. Multiple display bands must use independent queries and cursor chains (as Topic and AgentSession do), never one sentinel cursor.
