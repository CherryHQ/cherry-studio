...
- **Transactions** — wrap multi-statement / read-then-write mutations in `application.get('DbService').withWriteTx(...)` for atomicity and post-commit DataApi effect publication. A direct `db.transaction(...)` has equivalent SQLite atomicity but no effect collector. A single autocommit write needs no transaction.
