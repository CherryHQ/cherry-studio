...
async function copyMaterializedEntry(
  ...
  if (existingDestination) {
    ...
    if (
      (await materializedSourceMetadataFingerprint(sourcePath, sourceWorkspaceRoot)) !==
      sourceSnapshot.metadataFingerprint
    ) {
...
async function copyIdentityFromWorkspace(
  ...
  for (const name of ['SOUL.md', 'USER.md', 'memory']) {
    ...
    const result = await copyMaterializedEntry(sourcePath, destinationPath, effectiveWorkspacePath, 'identity')
    if (result) {
...
async function copyWindowsSkillWorkspaceContent(
...
): Promise<WorkspaceCopyStats | undefined> {
  if (!isWin) return undefined

  const stats: WorkspaceCopyStats = {
    copiedEntries: 0,
  ...
  for (const entry of skillEntries) {
    ...
    const result = await copyMaterializedEntry(sourcePath, destinationPath, canonicalSkillsDataRoot, 'skill')
    if (!result) {
    ...
    if (result.copied) stats.materializedEntries++
    else stats.reusedEntries++
    stats.fileCount += result.fileCount
    stats.byteCount += result.byteCount
  }
  return stats
}
...
async function copyOrdinaryWorkspaceContent(
  ...
  destinationWorkspacePath: string,
  agentDataPath: string
): Promise<WorkspaceCopyStats> {
  const sourceStat = await lstatIfExists(sourceWorkspacePath)
  if (!sourceStat?.isDirectory() || sourceStat.isSymbolicLink()) {
    return { copiedEntries: 0, materializedEntries: 0, reusedEntries: 0, fileCount: 0, byteCount: 0 }
  }
  ...
  for (const entry of entries) {
    ...
    if (entry.toLowerCase() === 'skills') {
      const skillStats = await copyWindowsSkillWorkspaceContent(
        agentsDataRoot,
        skillsDataRoot,
      ...
      if (skillStats) {
        copiedEntries += skillStats.copiedEntries
        materializedEntries += skillStats.materializedEntries
        reusedEntries += skillStats.reusedEntries
        fileCount += skillStats.fileCount
        byteCount += skillStats.byteCount
        continue
      }
    }
  ...
  return { copiedEntries, materializedEntries, reusedEntries, fileCount, byteCount }
}
...
export async function stageLegacyAgentFiles(input: {
  ...
  let materializedWorkspaceEntries = 0
  ...
  try {
    for (const { sourceAgentId, finalAgentId } of input.agents) {
      ...
      for (const session of systemSessions) {
        ...
        materializedWorkspaceEntries += workspaceStats.materializedEntries
    ...
    copiedWorkspaceEntries = publicationStats.publishedEntries + materializedWorkspaceEntries
