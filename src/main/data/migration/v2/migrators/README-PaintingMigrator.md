...
- SQLite `creation` table (one row per painting, stamped `kind: 'image'`)
- SQLite `creation_file_ref` table (one row per referenced file)
...
- `orderKey` is assigned across the **whole** migrated set in source order via
  ...
  `sortOrder` column)
- Output / input file ids do **not** live on the creation row. They are emitted
  as `creation_file_ref` rows: `sourceId=creation.id`, `role='output' | 'input'`
- `creation_file_ref` emission is **pre-filtered against `file_entry`**: ids the
...
- A cross-namespace duplicate painting id is rewritten to a fresh `uuidv4()`
  (not a composite string) so the emitted `creation_file_ref.sourceId` stays a valid
  `z.uuidv4()`
...
- The JSON files column (replaced by `creation_file_ref` rows)
