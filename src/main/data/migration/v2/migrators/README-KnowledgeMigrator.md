...
- The resulting paths are **path-shaped but unbacked**: no bytes are copied (v1 never stored the folder inside Cherry), so a migrated child still cannot be reindexed from disk. Reindexing the *container* while the original folder still exists fills `raw/<prefix>` for real, converging the migrated shape onto the native one. The prefix is normally re-picked identically — `chooseDirectoryPathPrefix` excludes the container itself and derives from the same basename — so paths and display names do not churn. The exception is a prefix that took a `_N` at migration time: if whatever forced that suffix has since been deleted, the reindex reclaims the shorter name and the paths do change.
