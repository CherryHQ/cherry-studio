...
| **Missing assistantId** | Topic not in any `assistant.topics[]`, or empty/null `assistantId` | Fall back to `DEFAULT_ASSISTANT_ID` (`'default'`). `AssistantMigrator`'s default-assistant backstop guarantees that row exists by the time `verifyForeignKeys()` runs (the post-init `DefaultAssistantSeeder` fires too late). Renderer hooks like `useAssistant(topic.assistantId)` need a routable id; `null` would drive `PATCH /assistants/` 400 loops. |
