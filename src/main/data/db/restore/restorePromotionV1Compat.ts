...
const logger = loggerService.withContext('RestorePromotionV1Compat')
...
export async function runRestorePromotion(): Promise<void> {
  ...
  if (read.kind === 'corrupt') {
    throw new Error(`v1 restore journal is unreadable — refusing to discard recovery evidence: ${read.error}`)
  }
...
/**
 * Release the artifacts of a converged v1 restore after an upgrade. Journal v1
 * never offered the v2 explicit rollback contract, so keeping its DB aside
 * would only leak plaintext state while the v2 service cannot acknowledge it.
 */
export function clearConvergedV1Restore(): void {
  const read = readRestoreJournal()
  if (read.kind !== 'ok') return
  if (read.journal.state === 'staged' || read.journal.state === 'promoting') return

  const userData = application.getPath('app.userdata')
  fs.rmSync(path.resolve(userData, read.journal.db.aside), { force: true })
  fs.rmSync(path.join(application.getPath('feature.backup.restore.staging'), read.journal.restoreId), {
    recursive: true,
    force: true
  })
  clearRestoreJournal()
...
export function isRestoreRecoveryPending(): boolean {
  ...
  return read.kind === 'ok' && (read.journal.state === 'staged' || read.journal.state === 'promoting')
}
 ...
 * silently rolled back by the filesystem. Windows cannot prove directory-entry
 * durability through Node/libuv; process crashes are recoverable, but sudden
 * power loss remains outside the compatibility bridge's guarantee.
 */
