...
| File | Exports | Role |
|---|---|---|
| `restoreJournal.ts` | `RestoreJournal(Schema)`, `PROMOTION_STEP_ORDER`, `readRestoreJournal` / `writeRestoreJournal` / `removeRestoreJournal`, `hasPendingRestore` | Crash-safe journal contract and durable pre-relaunch disclosure summary (sidecar `restore-journal.json`, `feature.backup.restore.file`; MUST stay in the DB's directory — journal dir-fsyncs are what make a commit-step marker imply the DB rename is durable) |
| `restorePromotion.ts` | `runRestorePromotion`, `cleanupTerminalRestoreArtifacts`, `markRestoreFailedAfterCrash`, `isLiveDbStranded`, `assertPathInsideUserData` / `assertRestoreJournalPathsInsideUserData` | Preboot promotion state machine — consumes a `staged` journal, swaps `work.sqlite` in by atomic rename (write-ahead markers + aside-undo + crash-recovery probes), and converges every exit to a clean terminal state. The trust boundary for journal-supplied paths (containment assertions live here). |
| `notesTreeHash.ts` | `computeNotesTreeHash` / `computeNotesTreeHashSync` | Ordered sha256-merkle digest over a Notes tree's `.md` leaves (versioned `sha256-merkle-v1:` prefix). Backs the notes-tree-swap promotion step's staging↔descriptor hash check so a tampered/corrupt merged tree cannot replace the live Notes tree (dir-swap safety invariant). Non-`.md` files are pass-through local data and excluded from the digest. |
