...
import type * as RestoreJournalModule from '@data/db/restore/restoreJournalV1Compat'
import type { RestoreJournal } from '@data/db/restore/restoreJournalV1Compat'
import { readRestoreJournal, writeRestoreJournal } from '@data/db/restore/restoreJournalV1Compat'
import {
  isLiveDbStranded,
...
describe('runRestorePromotion', () => {
  ...
  it('fails closed on a corrupt v1 journal and preserves recovery evidence', async () => {
    makeDb(livePath(), 'old')
    mkdirSync(stagingDir(), { recursive: true })
    writeFileSync(join(stagingDir(), 'leftover.bin'), 'x')
    writeFileSync(journalPath(), '{ definitely not a journal')

    await expect(runRestorePromotion()).rejects.toThrow(/refusing to discard recovery evidence/)
