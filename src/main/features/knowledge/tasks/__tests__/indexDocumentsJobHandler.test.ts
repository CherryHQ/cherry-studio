...
describe('index-documents job handler', () => {
  ...
  it('restore mode indexes a completed transported snapshot without transient status rewrites', async () => {
    ...
    await handler.execute(
      createCtx({ baseId: 'kb-1', itemId: NOTE_ITEM_ID, parentJobId: null, restoreId: 'restore-1' })
    )
  ...
  it('restore mode refuses to fetch a URL whose transported snapshot is absent', async () => {
    ...
    await expect(
      handler.execute(createCtx({ baseId: 'kb-1', itemId: 'url-restore', parentJobId: null, restoreId: 'restore-1' }))
