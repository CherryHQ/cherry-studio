...
describe('materializePortableDatabase', () => {
  ...
  describe('managed-path rebasing', () => {
    it('rebases managed roots and resets external workspaces to managed inert paths', async () => {
      dbh.db
        .insert(noteTable)
        .values([
      ...
      expect(result.summary.degradations).toEqual([
        { table: 'agent_workspace', rowId: 'w-user', reason: 'external-workspace-reset', detail: undefined }
      ])
      ...
      expect(state.workspaces.find((workspace) => workspace.id === 'w-system')).toEqual({
        id: 'w-system',
        path: `${TARGET_WORKSPACES}/session-1`
      })
      expect(state.workspaces.find((workspace) => workspace.id === 'w-user')?.path).toMatch(
        new RegExp(`^${TARGET_WORKSPACES}/\\.restored-unconfirmed/[0-9a-f]{64}$`)
      )
    })

    it('replaces a malformed workspace path before any runtime can follow it', async () => {
      insertWorkspace('w-unc', '\\\\server\\share\\workspace', 'user')

      const dbPath = snapshot()
      const result = await materializePortableDatabase({ dbPath, mode: restoreMode() })

      expect(result.summary.degradations).toEqual([
        { table: 'agent_workspace', rowId: 'w-unc', reason: 'path-unportable', detail: 'not-absolute' },
        { table: 'agent_workspace', rowId: 'w-unc', reason: 'external-workspace-reset', detail: undefined }
      ])
      const [workspace] = inspect(dbPath, (db) =>
        db.select({ path: agentWorkspaceTable.path }).from(agentWorkspaceTable).all()
      )
      expect(workspace.path).toMatch(new RegExp(`^${TARGET_WORKSPACES}/\\.restored-unconfirmed/[0-9a-f]{64}$`))
      expect(workspace.path).not.toContain('server')
    })
    ...
    it('degrades an unportable managed note suffix instead of rewriting it', async () => {
    ...
    it('avoids an archive-controlled placeholder collision while clearing external workspaces', async () => {
      const firstId = 'w-external'
      const occupiedLeaf = createHash('sha256').update(firstId).digest('hex')
      insertWorkspace(firstId, '/producer/code/project', 'user')
      insertWorkspace('w-occupier', `${TARGET_WORKSPACES}/.restored-unconfirmed/${occupiedLeaf}`, 'user')

      const dbPath = snapshot()
      await materializePortableDatabase({ dbPath, mode: restoreMode() })

      const workspaces = inspect(dbPath, (db) =>
        db.select({ id: agentWorkspaceTable.id, path: agentWorkspaceTable.path }).from(agentWorkspaceTable).all()
      )
      expect(workspaces.find((workspace) => workspace.id === firstId)?.path).toBe(
        `${TARGET_WORKSPACES}/.restored-unconfirmed/${occupiedLeaf}-1`
      )
      expect(workspaces.find((workspace) => workspace.id === 'w-occupier')?.path).not.toContain(occupiedLeaf)
    })

    it('replaces an external workspace before its target alias can block a managed rebase', async () => {
      // Crafted archive: a user workspace names the future target path literally.
      // It must not retain that path or block the system workspace's safe rebase.
      ...
      expect(result.summary.pathsRebased).toBe(1)
      expect(result.summary.degradations).toEqual([
        { table: 'agent_workspace', rowId: 'w-b-user', reason: 'external-workspace-reset', detail: undefined }
      ])
      const state = inspect(dbPath, (db) => ({
        workspaces: db
          .select({ id: agentWorkspaceTable.id, path: agentWorkspaceTable.path })
      ...
      // Both rows survive with distinct managed paths; the session is untouched.
      expect(state.workspaces.find((workspace) => workspace.id === 'w-a-system')).toEqual({
        id: 'w-a-system',
        path: `${TARGET_WORKSPACES}/session-1`
      })
      expect(state.workspaces.find((workspace) => workspace.id === 'w-b-user')?.path).toMatch(
        new RegExp(`^${TARGET_WORKSPACES}/\\.restored-unconfirmed/[0-9a-f]{64}$`)
      )
