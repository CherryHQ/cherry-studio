...
import { classifyManagedPath, type ManagedRootRebaseTable } from './managedPathRebase'
 ...
 * file.
 *
 * Lite carries one complete database: no domain is stripped and no target row is
 * consulted. Materialization only rewrites the archive's own file, which keeps
 * it archive processing rather than a row merge.
 *
...
export type MaterializationDegradationReason =
  ...
  | 'path-collision'
  /** An external workspace was replaced with a managed inert placeholder. */
  | 'external-workspace-reset'
 ...
 * deterministic archive output for the same source snapshot.
 */
...
interface RebaseCandidate<TWrite> {
  ...
  /** Do not reserve the old key: this row is guaranteed to move to a controlled inert value. */
  readonly releaseCurrent?: boolean
...
function resolveRebaseWrites<TWrite>(candidates: readonly RebaseCandidate<TWrite>[]): ResolvedRebases<TWrite> {
  ...
  for (const candidate of candidates) {
    if (!candidate.releaseCurrent) owner.set(candidate.currentKey, candidate.id)
  }
...
function noteKey(rootPath: string, path: string): string {
  return `${portableCollisionKey(rootPath)}\u0000${portableCollisionKey(path)}`
}

/**
 * Rebase `agent_workspace.path` (unique on `path`). A SYSTEM workspace is built
 * under the managed workspaces root, so it rebases. A user-selected external
 * workspace cannot survive: runtime readers can stat it, including a UNC path.
 * Preserve sessions by replacing that reference with a non-existent path under
 * the trusted managed root; the user must explicitly select a workspace again.
 */
function rebaseAgentWorkspaces(db: DbOrTx, table: ManagedRootRebaseTable, summary: SummaryBuilder): void {
  ...
  const managedRoot = table.pairings.find((pairing) => pairing.key === 'feature.agents.system_workspaces')?.targetPath
  if (!managedRoot) throw new Error('materializePortableDatabase: missing managed agent workspace root')
  const separator = table.targetPlatform === 'win32' ? '\\' : '/'

  const classified = rows.map((row) => ({ row, verdict: classifyManagedPath(table, row.path) }))
  // An archive may predeclare a would-be placeholder path. Reserve every live
  // and managed-rebased key before choosing placeholders, so no unsafe workspace
  // can survive merely because its deterministic first choice was occupied.
  const occupiedKeys = new Set<string>(rows.map((row) => portableCollisionKey(row.path)))
  for (const { verdict } of classified) {
    if (verdict.kind === 'managed') occupiedKeys.add(portableCollisionKey(verdict.rebasedPath))
  }
  const nextPlaceholder = (id: string): string => {
    const base = createHash('sha256').update(id).digest('hex')
    for (let suffix = 0; ; suffix++) {
      const leaf = suffix === 0 ? base : `${base}-${suffix}`
      const candidate = `${managedRoot}${separator}.restored-unconfirmed${separator}${leaf}`
  ...
  for (const { row, verdict } of classified) {
    const currentKey = portableCollisionKey(row.path)
    if (verdict.kind !== 'managed') {
      ...
      const replacement = nextPlaceholder(row.id)
      summary.degrade('agent_workspace', row.id, 'external-workspace-reset')
      candidates.push({
        id: row.id,
        currentKey,
        nextKey: portableCollisionKey(replacement),
        releaseCurrent: true,
    ...
    candidates.push({
      id: row.id,
      currentKey,
      nextKey: portableCollisionKey(verdict.rebasedPath),
      write: { path: verdict.rebasedPath }
    })
  }

  const { applied, collided } = resolveRebaseWrites(candidates)
  // Release external aliases before a managed row takes their target key.
  const orderedApplied = [...applied].sort(
    (a, b) => Number(managedCandidateIds.has(a.id)) - Number(managedCandidateIds.has(b.id))
  )
  for (const { id, write } of orderedApplied) {
  ...
  summary.pathsRebased += applied.filter(({ id }) => managedCandidateIds.has(id)).length
  for (const id of collided) summary.degrade('agent_workspace', id, 'path-collision')
}
 ...
 * they survive `VACUUM INTO` transport, so no index maintenance belongs here.
 */
