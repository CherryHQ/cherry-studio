...
//
// Wiring status: the engine is landed + unit-tested with Stage-4 engine-internal enhancements
// (global junction phase, FTS rebuild backstop, FTS + app_state consistency checks), but NOT
// yet wired into the restore spine — BackupService.startRestore still throws
// RestoreMergeNotImplementedError and ImportOrchestrator retains the 2-arg mergeBackupIntoWork
// stub. Spine wiring (signature widening + domains/strategy/skippedFileEntryIds plumbing) lands
// in task `merge-engine-spine-wiring`. Until then the barrel's only consumer is this module's
// own test; the engine is fail-closed in production BY CONSTRUCTION (no caller reaches it).
//
