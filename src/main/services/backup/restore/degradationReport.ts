...
function resourceDegradationCode(reason: string): BackupDegradationCode {
  ...
  if (reason === 'changed-after-snapshot' || reason === 'changed-during-capture') return 'resource-changed'
  if (reason === 'knowledge-index-rebuild-required') return 'knowledge-index-rebuild'
