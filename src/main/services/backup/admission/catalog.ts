...
/**
 * The legal Lite archive shape after pre-extraction validation: exactly one
 * manifest and exactly one database. Any third entry, including a directory,
...
/** Enforce the closed two-file Lite layout before any untrusted byte is extracted. */
function classifyShape(
  ...
  for (const entry of entries) {
    if (entry.isDirectory) {
      throw new ArchiveAdmissionError('layout', `Lite archive contains directory: ${renderUntrustedName(entry.path)}`)
    }
    ...
    throw new ArchiveAdmissionError('layout', `Lite archive contains extra entry: ${renderUntrustedName(entry.path)}`)
  }
  ...
  return { manifest, db, declaredTotalBytes }
}
