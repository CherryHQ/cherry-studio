...
/** Copy the production chain and append one schema-neutral test migration. */
export function buildExtendedMigrations(sourceFolder: string, destFolder: string): void {
  const journal = readJournal(sourceFolder)
  const index = journal.entries.length
  const previous = journal.entries.at(-1)
  const entry: JournalEntry = {
    idx: index,
    tag: `${String(index).padStart(4, '0')}_backup_test_noop`,
    when: (previous?.when ?? 0) + 1,
    breakpoints: true,
    version: previous?.version ?? '7'
  }
  mkdirSync(path.join(destFolder, 'meta'), { recursive: true })
  writeFileSync(
    path.join(destFolder, 'meta', '_journal.json'),
    JSON.stringify({ dialect: journal.dialect, entries: [...journal.entries, entry] })
  )
  for (const sourceEntry of journal.entries) {
    cpSync(path.join(sourceFolder, `${sourceEntry.tag}.sql`), path.join(destFolder, `${sourceEntry.tag}.sql`))
  }
  writeFileSync(path.join(destFolder, `${entry.tag}.sql`), '-- schema-neutral migrate-forward fixture\nSELECT 1;\n')
}
