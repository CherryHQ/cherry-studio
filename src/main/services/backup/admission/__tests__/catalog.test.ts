...
describe('validateArchiveShape — valid layouts', () => {
  ...
  it('rejects every third entry, including a resource entry', async () => {
    expect(await reasonOf([MANIFEST, DB, { name: 'resources/blob.bin', data: Buffer.from('B') }])).toBe('layout')
  })
})
...
describe('validateArchiveShape — ceilings', () => {
  ...
  it('accepts a compression ratio exactly at the ceiling (bigint boundary)', async () => {
    ...
    expect(
      await reasonOf([MANIFEST, { name: 'backup.sqlite', centralUncompressedSize: 1000, centralCompressedSize: 10 }])
    ).toBe('OK')
  })
