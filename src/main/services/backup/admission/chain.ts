...
export async function admitStagedDatabase(
  ...
  try {
    ...
    if (decision.kind === 'ahead' || decision.kind === 'fork') {
      // The renderer gets one bounded incompatibility outcome; raw lineage
      // diagnostics remain internal until a product demand proves that surface.
      throw new ArchiveAdmissionError('chain-incompatible', 'archive migration lineage is unsupported by this build')
    }
