...
import { verifyDbPayload, verifyStagedTree } from './verify'
 ...
 * 3. re-proves the two staged files against filesystem facts (no symlink/special,
 *    no realpath escape) and recomputes the DB size + hash;
 ...
 *
 * It creates NO restore journal and touches NO live DB — this module has no
 * such dependency, so every failure necessarily precedes a mutating stage. On success the caller OWNS the returned staging tree (and can
...
export async function admitArchive(inputs: AdmitArchiveInputs): Promise<AdmittedArchive> {
  ...
  try {
    ...
    // Re-prove the two-file layout and exact DB bytes over the owned staging tree.
    await verifyStagedTree(stagingDir, signal)
