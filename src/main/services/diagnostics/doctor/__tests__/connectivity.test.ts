...
describe('model connectivity against an HTTP provider', () => {
  ...
  it('resolves Agent ownership and isolates concurrent scopes, cancellation, and existing reports', async () => {
    ...
    const before = structuredClone(cache.getShared('doctor.state.global'))
    holdConversation = true
    ...
    expect(cache.getShared('doctor.state.global')).toEqual(before)
    expect(cache.getShared('doctor.state.agent:agent')).toBeUndefined()
