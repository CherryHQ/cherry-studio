...
async function hashFile(filename: string): Promise<string | null> {
  try { return digest(await fs.readFile(filename)) } catch (error) {
...
async function files(root: string, prefix = ''): Promise<string[]> {
  ...
  const nested = await Promise.all(entries.filter((entry) => entry.name !== marker).map(async (entry) => {
...
export async function copyOwnedSkill(source: string, destination: string, render?: (bytes: Buffer, relative: string) => Buffer): Promise<boolean> {
  ...
  try {
    ...
    try { await fs.access(destination); return false } catch (missing) {
  ...
  for (const relative of sourceFiles) {
    ...
    const bytes = render ? render(sourceBytes, relative) : sourceBytes
...
export async function isApplicationOwnedSkill(directory: string): Promise<boolean> {
  try { return JSON.parse(await fs.readFile(path.join(directory,marker),'utf8')).owner === 'the-boss' } catch { return false }
}
