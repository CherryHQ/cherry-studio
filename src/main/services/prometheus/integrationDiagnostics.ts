...
export async function runIntegrationDiagnostics(workspace: WorkspaceIntegration, config: IntegrationConfig, signal: AbortSignal): Promise<IntegrationDiagnostic[]> {
  ...
  const check = async (id: string, run: () => Promise<void>) => {
    ...
    try { await run(); results.push({ id, state: 'operational' }) } catch (error) {
      signal.throwIfAborted()
      const message = error instanceof Error ? error.message : String(error)
      results.push({ id, state: 'failed', detail: Object.values(secrets).filter(Boolean).reduce((value,secret) => value.split(secret).join('[redacted]'), message) })
    }
  }
    ...
    await check(`mcp:${server.reference}`, async () => {
      await runtime.withClient(server.id, async (client) => {
        ...
        if (server.reference?.startsWith('filesystem:')) {
          ...
          try {
            if (config.filesystem.allowWrite) text(await client.callTool({ name: 'write_file', arguments: { path: filename, content: token } }, undefined, { signal }))
            else await fs.writeFile(filename, token)
            const response = text(await client.callTool({ name: 'read_text_file', arguments: { path: filename } }, undefined, { signal }))
          ...
          text(await client.callTool({ name: 'query_graph', arguments: { query: 'find symbols named main' } }, undefined, { signal }))
        } else if (server.reference === 'surreal-memory') {
          const token = `The Boss diagnostic ${randomUUID()}`
          const created = JSON.parse(text(await client.callTool({ name: 'add_memory', arguments: { content: token, agent_id: 'the-boss-diagnostics', user_id: 'anonymous' } }, undefined, { signal }))) as { id: string }
          if (!created.id) throw new Error('prometheus.error.roundTrip')
          try {
            const response = text(await client.callTool({ name: 'get_memory', arguments: { id: created.id } }, undefined, { signal }))
            if (!response.includes(token)) throw new Error('prometheus.error.roundTrip')
          } finally { text(await client.callTool({ name: 'delete_memory', arguments: { id: created.id } })) }
        }
      })
    })
  }
  if (secrets.compassPassword) await check('surrealdb', async () => {
    const response = await fetch(new URL('/health', config.compass.endpoint), { signal: AbortSignal.any([signal, AbortSignal.timeout(5000)]) })
    ...
    await surrealSql(config.compass.endpoint, 'RETURN 1;', { ...config.compass, password: secrets.compassPassword!, database: workspaceDatabase(workspace.id) }, signal)
  ...
  for (const role of ['judge', 'critic'] as const) {
    if (!config.services[role].name) { results.push({ id: `liter:${role}`, state: 'disabled' }); continue }
    await check(`liter:${role}`, async () => {
      const response = await fetch(`${config.services.literEndpoint.replace(/\/$/,'')}/v1/chat/completions`, {
        ...
        body: JSON.stringify({ model: `kbd-${role}`, messages: [{ role: 'user', content: 'Reply with OK.' }], max_tokens: 16 })
      })
