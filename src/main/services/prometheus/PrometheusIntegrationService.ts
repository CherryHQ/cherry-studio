...
import { integrationConfigSchema, type IntegrationAction, type IntegrationConfig, type IntegrationOperation, type IntegrationSecret, type IntegrationSnapshot, type WorkspaceIntegration } from '@shared/types/prometheusIntegration'
...
import { describeWorkspace, indexWorkspace, installCompassProjectSkills, loadWorkspaceState, saveWorkspaceState, MANAGED_TAG, registerWorkspaceServers, workspaceIdentity } from './workspaceMcp'
...
export class PrometheusIntegrationService extends BaseService {
  ...
  protected onAllReady(): void {
    this.initialization = this.initialize().catch((error) => {
      const id = randomUUID()
      this.operations.set(id, { id, action: 'repair-path', startedAt: Date.now(), status: 'failed', output: '', error: String(error) })
    })
  }
  ...
  async snapshot(): Promise<IntegrationSnapshot> {
    ...
    try {
      inventory = JSON.parse(await fs.readFile(path.join(application.getPath('feature.prometheus.pack.runtime'), 'release-manifest.json'), 'utf8'))
    } catch (error) {
    ...
    return { config: readIntegrationConfig(), secrets: Object.fromEntries(Object.entries(secrets).map(([key,value]) => [key,Boolean(value)])),
      operations: [...this.operations.values()].slice(-20).reverse(), workspaces: [...this.workspaces.values()],
      commandDirectory: application.getPath('feature.prometheus.commands'), serviceDirectory: serviceDirectory(),
      servers: mcpServerService.list({}).items.filter((server) => server.tags?.includes(MANAGED_TAG)).map((server) => ({
        id: server.id, name: server.name, workspace: server.cwd, binary: server.command,
        status: application.get('CacheService').getShared(`mcp.status.${server.id}`)?.state ?? 'disabled'
      })),
      pathInstalled: await commandPathInstalled(), inventory }
  }

  async configure(rawConfig: IntegrationConfig, secretPatch: Partial<Record<IntegrationSecret, string>>): Promise<IntegrationSnapshot> {
    ...
    for (const root of config.filesystem.additionalRoots) {
      if (!path.isAbsolute(root) || !(await fs.stat(root)).isDirectory()) throw new Error('prometheus.error.workspaceDirectory')
    }
    if (new Set([config.services.surrealPort,config.services.memoryPort,config.services.literPort]).size !== 3) throw new Error('prometheus.error.portsDistinct')
  ...
  async resolveSession(session: AgentSessionEntity, sourceAgent: AgentEntity): Promise<{ agent: AgentEntity; servers: McpServer[] }> {
    await this.initialization
    const config = readIntegrationConfig()
    ...
    const manualIds = (sourceAgent.mcps ?? []).filter((id) => !mcpServerService.findByIdOrName(id)?.tags?.includes(MANAGED_TAG))
  ...
  start(action: IntegrationAction, workspacePath?: string): IntegrationOperation {
    if (this.controllers.size && !['status','logs','diagnose'].includes(action)) throw new Error('prometheus.error.operationRunning')
    return this.runOperation(action, workspacePath, async (signal, output, operation) => {
      const config = readIntegrationConfig()
      if (action === 'repair-path') { await installCommandPath(); return }
      if (['pull','start','stop','restart','status','logs'].includes(action)) {
        ...
        if (action === 'status') {
          const status = JSON.parse(result) as { docker: { state: string; compose: boolean; detail?: string }; endpoints: Record<string, { reached: boolean; status?: number; detail?: string }> }
          operation.diagnostics = [
            { id: 'Docker CLI', state: status.docker.state === 'absent' ? 'failed' : 'operational' },
            { id: 'Docker daemon', state: status.docker.state === 'running' ? 'operational' : 'failed', detail: status.docker.detail },
            { id: 'Docker Compose', state: status.docker.compose ? 'operational' : 'failed' },
            ...Object.entries(status.endpoints).map(([id, endpoint]) => ({ id, state: endpoint.reached ? 'listening' as const : 'failed' as const, detail: endpoint.detail }))
          ]
        }
        output(result); return
      }
      if (!workspacePath || !path.isAbsolute(workspacePath) || !(await fs.stat(workspacePath)).isDirectory()) throw new Error('prometheus.error.workspaceDirectory')
      const workspace = await describeWorkspace(await fs.realpath(workspacePath), config)
      this.workspaces.set(workspace.id, workspace)
      if (action === 'install-skills') { output(await installCompassProjectSkills(workspace.path, signal, output)); return }
      if (action === 'diagnose') {
        ...
        if (operation.diagnostics.some((result) => result.state === 'failed')) throw new Error('prometheus.error.toolOperation')
        return
      }
      await indexWorkspace(workspace, config, signal, output)
      workspace.indexed = true
  ...
  private runOperation(action: IntegrationAction, workspacePath: string | undefined, execute: (signal: AbortSignal, output: (value: string) => void, operation: IntegrationOperation) => Promise<void>): { operation: IntegrationOperation; completion: Promise<void> } {
    const id = randomUUID()
    const operation: IntegrationOperation = { id, action, workspacePath, status: 'running', output: '', startedAt: Date.now() }
    const controller = new AbortController()
    this.operations.set(id, operation)
    this.controllers.set(id, controller)
    const completion = execute(controller.signal, (value) => { operation.output = value.slice(-262144) }, operation).then(() => {
