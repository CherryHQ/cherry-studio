...
export async function writeMiniConfiguration(): Promise<void> {
  ...
  await fs.writeFile(path.join(integrationDirectory(), 'mini-runtime.json'), JSON.stringify({
    ...
    PROMETHEUS_KBD_JUDGE_MODEL: 'kbd-judge', PROMETHEUS_KBD_CRITIC_MODEL: 'kbd-critic'
  }), { mode: 0o600 })
}
...
export async function installMiniCommands(directory: string): Promise<void> {
  ...
  await fs.writeFile(path.join(directory, 'prometheus-host.json'), host)
  await fs.writeFile(path.join(application.getPath('cherry.bin'), 'prometheus-host.json'), host)
...
`
  await fs.writeFile(path.join(directory, 'mini-runner.cjs'), runner)
