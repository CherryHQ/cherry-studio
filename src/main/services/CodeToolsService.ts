...
class CodeToolsService {
   ...
   */
  private async checkTerminalAvailability(
    ...
    try {
      if (isMac && terminal.bundleId) {
        ...
        const { stdout } = await execAsync(
          `mdfind "kMDItemCFBundleIdentifier == '${terminal.bundleId}'"`,
          {
            timeout: 3000,
          },
        );
   ...
   */
  private async checkWindowsTerminalAvailability(
    terminal: TerminalConfig,
  ): Promise<TerminalConfig | null> {
    try {
      switch (terminal.id) {
      ...
      logger.debug(
        `Windows terminal ${terminal.id} not available:`,
        error as Error,
      );
      return null;
    }
   ...
   */
  private async checkCustomTerminalPath(
    terminal: TerminalConfig,
  ): Promise<TerminalConfig | null> {
    ...
    try {
      const command =
        terminal.id === terminalApps.alacritty ? "alacritty" : "wezterm";
  ...
  private async getAvailableTerminals(): Promise<TerminalConfig[]> {
    ...
    if (
      this.terminalsCache &&
      now - this.terminalsCache.timestamp < this.TERMINALS_CACHE_DURATION
    ) {
      logger.info(
        `Using cached terminals list (${this.terminalsCache.terminals.length} terminals)`,
      );
    ...
    const terminalPromises = terminalList.map((terminal) =>
      this.checkTerminalAvailability(terminal),
    );
    ...
    try {
      ...
      results.forEach((result, index) => {
        if (result.status === "fulfilled" && result.value) {
          ...
          logger.debug(
            `Terminal check failed for ${MACOS_TERMINALS[index].id}:`,
            result.reason,
          );
        }
      });
   ...
   */
  private async getTerminalConfig(
    terminalId?: string,
  ): Promise<TerminalConfigWithCommand> {
    const availableTerminals = await this.getAvailableTerminals();
    const terminalCommands = isWin
      ? WINDOWS_TERMINALS_WITH_COMMANDS
      : MACOS_TERMINALS_WITH_COMMANDS;
    const defaultTerminal = isWin
      ? terminalApps.cmd
      : terminalApps.systemDefault;
    ...
    if (terminalId) {
      ...
      if (requestedTerminal) {
        ...
        if (customPath && isWin) {
          requestedTerminal = this.applyCustomPath(
            requestedTerminal,
            customPath,
          );
        }
        ...
        logger.warn(
          `Requested terminal ${terminalId} not available, falling back to system default`,
        );
      }
    }
    ...
    const firstAvailable = terminalCommands.find((t) =>
      availableTerminals.some((at) => at.id === t.id),
    );
    if (firstAvailable) {
   ...
   */
  private applyCustomPath(
    terminal: TerminalConfigWithCommand,
    customPath: string,
  ): TerminalConfigWithCommand {
    return {
      ...terminal,
  ...
  public async getVersionInfo(cliTool: string): Promise<VersionInfo> {
    ...
    if (isInstalled) {
      ...
      try {
        ...
        installedVersion = versionMatch
          ? versionMatch[0]
          : stdout.trim().split(" ")[0];
        logger.info(
          `${cliTool} current installed version: ${installedVersion}`,
        );
      } catch (error) {
        logger.warn(
          `Failed to get installed version for ${cliTool}:`,
          error as Error,
        );
      }
    } else {
    ...
    if (cached && now - cached.timestamp < this.CACHE_DURATION) {
      logger.info(
        `Using cached latest version for ${packageName}: ${cached.version}`,
      );
      ...
      try {
        ...
        if (cached) {
          logger.info(
            `Using expired cached version for ${packageName}: ${cached.version}`,
          );
          latestVersion = cached.version;
        }
      }
  ...
  async run(
    ...
    logger.info(
      `Starting CLI tool launch: ${cliTool} in directory: ${directory}`,
    );
    ...
    if (isInstalled && options.autoUpdateToLatest) {
      ...
      try {
        ...
        if (versionInfo.needsUpdate) {
          logger.info(
            `Update available for ${cliTool}: ${versionInfo.installed} -> ${versionInfo.latest}`,
          );
    ...
    let baseCommand = isWin
      ? `"${executablePath}"`
      : `"${bunPath}" "${executablePath}"`;

    // Add configuration parameters for OpenAI Codex
    if (
      cliTool === codeTools.openaiCodex &&
      env.OPENAI_MODEL_PROVIDER &&
      env.OPENAI_MODEL_PROVIDER != "openai"
    ) {
    ...
    switch (platform) {
      case "darwin": {
        ...
        logger.info(
          `Using terminal: ${terminalConfig.name} (${terminalConfig.id})`,
        );

        const { command: cmd, args } = terminalConfig.command(
          directory,
          fullCommand,
        );
        terminalCommand = cmd;
      ...
      case "linux": {
        ...
        if (foundTerminal === "gnome-terminal") {
          ...
          terminalArgs = [
            "-e",
            `cd "${directory}" && clear && ${command} && bash`,
          ];
        }
        break;
      }
