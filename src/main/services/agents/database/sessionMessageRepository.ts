...
class AgentMessageRepository extends BaseService {
  ...
  async removeBlockFromMessage(sessionId: string, messageId: string, blockId: string): Promise<boolean> {
    try {
      const database = await this.getDatabase()
      const rows = await database
        .select()
      ...
      for (const row of rows) {
        ...
        try {
          ...
          // Try to remove blockId from message.blocks array (if it exists there)
          const blockIds = parsed.message.blocks ?? []
          const filteredBlockIds = blockIds.filter((id) => id !== blockId)
          if (filteredBlockIds.length !== blockIds.length) {
            parsed.message.blocks = filteredBlockIds
            modified = true
          }

          // Remove the actual block data from blocks array
          const originalBlocksCount = parsed.blocks?.length ?? 0
          if (parsed.blocks) {
            parsed.blocks = parsed.blocks.filter((block) => block.id !== blockId)
          ...
          if (!modified) {
            logger.warn(`Block ${blockId} not found in message ${messageId}`)
            return false
          }
          ...
          logger.info(`Removed block ${blockId} from message ${messageId}`)
          return true
        } catch (error) {
          logger.warn('Failed to parse session message content during block removal', error as Error)
        }
      }
      ...
      logger.error('Failed to remove block from message', error as Error)
      throw error
    }
  }

  async removeBlocksFromMessage(
    sessionId: string,
    messageId: string,
    blockIds: string[]
  ): Promise<{ success: boolean; removedCount: number; failedIds: string[] }> {
    const result = { success: false, removedCount: 0, failedIds: [] as string[] }
