  ...
  private async restoreDirect(extractionDir: string): Promise<void> {
    ...
    try {
      ...
      if (!this.isChainBundledPrefix(chain)) {
        throw new Error(
          `${BACKUP_NEWER_VERSION_ERROR_CODE}: This backup was created by a newer version of Cherry Studio (database is ahead of this version) and cannot be restored here. Please update Cherry Studio and try again. Backup appVersion: ${String((metadata as Record<string, unknown>).appVersion ?? 'unknown')}, current: ${app.getVersion()}.`
        )
      }
