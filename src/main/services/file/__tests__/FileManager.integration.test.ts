...
describe('FileManager (integration)', () => {
  ...
  it('INT-4g: owns compound entry mutations at the FileManager boundary', async () => {
    ...
    await fm.batchPermanentDeleteFromTrash([copied.id])
    await fm.trash(source.id)
    await fm.batchRemoveFromLibrary([source.id])
    ...
    expect(labels).toEqual(
      expect.arrayContaining([
        ...
        'file-manager:batch-permanent-delete-from-trash',
        'file-manager:batch-remove-from-library'
      ])
    )
  })
