 ...
 * knowledge_item checkers. Phase 2 adds chat_message and painting. Other
 * business domains (note) will be added when their owning DB tables migrate to
 * v2 — each new variant lands as a single PR introducing (a) the ref schema
 * variant, (b) the source-type tuple entry, AND (c) the checker below, so the
 * three surfaces stay in lockstep.
 *
 * Currently registered checkers: temp_session, knowledge_item, chat_message, painting.
 */
