...
class BackupManager {
  ...
  async backup(
    ...
    try {
      ...
      // TODO(v2 backup SQLite): the SQLite DB at `app.database.file` is intentionally
      // NOT included in this backup. The Phase 2 Batch 0 attempt was reverted because
      // the runtime restore path needs atomic-rename + WAL/SHM aux-file cleanup +
      // DbService client coordination to avoid data loss on relaunch. Until that
      // restore path lands, v1 Dexie data in `Data/`/`IndexedDB/` is the only source
      // for v2 file_entry, regenerated by FileMigrator on the next startup.
  ...
  private async restoreDirect(): Promise<void> {
    ...
    try {
      ...
      // TODO(v2 backup SQLite restore): SQLite is intentionally NOT restored here.
      // Runtime swap of `app.database.file` needs atomic-rename + delete of
      // `<db>-wal` / `<db>-shm` aux files (otherwise SQLite replays pre-restore
      // frames into the restored DB on next open) + DbService client coordination
      // (a live libsql client over the in-place file races the swap). Deferred to
      // a follow-up PR that introduces a proper atomic-restore mechanism.
      // Until then, FileMigrator regenerates v2 file_entry from v1 Dexie data
      // (already restored above as part of IndexedDB) on next startup.
