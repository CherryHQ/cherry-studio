...
import type { LanguageModelMiddleware, Provider as AiSdkProvider } from 'ai'
...
const mainProcessSdkContext: AiSdkConfigContext = {
  ...
  getIncludeUsageSetting: () => undefined,
...
async function executeStream(config: ExecuteStreamConfig): Promise<{
  ...
  const model =
    middlewares.length > 0 && typeof baseModel === 'object'
      ? (wrapLanguageModel({ model: baseModel as never, middleware: middlewares as never }) as typeof baseModel)
      : baseModel
...
export async function processMessage(config: MessageConfig): Promise<void> {
  ...
  try {
    ...
    const allMiddlewares = isStreaming
      ? middlewares
      : ([simulateStreamingMiddleware(), ...middlewares] as unknown as LanguageModelMiddleware[])
