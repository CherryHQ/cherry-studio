...
export class PiRuntimeDriver implements AgentSessionRuntimeDriver {
  ...
  async validateSession(session: AgentSessionEntity): Promise<void> {
    ...
    if (!agentService.getAgent(session.agentId)) {
      throw new Error(`pi agent ${session.agentId} is unavailable`)
    }
    if (!session.modelId) {
      throw new Error(`pi agent session ${session.id} has no model configured`)
    }
    ...
    await assertPiProviderUsable(session.modelId)
  }
