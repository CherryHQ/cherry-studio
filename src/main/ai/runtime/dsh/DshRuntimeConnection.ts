  ...
  async stopTask(taskId: string): Promise<boolean> {
    ...
    try {
      await bridge.request(
        'subagent/interrupt',
        // Both fixes matter: the runtime session id follows a fork, and the child id must be the
        // child that currently owns this task run rather than the raw task id.
        { sessionId: this.runtimeSessionId, childSessionId },
        { timeoutMs: 5_000 }
      )
      return true
    } catch (error) {
