...
export function toDshPluginUrl(pluginPath: string): string {
  return pathToFileURL(pluginPath, { windows: isWin }).href
}
...
export function buildDshCompositionYaml(input: DshCompositionInput): string {
  const pluginName = (specifier: string) => q(toDshPluginUrl(resolveDshPluginPath(specifier)))
  const sandboxMode = input.permissionMode === 'bypassPermissions' ? 'danger-full-access' : 'workspace-write'
  const shellExecutor = isWin ? '@deepseek-ai/dsh-pwsh-sandbox' : '@deepseek-ai/dsh-bash-sandbox'
  const lines: string[] = [
    ...
    ...(isWin ? [`    toolBash: false`] : [`    toolBash:`, `      enableRunInBackground: false`]),
    `    toolJobs: false`,
    ``,
    ...(isWin
      ? [
