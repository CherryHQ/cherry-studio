...
function agentTranscriptStepLine(
  messageId: string,
  inputTokens: number,
  outputTokens: number,
...
function appendAssistantTranscriptStep(
  ...
  appendFileSync(transcriptPath, agentTranscriptStepLine(messageId, inputTokens, outputTokens, toolUses))
}
...
function createAgentTranscriptFixture(agentId = 'agent-1', sessionId = 'sdk-agent') {
  ...
  return {
    ...
    // Half-flushes a step, the way a transcript write races a read, and returns the writer that ends it.
    appendAssistantStepPartially: (messageId: string, inputTokens: number, outputTokens: number) => {
      const line = agentTranscriptStepLine(messageId, inputTokens, outputTokens)
      const flushedLength = Math.floor(line.length / 2)
      appendFileSync(transcriptPath, line.slice(0, flushedLength))
      return () => appendFileSync(transcriptPath, line.slice(flushedLength))
    },
    cleanup: () => rmSync(claudeConfigDir, { recursive: true, force: true })
  }
}
...
/**
 * Real subagent transcripts inline tool results, so they routinely outgrow a single read pass. This
 * appends one complete non-assistant JSONL line of roughly `bytes`, which the reader parses but never
 * counts towards usage.
 */
function appendBulkTranscriptLine(transcriptPath: string, bytes: number) {
  appendFileSync(
    transcriptPath,
    `${JSON.stringify({
      type: 'user',
...
describe('ClaudeCodeStreamAdapter', () => {
  ...
  it('reconciles the final foreground Agent step after completion races its transcript write', () => {
    ...
    try {
      ...
      // The result is the CLI's write barrier, so the step that raced the completion lands right away
      // instead of after the retry delay.
  ...
  it('keeps reconciling a terminal Agent until the turn result flushes its transcript', () => {
    vi.useFakeTimers()
    const fixture = createAgentTranscriptFixture('agent-foreground-terminal-barrier')
    try {
      const { adapter, statusEvents } = createAdapter({ claudeConfigDir: fixture.claudeConfigDir })
      startAgentTask(adapter, fixture)
      completeForegroundAgent(adapter, fixture, { contextTokens: 240, toolUses: 2, durationMs: 3000 })

      // A changed total is published, but it cannot settle the task while its turn runs.
      fixture.appendAssistantStep('message-1', 100, 20, [{ id: 'tool-1' }])
      vi.advanceTimersByTime(1000)
      expect(getLastBackgroundTaskEvent(statusEvents)?.data.usage?.totalTokens).toBe(120)

      // A queued write can land well after any quiet window a heuristic would wait out.
      ...
      // Past the barrier the task is settled, so nothing written later is attributed to it.
      const settledEvents = statusEvents.length
      fixture.appendAssistantStep('message-3', 1000, 0)
      vi.advanceTimersByTime(10_000)
      expect(statusEvents).toHaveLength(settledEvents)
    } finally {
      fixture.cleanup()
      vi.useRealTimers()
    }
  ...
  it('keeps the published cumulative total when the terminal reconciliation reads a shorter transcript', () => {
    vi.useFakeTimers()
    const fixture = createAgentTranscriptFixture('agent-terminal-transcript-regression')
    let adapter: InstanceType<typeof ClaudeCodeStreamAdapter> | undefined
    try {
      const created = createAdapter({ claudeConfigDir: fixture.claudeConfigDir })
      adapter = created.adapter
      ...
      // Both steps are only visible in the forwarded flow, so the terminal edge publishes their total
      ...
      // The flushed transcript still holds only the first step, i.e. a smaller cumulative total.
      fixture.appendAssistantStep('message-1', 200, 30, [{ id: 'tool-1' }])
      vi.advanceTimersByTime(1000)

      expect(getLastBackgroundTaskEvent(statusEvents)?.data.usage).toMatchObject({
        totalTokens: 350,
        contextTokens: 240
      })
    } finally {
  ...
  it('defers the terminal Agent reconciliation until the raced transcript write is readable', () => {
    vi.useFakeTimers()
    const fixture = createAgentTranscriptFixture('agent-terminal-partial-write')
    let adapter: InstanceType<typeof ClaudeCodeStreamAdapter> | undefined
    try {
      const created = createAdapter({ claudeConfigDir: fixture.claudeConfigDir })
      adapter = created.adapter
      ...
      // The first reconcile attempt lands mid-write: step 1 is flushed, step 2 only half of it.
      fixture.appendAssistantStep('message-1', 100, 20, [{ id: 'tool-1' }])
      const finishStepTwo = fixture.appendAssistantStepPartially('message-2', 200, 30)
      vi.advanceTimersByTime(1000)
      expect(getLastBackgroundTaskEvent(statusEvents)?.data.usage).toEqual({
        contextTokens: 240,
        toolUses: 2,
      ...
      // The write lands and the deferred re-read publishes the transcript's complete total.
      finishStepTwo()
      vi.advanceTimersByTime(1000)
      expect(getLastBackgroundTaskEvent(statusEvents)?.data).toMatchObject({
        event: 'notification',
  ...
  it('publishes terminal Agent totals from a transcript larger than one read pass', () => {
    vi.useFakeTimers()
    const fixture = createAgentTranscriptFixture('agent-terminal-large-transcript')
    let adapter: InstanceType<typeof ClaudeCodeStreamAdapter> | undefined
    try {
      const created = createAdapter({ claudeConfigDir: fixture.claudeConfigDir })
      adapter = created.adapter
      ...
      // Complete JSONL, only bigger than the bytes one capped read pass covers: not a flush in progress.
      fixture.appendAssistantStep('message-1', 100, 20, [{ id: 'tool-1' }])
      appendBulkTranscriptLine(fixture.transcriptPath, 1_800_000)
      fixture.appendAssistantStep('message-2', 200, 30, [{ id: 'tool-2' }])
  ...
  it('retries a tail the per-attempt read ceiling left unread and finishes it on the next attempt', () => {
    vi.useFakeTimers()
    const fixture = createAgentTranscriptFixture('agent-terminal-read-ceiling')
    let adapter: InstanceType<typeof ClaudeCodeStreamAdapter> | undefined
    try {
      const created = createAdapter({ claudeConfigDir: fixture.claudeConfigDir })
      adapter = created.adapter
      ...
      // Complete JSONL well past what one attempt is allowed to read.
      fixture.appendAssistantStep('message-1', 100, 20, [{ id: 'tool-1' }])
      for (let line = 0; line < 4; line += 1) appendBulkTranscriptLine(fixture.transcriptPath, 600_000)
      fixture.appendAssistantStep('message-2', 200, 30, [{ id: 'tool-2' }])
      for (let line = 0; line < 4; line += 1) appendBulkTranscriptLine(fixture.transcriptPath, 600_000)

      // The unread tail is a retry reason, not something to publish a partial total from.
      vi.advanceTimersByTime(1000)
      expect(getLastBackgroundTaskEvent(statusEvents)?.data.usage).toEqual({
        contextTokens: 240,
        toolUses: 2,
      ...
      // The next attempt continues from the cached read offset and reaches the end.
      vi.advanceTimersByTime(1000)
      expect(getLastBackgroundTaskEvent(statusEvents)?.data).toMatchObject({
        event: 'notification',
        taskId: fixture.agentId,
  ...
  it('reconciles a half-written transcript tail that stopped changing instead of waiting for its writer', () => {
    vi.useFakeTimers()
    const fixture = createAgentTranscriptFixture('agent-terminal-frozen-tail')
    let adapter: InstanceType<typeof ClaudeCodeStreamAdapter> | undefined
    try {
      const created = createAdapter({ claudeConfigDir: fixture.claudeConfigDir })
      adapter = created.adapter
      ...
      // The CLI stops mid-line: step 1 is complete, step 2's line will never be finished.
      fixture.appendAssistantStep('message-1', 100, 20, [{ id: 'tool-1' }])
      fixture.appendAssistantStepPartially('message-2', 200, 30)

      // A first sighting cannot tell a dead writer apart from a live one, so it still waits.
      vi.advanceTimersByTime(1000)
      expect(getLastBackgroundTaskEvent(statusEvents)?.data.usage).toEqual({
        contextTokens: 240,
        toolUses: 2,
      ...
      // One unchanged interval can still be a live writer pausing, so the half-written tail is not
      // final yet.
      vi.advanceTimersByTime(1000)
      expect(getLastBackgroundTaskEvent(statusEvents)?.data.usage).toEqual({
        contextTokens: 240,
      ...
      // Unchanged size and mtime across two consecutive attempts mean no writer is left: the complete
      // prefix is the whole story, and it still lands inside the retry budget.
      vi.advanceTimersByTime(1000)
      expect(getLastBackgroundTaskEvent(statusEvents)?.data).toMatchObject({
        event: 'notification',
        taskId: fixture.agentId,
        status: 'completed',
        usage: { totalTokens: 120, contextTokens: 240, toolUses: 2, durationMs: 3000 }
      })
    } finally {
      adapter?.dispose()
      fixture.cleanup()
  ...
  it('keeps the largest published Agent total while a growing transcript outruns the read budget', () => {
    vi.useFakeTimers()
    const fixture = createAgentTranscriptFixture('agent-terminal-growing-transcript')
    let adapter: InstanceType<typeof ClaudeCodeStreamAdapter> | undefined
    try {
      const created = createAdapter({ claudeConfigDir: fixture.claudeConfigDir })
      adapter = created.adapter
      const { statusEvents } = created
      startAgentTask(adapter, fixture, { backgrounded: true })
      // Forwarded steps publish 1500 while the transcript is still empty.
      emitForwardedAgentStep(adapter, fixture, { messageId: 'message-flow-1', inputTokens: 1200, outputTokens: 100 })
      emitForwardedAgentStep(adapter, fixture, { messageId: 'message-flow-2', inputTokens: 200, outputTokens: 0 })
      completeBackgroundAgent(adapter, fixture, { contextTokens: 240, toolUses: 2, durationMs: 3000 })
      expect(getLastBackgroundTaskEvent(statusEvents)?.data.usage?.totalTokens).toBe(1500)

      // The task is still flushing: a half-line keeps the first attempt incomplete.
      fixture.appendAssistantStep('message-1', 100, 20, [{ id: 'tool-1' }])
      appendBulkTranscriptLine(fixture.transcriptPath, 1_200_000)
      const finishStepTwo = fixture.appendAssistantStepPartially('message-2', 200, 30)
      vi.advanceTimersByTime(1000)
      expect(getLastBackgroundTaskEvent(statusEvents)?.data.usage?.totalTokens).toBe(1500)

      // The writer flushes and grows the transcript past the whole retry budget of a capped reader.
      finishStepTwo()
      fixture.appendAssistantStep('message-3', 1300, 180, [{ id: 'tool-3' }])
      appendBulkTranscriptLine(fixture.transcriptPath, 1_200_000)
      vi.advanceTimersByTime(1000)

      // The newest transcript total is larger, so it must replace the streamed one — never shrink it.
      expect(getLastBackgroundTaskEvent(statusEvents)?.data.usage).toEqual({
        totalTokens: 1830,
        contextTokens: 240,
        toolUses: 2,
      ...
      const publishedTotals = statusEvents.flatMap((event) => {
        const totalTokens = event.type === 'background-task-event' ? event.data.usage?.totalTokens : undefined
        return typeof totalTokens === 'number' ? [totalTokens] : []
      })
      expect(publishedTotals.every((total, index) => index === 0 || total >= publishedTotals[index - 1])).toBe(true)

      const settledEvents = statusEvents.length
      vi.advanceTimersByTime(1000)
      expect(statusEvents.length).toBe(settledEvents)
    } finally {
      adapter?.dispose()
      fixture.cleanup()
      vi.useRealTimers()
  ...
  it('never publishes an unread terminal transcript tail and, while idle, stops reconciling after the budget', () => {
    vi.useFakeTimers()
    const fixture = createAgentTranscriptFixture('agent-terminal-unread-transcript')
    let adapter: InstanceType<typeof ClaudeCodeStreamAdapter> | undefined
    try {
      const created = createAdapter({ claudeConfigDir: fixture.claudeConfigDir })
      adapter = created.adapter
      const { statusEvents } = created
      startAgentTask(adapter, fixture, { backgrounded: true })
      // With the turn over, no result is coming to vouch for the transcript.
      adapter.handleMessage(successResult({ session_id: fixture.sessionId }))
      emitForwardedAgentStep(adapter, fixture, { messageId: 'message-flow-1', inputTokens: 1200, outputTokens: 100 })
      completeBackgroundAgent(adapter, fixture, { contextTokens: 240, toolUses: 2, durationMs: 3000 })
      expect(getLastBackgroundTaskEvent(statusEvents)?.data.usage?.totalTokens).toBe(1300)

      // More unread transcript than the whole retry budget can drain, so the step line stays unreachable.
      for (let line = 0; line < 20; line += 1) appendBulkTranscriptLine(fixture.transcriptPath, 1_200_000)
      fixture.appendAssistantStep('message-1', 5000, 700, [{ id: 'tool-1' }])
      vi.advanceTimersByTime(1000)

      // The ceiling is genuinely short of the file, so the step line stays beyond the read offset.
      const firstAttempt = (adapter as any).agentTranscriptStatsCaches.get(fixture.agentId)
      expect(firstAttempt.readOffset).toBeLessThan(firstAttempt.sizeBytes)

      for (let attempt = 0; attempt < 3; attempt += 1) vi.advanceTimersByTime(1000)

      // An incomplete read is never published as final, so the streamed total stays the largest one.
      expect(getLastBackgroundTaskEvent(statusEvents)?.data.usage?.totalTokens).toBe(1300)

      // The window is bounded: records written after it stay unreconciled for this task.
      const settledEvents = statusEvents.length
      fixture.appendAssistantStep('message-2', 9000, 900, [{ id: 'tool-2' }])
  ...
  it('reads the transcript once per burst of forwarded child steps, off the SDK event path', () => {
    vi.useFakeTimers()
    const fixture = createAgentTranscriptFixture('agent-transcript-refresh')
    try {
      const { adapter, statusEvents } = createAdapter({ claudeConfigDir: fixture.claudeConfigDir })
      startAgentTask(adapter, fixture, { backgrounded: true })
      adapter.handleMessage(successResult({ session_id: fixture.sessionId }))
      const publishedTotal = () => getLastBackgroundTaskEvent(statusEvents)?.data.usage?.totalTokens

      // The forwarded steps carry no usage, so a total can only come from reading the transcript.
      fixture.appendAssistantStep('message-1', 200, 30, [{ id: 'tool-1' }])
      emitForwardedAgentStep(adapter, fixture, { messageId: 'message-1', inputTokens: 0, outputTokens: 0 })
      expect(publishedTotal()).toBeUndefined()
      vi.advanceTimersByTime(100)
      fixture.appendAssistantStep('message-2', 500, 70, [{ id: 'tool-2' }])
      emitForwardedAgentStep(adapter, fixture, { messageId: 'message-2', inputTokens: 0, outputTokens: 0 })
      expect(publishedTotal()).toBeUndefined()

      // One read after the burst's window covers both of its steps.
      vi.advanceTimersByTime(100)
      expect(getLastBackgroundTaskEvent(statusEvents)?.data.usage).toEqual({
        totalTokens: 800,
        contextTokens: 570,
      ...
      // The step inside the window queued no read of its own; the next step does.
      fixture.appendAssistantStep('message-3', 1000, 0)
      vi.advanceTimersByTime(1_000)
      expect(publishedTotal()).toBe(800)
      emitForwardedAgentStep(adapter, fixture, { messageId: 'message-3', inputTokens: 0, outputTokens: 0 })
      vi.advanceTimersByTime(200)
      expect(publishedTotal()).toBe(1800)
    } finally {
      fixture.cleanup()
      vi.useRealTimers()
    }
  ...
  it('drops a pending transcript refresh once its Agent settles or the adapter goes away', () => {
    vi.useFakeTimers()
    const fixture = createAgentTranscriptFixture('agent-refresh-settled')
    try {
      const settled = createAdapter({ claudeConfigDir: fixture.claudeConfigDir })
      startAgentTask(settled.adapter, fixture, { backgrounded: true })
      settled.adapter.handleMessage(successResult({ session_id: fixture.sessionId }))
      fixture.appendAssistantStep('message-1', 200, 30, [{ id: 'tool-1' }])
      emitForwardedAgentStep(settled.adapter, fixture, { messageId: 'message-1', inputTokens: 0, outputTokens: 0 })
      completeBackgroundAgent(settled.adapter, fixture, { contextTokens: 240, toolUses: 1, durationMs: 3000 })
      // The CLI can still be flushing the transcript after the terminal edge.
      fixture.appendAssistantStep('message-2', 500, 70)
      vi.advanceTimersByTime(5_000)

      // A progress edge after the terminal one would put the settled Agent back in progress.
      const taskEvents = settled.statusEvents.filter((event) => event.type === 'background-task-event')
      const settledAt = taskEvents.findIndex((event) => event.data.status === 'completed')
      expect(settledAt).toBeGreaterThanOrEqual(0)
      expect(taskEvents.slice(settledAt).map((event) => event.data.status)).not.toContain('in_progress')

      const disposed = createAdapter({ claudeConfigDir: fixture.claudeConfigDir })
      startAgentTask(disposed.adapter, fixture, { backgrounded: true })
      emitForwardedAgentStep(disposed.adapter, fixture, { messageId: 'message-1', inputTokens: 0, outputTokens: 0 })
      const published = disposed.statusEvents.length
      disposed.adapter.dispose()
      vi.advanceTimersByTime(1_000)
      expect(disposed.statusEvents).toHaveLength(published)
    } finally {
      fixture.cleanup()
      vi.useRealTimers()
    }
    ...
    it('stops background Bash polling and clears task associations at quiescent idle', () => {
      ...
      try {
        ...
        expect(statusEvents).toEqual([
          ...
          // A Bash-only membership holds no reply: only a background Agent keeps the await open.
          { type: 'background-work-state', active: true, awaitingReply: false }
        ])
        adapter.dispose()
      } finally {
