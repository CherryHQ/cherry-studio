...
describe('ClaudeCodeWarmQueryManager', () => {
  ...
  it('consumes a matching warm query once', async () => {
    ...
    manager.prewarm({ key: 'session-1', options: { model: 'sonnet', resume: 'sdk-1', abortController } })
  ...
  it('closes a stale warm query when session options change', async () => {
    ...
    manager.prewarm({ key: 'session-1', options: { model: 'sonnet', resume: 'sdk-1' } })
    manager.prewarm({ key: 'session-1', options: { model: 'opus', resume: 'sdk-1' } })
  ...
  it('closes unused warm queries after the idle ttl', async () => {
    ...
    manager.prewarm({ key: 'session-1', options: { model: 'sonnet' } })
    await Promise.resolve()
