...
describe('resolveMountedMcpServers', () => {
  ...
  it('keeps Cherry Assistant host servers once the session is channel-linked', () => {
    ...
    expect(resolveMountedMcpServers(agentRef, { channelLinked: true }).has('assistant')).toBe(true)
    expect(resolveMountedMcpServers(agentRef, { channelLinked: true }).has('assistant-files')).toBe(true)
  })
})
