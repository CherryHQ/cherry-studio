...
describe('adjustAllowedToolsForMcp', () => {
  it('lists auto-approved cherry-tools + agent-memory for every agent, excluding the mutating kb_manage', () => {
    ...
    expect(allowed).not.toContain('mcp__mcp_manager__installMcpServer__15c915834e62')
  })
  ...
  it('removes an exact disabled tool without affecting sibling auto-approvals', () => {
    const allowed = adjustAllowedToolsForMcp(WITHOUT_HOST_TOOLS, ['mcp__cherry_tools__webFetch__0d46b7903981'])
  ...
  it.each(['mcp__cherry_tools', 'mcp__cherry_tools__*'])('removes server-wide disabled tools for %s', (rule) => {
    const allowed = adjustAllowedToolsForMcp(WITHOUT_HOST_TOOLS, [rule])

    expect(allowed.some((toolName) => toolName.startsWith('mcp__cherry_tools__'))).toBe(false)
  ...
  it('does not let the memory auto-approval override an exact disabled tool', () => {
    const allowed = adjustAllowedToolsForMcp(WITHOUT_HOST_TOOLS, ['mcp__agent_memory__memory__b472a1250bce'])
...
describe('buildMcpServers', () => {
  ...
  it('injects the agent-memory and skills servers for every agent (REGRESSION agents-jobs-3)', async () => {
    ...
    expect(Object.keys(result ?? {})).toEqual(
      expect.arrayContaining(['cherry_tools', 'agent_memory', 'skills', 'mcp_manager'])
    )
