...
describe('buildClaudeCodeQueryRequestForAgentSession resume-token precedence', () => {
  ...
  it('preserves an explicit API timeout for local API gateway routes', async () => {
    ...
    mocks.getLastRuntimeResumeToken.mockReturnValue(null)
    mocks.buildSessionSettings.mockResolvedValue({ env: { API_TIMEOUT_MS: '3600000' } })

    const request = await buildClaudeCodeQueryRequestForAgentSession('session-1')

    expect(request?.options.env?.API_TIMEOUT_MS).toBe('3600000')
  })
