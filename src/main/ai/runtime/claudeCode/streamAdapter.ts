...
/** One reconciliation attempt drains the transcript; only this ceiling makes a large tail a retry. */
const MAX_AGENT_TRANSCRIPT_READ_BYTES_PER_ATTEMPT = 8 * MAX_AGENT_TRANSCRIPT_READ_BYTES_PER_PASS
/** Forwarded child steps arrive in bursts, so the transcript is read once a burst has had this long. */
const AGENT_TRANSCRIPT_REFRESH_DELAY_MS = 200
const TERMINAL_TASK_RECONCILE_DELAY_MS = 1000
const TERMINAL_TASK_RECONCILE_MAX_ATTEMPTS = 3
/** A half-written tail only counts as final once this many consecutive attempts read the same stamp. */
const TERMINAL_TRANSCRIPT_QUIET_ATTEMPTS = 2
...
type PendingTerminalAgentState = PendingTerminalWorkflowState & {
  sdkSessionId: string
  /** Transcript size and mtime as the previous attempt left them; a freeze means its writer is gone. */
  lastTranscriptStamp?: { sizeBytes: number; mtimeMs: number }
  /** Consecutive attempts that read `lastTranscriptStamp` unchanged; a lone one can still be a pause. */
  quietTranscriptAttempts: number
  /** A result arrived after the terminal edge, so every transcript write the CLI had queued is on disk. */
  flushed?: boolean
}
...
function readAgentTranscriptStats(
  ...
  maxBytesPerPass = MAX_AGENT_TRANSCRIPT_READ_BYTES_PER_PASS
  ...
  try {
    ...
    while (cache.readOffset < stat.size && bytesReadThisPass < maxBytesPerPass) {
      const length = Math.min(
        AGENT_TRANSCRIPT_READ_CHUNK_BYTES,
        stat.size - cache.readOffset,
        maxBytesPerPass - bytesReadThisPass
      )
...
/**
 * Drains the transcript up to the per-attempt ceiling. The cache carries `readOffset`, so a continued
 * pass re-parses nothing; only a tail the ceiling left unread is worth another attempt.
 */
function drainAgentTranscriptStats(
  transcriptPath: string,
  cached?: AgentTranscriptStatsCacheEntry
): AgentTranscriptStatsCacheEntry {
  let stats = readAgentTranscriptStats(transcriptPath, cached, MAX_AGENT_TRANSCRIPT_READ_BYTES_PER_PASS)
  let readBytes = Math.max(0, stats.readOffset - (cached?.readOffset ?? 0))
  while (readBytes < MAX_AGENT_TRANSCRIPT_READ_BYTES_PER_ATTEMPT) {
    const previousOffset = stats.readOffset
    const next = readAgentTranscriptStats(
      transcriptPath,
      stats,
      Math.min(MAX_AGENT_TRANSCRIPT_READ_BYTES_PER_PASS, MAX_AGENT_TRANSCRIPT_READ_BYTES_PER_ATTEMPT - readBytes)
    )
    // No forward progress means the file ended where the pass stat'd it, so this attempt is done.
    if (next.readOffset <= previousOffset) return next
    readBytes += next.readOffset - previousOffset
    stats = next
  }
  return stats
}
...
export class ClaudeCodeStreamAdapter {
  ...
  /** Whether the SDK's latest membership still lists an Agent, retained stragglers aside. */
  private membershipHasAgent = false
  ...
  private readonly agentTranscriptRefreshes = new Map<string, AgentTranscriptRefresh>()
  ...
  handleMessage(message: SDKMessage): ClaudeCodeStreamAdapterResult {
    ...
    if (message.type === 'result') this.reconcileFlushedTerminalAgents()
  ...
  private handleSystemMessage(message: SdkRuntimeSystemMessage, ctx: StreamContext): void {
    switch (message.subtype) {
      ...
      case 'background_tasks_changed': {
        ...
        // Membership stays authoritative while a vanished task waits for its terminal edge.
        this.membershipHasAgent = message.tasks.some(
          (task) => task.task_type === 'subagent' || task.task_type === 'local_agent'
        )
        if (this.membershipHasAgent) this.awaitingBackgroundReply = true
    ...
    const contextTokens =
      terminal && reported?.contextTokens !== undefined
        ...
        : hasTokenStats && stats && stats.latestTokens > 0
          ? stats.latestTokens
          : reported?.contextTokens
    // Cumulative tokens only grow, so a transcript read that lags a reported total must not shrink it.
    const totalTokens = Math.max(hasTokenStats && stats ? stats.cumulativeTokens : 0, reported?.totalTokens ?? 0)
    const usage = {
  ...
  private updateAgentFlowStats(rootToolCallId: string, message: SDKAssistantMessage): void {
    ...
    const timestamp = getSdkMessageTimestamp(message)
    this.scheduleAgentTranscriptRefresh(taskId, rootToolCallId, message.session_id, timestamp)
    if (flowChanged) this.publishAgentProgress(taskId, rootToolCallId, timestamp)
  }

  /** The transcript read is synchronous file work, so a burst of forwarded steps shares one read that
  ...
  private refreshAgentTranscriptProgress(taskId: string): void {
    const pending = this.agentTranscriptRefreshes.get(taskId)
    if (!pending) return
    this.agentTranscriptRefreshes.delete(taskId)
    // A settled task already published its terminal usage; a late progress edge would reopen it.
    ...
    if (
      !transcriptStats ||
      (transcriptStats.cumulativeTokens === (previous?.cumulativeTokens ?? 0) &&
        transcriptStats.latestTokens === (previous?.latestTokens ?? 0) &&
        transcriptStats.totalToolCalls === (previous?.totalToolCalls ?? 0))
    ) {
      return
    }
  ...
  private publishAgentProgress(taskId: string, rootToolCallId: string, timestamp: string): void {
    ...
    const eventData: AgentTaskEventPartData = {
      ...
      ...this.getTaskTimeline(taskId, 'in_progress', timestamp),
      usage
    }
  ...
  private clearBackgroundTaskState(): void {
    ...
    for (const pending of this.agentTranscriptRefreshes.values()) clearTimeout(pending.timer)
    ...
    this.agentTranscriptRefreshes.clear()
  ...
  private resolveAgentTranscriptPath(taskId: string, sdkSessionId: string): string | undefined {
    if (!SAFE_TRANSCRIPT_ID.test(taskId)) return undefined
    const cachedPath = this.agentTranscriptStatsCaches.get(taskId)?.transcriptPath
    ...
    try {
      ...
      const transcriptPath = realpathSync(path.join(transcriptDir, `agent-${taskId}.jsonl`))
      return path.dirname(transcriptPath) === transcriptDir ? transcriptPath : undefined
    } catch {
      return undefined
    }
  }

  private readAgentTaskTranscriptStats(
    taskId: string,
    sdkSessionId: string
  ...
  /** The terminal reconciliation drains the whole file instead of trusting a single capped pass. */
  private drainAgentTaskTranscriptStats(
    taskId: string,
    sdkSessionId: string
  ): AgentTranscriptStatsCacheEntry | undefined {
    const cached = this.agentTranscriptStatsCaches.get(taskId)
  ...
  private scheduleTerminalAgentReconciliation(eventData: AgentTaskEventPartData, sdkSessionId: string): void {
    ...
    timer.unref?.()
    this.pendingTerminalAgentEvents.set(eventData.taskId, {
      eventData,
      sdkSessionId,
      timer,
      attempts: 0,
      quietTranscriptAttempts: 0
    })
  }
  ...
  private reconcileTerminalAgentStats(taskId: string): void {
    ...
    const unchangedStamp = Boolean(
      transcriptStats &&
      stamp &&
      stamp.sizeBytes === transcriptStats.sizeBytes &&
      stamp.mtimeMs === transcriptStats.mtimeMs
    )
    pending.quietTranscriptAttempts = unchangedStamp ? pending.quietTranscriptAttempts + 1 : 0
    if (transcriptStats) {
      pending.lastTranscriptStamp = { sizeBytes: transcriptStats.sizeBytes, mtimeMs: transcriptStats.mtimeMs }
    }
    const frozenTail = Boolean(
      transcriptStats &&
      !unreadTail &&
    ...
    const transcriptReadable =
      hasAgentUsageStats(transcriptStats) && !unreadTail && (transcriptStats.pendingLine.length === 0 || frozenTail)
    const usage = transcriptReadable ? this.getAgentUsage(transcriptStats, pending.eventData.usage, true) : undefined
    const usageChanged = Boolean(usage && hasAgentUsageChanged(pending.eventData.usage, usage))
    if (usage && usageChanged) {
    ...
    pending.attempts += 1
    if (pending.attempts < TERMINAL_TASK_RECONCILE_MAX_ATTEMPTS || (!pending.flushed && this.turnActive)) {
    ...
    if (!transcriptReadable) {
      // Never usable within the retry budget: keep the larger usage already published for this task.
      logger.warn('Terminal Agent reconciliation read an incomplete transcript', {
        sessionId: this.sessionId,
        sdkSessionId: pending.sdkSessionId,
        taskId,
        readOffset: transcriptStats?.readOffset,
        sizeBytes: transcriptStats?.sizeBytes,
        pendingLineLength: transcriptStats?.pendingLine.length
      })
    }
    this.finishTerminalTaskReconciliation(taskId)
  }

  /** With `CLAUDE_CODE_EAGER_FLUSH` the CLI writes out every queued transcript entry before yielding a
   *  result, so an Agent that settled before it can take its final attempt now. */
  private reconcileFlushedTerminalAgents(): void {
    for (const [taskId, pending] of this.pendingTerminalAgentEvents) {
      if (pending.flushed) continue
      clearTimeout(pending.timer)
      pending.flushed = true
      pending.attempts = 0
      pending.timer = setTimeout(() => this.reconcileTerminalAgentStats(taskId), 0)
      pending.timer.unref?.()
    }
  }
  ...
  private releaseTerminalTaskState(taskId: string): void {
    ...
    const pendingRefresh = this.agentTranscriptRefreshes.get(taskId)
    if (pendingRefresh) clearTimeout(pendingRefresh.timer)
  ...
  private handleSessionStateChangedSystemMessage(message: SDKSessionStateChangedMessage): void {
    ...
    // A drained background-agent loop releases the awaiting-reply hold before the release sequence.
    if (this.awaitingBackgroundReply && !this.membershipHasAgent) {
