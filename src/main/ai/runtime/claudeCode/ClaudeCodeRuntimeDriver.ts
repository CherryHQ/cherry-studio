...
class ClaudeCodeRuntimeConnection implements AgentRuntimeConnection {
  ...
  /** Staleness identity captured at start(); live facts are advanced in place by reconcile. */
  ...
  async start(): Promise<this> {
    ...
    // Capture the staleness identity AFTER materialization: the gateway key (if this route uses
    // it) has been generated by now, so the connect-time and reconcile-time derives read the same
    // persisted value. A failed derive leaves it unset — reconcile then reports 'rebuild'.
    const derived = await deriveConnectionConfig(this.input.sessionId, this.input.modelId)
    this.connectionConfig = derived.ok ? derived.config : undefined
  ...
  /**
   * Permission-mode applier shared by the policy push and reconcile. Awaits the SDK call FIRST,
   * mutates the snapshot only on success: the snapshot's `permissionMode` gates `canUseTool`, so
   * mutating it before the SDK confirms would leave `canUseTool` enforcing a mode the running query
   * never adopted (and on a rejection, a tightened mode silently abandoned).
   */
  private async applyPermissionMode(permissionMode: AgentPermissionMode | undefined): Promise<void> {
    if (!this.query) throw new Error('No live query to apply the permission mode to')
    // Unchanged mode → no SDK round-trip, no snapshot churn.
    if (this.toolPolicySnapshot?.getPermissionMode() === permissionMode) return
    await this.query.setPermissionMode(permissionMode ?? 'default')
    this.toolPolicySnapshot?.setPermissionMode(permissionMode)
  }
  ...
  private async reconcileOnce(input: { modelId: UniqueModelId }): Promise<AgentRuntimeReconcileResult> {
    ...
    // No baseline captured at start (derive failed then) — can't tell what this connection serves.
    ...
    if (!toolPolicyFactsEqual(baseline.live.toolPolicy, fresh.live.toolPolicy)) {
      try {
        ...
        await this.applyPermissionMode(
          (fresh.live.toolPolicy.permissionMode ?? undefined) as AgentPermissionMode | undefined
        )
      } catch (error) {
