...
// The declared window is the budget basis for every channel alike: channel identity is not
// evidence that the window is overstated, so no per-channel derating applies here. The uniform
// margin below covers estimator error only; a window that overstates the real provider limit is
// a catalog/config error — correct `contextWindow` to the channel's real value instead.
// Overstatement beyond this headroom (e.g. #18894: 256K declared / 128K real) cannot be told
// apart from an accurate config, so it is not budgeted for automatically.
