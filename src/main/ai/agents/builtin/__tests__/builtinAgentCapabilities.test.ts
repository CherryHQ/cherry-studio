...
describe('resolveAgentCapabilities', () => {
  ...
  it('keeps diagnostic draft preparation in the reusable channel-linked Support tool set', () => {
    ...
    expect(assistant.hostTools?.tools).toBeUndefined()
    expect(assistant.hostTools?.toolsInChannelSessions).toEqual([
      'navigate',
      'diagnose',
      'product_info',
      'apply_setting',
...
describe('hostToolsEnabled', () => {
  it.each(AGENT_TYPES)('keeps host tools available to channel-linked Cherry Assistant sessions on %s', (type) => {
    expect(hostToolsEnabled(agentOf(type, BUILTIN_AGENT_ROLE.ASSISTANT), { channelLinked: false })).toBe(true)
    expect(hostToolsEnabled(agentOf(type, BUILTIN_AGENT_ROLE.ASSISTANT), { channelLinked: true })).toBe(true)
  })
