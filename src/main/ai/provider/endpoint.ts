...
export function resolveEffectiveEndpoint(provider: Provider, model: Model): ResolvedEndpoint {
  // A claim can only come from an endpoint the provider declares, so an incomplete row cannot route
  // to a backend that isn't there — the claim and the connection config are the same object.
  const { endpointType, providerOptionsKey } = resolveModelEndpoint({
