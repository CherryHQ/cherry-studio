...
import { formatApiHost, withoutTrailingApiVersion, withoutTrailingSlash, SUPPORTED_ENDPOINT_LIST } from '@shared/utils/api'
...
const openAICompatibleFetcher: ModelFetcher = {
  ...
  fetch: async (provider, signal) => {
    ...
    const endpointMatch = SUPPORTED_ENDPOINT_LIST.find((ep) => rawBaseUrl.endsWith(ep))
    const host = endpointMatch ? rawBaseUrl.slice(0, rawBaseUrl.length - endpointMatch.length) : rawBaseUrl
