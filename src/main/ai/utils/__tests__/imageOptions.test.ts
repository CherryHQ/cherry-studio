...
import { buildImageProviderOptions, type ImageOptionParams } from '../imageOptions'

function params(overrides: Partial<ImageOptionParams> = {}): ImageOptionParams {
  return {
    imageSize: '1024x1024',
    ...overrides
  }
...
describe('buildImageProviderOptions', () => {
  ...
  it('maps personGeneration + imageSize (as imageConfig.imageSize) under the google key', () => {
    const result = buildImageProviderOptions(
      'google',
      params({ personGeneration: 'allow_adult' as ImageOptionParams['personGeneration'] })
    )
    expect(result).toEqual({
  ...
  it('lowercases the registry-uppercase personGeneration for the @ai-sdk/google schema', () => {
    const result = buildImageProviderOptions(
      'google',
      params({ imageSize: undefined, personGeneration: 'ALLOW_ALL' as ImageOptionParams['personGeneration'] })
    )
