...
describe('PersistentChatContextProvider — prepareBudgetContinue pending-flip safety', () => {
  beforeEach(() => {
    vi.clearAllMocks()
  })

  it('does NOT flip the anchor to pending when resolveCompactedHistory throws (row stays success)', async () => {
    // Simulate the scenario from bug #437: the assistant row was already persisted as `success`
    // by broadcastExecutionDone before the budget-continue resume was scheduled. If
    // resolveCompactedHistory (or its getPathToNode dependency) throws, the row must NOT be
    // flipped to `pending` — otherwise it strands as a permanently-spinning bubble on reload.
    const { messageService } = await import('@main/data/services/MessageService')

    // getById returns a valid assistant anchor row (status: 'success' from the prior turn).
    vi.mocked(messageService.getById).mockResolvedValue(
      fakeMsg('a1', 'assistant', 'partial answer from completed turn') as any
    )

    // getPathToNode (called inside resolveCompactedHistory) throws — e.g. SQLITE_BUSY.
    mockGetPathToNode.mockRejectedValue(new Error('SQLITE_BUSY: database is locked'))

    // resolveRequestContextSettings is called inside resolveCompactedHistory; let it return a
    // compression-off shape so the throw from getPathToNode is the first failure.
    mockResolveRequestContextSettings.mockResolvedValue({
      contextSettings: { enabled: false },
    ...
    await expect(
      provider.prepareDispatch(
        makeSubscriber(),
        { trigger: 'budget-continue', topicId: 'topic-1', parentAnchorId: 'a1' } as any,
        { hasLiveStream: false }
      )
    ).rejects.toThrow('SQLITE_BUSY')

    // The critical assertion: messageService.update must NOT have been called with status: 'pending'.
    // If it had been called, the success row would be stranded as pending forever.
    const updateCalls = vi.mocked(messageService.update).mock.calls
    const pendingFlips = updateCalls.filter(([_id, patch]) => (patch as Record<string, unknown>).status === 'pending')
