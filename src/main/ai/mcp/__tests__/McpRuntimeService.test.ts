...
describe('McpRuntimeService write quiesce', () => {
  beforeEach(() => {
    BaseService.resetInstances()
    MockMainCacheServiceUtils.resetMocks()
  })

  it('queues new runtime work outside the drain wait-set and admits it after resume', async () => {
    const service = new McpRuntimeService()
    const work = vi.fn().mockResolvedValue('done')
    const hold = service.pause('backup')

    const operation = (service as any).runRuntimeWork('tool:server-1', work) as Promise<string>
  ...
  it('drains runtime work and client initialization admitted before pause', async () => {
    const service = new McpRuntimeService()
    const tool = createDeferred<void>()
    const client = createDeferred<object>()
    const operation = (service as any).runRuntimeWork('tool:server-1', () => tool.promise) as Promise<void>
    ;(service as any).pendingClients.set(serverKeyFor('server-2'), client.promise)
    await Promise.resolve()
    const hold = service.pause('backup')

    let settled = false
    const drain = service.drainInFlight({ timeoutMs: 5_000 }).finally(() => {
      settled = true
    })
    await Promise.resolve()
    ...
    expect(service.listActiveWork()).toEqual(
      expect.arrayContaining([
        { id: 'tool:server-1', summary: 'MCP runtime work in flight' },
        { id: 'client-init:server-2', summary: 'MCP client initializing' }
      ])
    )

    tool.resolve(undefined)
    client.resolve({})
    await operation
