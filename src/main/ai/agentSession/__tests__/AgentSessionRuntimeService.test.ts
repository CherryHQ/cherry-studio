...
describe('AgentSessionRuntimeService', () => {
  ...
  describe('background tasks', () => {
    ...
    it('acknowledges a replayed orphan only after the row is durably persisted', async () => {
      // Reset the trio outright — earlier cases leave implementations behind (clearAllMocks
      // keeps them), which would otherwise decide this case's lookups.
      mocks.findFlowHostMessageId.mockReset()
      mocks.getSessionMessage.mockReset()
      mocks.cacheGetPersist.mockReset()
      mocks.replaceMessageParts.mockReset()
      // The persisted batch stays until the flush lands the chunks in the message row, so a
      // crash before persistence replays it again instead of losing it.
      mocks.findFlowHostMessageId.mockImplementation(() => 'assistant-1')
      const orphanChunk = { type: 'text-start', id: 'orphaned-text' }
      const orphannedAt = Date.now()
      mocks.cacheGetPersist.mockImplementation((key: string) =>
        key === 'agent.session.flow_recovery_orphans'
          ? [{ sessionId: 'session-1', rootToolCallId: 'task-root-ack', orphannedAt, chunks: [orphanChunk] }]
          : undefined
      )
      mocks.getSessionMessage.mockImplementation(() => ({
        id: 'assistant-1',
      ...
      // The durable copy is dropped only once the row committed.
      const cleared = mocks.cacheSetPersist.mock.calls.find(
        ([key, value]) => key === 'agent.session.flow_recovery_orphans' && Array.isArray(value) && value.length === 0
      )
