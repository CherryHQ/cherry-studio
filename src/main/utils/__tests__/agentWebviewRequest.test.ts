...
import { AgentDevPreviewRequestPolicy, AgentHtmlArtifactRequestPolicy } from '../agentWebviewRequest'
...
describe('AgentHtmlArtifactRequestPolicy', () => {
  ...
  it('allows public remote assets while blocking local network access', async () => {
    const policy = new AgentHtmlArtifactRequestPolicy()
    await policy.isAllowed(request(artifactUrl, 'mainFrame'))

    await expect(policy.isAllowed(request('https://cdn.example.com/app.js', 'script'))).resolves.toBe(true)
    await expect(policy.isAllowed(request('http://cdn.example.com/legacy.js', 'script'))).resolves.toBe(true)
    await expect(policy.isAllowed(request('ws://cdn.example.com/live', 'webSocket'))).resolves.toBe(true)
