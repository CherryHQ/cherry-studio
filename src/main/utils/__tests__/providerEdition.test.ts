...
const provider = (availableInEditions?: AppEdition[]): Provider => ({ availableInEditions }) as Provider
...
describe('isProviderAvailableInEdition', () => {
  ...
  ])('%s', (_label, availableInEditions, edition, expected) => {
    expect(isProviderAvailableInEdition(provider(availableInEditions), edition)).toBe(expected)
  })
})
