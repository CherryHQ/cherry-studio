...
export async function registerIpc(
  ...
  ipcMain.handle(
    ...
    async (_event, payload) => {
      try {
        ...
        logger.error(
          "Failed to persist agent session messages",
          error as Error,
        );
        throw error;
      }
  ...
  ipcMain.handle(IpcChannel.System_GetGitBashPath, () => {
    ...
    const customPath = configManager.get(ConfigKeys.GitBashPath) as
      | string
      | undefined;
  ...
  ipcMain.handle(
    IpcChannel.System_SetGitBashPath,
    (_, newPath: string | null) => {
      if (!isWin) {
        return false;
      }
  ...
  ipcMain.handle(
    IpcChannel.Backup_CreateLanTransferBackup,
    backupManager.createLanTransferBackup.bind(backupManager),
  );
  ipcMain.handle(
    IpcChannel.Backup_DeleteLanTransferBackup,
    backupManager.deleteLanTransferBackup.bind(backupManager),
  );
  ...
  ipcMain.handle(
    ...
    fileManager.isDirectory.bind(fileManager),
  );
  ipcMain.handle(
    IpcChannel.File_ListDirectory,
    fileManager.listDirectory.bind(fileManager),
  );
  ipcMain.handle(
  ...
  ipcMain.handle(
    IpcChannel.File_PauseWatcher,
    fileManager.pauseFileWatcher.bind(fileManager),
  );
  ipcMain.handle(
    IpcChannel.File_ResumeWatcher,
    fileManager.resumeFileWatcher.bind(fileManager),
  );
  ipcMain.handle(
    IpcChannel.File_BatchUploadMarkdown,
    fileManager.batchUploadMarkdownFiles.bind(fileManager),
  );
  ipcMain.handle(
    IpcChannel.File_ShowInFolder,
    fileManager.showInFolder.bind(fileManager),
  );

  // pdf
  ipcMain.handle(
    IpcChannel.Pdf_ExtractText,
    (_, data: Uint8Array | ArrayBuffer | string) => extractPdfText(data),
  );
  ...
  // memory
  ipcMain.handle(IpcChannel.Memory_Add, (_, messages, config) =>
    memoryService.add(messages, config),
  );
  ipcMain.handle(IpcChannel.Memory_Search, (_, query, config) =>
    memoryService.search(query, config),
  );
  ipcMain.handle(IpcChannel.Memory_List, (_, config) =>
    memoryService.list(config),
  );
  ipcMain.handle(IpcChannel.Memory_Delete, (_, id) => memoryService.delete(id));
  ipcMain.handle(IpcChannel.Memory_Update, (_, id, memory, metadata) =>
    memoryService.update(id, memory, metadata),
  );
  ipcMain.handle(IpcChannel.Memory_Get, (_, memoryId) =>
    memoryService.get(memoryId),
  );
  ipcMain.handle(IpcChannel.Memory_SetConfig, (_, config) =>
    memoryService.setConfig(config),
  );
  ipcMain.handle(IpcChannel.Memory_DeleteUser, (_, userId) =>
    memoryService.deleteUser(userId),
  );
  ipcMain.handle(IpcChannel.Memory_DeleteAllMemoriesForUser, (_, userId) =>
    memoryService.deleteAllMemoriesForUser(userId),
  );
  ipcMain.handle(IpcChannel.Memory_GetUsersList, () =>
    memoryService.getUsersList(),
  );
  ...
  ipcMain.handle(
    IpcChannel.Mcp_ResolveHubTool,
    async (_event, nameOrId: string) => {
      const { resolveHubToolName } = await import(
        "@main/mcpServers/hub/mcp-bridge"
      );
  ...
  // CherryIN OAuth
  ipcMain.handle(
    IpcChannel.CherryIN_SaveToken,
    CherryINOAuthService.saveToken.bind(CherryINOAuthService),
  );
  ipcMain.handle(
    IpcChannel.CherryIN_HasToken,
    CherryINOAuthService.hasToken.bind(CherryINOAuthService),
  );
  ipcMain.handle(
    IpcChannel.CherryIN_GetBalance,
    CherryINOAuthService.getBalance.bind(CherryINOAuthService),
  );
  ipcMain.handle(
    IpcChannel.CherryIN_Logout,
    CherryINOAuthService.logout.bind(CherryINOAuthService),
  );
  ipcMain.handle(
    IpcChannel.CherryIN_StartOAuthFlow,
    CherryINOAuthService.startOAuthFlow.bind(CherryINOAuthService),
  );
  ipcMain.handle(
    IpcChannel.CherryIN_ExchangeToken,
    CherryINOAuthService.exchangeToken.bind(CherryINOAuthService),
  );
  ...
  // search window
  ipcMain.handle(
    IpcChannel.SearchWindow_Open,
    async (_, uid: string, show?: boolean) => {
  ...
  ipcMain.handle(
    IpcChannel.Webview_PrintToPDF,
    async (_, webviewId: number) => {
  ...
  ipcMain.handle(
    IpcChannel.App_SetUseSystemTitleBar,
    (_, isActive: boolean) => {
  ...
  // ExternalApps
  ipcMain.handle(IpcChannel.ExternalApps_DetectInstalled, () =>
    externalAppsService.detectInstalledApps(),
  );
  ...
  if (isOvmsSupported) {
    ...
    if (ovmsManager) {
      ...
      ipcMain.handle(IpcChannel.Ovms_IsRunning, () =>
        ovmsManager.initializeOvms(),
      );
      ipcMain.handle(IpcChannel.Ovms_GetStatus, () =>
        ovmsManager.getOvmsStatus(),
      );
      ipcMain.handle(IpcChannel.Ovms_RunOVMS, () => ovmsManager.runOvms());
      ipcMain.handle(IpcChannel.Ovms_StopOVMS, () => ovmsManager.stopOvms());
    } else {
      logger.error(
        "Unexpected behavior: undefined ovmsManager, but OVMS should be supported.",
      );
    }
  } else {
    const fallback = () => {
  ...
  ipcMain.handle(
    IpcChannel.ClaudeCodePlugin_UninstallPackage,
    async (_, options) => {
      try {
  ...
  ipcMain.handle(
    IpcChannel.ClaudeCodePlugin_ListInstalled,
  ...
  ipcMain.handle(
    IpcChannel.ClaudeCodePlugin_WriteContent,
    async (_, options) => {
      try {
        await pluginService.writeContent(
          options.agentId,
          options.filename,
          options.type,
          options.content,
  ...
  ipcMain.handle(
    IpcChannel.ClaudeCodePlugin_InstallFromZip,
    async (_, options) => {
      try {
  ...
  ipcMain.handle(
    IpcChannel.ClaudeCodePlugin_InstallFromDirectory,
    async (_, options) => {
      try {
  ...
  ipcMain.handle(IpcChannel.LocalTransfer_ListServices, () =>
    localTransferService.getState(),
  );
  ipcMain.handle(IpcChannel.LocalTransfer_StartScan, () =>
    localTransferService.startDiscovery({ resetList: true }),
  );
  ipcMain.handle(IpcChannel.LocalTransfer_StopScan, () =>
    localTransferService.stopDiscovery(),
  );
  ipcMain.handle(
    IpcChannel.LocalTransfer_Connect,
    (_, payload: LocalTransferConnectPayload) =>
      lanTransferClientService.connectAndHandshake(payload),
  );
  ipcMain.handle(IpcChannel.LocalTransfer_Disconnect, () =>
    lanTransferClientService.disconnect(),
  );
  ipcMain.handle(
    IpcChannel.LocalTransfer_SendFile,
    (_, payload: { filePath: string }) =>
      lanTransferClientService.sendFile(payload.filePath),
  );
  ipcMain.handle(IpcChannel.LocalTransfer_CancelTransfer, () =>
    lanTransferClientService.cancelTransfer(),
  );
  ...
  ipcMain.handle(
    ...
    openClawService.checkInstalled,
  );
  ...
  ipcMain.handle(
    ...
    openClawService.getDashboardUrl,
  );
  ipcMain.handle(
    IpcChannel.OpenClaw_SyncConfig,
    openClawService.syncProviderConfig,
  );
  ipcMain.handle(
    IpcChannel.OpenClaw_GetChannels,
    openClawService.getChannelStatus,
  );
  ipcMain.handle(IpcChannel.OpenClaw_CheckUpdate, openClawService.checkUpdate);
  ipcMain.handle(
    IpcChannel.OpenClaw_PerformUpdate,
    openClawService.performUpdate,
  );

  // Analytics
  ipcMain.handle(
    IpcChannel.Analytics_TrackTokenUsage,
    (_, data: TokenUsageData) => analyticsService.trackTokenUsage(data),
  );
}
