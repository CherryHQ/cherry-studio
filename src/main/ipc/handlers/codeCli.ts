...
export const codeCliHandlers: IpcHandlersFor<typeof codeCliRequestSchemas> = {
  ...
  'code_cli.mcode_provider.apply': async (input) => {
    return application.get('CodeCliService').applyMiniMaxCodeProvider(input)
  },
  'code_cli.mcode_provider.clear': async () => {
    return application.get('CodeCliService').clearMiniMaxCodeProviders()
  },
  'code_cli.mcode_provider.activate_official': async () => {
    return application.get('CodeCliService').activateMiniMaxCodeOfficial()
  },
