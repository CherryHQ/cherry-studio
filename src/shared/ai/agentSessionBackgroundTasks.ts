...
export function mergeAgentSessionTaskEvent(
  ...
  if (superseded) {
    ...
    delete merged.error
  } else if (incoming.synthetic === true && isTerminalAgentSessionTaskStatus(existing.status)) {
    // The reverse never holds either: a late guess cannot demote an outcome the runtime reported.
    return merged
  }
  const existingIsTerminal = !superseded && isTerminalAgentSessionTaskStatus(existing.status)
