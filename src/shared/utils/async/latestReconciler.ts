...
export interface LatestReconcilerOptions<T> {
  ...
  /**
   * Invoked when `getSnapshot` or `apply` throws. The owner supplies error reporting.
...
export function createLatestReconciler<T>(options: LatestReconcilerOptions<T>): LatestReconciler {
  const { getSnapshot, isSettled, apply, onError } = options

  /** A convergence loop is currently running. Guarantees single-flight `apply`. */
  let running = false
  // Clear before reading so requests during a snapshot or apply cannot be lost.
  let dirty = false
  ...
  const runLoop = async (): Promise<void> => {
    ...
    try {
      while (true) {
        ...
        // A request during the snapshot read supersedes it; re-read before acting.
        if (dirty) continue
