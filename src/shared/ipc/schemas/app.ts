...
export const appRequestSchemas = {
  ...
  // `temp.data_reset` marker and relaunches; runDataReset performs the
  // wipe at preboot on the next boot. Resolving void is ambiguous (user
  // cancelled at the native dialog, or the relaunch killed this process
  // first) — callers must not rely on it. A *rejection* is meaningful: the
  // marker could not be persisted and no relaunch was triggered.
