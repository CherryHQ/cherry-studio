...
export interface UseConversationTurnControllerOptions<TInput, TConversation> {
  ...
  openStream?: (input: TInput, request: AiStreamOpenRequest) => Promise<AiStreamOpenResponse>
  ...
  const send = useCallback(
    async (input: TInput): Promise<boolean> => {
      ...
      try {
        ...
        const ack = openStream ? await openStream(input, request) : await ipcApi.request('ai.stream.open', request)
