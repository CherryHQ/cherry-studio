...
export type AvatarMutationInput =
  | { kind: 'image'; data: Uint8Array<ArrayBufferLike> }
  | { kind: 'emoji'; emoji: string }

function cloneImageData(avatar: AvatarMutationInput): SetAvatarIntent {
...
export function useAvatarMutations() {
  ...
  const setAssistantAvatar = useCallback(
    async (assistantId: string, avatar: AvatarMutationInput): Promise<Assistant> => {
  ...
  const setAgentAvatar = useCallback(
    async (agentId: string, avatar: AvatarMutationInput): Promise<AgentEntity> => {
