...
export interface MarkdownHost {
  ...
  /** Export the rendered table rows to an Excel file; returns whether it succeeded. */
  ...
  /** Render an HTML code fence as an inline artifact preview (chat immersive HTML). The
   *  renderer supplies the code-block-derived context (stable id, editability, save handler)
   *  so the host stays free to classify/route it. When absent, HTML fences fall back to the
   *  default `HtmlArtifactsCard`. */
  renderHtmlArtifact?: (
    html: string,
    options: { isStreaming: boolean; artifactId: string; editable: boolean; onSave: (html: string) => void }
  ) => ReactNode
}
