...
export function useAssistantPresetCreation({ enabled = true }: { enabled?: boolean } = {}) {
  ...
  const { models, isLoading: modelsLoading } = useModels(undefined, { fetchEnabled: enabled })
  const { providers, isLoading: providersLoading } = useProviders(undefined, { enabled })
  ...
  const resolvePreset = useCallback(
    ...
    [defaultModelId, isLoading, models, providers]
  )
  ...
  return { createFromPreset, resolvePreset, isLoading }
}
