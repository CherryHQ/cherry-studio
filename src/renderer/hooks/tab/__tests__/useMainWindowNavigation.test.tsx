...
describe('useMainWindowNavigation', () => {
  ...
  it('focuses the translate session named by a completion notification route', () => {
    ...
    mocks.ipcListeners.get('navigation.open_route_requested')?.({
      to: '/app/translate?sessionId=translate-1'
    })
  ...
  it('opens a fresh translate tab when the session id is already owned by another route', () => {
    mocks.tabs = [{ id: 'closed-session', type: 'route', url: '/app/knowledge', title: 'Knowledge' }]
    render(<MainWindowNavigationHarness />)
    ...
    expect(mocks.openRoute).toHaveBeenCalledWith('/app/translate?sessionId=closed-session', { forceNew: true })
    expect(mocks.setActiveTab).not.toHaveBeenCalled()
  })

  it('reuses a translate tab whose routed session id collided with another tab id', () => {
    mocks.tabs = [
      { id: 'closed-session', type: 'route', url: '/app/knowledge', title: 'Knowledge' },
      {
        id: 'replacement-tab',
        type: 'route',
        url: '/app/translate?sessionId=closed-session',
        title: 'Translate'
      }
    ]
    render(<MainWindowNavigationHarness />)
    ...
    mocks.ipcListeners.get('navigation.open_route_requested')?.({
      to: '/app/translate?sessionId=closed-session&historyId=history-1'
    })

    expect(mocks.updateTab).toHaveBeenCalledWith('replacement-tab', {
      url: '/app/translate?sessionId=closed-session&historyId=history-1',
      lastAccessTime: expect.any(Number)
    })
    expect(mocks.setActiveTab).toHaveBeenCalledWith('replacement-tab')
    expect(mocks.openRoute).not.toHaveBeenCalled()
  })
