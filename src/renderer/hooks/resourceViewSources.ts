...
/**
 * Shared page-level data sources for the new-view layout.
 *
 * In new view the entity rail and the right-panel resource list are two separate components that
...
/** The shared full-topics source for the assistant new-view rail + right-panel topic list. */
export function useAssistantTopicsSource() {
  return useTopics({ loadAll: true })
}

/** The shared full-sessions source for the agent new-view rail + right-panel session list. */
