...
const useTestMiniAppPopup = () => {
  ...
  return {
    ...popup,
    // Mutations from useMiniApps; the popup surface no longer owns close paths
    exitMiniApp: miniApps.exitMiniApp,
...
describe('useMiniAppPopup', () => {
  ...
  describe('split pane', () => {
    ...
    it('should not evict the split app when another app opens under cap pressure', async () => {
      ...
      await act(async () => {
        result.current.openMiniAppKeepAlive(createMiniApp('third-app'))
      })
    ...
    it('should drop the split pane when its app is closed', async () => {
      ...
      await act(async () => {
        result.current.exitMiniApp('right-app')
      })
    ...
    it('should keep the split pane when the app closed is not the one in it', async () => {
      ...
      await act(async () => {
        result.current.exitMiniApp('left-app')
      })
    ...
    it('should collapse the split when every mini app closes', async () => {
      ...
      await act(async () => {
        for (const app of getKeepAlive()) {
          result.current.exitMiniApp(app.appId)
        }
      })
