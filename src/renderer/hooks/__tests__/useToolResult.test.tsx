...
describe('useToolResult', () => {
  ...
  it('re-reads the result on an interval only while the caller asks for one', async () => {
    vi.useFakeTimers()
    try {
      ipcRequestMock.mockResolvedValue({ found: true, output: 'growing output' })
      const initialProps: { intervalMs?: number } = { intervalMs: 1_000 }
      const { rerender } = renderHook(
        ({ intervalMs }: { intervalMs?: number }) =>
          useToolResult(TOOL_REF, { refreshToken: 'running:1', refreshIntervalMs: intervalMs }),
  ...
  it('does not stack interval reads behind a read that is still in flight', async () => {
    vi.useFakeTimers()
    try {
      const slow = deferResult()
      ipcRequestMock
        .mockResolvedValueOnce({ found: true, output: 'first output' })
        .mockReturnValueOnce(slow.promise)
        .mockResolvedValue({ found: true, output: 'latest output' })
      const { result } = renderHook(() => useToolResult(TOOL_REF, { refreshIntervalMs: 1_000 }), {
        wrapper: createWrapper()
      })
      await act(async () => undefined)
      expect(ipcRequestMock).toHaveBeenCalledTimes(1)
      ...
      await act(async () => slow.resolve({ found: true, output: 'slow output' }))
      await act(async () => {
        await vi.advanceTimersByTimeAsync(1_000)
      })
      expect(ipcRequestMock).toHaveBeenCalledTimes(3)
      expect(result.current.output).toBe('latest output')
    } finally {
      vi.useRealTimers()
    }
  })
