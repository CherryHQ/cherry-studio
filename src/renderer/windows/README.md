...
- **Main-only** concerns stay in `MainWindowRuntime`, explicitly outside `useWindowRuntime`: the boot spinner + `init` timer teardown (paired with markup only `main/index.html` creates), `useAppUpdateHandler`, `useStorageMonitorNotification`, `useTopicNamingErrorNotification`, `useBackupRestoreNotice` (main-window-targeted toasts that must not duplicate across windows).
