 ...
 * Only the Blob wrapper is adapted for jsdom's FileReader; the PNG encode itself is real,
 * which is the point — an assertion that "an image rendered" would not notice an export
...
function makeCanvas(width: number, height: number) {
  ...
  Object.defineProperty(canvas, 'toBlob', {
    value: (callback: (blob: Blob | null) => void) => {
      callback(new Blob([new Uint8Array(canvas.encodeSync('png'))], { type: 'image/png' }))
    }
  })
  return canvas
}
...
async function decode(dataUrl: string) {
  const image = await loadImage(Buffer.from(dataUrl.slice(dataUrl.indexOf(',') + 1), 'base64'))
...
describe('generateSelectionDataUrl', () => {
  ...
  it('exports a real PNG cropped at physical resolution', async () => {
    const dataUrl = await generateSelectionDataUrl(await makeSourceImage(), { x: 50, y: 25, width: 100, height: 50 }, 2)

    expect(dataUrl).toMatch(/^data:image\/png;base64,/)

    const png = await decode(dataUrl)
