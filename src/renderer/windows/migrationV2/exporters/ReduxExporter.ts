...
export class ReduxExporter {
  ...
  private async writeSlice(
    ...
    const flush = async (force = false) => {
      if (!force && pending.length < EXPORT_CHUNK_CHAR_LIMIT) return
      if (!pending && !force) return

      const requestedEnd = Math.min(EXPORT_CHUNK_CHAR_LIMIT, pending.length)
      const end = clampSurrogateBoundary(pending, requestedEnd)
      if (end === 0 && pending.length > 0) return
