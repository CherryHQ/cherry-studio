...
- The diagnostic panel appears only for migration errors and version-incompatible blocks (including a renderer export failure after Main records it). Saving sends only the current migration-window locale: Main chooses the destination, builds the bundle, and retains the most recently saved path.
- While a diagnostic save is active, controls that could exit or replace the failure state are disabled, and Main independently rejects state-changing IPC. Closing waits for the save to settle without a timeout. The saved notice reports the log result, states that the bundle was not transmitted automatically, and then emphasizes that the user must attach the complete bundle manually; large-file guidance stays with the attachment reminder.
