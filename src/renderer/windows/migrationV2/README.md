...
- While a diagnostic save is active, controls that could exit or replace the failure state are disabled, and Main independently rejects state-changing IPC. Closing waits for the save to settle without a timeout. The saved notice uses one line for the log result and a second, emphasized line reminding the user to attach the complete bundle manually; large-file guidance stays on that second line.
