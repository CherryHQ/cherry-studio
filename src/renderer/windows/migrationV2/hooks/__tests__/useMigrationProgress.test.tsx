...
describe('useMigrationActions diagnostics', () => {
  ...
  it('passes the current migration window locale when saving diagnostics', async () => {
    const { result } = renderHook(() => useMigrationActions())

    await act(async () => {
      await result.current.saveDiagnostics('zh-CN')
    })

    expect(invoke).toHaveBeenCalledOnce()
    expect(invoke).toHaveBeenCalledWith(MigrationIpcChannels.SaveDiagnosticBundle, { locale: 'zh-CN' })
  })
