...
function AgentFlowHeaderTitle({
  ...
  return (
    <div className="flex min-w-0 items-center gap-0.5">
      ...
      <span title={tab.title} className="min-w-0 flex-1 truncate px-1">
        {tab.title}
      </span>
      {tab.agentName ? (
        <>
...
function AgentFlowRightPanel({ active, panelId, scope }: RightPanelComponentProps<AgentRightPanelScope>) {
  ...
  const task = status.runTasks.find((runTask) => runTask.toolUseId === tab.toolCallId)
  const statusLabels = {
      ...
      {phases.map((phase) => (
        <AccordionItem
          ...
          <AccordionContent className="px-2.5 pt-1 pb-2">
            {/* The floor keeps five numeric columns legible; above it the table tracks the pane width,
                so a normally sized sidebar shows every column instead of hiding the last one. */}
            <div className="overflow-x-auto pb-1 [scrollbar-color:var(--scrollbar-thumb-hover)_transparent]">
              <table className="w-full min-w-[200px] table-fixed border-collapse text-[11px] leading-5">
...
function ShellRunTaskCard({
  ...
  const { output: resolvedOutput } = useToolResult(shouldResolveDeferredOutput ? deferredResultRef : undefined, {
    ...
    // While the command runs the version above never changes, yet the deferred file keeps growing.
    refreshIntervalMs: task.status === 'in_progress' ? RUNNING_DEFERRED_OUTPUT_POLL_MS : undefined
  })
...
// A session's run history only grows: every finished shell, agent and workflow keeps its row. Rows
// therefore mount through a virtual window. The cap only has to keep a runaway history from burying
// the rest of the pane, so it tracks the window rather than a fixed height — a short group renders
// at its natural height, uncapped in practice.
const RUN_TASK_ROW_ESTIMATE_PX = 64
const RUN_TASK_ROW_STYLE: CSSProperties = { paddingBottom: 6 }
const RUN_TASK_LIST_STYLE: CSSProperties = { height: 'auto', maxHeight: '65vh' }
const estimateRunTaskRow = () => RUN_TASK_ROW_ESTIMATE_PX

/** How often an expanded, still-running shell re-reads its deferred output. */
const RUNNING_DEFERRED_OUTPUT_POLL_MS = 1000
...
const AgentRunActivitySection = memo(function AgentRunActivitySection({
  ...
  const renderTask = useCallback(
    ...
    [durationFormatter, sessionId, workflowDurationFormatter]
  )
  const getItemKey = useCallback((index: number) => tasks[index]?.id ?? index, [tasks])
  ...
  return (
    <AccordionItem value={value} role="region" aria-label={label} className="border-0 first:border-t-0 last:border-b-0">
      ...
      <AccordionContent className="pt-2 pb-0">
        {/* A row that expands animates its own height, so the list has to reserve the scrollbar gutter:
            otherwise the 6px scrollbar toggles mid-animation and rewraps every card. */}
        <DynamicVirtualList
          list={tasks}
          estimateSize={estimateRunTaskRow}
          getItemKey={getItemKey}
          itemContainerStyle={RUN_TASK_ROW_STYLE}
          scrollerStyle={RUN_TASK_LIST_STYLE}
  ...
  AGENT_TRACE_PANE_CAPABILITY,
  {
    ...
    resolve: (scope) => {
      ...
      return {
        ...
        title: <AgentFlowHeaderTitle tab={tab} previousTab={scope.previousFlowTab} goBack={scope.goBackFlow} />,
        readiness: scope.meta.conversationState
      }
    }
  }
