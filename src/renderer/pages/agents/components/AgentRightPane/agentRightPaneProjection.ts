...
export function buildAgentToolFlowProjection(
  ...
  if (selectedToolCallIds.size) {
    ...
    if (selectedOutputText) {
      if (isInternalAgentLaunchReceipt(selectedOutputText)) {
        launchReceipt = selectedOutputText
      } else if (!isBackgroundAgentLaunchReceipt(selectedOutput, selectedOutputText)) {
        const separated = splitAgentCompletionReceipt(selectedOutputText)
