...
describe('agent right pane projections', () => {
  ...
  it('shows a task as completed when its real terminal edge follows a synthesized interruption', () => {
    const interrupted = [
      {
        type: 'data-agent-task-event',
        data: {
    ...
    // The command outlived the restart and reported its real outcome in a later delivery message.
    const completed = [
      {
        type: 'data-agent-task-event',
        data: {
          ...
          taskId: 'bg-1',
          status: 'completed',
          completedAt: '2026-05-21T00:00:30.000Z'
        }
      }
    ] as unknown as CherryMessagePart[]

    const status = buildAgentRightPaneStatus([message('m1', interrupted), message('m2', completed)], {
      m1: interrupted,
      m2: completed
    })

    expect(status.runTasks).toEqual([
      expect.objectContaining({ id: 'bg-1', status: 'completed', completedAt: '2026-05-21T00:00:30.000Z' })
    ])
  })
