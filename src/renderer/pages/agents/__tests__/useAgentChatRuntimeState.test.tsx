...
describe('useAgentChatRuntimeState', () => {
  ...
  it('removes temporary AskUserQuestion answers when the persisted result is deferred', async () => {
    const part = makeAskUserQuestionPart()
    const { result, rerender } = renderHook(() =>
      useAgentChatRuntimeState({
        sessionId: 'session-1',
    ...
    expect(result.current.optimisticAskUserQuestionInputsByToolCallId).toHaveProperty('call-ask')
