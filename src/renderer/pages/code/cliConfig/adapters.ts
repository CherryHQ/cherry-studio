...
const codexAdapter: CliConfigAdapter = {
  ...
  async buildDraft(args, context) {
    ...
    const [config, auth] = await Promise.all([
      readAndParseDraftFile('codex-config', parseTomlOrThrow, args.files),
      readAndParseDraftFile('codex-auth', parseJsonOrThrow, args.files)
    ])
  ...
  async buildClearFiles() {
    const [existing, existingAuth] = await Promise.all([
      resolveAbs(CODEX_CONFIG_PATH).then((path) => readValidatedTomlOrNull(path, 'Codex config')),
      resolveAbs(CODEX_AUTH_PATH).then((path) => readValidatedJsonOrNull(path, 'Codex auth'))
    ])
...
const geminiAdapter: CliConfigAdapter = {
  ...
  async buildDraft(args, context) {
    ...
    const [envText, settings] = await Promise.all([
      readDraftFileText('gemini-env', args.files),
      readAndParseDraftFile('gemini-settings', parseJsonOrThrow, args.files)
    ])
  ...
  async buildClearFiles() {
    ...
    const [envText, settings] = await Promise.all([
      ...
      resolveAbs(GEMINI_SETTINGS_PATH).then((path) => readValidatedJsonOrNull(path, 'Gemini CLI settings'))
    ])
    if (envText !== null) {
