...
describe('TranslatePage', () => {
  ...
  it('waits for a pending history restore before reporting OCR success', async () => {
    fileMock.onSelectFile.mockResolvedValue([{ path: '/tmp/image.png', size: 10, type: 'image' }])
    const { persistLanguages, resolvePersist } = createDeferredLanguagePersist()
  ...
  it('keeps a newer OCR job visible when a cancelled completion settles', async () => {
    fileMock.onSelectFile.mockResolvedValue([{ path: '/tmp/image.png', size: 10, type: 'image' }])
    fileMock.startJob
      .mockResolvedValueOnce({ id: 'job-ocr-1', type: 'file-processing.background', status: 'pending' })
      .mockResolvedValueOnce({ id: 'job-ocr-2', type: 'file-processing.background', status: 'pending' })
    const persistLanguages = vi.fn(
      () =>
        new Promise<void>((_resolve, reject) => {
          rejectPersist = () => reject(new Error('save failed'))
    ...
    await MockUsePreference.useMultiplePreferences.withImplementation(
      ...
      async () => {
        ...
        useJobMock.mockImplementation((jobId: string) =>
          jobId === 'job-ocr-1'
            ? {
                data: {
                  id: jobId,
                  type: 'file-processing.background',
                  status: 'completed',
                  output: { artifact: { kind: 'text', format: 'plain', text: 'old OCR text' } }
                },
                isTerminal: true
              }
            : { data: undefined, isTerminal: false }
        )
        rerender(<TranslatePage />)
        fireEvent.click(screen.getByRole('button', { name: 'common.cancel' }))

        fireEvent.click(screen.getByRole('button', { name: 'translate.files.upload' }))
        await waitFor(() => expect(fileMock.startJob).toHaveBeenCalledTimes(2))
        await waitFor(() => expect(useJobMock).toHaveBeenCalledWith('job-ocr-2'))

        await act(async () => rejectPersist())
        rerender(<TranslatePage />)
        expect(screen.getByTestId('translate-input-ocr-processing')).toBeInTheDocument()
  ...
  it('keeps buffered translation output when a later history restore fails', async () => {
    smoothStreamMock.deferUpdates = true
    MockUsePreferenceUtils.setMultiplePreferenceValues({
      'feature.translate.model_id': 'openai::gpt-4.1',
      'feature.translate.page.source_language': 'zh-cn'
