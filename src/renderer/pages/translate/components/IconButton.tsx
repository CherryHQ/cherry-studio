...
const toneClass = (tone: IconButtonTone, active: boolean): string => {
  if (tone === 'destructive') {
    return 'text-foreground-tertiary hover:bg-accent hover:text-destructive'
  }
  if (tone === 'star') {
    return active ? 'text-amber-500 bg-amber-500/10' : 'text-foreground-tertiary hover:bg-accent hover:text-amber-500'
  }
  return active ? 'bg-accent text-foreground' : 'text-foreground-tertiary hover:bg-accent hover:text-foreground'
}
