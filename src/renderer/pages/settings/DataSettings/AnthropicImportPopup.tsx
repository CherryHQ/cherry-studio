  ...
  const onOk = async () => {
    ...
    try {
      ...
      const file = await window.api.file.open({
        filters: [{ name: t('import.claude.title'), extensions: ['json'] }]
      })
      ...
      if (result.success) {
        ...
        toast.error(result.error || t('import.claude.error.unknown'))
      }
    } catch (error) {
      logger.error('Claude import failed:', error as Error)
      toast.error(t('import.claude.error.unknown'))
      resolve({})
    } finally {
      setSelecting(false)
      setImporting(false)
...
const AnthropicImportPopup = createPopup<Record<string, never>, PopupResult>(PopupContainer, { dismissResult: {} })

export default AnthropicImportPopup
