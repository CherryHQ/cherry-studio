...
describe('BackupSettings', () => {
  ...
  it('ignores forged compatibility data and falls back to the plain sentence', async () => {
    ...
    await waitFor(() => expect(toast.error).toHaveBeenCalledWith('settings.data.backup.compatibility.ahead_title'))
    expect(popup.confirm).not.toHaveBeenCalled()
    expect(popup.info).not.toHaveBeenCalled()
  ...
  it.each(Object.values(backupErrorCodes).filter((code) => !GENERIC_CODES.includes(code)))(
    ...
    async (code) => {
      ...
      expect(toast.error).not.toHaveBeenCalledWith('settings.data.backup.error.unexpected')
    }
  )
  ...
  describe('cancelling work in flight', () => {
    ...
    it('holds the dialog stop button on "Stopping…" until the operation unwinds', async () => {
      ...
      await renderSettings()
      click(EXPORT_BUTTON)
      await waitFor(() => expect(screen.getByText('settings.data.backup.progress.stop')).toBeInTheDocument())

      click('settings.data.backup.progress.stop')

      await waitFor(() =>
        expect(screen.getByRole('button', { name: 'settings.data.backup.progress.cancelling' })).toBeDisabled()
      )
      stalled.finish({ status: 'canceled' })
      await waitFor(() => expect(screen.getByRole('button', { name: EXPORT_BUTTON })).toBeEnabled())
      expect(screen.queryByText('settings.data.backup.progress.cancelling')).not.toBeInTheDocument()
    })
