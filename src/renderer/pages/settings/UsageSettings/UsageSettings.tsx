...
import type { AiUsageRecordModality, AiUsageRecordSourceType } from '@shared/data/types/aiUsageRecord'
...
const WINDOW_KEYS = ['30d', '90d', '365d'] as const
...
const EMPTY_TIMELINE_BUCKETS: AiUsageRecordTimelineBucket[] = []
const EMPTY_STATS_BUCKETS: AiUsageRecordStatsBucket[] = []
const EMPTY_STATS_METRICS: AiUsageRecordStatsMetrics = {
  costCurrency: null,
  totalCost: 0,
  totalInputTokens: 0,
  totalOutputTokens: 0,
...
const MODALITY_LABEL_KEYS: Record<AiUsageRecordModality, string> = {
...
function getWindowRange(windowKey: WindowKey): BoundedTimeRange {
...
function getPreviousWindowRange(windowKey: WindowKey): BoundedTimeRange {
  const days = getWindowDays(windowKey)
...
function rangeFromDateKey(value: string): BoundedTimeRange {
...
function applyTimelineCurrency(
...
/** Stable display identity; null-key auth and unattributed records stay separate. */
function getBucketKey(bucket: AiUsageRecordGroupIdentity & { isOther?: true }): string {
  if (bucket.isOther) return 'other'
  const apiKeyIdentity = bucket.apiKeyId ?? `attribution:${bucket.apiKeyAttribution ?? 'none'}`
...
function toQueryRange(range: BoundedTimeRange): BoundedTimeRange {
  return range
}
...
/**
 * One series per server-ranked group plus its explicit trailing "other"
 * buckets. The local cap remains a defensive display bound.
 */
...
function UsageSettings() {
  ...
  // The ungrouped timeline is bounded to the selected window and returns the
  // closed currency list plus daily per-currency totals. It does not need a
  // page currency in order to establish the stable default.
  ...
  const overviewBuckets = overviewRows
  const exploreBuckets = exploreRows
  const overviewTotals = overviewStatsResult.data?.totals ?? EMPTY_STATS_METRICS
  const previousOverviewTotals = previousOverviewStatsResult.data?.totals ?? EMPTY_STATS_METRICS
  ...
  const exploreTotals = exploreStatsResult.data?.totals ?? EMPTY_STATS_METRICS
  const exploreOther = exploreStatsResult.data?.other ?? EMPTY_STATS_METRICS
  ...
  const otherExploreTokens = exploreOther.totalTokens
  const otherExploreEntries = exploreOther.entryCount
  const otherExploreCost = exploreOther.totalCost
  ...
  const getModalityLabel = (modality: AiUsageRecordModality) => t(MODALITY_LABEL_KEYS[modality])
  ...
  const getEntryAriaSort = (sortBy: AiUsageRecordListSortBy) =>
    entrySortBy === sortBy ? (entrySortOrder === 'asc' ? 'ascending' : 'descending') : 'none'
  const renderEntrySortHeader = (sortBy: AiUsageRecordListSortBy, label: string, align: 'left' | 'right' = 'left') => {
                  ...
                  <>
                    ...
                    {hasNextEntryPage && (
                      <div className="flex justify-center pt-3">
                        <Button variant="outline" size="sm" disabled={entriesRefreshing} onClick={loadNextEntryPage}>
                          {entriesRefreshing
