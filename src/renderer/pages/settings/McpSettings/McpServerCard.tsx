        ...
        <SourceCell>
          {server.providerUrl && (
            <Button
              variant="ghost"
              size="icon-sm"
              className="size-7 rounded-md text-foreground/80 shadow-none hover:text-foreground [&_svg]:[stroke-width:var(--icon-stroke)]"
...
const ServerNameText = ({ className, ...props }: React.ComponentPropsWithoutRef<'span'>) => (
  <span
    className={cn('text-(length:--font-size-body-xs) min-w-0 truncate font-normal leading-5', className)}
    {...props}
  />
)
...
const MetaBadge = ({ className, ...props }: React.ComponentPropsWithoutRef<typeof Badge>) => (
  <Badge
    variant="secondary"
    className={cn(
      'text-(length:--font-size-body-xs) h-5 max-w-full rounded-md border-transparent px-2 font-normal leading-none',
      className
    )}
    {...props}
  />
