...
export function IntegrationOperations({
  ...
  return (
    <div className="mt-5 divide-y divide-border" aria-label={t('settings.prometheus.integration.activity')}>
          ...
          <div key={operation.id} className="py-3">
            <div className="flex flex-wrap items-center justify-between gap-2">
              <span className="flex min-w-0 items-center gap-2 text-sm" role="status">
                ...
                />
                {t(`settings.prometheus.integration.actions.${operation.action}`)} ·{' '}
                {t(`settings.prometheus.integration.states.${operation.status}`)}
              </span>
              {operation.status === 'running' && (
                ...
                {operation.diagnostics.map((result) => (
                  <div key={result.id} className="grid grid-cols-[minmax(0,1fr)_auto] gap-x-4 gap-y-1">
                    <dt className="break-all">
                      {t(`settings.prometheus.integration.diagnostic.${result.id}`, { defaultValue: result.id })}
                    </dt>
                    <dd className={result.state === 'failed' ? 'text-error' : 'text-foreground-secondary'}>
                      {t(`settings.prometheus.integration.states.${result.state}`)}
                    </dd>
                    {result.detail && (
