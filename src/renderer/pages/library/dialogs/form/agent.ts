...
function toUniqueModelDraft(value: string | undefined | null): UniqueModelId | '' {
  return isUniqueModelId(value) ? value : ''
}
...
export function buildInitialAgentFormState(agent?: AgentDetail | null): AgentFormState {
  ...
  return {
    ...
    model: agent?.model ?? '',
    planModel: toUniqueModelDraft(agent?.planModel),
    smallModel: toUniqueModelDraft(agent?.smallModel),
