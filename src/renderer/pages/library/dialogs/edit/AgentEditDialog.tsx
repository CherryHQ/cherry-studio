...
function AgentToolsFields({
  ...
  // This PR keeps the current main-branch `allowedTools` contract; the later
  // agent-runtime split can switch the storage contract to disabledTools.
  ...
  const enabledToolIds = allowedSet
  const setToolEnabled = (name: string, enabled: boolean) =>
    form.setValue(
      'allowedTools',
      enabled ? Array.from(new Set([...allowedTools, name])) : allowedTools.filter((n) => n !== name),
      {
        shouldDirty: true
      }
    )
