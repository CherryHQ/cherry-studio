...
const Artboard: FC<ArtboardProps> = ({
  ...
  const promptedStyleIndex = stylePresets?.findIndex((preset) => preset.prompt === painting.prompt) ?? -1
  const rememberedStyleIndex =
    styleSelection?.paintingId === painting.id
      ? (stylePresets?.findIndex((preset) => preset.id === styleSelection.presetId) ?? -1)
      : -1
  const selectedStyleIndex = rememberedStyleIndex >= 0 ? rememberedStyleIndex : promptedStyleIndex

  if (stylePresets?.length && onStyleSelect && painting.files.length === 0 && !isLoading) {
    ...
    return (
      <div
        ...
        role="group"
        aria-label={styleGroupLabel}>
        {stylePresets.map((preset, index) => {
          const isSelected = index === selectedStyleIndex
          const middlePosition = Math.floor(stylePresets.length / 2)
          const relativePosition =
            ((index - activeStyleIndex + middlePosition + stylePresets.length) % stylePresets.length) - middlePosition
          ...
          return (
            <NormalTooltip
              ...
              <Button
                ...
                onClick={() => {
                  setStyleSelection({ paintingId: painting.id, presetId: preset.id })
                  onStyleSelect(preset.prompt)
                }}>
