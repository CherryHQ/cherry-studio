...
export const Topics: React.FC<Props> = ({ assistant: _assistant, activeTopic, setActiveTopic, position }) => {
  ...
  const onToggleCacheReminder = useCallback(
    (topic: Topic) => {
    ...
    [activeTopic.id, setActiveTopic, updateTopic]
  )
  ...
  const renderTopicMenuItems = (topic: Topic) => {
    ...
    return (
      <>
        ...
        <ContextMenuItem onSelect={() => onToggleCacheReminder(topic)}>
          <ContextMenuItemContent icon={<Timer size={14} />}>
            {t('chat.topics.cache_reminder.label')}
          </ContextMenuItemContent>
        </ContextMenuItem>
