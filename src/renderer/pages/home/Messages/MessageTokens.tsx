...
const MessageTokens: React.FC<MessageTokensProps> = ({ message }) => {
  ...
  const getPrice = () => {
    ...
    // from pricing at completion time); fall back to legacy v1 pricing math.
  ...
  const getPriceString = () => {
    ...
    const currencySymbol = message.usage?.cost_currency === 'CNY' ? '¥' : message.model?.pricing?.currencySymbol || '$'
  ...
  if (message.role === 'assistant') {
    ...
    const cacheReadTokens = message?.usage?.cache_read_tokens
    const reasoningTokens = message?.usage?.thoughts_tokens
