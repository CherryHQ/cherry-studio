...
export class WebSearchService {
  ...
  public async processWebsearch(
    ...
    // RAG compression previously depended on the legacy v1 knowledge IPC. Use cutoff fallback until v2 websearch RAG is rebuilt.
    if (compressionConfig?.method === 'rag' && requestId) {
      logger.warn('Web search RAG compression is disabled while legacy knowledge v1 IPC is removed')
      if (compressionConfig.cutoffLimit) {
