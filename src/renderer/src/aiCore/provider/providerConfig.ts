  ...
  switch (provider.id) {
    ...
    case 'openai': {
      if (provider.authType === 'oauth') {
        ...
        config.options.fetch = async(url, options) => {
          // add specified body
          console.info('Fetching OpenAI OAuth with body', options.body)
