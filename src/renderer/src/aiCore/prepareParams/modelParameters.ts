 ...
 * - Disabled unconditionally for Claude 4.7+ (rejects sampling params with HTTP 400).
...
export function getTemperature(assistant: Assistant, model: Model): number | undefined {
  ...
  // Claude 4.7+ rejects sampling params (temperature/top_p/top_k) with HTTP 400
...
export function getTopP(assistant: Assistant, model: Model): number | undefined {
  ...
  // Claude 4.7+ rejects sampling params unconditionally (see getTemperature).
  if (isClaude47PlusModel(model)) {
 ...
 * the model rejects. Currently strips `topK` for Gemini 3.x models and Claude 4.7+
...
export function filterStandardParams(
  ...
  if (isClaude47PlusModel(model) && 'topK' in standardParams) {
    const { topK, ...rest } = standardParams
