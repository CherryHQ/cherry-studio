...
const fetchAndProcessAssistantResponseImpl = async (
  ...
  try {
    ...
    // Filter messages for multi-model conversation isolation
    // Each model should only receive conversation history where that model was @'d
    const currentModel = assistantMessage.model
    if (currentModel) {
      messagesForContext = messagesForContext.filter((message) => {
        // Always include user messages that mentioned this model
        if (message.role === 'user') {
          // If user message has mentions, only include if this model was mentioned
          if (message.mentions && message.mentions.length > 0) {
            return message.mentions.some((m) => m.id === currentModel?.id)
          }
          // If no mentions, include the user message (backward compatibility)
          return true
        }
        // Always include assistant messages that were generated by this model
        if (message.role === 'assistant') {
          return message.model?.id === currentModel?.id
        }
