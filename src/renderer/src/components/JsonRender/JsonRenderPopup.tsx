...
const CodePanel = memo<CodePanelProps>(({ codeEditorRef, spec, onSave, saved, onClickSave, saveLabel }) => {
  return (
    <CodeSection>
      <CodeEditor
        ref={codeEditorRef}
...
const PreviewPanel = memo<PreviewPanelProps>(({ spec: rawSpec }) => {
  const { spec, error } = useJsonRenderSpec(rawSpec, false)
      ...
      {spec ? (
        <PreviewContent>
          <Suspense fallback={null}>
            <JsonRenderPreview spec={spec} />
          </Suspense>
        </PreviewContent>
      ) : (
        <EmptyPreview>
          <p>{error || 'No content to preview'}</p>
        </EmptyPreview>
      )}
    </PreviewSection>
  )
...
interface JsonRenderPopupProps {
  open: boolean
  title: string
  spec: string
  onSave?: (spec: string) => void
...
const JsonRenderPopup: React.FC<JsonRenderPopupProps> = ({ open, title, spec, onSave, onClose }) => {
...
export default JsonRenderPopup
