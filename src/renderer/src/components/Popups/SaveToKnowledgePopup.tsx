...
import { useKnowledgeFiles, useKnowledgeNotes } from '@renderer/hooks/useKnowledges'
...
const PopupContainer: React.FC<Props> = ({ source, title, resolve }) => {
  ...
  const { addNote } = useKnowledgeNotes(selectedBaseId || '')
  const { addFiles } = useKnowledgeFiles(selectedBaseId || '')
  ...
  const formState = useMemo(() => {
    const hasValidBase = !!selectedBaseId && bases.some((base) => base.id === selectedBaseId)
  ...
  useEffect(() => {
    if (!selectedBaseId) {
      const firstAvailableBase = bases[0]
