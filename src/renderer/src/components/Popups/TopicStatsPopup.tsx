...
import { BarChart3, Bot, Coins, Cpu, Gauge, Hash, MessageSquare, User, Zap } from 'lucide-react'
...
const MHeader = styled.div` display: flex; justify-content: space-between; align-items: baseline; gap: 8px; `
...
const MBadge = styled.span`
  ...
  background: var(--color-background-soft); padding: 1px 5px; border-radius: 3px; flex-shrink: 0;
`
const MTrack = styled.div`
  height: 6px; border-radius: 3px; background: var(--color-background-soft); overflow: hidden;
`
...
const InfoVal = styled.div` font-size: 13px; font-weight: 500; color: var(--color-text); white-space: nowrap; `
    ...
    <Modal
      ...
      <CardRow>
        ...
        <Card $accent={C.card3}>
          ...
          <CardValue>{fmtCost(stats.totalCost)}</CardValue>
          <CardLabel>{t('stats.total_cost')}</CardLabel>
        </Card>
        <Card $accent={C.card4}>
          <CardIcon $c={C.card4}>
            <Zap size={13} />
      ...
      {stats.totalCost > 0 && (
        <Section>
          <SectionHeader>
            <Coins size={12} /> {t('stats.cost_breakdown')}
          </SectionHeader>
          <BarTrack>
            <BarSeg $w={(stats.inputCost / stats.totalCost) * 100} $c="#f59e0b" />
            <BarSeg $w={(stats.outputCost / stats.totalCost) * 100} $c="#ef4444" />
          </BarTrack>
          <Legend>
            <LegendItem>
              <Dot $c="#f59e0b" />
              {t('stats.input_cost')} <b style={{ color: 'var(--color-text)' }}>{fmtCost(stats.inputCost)}</b>
            </LegendItem>
            <LegendItem>
              <Dot $c="#ef4444" />
              {t('stats.output_cost')} <b style={{ color: 'var(--color-text)' }}>{fmtCost(stats.outputCost)}</b>
            </LegendItem>
          </Legend>
        </Section>
      )}
            ...
            {stats.modelStats.map((m, i) => (
              <MRow key={m.modelId}>
                ...
                <MMetrics>
                  ...
                  {m.cost > 0 && <span>{fmtCost(m.cost)}</span>}
      ...
      <Section>
        ...
        <InfoGrid>
          ...
          <div>
            ...
            <InfoVal>{stats.userMessages}</InfoVal>
          </div>
          <div>
            <InfoLabel>
              <Bot size={10} style={{ verticalAlign: 'middle', marginRight: 2 }} />
              {t('stats.assistant_messages')}
            </InfoLabel>
            <InfoVal>{stats.assistantMessages}</InfoVal>
          </div>
          <div>
