...
const DraggableTabItem: React.FC<DraggableTabItemProps> = ({ tab, isActive, onClick, onClose, index }) => {
  ...
  const {
    ...
    id: tab.id,
    disabled: tab.isPinned // Disable dragging for pinned tabs
  })
  ...
  const handleMouseEnter = () => {
    // Don't show preview if dragging or if tab is active
    if (isSortableDragging || isActive) return
  ...
  return (
    <TabWrapper>
      <motion.div
        ...
        whileTap={{ scale: 0.98 }}
        className={isSortableDragging ? 'dragging' : ''}
      ...
      <TabPreview tab={tab} isVisible={showPreview} />
    </TabWrapper>
  )
}
...
const TabContainer = styled.div<{ theme: string; isActive: boolean; isPinned: boolean; isDragging?: boolean }>`
  display: flex;
  align-items: center;
  height: 30px;
  ...
  cursor: ${({ isDragging, isPinned }) => (isDragging ? 'grabbing' : isPinned ? 'pointer' : 'grab')};
  user-select: none;
  position: relative;
  overflow: hidden;
...
export default DraggableTabItem
