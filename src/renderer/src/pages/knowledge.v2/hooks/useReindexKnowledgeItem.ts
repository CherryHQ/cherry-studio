...
const normalizeError = (error: unknown): Error => {
  ...
  return new Error('Failed to reindex the selected knowledge source')
}
...
export const useReindexKnowledgeItem = (baseId: string) => {
  ...
  const reindexItem = useCallback(
    async (item: KnowledgeItem): Promise<{ itemIds: string[] }> => {
      if (!baseId) {
      ...
      try {
        const result = await window.api.knowledgeRuntime.reindexItems(baseId, [item.id])
