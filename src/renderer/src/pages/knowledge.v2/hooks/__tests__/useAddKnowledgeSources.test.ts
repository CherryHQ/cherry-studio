import { act, renderHook } from '@testing-library/react'
import { beforeEach, describe, expect, it, vi } from 'vitest'

import { mockRendererLoggerService } from '../../../../../../../tests/__mocks__/RendererLoggerService'
import { useAddKnowledgeSources } from '../useAddKnowledgeSources'

const mockUseInvalidateCache = vi.fn()
const mockInvalidateCache = vi.fn()
const mockAddSources = vi.fn()
let loggerErrorSpy: ReturnType<typeof vi.spyOn>

vi.mock('@data/hooks/useDataApi', () => ({
  useInvalidateCache: () => mockUseInvalidateCache()
}))

describe('useAddKnowledgeSources', () => {
  beforeEach(() => {
    vi.clearAllMocks()
    loggerErrorSpy = vi.spyOn(mockRendererLoggerService, 'error').mockImplementation(() => {})
    mockUseInvalidateCache.mockReturnValue(mockInvalidateCache)
    mockInvalidateCache.mockResolvedValue(undefined)
    mockAddSources.mockResolvedValue({ itemIds: ['directory-1', 'url-1', 'sitemap-1'] })
    ;(window as any).api = {
      knowledgeRuntime: {
        addSources: mockAddSources
      }
    }
  })

  it('submits knowledge sources through orchestration IPC and refreshes the list', async () => {
    const items = [
      { type: 'directory' as const, data: { name: 'docs', path: '/Users/me/docs' } },
      { type: 'url' as const, data: { url: 'https://example.com/article', name: 'https://example.com/article' } },
      {
        type: 'sitemap' as const,
        data: {
          url: 'https://docs.cherry-ai.com/sitemap-pages.xml',
          name: 'https://docs.cherry-ai.com/sitemap-pages.xml'
        }
      }
    ]

    const { result } = renderHook(() => useAddKnowledgeSources('base-1'))

    await act(async () => {
      await expect(result.current.submit(items)).resolves.toEqual({ itemIds: ['directory-1', 'url-1', 'sitemap-1'] })
    })

    expect(mockAddSources).toHaveBeenCalledWith('base-1', items)
    expect(mockInvalidateCache).toHaveBeenCalledWith('/knowledge-bases/base-1/items')
    expect(result.current.error).toBeUndefined()
    expect(result.current.isSubmitting).toBe(false)
  })

  it('keeps submit rejected and exposes inline error when orchestration rejects', async () => {
    const submitError = new Error('create failed')
    mockAddSources.mockRejectedValueOnce(submitError)

    const { result } = renderHook(() => useAddKnowledgeSources('base-1'))

    await act(async () => {
      await expect(
        result.current.submit([
          { type: 'url' as const, data: { url: 'https://example.com/article', name: 'https://example.com/article' } }
        ])
      ).rejects.toBe(submitError)
    })

    expect(mockInvalidateCache).not.toHaveBeenCalled()
    expect(result.current.error).toBe(submitError)
    expect(result.current.isSubmitting).toBe(false)
    expect(loggerErrorSpy).toHaveBeenCalledWith('Failed to add knowledge sources', {
      baseId: 'base-1',
      sourceCount: 1,
      error: submitError
    })
  })
})
