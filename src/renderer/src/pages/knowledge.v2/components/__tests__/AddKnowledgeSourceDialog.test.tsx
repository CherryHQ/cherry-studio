...
describe('AddKnowledgeSourceDialog', () => {
  ...
  it('renders the note placeholder state and keeps add disabled before the real integration lands', () => {
    ...
    expect(
      screen.getByText('真实笔记列表接入后，将在这里展示可多选的笔记。当前可先使用文件、目录、网址或站点地图。')
    ).toBeInTheDocument()
  ...
  it('renders the sitemap form copy when the sitemap tab becomes active', () => {
    render(<AddKnowledgeSourceDialog open onOpenChange={vi.fn()} />)

    fireEvent.click(screen.getByRole('tab', { name: '站点地图' }))

    expect(screen.getByRole('tab', { name: '站点地图' })).toHaveAttribute('aria-selected', 'true')
    expect(screen.getByText('输入 Sitemap 地址：')).toBeInTheDocument()
    expect(screen.getByPlaceholderText('https://docs.cherry-ai.com/sitemap-pages.xml')).toBeInTheDocument()
    expect(screen.getByText('将读取 Sitemap 中包含的页面并建立索引')).toBeInTheDocument()
    expect(screen.queryByText('爬虫设置')).not.toBeInTheDocument()
    expect(screen.queryByLabelText('爬取深度')).not.toBeInTheDocument()
    expect(screen.queryByLabelText('最大页面数')).not.toBeInTheDocument()
  })
