  ...
  return (
    <div className="space-y-6">
      <div>
        ...
        <p className="text-muted-foreground/75 text-xs">{t('library.config.knowledge.desc')}</p>
      </div>
      ...
      <div>
        ...
        {linkedItems.length === 0 ? (
          <div className="flex flex-col items-center rounded-xs border border-border/20 border-dashed p-6">
            <Database size={20} strokeWidth={1.2} className="mb-2 text-muted-foreground/70" />
            <p className="mb-1 text-muted-foreground/80 text-xs">{t('library.config.knowledge.empty_title')}</p>
            <p className="text-muted-foreground/75 text-xs">{t('library.config.knowledge.empty_desc')}</p>
          </div>
        ) : (
            ...
            {linkedItems.map((kb) => (
              <div
                ...
                <Button
                  ...
                  className="flex h-6 min-h-0 w-6 items-center justify-center rounded-3xs font-normal text-muted-foreground/75 opacity-0 shadow-none transition-all hover:bg-destructive/10 hover:text-destructive focus-visible:ring-0 group-hover:opacity-100">
                  <Trash2 size={10} />
                </Button>
              </div>
            ))}
        ...
        <Popover open={pickerOpen} onOpenChange={setPickerOpen}>
          <PopoverTrigger asChild>
            <Button
              variant="ghost"
              disabled={isLoading}
              className="mt-2 flex h-auto min-h-0 items-center gap-1 rounded-2xs border border-border/20 px-2.5 py-1.5 font-normal text-muted-foreground/75 text-xs shadow-none transition-colors hover:border-border/40 hover:bg-accent/50 hover:text-foreground focus-visible:ring-0 disabled:opacity-50">
          ...
          <PopoverContent
            ...
            {unlinkedItems.length === 0 ? (
              <p className="px-2 py-5 text-center font-normal text-muted-foreground/70 text-xs">
