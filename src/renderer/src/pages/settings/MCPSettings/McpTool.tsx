...
const MCPToolsSection = ({ tools, server, onToggleTool, onToggleAutoApprove }: MCPToolsSectionProps) => {
  ...
  const renderToolProperties = (tool: MCPTool) => {
    ...
    // <Typography.Title level={5}>{t('settings.mcp.tools.inputSchema.key')}:</Typography.Title>
    return (
    ...
    },
    {
      title: (
        <Flex align="center" justify="center" gap={4}>
          ...
          <Typography.Text strong>{t('settings.mcp.tools.autoApprove.key')}</Typography.Text>
        </Flex>
      ),
      key: 'autoApprove',
