...
const ProviderOAuth: FC<Props> = ({ providerId }) => {
  ...
  let providerWebsite = providerConfig?.api?.url.replace('https://', '').replace('api.', '') || provider.name
  if (provider.id === 'ppio') {
  ...
  const renderProviderAction = (action: ProviderOAuthAction) => {
    ...
    if (action === 'bills') {
      return (
        <Button key={action} shape="round" icon={<ReceiptText size={16} />} onClick={() => onProviderAction(action)}>
    ...
    if (action === 'apiKey') {
      return (
        <Button key={action} shape="round" icon={<KeyRound size={16} />} onClick={() => onProviderAction(action)}>
  ...
  return (
    <Container>
      <ProviderLogo src={getProviderLogo(provider.id)} alt={getProviderLabel(provider.id)} />
      ...
      <Description>
        <Trans
          ...
          components={{
            website: <OfficialWebsite href={providerConfig?.websites?.official} target="_blank" rel="noreferrer" />
          }}
