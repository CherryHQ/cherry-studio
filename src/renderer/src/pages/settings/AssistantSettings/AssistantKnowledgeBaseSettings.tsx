...
const AssistantKnowledgeBaseSettings: React.FC<Props> = ({ assistant, updateAssistant }) => {
  ...
  return (
    <Container>
      ...
      <Row align="middle" style={{ marginTop: 10 }}>
        <Segmented
          value={assistant.knowledge_recognition}
          options={[
            { label: t('assistants.settings.knowledge_base.recognition.off'), value: 'off' },
            { label: t('assistants.settings.knowledge_base.recognition.on'), value: 'on' },
            { label: t('assistants.settings.knowledge_base.recognition.auto'), value: 'auto' }
          ]}
          onChange={(value) =>
            updateAssistant({
              ...assistant,
              knowledge_recognition: value as 'auto' | 'off' | 'on'
            })
          }
        />
      </Row>
