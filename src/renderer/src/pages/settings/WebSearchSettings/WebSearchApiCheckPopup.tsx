...
const PopupContainer: React.FC<Props> = ({ title, provider, apiKeys, resolve }) => {
  ...
  const checkAllKeys = async () => {
    ...
    try {
      for (let i = 0; i < newStatuses.length; i++) {
        ...
        const { valid } = await WebSearchService.checkSearch({ ...provider, apiKey: newStatuses[i].key })
  ...
  const checkSingleKey = async (keyIndex: number) => {
    ...
    try {
      const { valid } = await WebSearchService.checkSearch({ ...provider, apiKey: keyStatuses[keyIndex].key })
...
export default class WebSearchApiCheckPopup {
  static topviewId = 0
  static hide() {
    TopView.hide('WebSearchApiCheckPopup')
  }
