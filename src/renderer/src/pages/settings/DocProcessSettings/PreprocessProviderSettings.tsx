...
import { Avatar, Button, type CompoundIcon, Flex, Tooltip } from '@cherrystudio/ui'
    ...
    <>
      <SettingTitle>
        <Flex className="items-center gap-2">
          {(() => {
            const logo = getPreprocessProviderLogo(preprocessProvider.id)
            if (logo && typeof logo !== 'string') {
              const Icon = logo as CompoundIcon
              return <Icon.Avatar size={16} shape="rounded" />
            }
            return <Avatar radius="md" src={logo} className="h-4 w-4 border-[0.5px] border-[var(--color-border)]" />
          })()}
