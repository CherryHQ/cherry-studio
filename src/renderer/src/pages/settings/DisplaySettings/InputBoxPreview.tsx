...
const InputBoxPreview: FC<Props> = ({ inputBoxConfig, setInputBoxConfig }) => {
  ...
  const { leftIcons, rightIcons } = useMemo(() => {
    ...
    // 应用用户的自定义配置
    const configuredIcons = visibleIcons.map(icon => ({
      ...icon,
  ...
  // 更新配置的回调函数
  const handleConfigUpdate = (newConfig: Record<string, { visible: boolean; position: 'left' | 'right'; order: number }>) => {
  ...
  return (
    <Container>
            ...
            <ToolbarMenu>
              {leftIcons.map((icon) => (
                <Tooltip key={icon.id} title={t(icon.tooltip)} placement="top">
                  <ToolbarButton
                    onClick={icon.id === 'expand_collapse' ? handleExpandToggle : undefined}
                  >
                    {icon.id === 'expand_collapse'
      ...
      <InputBoxIconsManager
        icons={allIcons}
        inputBoxConfig={inputBoxConfig}
        setInputBoxConfig={handleConfigUpdate}
      />
    </Container>
  )
}
