...
import { FileMetadata, FileTypes, isKnowledgeImageItem, KnowledgeBase, KnowledgeItem } from '@renderer/types'
...
const logger = loggerService.withContext('KnowledgeImages')
...
interface KnowledgeImagesProps {
...
const KnowledgeImages: FC<KnowledgeImagesProps> = ({ selectedBase, progressMap, preprocessMap }) => {
  const { t } = useTranslation()
  const [windowHeight, setWindowHeight] = useState(window.innerHeight)

  const { base, imageItems, addImages, refreshItem, removeItem, getProcessingStatus } = useKnowledge(
    selectedBase.id || ''
  )
  ...
  const handleDrop = async (files: File[]) => {
    ...
    if (files) {
      const _files: FileMetadata[] = files
        ...
        .filter(({ ext }) => imageExts.includes(ext))
      const uploadedFiles = await FileManager.uploadFiles(_files)
      logger.debug('uploadedImages', uploadedFiles)
    ...
    <ItemContainer>
      ...
      <ItemFlexColumn>
        <Dragger
          ...
          <p className="ant-upload-text">{t('knowledge.drag_image')}</p>
          <p className="ant-upload-hint">{t('knowledge.image_hint', { image_types: 'JPG, JPEG, PNG' })}</p>
        </Dragger>
        {imageItems.length === 0 ? (
          <KnowledgeEmptyView />
        ) : (
          <DynamicVirtualList
            list={imageItems.reverse()}
