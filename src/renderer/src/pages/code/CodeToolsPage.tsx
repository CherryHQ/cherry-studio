...
const CodeToolsPage: FC = () => {
  ...
  const [terminalCustomPaths, setTerminalCustomPaths] = useState<
    ...
    (m: Model) => {
      ...
      if (selectedCliTool === codeTools.claudeCode) {
        ...
        return (
          ...
          CLAUDE_OFFICIAL_SUPPORTED_PROVIDERS.includes(m.provider)
        );
      }
      ...
      if (selectedCliTool === codeTools.openaiCodex) {
        ...
        return (
          ...
          OPENAI_CODEX_SUPPORTED_PROVIDERS.includes(m.provider)
        );
      }
  ...
  const validateLaunch = (): { isValid: boolean; message?: string } => {
    if (!canLaunch || !isBunInstalled) {
      return {
        isValid: false,
        message: !isBunInstalled
          ? t("code.launch.bun_required")
          : t("code.launch.validation_error"),
      };
    }
  ...
  // 准备启动环境
  const prepareLaunchEnvironment = async (): Promise<Record<
    string,
    string
  > | null> => {
    if (selectedCliTool === codeTools.githubCopilotCli) {
  ...
  const executeLaunch = async (env: Record<string, string>) => {
    const modelId =
      selectedCliTool === codeTools.githubCopilotCli ? "" : selectedModel?.id!;
  ...
  const handleLaunch = async () => {
    ...
    if (!validation.isValid) {
      window.toast.warning(
        validation.message || t("code.launch.validation_error"),
      );
      return;
    }
          ...
          <SettingsPanel>
            ...
            <SettingsItem>
                  ...
                  <Popover
                    content={
                      <div style={{ width: 200 }}>
                        ...
                        >
                          {getClaudeSupportedProviders(allProviders).map(
                            (provider) => {
                              return (
                                <Link
                                  key={provider.id}
              ...
              <ModelSelector
                ...
                value={
                  selectedModel ? getModelUniqId(selectedModel) : undefined
                }
            ...
            <SettingsItem>
              ...
              <Checkbox
                ...
                onChange={(e) => setAutoUpdateToLatest(e.target.checked)}
              >
