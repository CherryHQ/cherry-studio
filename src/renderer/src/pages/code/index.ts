...
export const GEMINI_SUPPORTED_PROVIDERS = [
  "aihubmix",
  "dmxapi",
  "new-api",
  "cherryin",
...
export const OPENAI_CODEX_SUPPORTED_PROVIDERS = [
  "openai",
  "openrouter",
  "aihubmix",
  "new-api",
...
// Provider 过滤映射
export const CLI_TOOL_PROVIDER_MAP: Record<
  string,
...
> = {
  [codeTools.claudeCode]: (providers) =>
    providers.filter(
      (p) =>
        p.type === "anthropic" || CLAUDE_SUPPORTED_PROVIDERS.includes(p.id),
    ),
  [codeTools.geminiCli]: (providers) =>
    providers.filter(
      (p) => p.type === "gemini" || GEMINI_SUPPORTED_PROVIDERS.includes(p.id),
    ),
  [codeTools.qwenCode]: (providers) =>
    providers.filter((p) => p.type.includes("openai")),
  [codeTools.openaiCodex]: (providers) =>
    providers.filter(
      (p) =>
        p.id === "openai" || OPENAI_CODEX_SUPPORTED_PROVIDERS.includes(p.id),
    ),
  [codeTools.iFlowCli]: (providers) =>
    providers.filter((p) => p.type.includes("openai")),
  ...
  switch (tool) {
    case codeTools.claudeCode:
      env.ANTHROPIC_BASE_URL =
        getCodeToolsApiBaseUrl(model, "anthropic") || modelProvider.apiHost;
    ...
    case codeTools.geminiCli: {
      const apiBaseUrl =
        getCodeToolsApiBaseUrl(model, "gemini") || modelProvider.apiHost;
...
export const getClaudeSupportedProviders = (providers: Provider[]) => {
  return providers.filter(
    (p) => p.type === "anthropic" || CLAUDE_SUPPORTED_PROVIDERS.includes(p.id),
  );
};
