...
const InputbarInner: FC<InputbarInnerProps> = ({ assistant: initialAssistant, setActiveTopic, topic, actionsRef }) => {
  ...
  const { isProviderEnabled } = useWebSearchProviders()
  ...
  useEffect(() => {
    ...
    if (
      assistant.webSearchProviderId &&
      (!isProviderEnabled(assistant.webSearchProviderId) || isMandatoryWebSearchModel(model))
    ) {
  ...
  }, [assistant, model, updateAssistant, isProviderEnabled])
