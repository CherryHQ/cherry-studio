...
const Inputbar: FC<Props> = ({ assistant: _assistant, setActiveTopic, topic }) => {
  ...
  const inputBoxIcons = useMemo(() => {
    ...
    const conditions = {
      ...
      showKnowledgeIcon,
      hasMcpServers: activedMcpServers.length > 0,
      isGenerateImageModel: isGenerateImageModel(model),
      showInputEstimatedTokens
    }
    ...
    const configuredIcons = visibleIcons
      .map((icon) => ({
        ...icon,
        visible: inputBoxIconsConfig?.[icon.id]?.visible ?? icon.visible,
        order: inputBoxIconsConfig?.[icon.id]?.order ?? icon.order,
        position: inputBoxIconsConfig?.[icon.id]?.position ?? icon.position
      }))
