...
const InputbarTools = ({
  ...
  const getQuickPanelMenuImpl = (params: {
    ...
    return [
      ...
      {
        label: t('chat.input.web_search.key'),
        description: '',
        icon: <Globe />,
        isMenu: true,
        action: () => {
      ...
      {
        label: couldAddImageFile ? t('chat.input.upload.key') : t('chat.input.upload.document'),
        description: '',
        icon: <Paperclip />,
        isMenu: true,
  ...
  const toolButtons = useMemo<ToolButtonConfig[]>(() => {
    return [
      ...
      {
        key: 'thinking',
        label: t('chat.input.thinking.key'),
        component: (
      ...
      {
        key: 'clear_topic',
        label: t('chat.input.clear.key', { Command: '' }),
        component: (
          <Tooltip
            placement="top"
            title={t('chat.input.clear.key', { Command: cleanTopicShortcut })}
            mouseLeaveDelay={0}
            arrow>
