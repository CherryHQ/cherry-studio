 ...
 * 组合中间件
 */
function compose<TContext extends BaseContext, TResult = any>(
  middlewares: Array<(ctx: TContext, next: Next) => Promise<void>>
): (ctx: TContext, finalHandler?: (ctx: TContext) => Promise<TResult>) => Promise<TResult> {
  if (!Array.isArray(middlewares)) throw new TypeError('Middleware stack must be an array!')
  for (const fn of middlewares) {
    if (typeof fn !== 'function') throw new TypeError('Middleware must be composed of functions!')
  }

  return async function composedMiddleware(ctx: TContext, finalHandler?: (ctx: TContext) => Promise<TResult>) {
    let index = -1
    let finalResult: TResult | undefined = undefined
    ...
    async function dispatch(i: number): Promise<void> {
      ...
      const middlewareToExecute: ((ctx: TContext, next: Next) => Promise<void>) | undefined = middlewares[i]

      if (i === middlewares.length) {
        // 到达中间件链末尾，检查是否需要调用最终处理函数
        if (finalHandler) {
          finalResult = await finalHandler(ctx) // 执行最终处理并捕获其结果
        }
        return
      }

      if (!middlewareToExecute) {
        return Promise.resolve() // 以防万一
      }

      const next: Next = () => dispatch(i + 1)

      await middlewareToExecute(ctx, next)
    }

    await dispatch(0)

    if (finalResult === undefined) {
      throw new Error('No result generated from middleware chain')
    }
...
export function applyCompletionsMiddlewares(
  ...
  console.log(
    `[applyCompletionsMiddlewares] Applying ${middlewares.length} middlewares for provider: ${originalApiClientInstance.provider?.id}`
  )

  if (middlewares.length === 0) {
    console.log(`[applyCompletionsMiddlewares] No middlewares to apply, returning original method`)
    return originalCompletionsMethod.bind(originalApiClientInstance)
  }
  ...
  const enhancedCompletionsMethod = async function (
    ...
    // 将增强后的方法保存到context中供递归调用使用
    // 注意：这里我们需要延迟绑定，因为函数正在定义中
    if (!completionsContext._internal.customState) {
      completionsContext._internal.customState = {}
    }
    completionsContext._internal.customState.enhancedCompletions = enhancedCompletionsMethod

    // 最终处理函数：调用原始方法
    const finalHandler = async () => {
      console.log(
        `[applyCompletionsMiddlewares] Calling original completions method for provider: ${originalApiClientInstance.provider?.id}`
      )
      return await originalCompletionsMethod.call(originalApiClientInstance, params)
    }

    console.log(`[applyCompletionsMiddlewares] Starting middleware chain execution`)
    try {
      const result = await composed(completionsContext, finalHandler)
      console.log(`[applyCompletionsMiddlewares] Middleware chain completed successfully`)
      return result
    } catch (error) {
      console.error(`[applyCompletionsMiddlewares] Middleware chain failed:`, error)
      throw error
    }
  }

  console.log(`[applyCompletionsMiddlewares] Created enhanced completions method`)
  return enhancedCompletionsMethod
}

/**
 * 应用通用方法中间件
 */
export function applyMethodMiddlewares<TArgs extends any[], TResult>(
  originalApiClientInstance: BaseApiClient,
  methodName: string,
  originalMethod: (...args: TArgs) => Promise<TResult>,
  ...
  if (middlewares.length === 0) {
    return originalMethod.bind(originalApiClientInstance)
  }

  const composed = compose(middlewares)

  return async function enhancedMethod(...args: TArgs): Promise<TResult> {
    // 为通用方法创建一个合适的参数对象
    const defaultParams: CompletionsParams = {
      messages: [],
      assistant: {} as any,
      model: {} as any,
      onChunk: () => {},
    ...
    // 最终处理函数：调用原始方法
    const finalHandler = async () => {
      return await originalMethod.apply(originalApiClientInstance, args)
    }

    return await composed(baseContext, finalHandler)
  }
}
