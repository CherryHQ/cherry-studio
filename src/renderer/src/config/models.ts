...
export const REASONING_REGEX = /^(o\d+(?:-[\w-]+)?|.*\b(?:reasoner|thinking)\b.*|.*-[rR]\d+.*|.*claude-3-7-sonnet.*)$/i
...
export function isClaudeThinkingModel(model: Model): boolean {
  if (!model) {
    return false
  }

  return model?.id?.includes('claude-3-7-sonnet') || false
}
