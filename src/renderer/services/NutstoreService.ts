...
import { getBackupSyncState, type RemoteSyncState, setBackupSyncState } from './BackupService'
...
export async function startNutstoreAutoSync() {
  ...
  async function scheduleNextBackup() {
    ...
    const { lastSyncTime } = getBackupSyncState().nutstore
    const timeUntilNextSync = lastSyncTime
      ? Math.max(1000, lastSyncTime + requiredInterval - Date.now())
      : requiredInterval
