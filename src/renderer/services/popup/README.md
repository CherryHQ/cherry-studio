...
- **two-phase close** — `open:false`, then unmount after `POPUP_EXIT_MS`; the delay exceeds the CSS exit duration
  by a small paint buffer so the compositor can present the final transparent frame.
