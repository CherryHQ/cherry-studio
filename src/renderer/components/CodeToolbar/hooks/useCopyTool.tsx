...
export const useCopyTool = ({ showPreviewTools, previewRef, onCopySource, setTools }: UseCopyToolProps) => {
  ...
  useEffect(() => {
    ...
    const baseTool = {
      ...TOOL_SPECS.copy,
      icon: copied ? <Check className="tool-icon" color="var(--cs-success)" /> : <CopyIcon className="tool-icon" />,
    ...
    const copyImageTool = {
      ...
      icon: copiedImage ? <Check className="tool-icon" color="var(--cs-success)" /> : <Image className="tool-icon" />,
