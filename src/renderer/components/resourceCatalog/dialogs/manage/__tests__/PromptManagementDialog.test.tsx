...
beforeEach(() => {
  vi.clearAllMocks()
  mocks.useQuery.mockImplementation((_path: string, options: { query?: { targetType?: string } }) => ({
    data: options.query?.targetType ? [promptsFixture[0]] : promptsFixture,
    error: undefined,
    isLoading: false,
...
describe('PromptManagementDialog', () => {
  ...
  it('passes search text to the prompts query', async () => {
    ...
    await waitFor(() => {
      expect(mocks.useQuery).toHaveBeenCalledWith('/prompts', expect.objectContaining({ query: { search: 'route' } }))
    })
  })
