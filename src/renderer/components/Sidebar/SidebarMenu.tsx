...
function IconMenuItems({ items, activeItem, onItemClick, onReorder, onContextMenuOpenChange }: MenuItemsProps) {
  return (
    <SidebarSortableList
      items={items}
      itemKey="id"
        ...
        return (
          <SidebarTooltip key={item.id} content={item.label}>
            <SidebarItemContextMenu items={item.contextMenuItems} onOpenChange={onContextMenuOpenChange}>
              <button
                type="button"
                onClick={guardClick(() => void onItemClick(item.id))}
...
function FullMenuItems({ items, activeItem, onItemClick, onReorder, onContextMenuOpenChange }: MenuItemsProps) {
  return (
    <SidebarSortableList
      items={items}
      itemKey="id"
