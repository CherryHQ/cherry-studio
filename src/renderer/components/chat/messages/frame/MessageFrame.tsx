...
const USER_MESSAGE_FOOTER_ACTIONS_CLASS =
  'absolute inset-0 flex items-center gap-2 opacity-0 transition-opacity duration-150 focus-within:opacity-100 group-hover/message:opacity-100 [@media(hover:none)]:opacity-100'
...
const MessageItemContent: FC<Omit<Props, 'messageParts'>> = ({
  ...
  const assistantFooterVisibilityClass = keepAssistantFooterVisible
    ? 'opacity-100'
    : 'opacity-0 transition-opacity duration-150 focus-within:opacity-100 group-hover/message:opacity-100 [@media(hover:none)]:opacity-100'
  ...
  return (
    <div
      ...
      className={cn(
        classNames({
          'message group/message transform-[translateZ(0)] relative flex w-full flex-col rounded-[10px] pt-2.5 pb-0 transition-colors duration-300 will-change-transform [&:hover_.menubar]:opacity-100 [&_.menubar.show]:opacity-100 [&_.menubar]:opacity-0 [&_.menubar]:transition-opacity [&_.menubar]:duration-200 [&_.menubar:focus-within]:opacity-100 [@media(hover:none)]:[&_.menubar]:opacity-100': true,
