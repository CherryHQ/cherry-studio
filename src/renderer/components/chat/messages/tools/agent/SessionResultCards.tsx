...
import { ArrowUpRight, GitBranchPlus, Send } from 'lucide-react'
...
export const SessionResultCards = React.memo(function SessionResultCards({
  ...
  return (
    <div className="mt-3 flex w-full flex-col gap-2" data-testid="session-result-cards">
      {targets.map((target) => {
        ...
        const TargetIcon = isCreate ? GitBranchPlus : Send
        ...
        return (
          <div
            ...
            <span className="flex size-8 shrink-0 items-center justify-center rounded-md bg-background-subtle text-muted-foreground">
              <TargetIcon aria-hidden="true" size={16} strokeWidth={1.8} />
            </span>
