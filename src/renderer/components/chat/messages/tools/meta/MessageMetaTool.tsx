  ...
  return (
    <BodyContainer>
      ...
      {toolResponse.status === 'done' && matchedNamespaces.length === 0 && (
        <Empty>{t('message.tools.metaTool.noToolsMatched')}</Empty>
      )}
...
const ToolInspectBody: FC<{ toolResponse: NormalToolResponse }> = ({ toolResponse }) => {
  ...
  return (
    <BodyContainer>
      <ArgsBlock args={args} />
      {jsDoc && (
        <ResponseBlock title={t('message.tools.metaTool.jsDoc')}>
...
const ToolInvokeBody: FC<{ toolResponse: NormalToolResponse }> = ({ toolResponse }) => {
  ...
  if (!innerName) {
    return (
      <BodyContainer>
        <Empty>{t('message.tools.metaTool.missingToolName')}</Empty>
      </BodyContainer>
    )
  }
  ...
  return (
    <BodyContainer>
      ...
      {response !== undefined && response !== null && (
        <ResponseBlock title={t('message.tools.metaTool.response')}>
...
const ToolExecBody: FC<{ toolResponse: NormalToolResponse }> = ({ toolResponse }) => {
  ...
  return (
    <BodyContainer>
      <ResponseBlock title={t('message.tools.metaTool.code')}>
        {highlighted ? (
      ...
      {out?.logs && out.logs.length > 0 && (
        <ResponseBlock title={t('message.tools.metaTool.logs', { count: out.logs.length })}>
      ...
      {!out?.isError && out?.result !== undefined && (
        <ResponseBlock title={t('message.tools.metaTool.result')}>
