...
describe('ChatAppShell', () => {
  ...
  it('paints the recessed conversation canvas on the center column, not the shell root', () => {
    ...
    expect(container.querySelector('#chat-main')).toHaveClass('bg-chat-canvas')
    expect(container.querySelector('[data-chat-app-shell-root]')).not.toHaveClass('bg-chat-canvas')
    expect(screen.getByTestId('pane').closest('[data-resource-list-pane]')).not.toHaveClass('bg-chat-canvas')
  })
  ...
  it('renders side panel in a root overlay host above center layers', () => {
    ...
    expect(navbarWrapper).toHaveClass('relative', 'shrink-0', 'bg-chat-canvas')
