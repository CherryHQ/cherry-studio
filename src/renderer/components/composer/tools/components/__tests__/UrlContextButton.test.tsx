...
describe('UrlContextButton', () => {
  beforeEach(() => {
    ...
    mocks.assistant = {
      id: 'assistant-1',
      name: 'Assistant',
      settings: { enableUrlContext: false, mcpMode: 'disabled' }
    }
  ...
  it('disables URL Context with a warning when Gemini 2.5 has MCP enabled', async () => {
    mocks.assistant.settings.mcpMode = 'auto'
    ...
    expect(launcher).toMatchObject({
      id: 'url-context',
      disabled: true,
      disabledReason: 'chat.mcp.warning.url_context'
    })
  })

  it('turns URL Context off when MCP becomes enabled for Gemini 2.5', async () => {
    mocks.assistant.settings = { enableUrlContext: true, mcpMode: 'auto' }
    render(<UrlContextToolRuntime assistantId="assistant-1" launcher={launcherApi} />)

    await waitFor(() => expect(mocks.updateAssistant).toHaveBeenCalledWith({ settings: { enableUrlContext: false } }))
    expect(toast.warning).toHaveBeenCalledWith('chat.mcp.warning.url_context')
  })

  it('allows URL Context with MCP enabled for Gemini 3', async () => {
    mocks.assistant.settings.mcpMode = 'auto'
    mocks.model = { ...geminiModel, id: 'gemini::gemini-3-pro', apiModelId: 'gemini-3-pro' }
    render(<UrlContextButton assistantId="assistant-1" launcher={launcherApi} />)
