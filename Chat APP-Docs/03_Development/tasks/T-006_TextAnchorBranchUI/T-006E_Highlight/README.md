...
- **多个并存高亮** —— **已起步**：P1（[[../../P1_MultiBranch/README|P1 概览]]）三步法 = S1 state foundation / S2 multi-branch UI + multi-highlight / S3 disposition。**P1-S1 已实施（2026-06-01）**：state shape 从单 anchor 推广成 `branches: Branch[]` 数组 + Context `{ anchors: BranchAnchorHighlight[] }` 列表化 + MainTextBlock effect filter+迭代；零行为变化（`branches.length ≤ 1` invariant 保持）。**P1-S2 才解约束 + 加多分支 UI + 给 `paintSourceHighlight` 加 per-branch span 标识让多个 anchor 可共存不互擦**（当前实现 `paintSourceHighlight` 体内首行 `clearSourceHighlight()` 全文档擦，length ≤ 1 时正确，length ≥ 2 时第 N 个 paint 会把前 N-1 个擦掉 —— 这是 S2 的核心改造点）
