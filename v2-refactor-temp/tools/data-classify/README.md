...
- 没有孤立的分类条目
- 命名规范一致
- 同来源或同优先级数据没有重复目标键；跨来源重复目标键按生成器优先级消解并作为 warning 报告
- 生成代码结构正确
...
`generate-migration.js` 和 `validate-consistency.js` 使用同一优先级规则：跨来源映射到同一个 preference / bootConfig `targetKey` 时，生成器选择最高优先级来源，校验器只报告 warning；同来源或同优先级的重复仍然是 error，需要修正分类。
