...
| `legacy_v1` | The verified targets in the audit below | File usage is read from disk; localStorage and IndexedDB are logical byte estimates | Best effort per target; a failure does not stop the remaining targets |
| `restore_staging` | Verified `Data.restore`, `IndexedDB.restore`, and `Local Storage.restore` directories | Regular-file disk usage | Removes the selected legacy restore markers and therefore cancels that pending restore |

Missing files contribute `0 B`. Symbolic links are never followed or counted.
A candidate path that is itself a symbolic link is not removed, and validated
legacy or restore directories containing a symbolic link are retained. An
unreadable or invalid candidate makes the group size partial or unavailable but
does not make adjacent candidates eligible.
...
| `{userData}/config.json` | Non-empty JSON object containing only known v1 electron-store keys | Removes the file |
...
## Restore staging validation

- `IndexedDB.restore` must be a regular directory with no symlinks and only
  Chromium IndexedDB entries (`*.indexeddb.*`).
- `Local Storage.restore` must be a regular directory with no symlinks and only
  its `leveldb` entry.
- `Data.restore` must be a regular directory with no symlinks and only known v1
  data roots (`Agents`, `Channels`, `Files`, `KnowledgeBase`, `Memory`, `Notes`,
  `Skills`, `Workspace`, and `agents.db` plus its SQLite sidecars). An empty
  directory is valid because the legacy reset flow intentionally stages one.

Unknown entries make the restore target ineligible; they are not counted or
deleted.
