...
> `install_skill` is not in the explicit auto-approve list: Cherry applies a mandatory
...
9. **Final classifications:**
   ...
   - claw `cron` → **user**; `agent-memory` `memory` → **user**; `skills` `search_skills`/`install_skill` → **internal**, wired for every session (`install_skill` always requires interactive confirmation and is denied headlessly); claw `notify` + `config` → **user** (no channel gate; `notify` self-degrades at call time when no channel is connected).
...
- `settingsBuilder.buildMcpServers()` injects `cherry-tools` and `agent-memory` for every session; per-tool policy is enforced by `disallowedTools`/hooks, not by mounting/unmounting the server. `skills` is injected for every session too (`search_skills` auto-approved; `install_skill` always asks interactively and is denied headlessly).
...
| install_skill | skills | context | internal | installs one marketplace skill; mandatory interactive approval, denied headlessly |
...
- **`skills` wiring (done):** wired in skill-install-visibility-sync. `search_skills` is read-only (auto-approved); `install_skill` always requires interactive approval and a PreToolUse hook denies headless turns. Install resolves the exact repo directory and refuses to overwrite builtin/system/local/other-source skills.
