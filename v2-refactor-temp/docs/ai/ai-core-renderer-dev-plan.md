...
IpcChatTransport       ✅       现有组件渲染验证     ✅   消息删除   ✅             Batch1(稳定) ✅
共享 Schema            ✅                                Topic 双写  ✅             Batch2(适配) ✅
useAiChat hook         ✅                                                         Batch3(协调) ⬜
TestChat 页面验证      ✅                                                       删旧代码 ⬜
                                                                                Agent 统一 ⬜
```
...
| **P3.2b** | **Parts 直渲染 — Batch 1+2 ✅, Batch 3 阻塞于 Main 侧 Part 元数据补全** | **Person B** | **🔶** |
...
> **P3.2b Parts 直渲染 — 废弃 Block 类型系统 ⬜**
>
> **目标**: 所有渲染组件直接消费 `CherryMessagePart`，彻底废弃 `MessageBlock` 类型系统。
> V1 兼容不保留，Block 组件原地重构为 Part 消费者。
>
> **策略**: 在现有 Block 组件基础上原地重构（而非新建 Part 组件），保证 UI 和逻辑完全一致。
> 仅新建 `PartsRenderer` 替代 `MessageBlockRenderer` 的路由/分组逻辑。
>
> **架构变化**:
> ```
> 当前:
>   PartsContext[msgId] → partToBlock() → MessageBlock[] → groupSimilarBlocks()
>     → Block 组件（props: block: XXXMessageBlock）
>
> 目标:
>   PartsContext[msgId] → CherryMessagePart[] → groupSimilarParts()
>     → Part 组件（props: part: CherryMessagePart 子类型, partId, isStreaming）
> ```
>
> #### Block → Part 数据结构变化表
>
> **通用变化**: `BaseMessageBlock { id, messageId, type, createdAt, status, metadata, error }` 被废弃。
> 公共信息由 PartsRenderer 注入：`partId`（`${messageId}-part-${index}`）、`isStreaming`（从 message.status 推导）。
>
> | # | Block 类型 | Part 类型 | 字段映射 | 缺失/待设计 | 稳定度 |
> |---|-----------|-----------|---------|------------|--------|
> | 1 | `ThinkingMessageBlock` | `ReasoningUIPart` | `content→text`, `thinking_millsec→providerMetadata.cherry.thinkingMs`, `status→isStreaming` | 无 | ✅ 可立即改造 |
> | 2 | `CompactMessageBlock` | `DataUIPart<'compact'>` | `content→data.content`, `compactedContent→data.compactedContent` | 无 | ✅ 可立即改造 |
> | 3 | `TranslationMessageBlock` | `DataUIPart<'translation'>` | `content→data.content`, `targetLanguage→data.targetLanguage`, `sourceLanguage→data.sourceLanguage` | 无 | ✅ 可立即改造 |
> | 4 | `MainTextMessageBlock` | `TextUIPart` | `content→text`, citations: `citationReferences[].citationBlockId → providerMetadata.cherry.references: ContentReference[]` | citation 从独立 block 引用变为内联 references，需配合 CitationPart | ⚠️ citation 逻辑复杂 |
> | 5 | `ImageMessageBlock` | `FileUIPart` (image mediaType) | `url→url` | `file: FileMetadata` 无等价；`metadata.generateImageResponse` 无等价（Main 侧应为每张图输出独立 FileUIPart） | ⚠️ 需确认 Main 侧图片生成输出 |
> | 6 | `FileMessageBlock` | `FileUIPart` (non-image) | `file.path→url`, `file.name→filename` | FileUIPart 缺少 `size/ext/type/created_at`（FileMetadata 字段），下游 MessageAttachments 深度依赖 FileMetadata | ⚠️ 需重构 MessageAttachments |
> | 7 | `VideoMessageBlock` | `DataUIPart<'video'>` | `url→data.url`, `filePath→data.filePath` | `metadata.type`、`metadata.video.path`、`metadata.startTime` 在 VideoPartData 中无等价 | ⚠️ 需评估 V2 视频功能范围 |
> | 8 | `ToolMessageBlock` | `ToolUIPart` / `DynamicToolUIPart` | `toolId→toolCallId`, `toolName→从type解析或toolName`, `arguments→input`, `content→output` | `metadata.rawMcpToolResponse`（MCP状态/审批）无等价，需存入 `providerMetadata.cherry` | ❌ 需 Main 侧协调 |
> | 9 | `ErrorMessageBlock` | `DataUIPart<'error'>` | `error.message→data.message`, `error.name→data.name` | ErrorPartData 缺少 `i18nKey/status/statusCode/providerId/modelId/stack`；删除操作从 Redux dispatch 改为 DataApi patch | ❌ 需扩展 ErrorPartData |
> | 10 | `CitationMessageBlock` | `DataUIPart<'citation'>` | `response→data.?`, `knowledge→data.?`, `memories→data.?` | **data schema 当前为空 `{}`**，需完整定义 citation 数据结构；`askId` 需从 message 传入 | ❌ 需设计 citation data schema |
> | 11 | `CodeMessageBlock` | `DataUIPart<'code'>` | `content→data.content`, `language→data.language` | 无（极少独立出现，多在 TextUIPart markdown 中） | ✅ 可立即改造 |
>
> #### 下游委托组件调整
>
> | 委托组件 | 当前接收 | 改为接收 | 改动量 |
> |---------|---------|---------|-------|
> | `Markdown.tsx` | `block: MainText\|Translation\|Thinking\|Compact` (实际只用 id/content/status) | `MarkdownSource { id, content, status }` 轻量接口 | 低 |
> | `MessageTranslate.tsx` | `block: TranslationMessageBlock` | `MarkdownSource` + 翻译元数据 | 低 |
> | `MessageVideo.tsx` | `block: VideoMessageBlock` | `VideoPartData { url?, filePath? }` | 低 |
> | `MessageAttachments.tsx` | `block: FileMessageBlock` (深度依赖 FileMetadata) | `{ url, filename, mediaType }` 精简接口 | 中 |
> | `MessageTools.tsx` → `MessageMcpTool/MessageTool` | `block: ToolMessageBlock` | ToolUIPart + providerMetadata | 高 |
> | `CodeBlock.tsx` (Markdown 子组件) | `useResolveBlock(blockId)` | MarkdownContext `{ content, status, partId }` | 低 |
> | `Table.tsx` (Markdown 子组件) | `useResolveBlock(blockId)` | MarkdownContext `{ content, status, partId }` | 低 |
>
> #### 实施分批
>
> **Batch 1（稳定，可立即改造）**:
...
> | # | Task | 涉及文件 | 说明 | 状态 |
> |---|------|----------|------|------|
> | 1 | 定义 `MarkdownSource` 接口，Markdown.tsx props 放宽 | `Markdown.tsx` | `{ id: string; content: string; status: string }` 替代 block 联合类型 | ✅ |
> | 2 | 创建 `PartsRenderer` | `Blocks/PartsRenderer.tsx` | 从 PartsContext 读 parts，按 type 路由+分组，未迁移类型 fallback 到 partToBlock()→legacy Block | ✅ |
> | 3 | `MessageContent.tsx` 条件渲染 | `MessageContent.tsx` | `isV2 ? <PartsRenderer> : <MessageBlockRenderer>` | ✅ |
> | 4 | 重构 `ThinkingBlock` → 纯视图 props | `ThinkingBlock.tsx` | props: `{ id, content, isStreaming, thinkingMs }`，数据提取由 PartsRenderer/MessageBlockRenderer 负责 | ✅ |
> | 5 | 重构 `CompactBlock` → 纯视图 props | `CompactBlock.tsx` | props: `{ id, content, compactedContent }` | ✅ |
> | 6 | 重构 `TranslationBlock` + `MessageTranslate` → 纯视图 props | `TranslationBlock.tsx`, `MessageTranslate.tsx` | props: `{ id, content, isStreaming }`; MessageTranslate 接收 `MarkdownSource` | ✅ |
> | 7 | typecheck + test | — | 0 TS error, 294/295 tests pass (1 已有失败) | ✅ |
>
> **Batch 2（需小幅适配）**:
>
> | # | Task | 涉及文件 | 说明 | 状态 |
> |---|------|----------|------|------|
> | 1 | 重构 `MainTextBlock` → 纯视图 props | `MainTextBlock.tsx` | props: `{ id, content, isStreaming, citations, citationReferences, role, mentions }`，移除 Redux/useResolveBlock 依赖 | ✅ |
> | 2 | 重构 `ImageBlock` → 纯视图 props | `ImageBlock.tsx` | props: `{ images, isPending, isSingle }`，图片 URL 提取由 caller 负责 | ✅ |
> | 3 | 新增 `MarkdownBlockContext` | `Markdown.tsx` | 提供 `{ content, isStreaming }` 给 CodeBlock/Table 子组件 | ✅ |
> | 4 | `CodeBlock` 消费 MarkdownBlockContext | `CodeBlock.tsx` | `useMarkdownBlockContext()` 替代 `useResolveBlock(blockId)`，Redux fallback 保留 | ✅ |
> | 5 | `Table` 消费 MarkdownBlockContext | `Table.tsx` | `extractTableMarkdown` 改为接收 `markdownContent` 字符串，Redux fallback 保留 | ✅ |
> | 6 | V1 路径适配 `MainTextBlockWithCitations` | `Blocks/index.tsx` | 包装器处理 Redux citation 解析，传纯 props 给 MainTextBlock | ✅ |
> | 7 | PartsRenderer 直接路由 text/image | `PartsRenderer.tsx` | `text` → MainTextBlock + `convertReferencesToCitations`；image group 直接提取 URL | ✅ |
> | 8 | 修复 `data-code` 静默丢弃 | `PartsRenderer.tsx` | `data-code` part 包装为 markdown code fence 交给 MainTextBlock 渲染 | ✅ |
> | 9 | V2 citation 解析 | `partsToBlocks.ts` | 新增 `convertReferencesToCitations`，支持 web/knowledge/memory 三种 citation 类型 | ✅ |
> | 10 | typecheck + test | — | 0 新增 TS error，14/14 MainTextBlock + 10/10 PartsRenderer 测试通过 | ✅ |
>
> **Batch 3（需 Part 数据类型扩展或 Main 侧协调）**:
>
> | # | Task | 涉及文件 | 阻塞项 | 状态 |
> |---|------|----------|--------|------|
> | 1 | 重构 `ToolBlock/ToolBlockGroup` + `MessageTools` 链 | 多文件 | `rawMcpToolResponse` 需存入 providerMetadata | ⬜ |
> | 2 | 重构 `ErrorBlock` | `ErrorBlock.tsx` | ErrorPartData 需扩展 (i18nKey/status/providerId 等) | ⬜ |
> | 3 | 重构 `CitationBlock` | `CitationBlock.tsx` | `data-citation` data schema 需设计 | ⬜ |
> | 4 | 重构 `FileBlock` + `MessageAttachments` | `FileBlock.tsx`, `MessageAttachments.tsx` | FileUIPart 缺少 FileMetadata 字段 | ⬜ |
> | 5 | 重构 `VideoBlock` + `MessageVideo` | `VideoBlock.tsx`, `MessageVideo.tsx` | VideoPartData 需评估是否扩展 | ⬜ |
>
> #### Batch 3 可行性分析
>
> **结论**: Batch 3 中大部分组件无法纯 Renderer 侧解决，核心原因是 V2 Part 类型在持久化时丢失了大量 V1 Block 携带的元数据。
> Renderer 侧只能消费 Part 中已有的字段，缺失字段必须由 Main 侧在生成/持久化 Part 时注入。
>
> **逐组件可行性判定**:
>
> | # | 组件 | Renderer 侧可解决？ | 原因 |
> |---|------|---------------------|------|
> | 1 | ToolBlock + MessageTools 链 | ❌ 不能 | Part (`dynamic-tool`/`tool-*`) 仅提供 `toolCallId`, `toolName`, `state`, `input`, `output`。缺少 `serverName`, `serverId`, `tool.type`(mcp/builtin/provider), 细粒度 `status`, `partialArguments`(流式JSON)。 |
> | 2 | ErrorBlock | ⚠️ 部分可以 | `ErrorPartData` 只有 `{name?, message?, code?}`。`providerId`/`modelId` 可从 message 级别字段获取，`i18nKey` 可 Renderer 侧模式匹配推导。但 `statusCode` 和 `stack` 必须 Main 侧写入。 |
> | 3 | CitationBlock | ❌ 不能 | V2 **根本没有定义 `DataUIPart<'citation'>`**。需从零设计 `CitationPartData` schema + Main 侧生成逻辑。 |
> | 4 | FileBlock + MessageAttachments | ⚠️ 部分可以 | `FileUIPart` 提供 `{url, filename?, mediaType}`，但 `MessageAttachments` 深度依赖 `FileMetadata` 的 `size`, `ext`, `type`, `created_at`。部分可推导，`size`/`created_at` 无法推导。 |
> | 5 | VideoBlock + MessageVideo | ✅ 基本可以 | `VideoPartData {url?, filePath?}` 覆盖核心字段。`metadata.startTime` 缺失但属边缘功能，可暂不迁移。 |
>
> **Main 侧需求详细表**:
>
> | # | 组件 | 缺失字段 | Main 侧改动位置 | 具体方案 | 复杂度 |
> |---|------|---------|----------------|---------|--------|
> | 1a | ToolBlock | `tool.serverName`, `serverId`, `type` | AiCompletionService / toolCallbacks | 生成 `dynamic-tool` Part 时将 MCP tool 元信息写入 `providerMetadata.cherry: { tool: { serverName, serverId, type } }` | 中 |
> | 1b | ToolBlock | `status` 细粒度 (pending/streaming/invoking/cancelled) | toolCallbacks / MCP handler | AI SDK `state` 只有 3 值，需在 `providerMetadata.cherry` 中补充 `mcpStatus` 字段 | 中 |
> | 1c | ToolBlock | `partialArguments` (流式 JSON) | Stream delta 处理 | 流式阶段可从 `tool-*` Part 增量读取；**持久化后不需要**，仅影响流式 UX | 低（可跳过） |
> | 1d | ToolBlock | Approval 状态 | 独立于 Part 系统 | 通过 Redux `toolPermissions` 管理，V2 可保持独立状态管理，**不需要写入 Part** | 无需改动 |
> | 2a | ErrorBlock | `statusCode` | AiCompletionService `onError` | 构建 `data-error` Part 时从 `AISDKAPICallError.statusCode` 提取写入 `ErrorPartData`（需扩展类型） | 低 |
> | 2b | ErrorBlock | `stack` | AiCompletionService `onError` | 写入 `providerMetadata.cherry.error.stack`（类型已有此字段） | 低 |
> | 2c | ErrorBlock | `i18nKey` | **Renderer 侧可解决** | 根据 error message/code 模式匹配推导 | 无需改动 |
> | 2d | ErrorBlock | `providerId`, `modelId` | **Renderer 侧可解决** | 从 `message.modelMeta.provider` 和 `message.modelId` 读取 | 无需改动 |
> | 3a | CitationBlock | **整个 Part 类型不存在** | `uiParts.ts` 类型定义 | 新增 `CitationPartData` 到 `CherryDataPartTypes`，schema 含 `web`, `knowledge`, `memories` | 高 |
> | 3b | CitationBlock | Part 生成逻辑 | citationCallbacks → Main 侧等价物 | V2 citation 数据由 Main 侧外部工具回调产生，需构建 `data-citation` Part 写入 message.parts | 高 |
> | 3c | CitationBlock | `askId`（用户消息关联） | Message 级别 | V2 message 已有 `askId`，**不需要写入 Part** | 无需改动 |
> | 4a | FileBlock | `size`, `created_at` | Main 侧文件处理 | 生成 `FileUIPart` 时将文件元数据写入 `providerMetadata.cherry: { file: { size, createdAt, ext, type } }` | 低 |
> | 4b | FileBlock | `id` (FileMetadata.id) | **Renderer 侧可解决** | 用 `partId` (`${messageId}-part-${index}`) 替代 | 无需改动 |
> | 5a | VideoBlock | `startTime` | 类型扩展 | 在 `VideoPartData` 中新增 `startTime?: number` 字段 | 极低 |
>
> **建议执行顺序**: VideoBlock → ErrorBlock → FileBlock → ToolBlock → CitationBlock（按 Main 侧依赖从低到高）
>
> **Batch 4（清理）**:
>
> | # | Task | 涉及文件 | 状态 |
> |---|------|----------|------|
> | 1 | MessageOutline/MessageMenubar/MessageGroupMenuBar/MessageAnchorLine 删除 V1 fallback | 4 文件 | ⬜ |
> | 2 | MessageBlockRenderer 标记 deprecated（仅保留 V1 路径） | `Blocks/index.tsx` | ⬜ |
> | 3 | V2Contexts.ts 清理 deprecated hooks | `V2Contexts.ts` | ⬜ |
> | 4 | V2ChatContent 简化（移除 blocksToparts 依赖） | `V2ChatContent.tsx` | ⬜ |
> | 5 | Messages.tsx 事件处理器直读 partsMap | `Messages.tsx` | ⬜ |
