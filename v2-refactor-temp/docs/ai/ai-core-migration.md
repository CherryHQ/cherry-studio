...
| 操作 | 数量 | 说明 |
|------|------|------|
| **新建/重写** | ~9 个 | AiService (已存在,更新)、AiCompletionService (已存在,重写)、IpcChatTransport、shared schemas、`AiStreamManager` + 三个 Listener 实现 + `InternalStreamTarget` + `ClaudeCodeStreamAdapter`(全部在 Phase 2 完成;**不**造 `UIMessageAccumulator`,**不**造 `MessagePersistenceService` —— 前者复用 AI SDK 工具,后者直接用 Main 端已有的 `messageService` singleton) |
...
Renderer 侧:
4. `useChat` 调用处(`Chat.tsx`)配置 `sendAutomaticallyWhen: lastAssistantMessageIsCompleteWithApprovalResponses`
...
**需要修改的文件**:
```
src/renderer/src/pages/home/Chat.tsx                 # useChat 调用处添加 sendAutomaticallyWhen
src/renderer/src/pages/home/Messages/Tools/
...
```
  ...
  ㉑ Tool 权限审批接入官方 sendAutomaticallyWhen
  ㉒ 删除旧 Agent 代码 + E2E 测试
```
...
> **注**:更早版本的文档把 AiStreamManager 作为"Phase 7 补丁"放在最后 —— 那是基于"Phase 3 落地后才发现问题"的 retrospective 框架。现在已经前置到 Phase 2,正文章节 [Phase 2 架构详述: AiStreamManager](#phase-2-架构详述-aistreambroker) 保留了设计完整内容。Phase 6(ClaudeCodeService → ToolLoopAgent)和 Phase 2 AiStreamManager 仍然相互独立,可并行推进。
```
