...
New `preferred_endpoint_type` column on `user_model` (migration `0021_model_endpoint_preference.sql`, additive and nullable). New i18n keys under `settings.models.add.preferred_endpoint.*`: `label`, `tooltip`, `inherit`, `inherit_resolved`.
