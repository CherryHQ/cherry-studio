...
Use Settings → Prompt Management to create, edit, delete, reorder, or change prompt visibility.
Use an Assistant or Agent's Quick Phrases tab to link and order restricted prompts. A restricted
prompt with no links remains in Prompt Management but does not appear in chat.

## Notes for release manager

Changing a restricted prompt to global removes its restricted links after confirmation because the
prompt becomes available everywhere automatically. Editing or deleting a restricted prompt affects
