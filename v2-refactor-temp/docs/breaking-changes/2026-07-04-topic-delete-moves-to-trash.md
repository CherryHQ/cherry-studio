...
One thing does need a decision before upgrading: anything already sitting in the trash from an earlier version becomes subject to auto-purge for the first time, and whatever is older than the retention period is removed on the next scheduled run. If you want to keep it, set the trash retention to 0 before you upgrade, then restore what you need.
