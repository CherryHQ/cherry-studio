...
"Auto-accept Edits". In that mode the agent works on its own and only asks before a destructive shell command
(`rm`, `sudo`, `git reset --hard`, a piped remote script, …) or before touching a file outside its workspace and
data directory.
...
(and carries a "depends on the model" caveat), on Pi it is Cherry's own deterministic rule set. The Pi command
check is a best-effort net against ordinary mistakes, not a defense against a deliberately obfuscated command —
do not describe it as a security guarantee.
