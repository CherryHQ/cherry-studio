...
位于 [`src/shared/file/types/info.ts`](../../../src/shared/file/types/info.ts)：
...
async function ensureExternalEntry(
  ...
  // params.externalPath 已是 FilePath —— IPC 边界由 FilePathSchema 完成
  // canonicalize（segment resolve + NFC + trailing-sep strip），无需在 upsert
  // 路径里再跑一次。不含 fs.realpath（case-insensitive FS 去重由
  // findCaseInsensitivePeers + realpath probe 在 INSERT 之前完成）。
  // External 恒非 trashed（fe_external_no_delete CHECK），所以不需要 includeTrashed。
  const existing = await fileEntryService.findByExternalPath(params.externalPath);
  if (existing) return existing; // name/ext 来自 externalPath，不会漂移；size 不存

  await ops.stat(params.externalPath); // 纯探测：路径必须存在；副作用是更新 DanglingCache
  const { name, ext } = splitName(path.basename(params.externalPath));
...
位于 `src/shared/data/api/schemas/files.ts`。所有端点**只做 SQL**——不触 FS、不调 main-side resolver、不查 in-memory cache。响应 shape 按端点固定，**不使用 opt-in 派生字段**。任何需要 FS IO 或 main-side 计算的派生都搬到 File IPC（§7.2）。
...
详细类型契约见 [`src/shared/file/types/ipc.ts`](../../../src/shared/file/types/ipc.ts)。
...
- `canonicalizeAbsolutePath` / `FilePathSchema` 实现（契约与签名 Phase 1a 锁定，实现在 1b.1）
...
- `canonicalizeAbsolutePath` 真实现（segment resolve + NFC + trailing-sep strip）+ `FilePathSchema` 在 `packages/shared/file/types/common.ts` 中包装为 branded entry 点；8-10 条边界测试（NFC/NFD / trailing / `./a/../a` / Windows `\\` / 盘符大小写）
...
| `externalPath` 大小写不敏感 FS 导致同文件双 entry（macOS APFS / Windows NTFS） | 文件页用户看到两份同文件、file_ref 分裂 | Phase 1b.1 `FilePathSchema` 在 IPC 边界做同步廉价规范化（resolve + NFC + trailing-sep），case 去重由 `ensureExternalEntry` 的 `findCaseInsensitivePeers` + `fs.realpath` probe 在 INSERT 之前完成；DB 端由 `UNIQUE(lower(externalPath))` 功能索引兜底 |
...
> - 类层级:`TreeNode` / `TreeFile` / `TreeDir` / `TreeDirRoot` 落在 `src/shared/file/types/tree.ts`,主进程与渲染进程共享。
...
- 保留 `id` 与所有 `file_ref`
- 调用 `FilePathSchema.parse` 归一化新路径
