...
> - [`migration-plan.md`](./migration-plan.md) — 字段级退役 + 消费域切换 + 跨模块协调的详细调研
> - [`phase2-consumer-migration.md`](./phase2-consumer-migration.md) — **Phase 2 消费方迁移实施计划**（任务分解、依赖图、Batch 划分，取代本文档 §9.7 原有的 Batch A-E）
...
> **本节原有的 Batch A-E 划分已废弃。** 详细的任务分解、依赖分析与实施计划见 [`phase2-consumer-migration.md`](./phase2-consumer-migration.md)。以下仅保留概述。

**Batch 0（已完成）**：FileMigrator + 跨模块协调（PR #15067）。Dexie `db.files` 一次性搬到 `file_entry`；KnowledgeMigrator 内新增 file_ref 创建。ChatMigrator file_ref 延后到 chat 域整体迁移（§8.4）；PaintingMigrator 同样延后（§8.5）。

**Batch A-E（消费方迁移）**：按业务域从小到大推进（Translate/Agent/Export → Paste/OCR/AI Core → Painting → Knowledge → Messages），每个域原子切换——消费者直接从 `FileMetadata` 迁移到 `FileEntry`/`FileInfo`/`FileHandle`，不经过中间态（无 Dexie freeze、无 `toFileMetadata` shim、无独立字段退役步骤）。

**前置条件**：
- **PREREQ-INFRA**：v2 File IPC 接线（当前 48 个方法中仅 7 个已接线）、共享工具提取、renderer hooks
- Backup/Restore 的 v2 感知由 [#12659](https://github.com/CherryHQ/cherry-studio/issues/12659) 独立处理，不阻塞消费方迁移

**CLEANUP**：所有域迁移完成后，一次性移除 Dexie `files` 表、`FileMetadata` 类型、`FileStorage` 服务、v1 preload APIs。
