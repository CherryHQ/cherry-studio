# KnowledgeFacade 设计文档

> **最后更新**: 2026-01-18
>
> **状态: 计划功能 (尚未实现)**
>
> 本文档描述的 KnowledgeFacade 是计划中的功能，旨在为 MCP Tools、HTTP API、Agent 提供统一的简化接口。
> 当前访问知识库的方式是通过 DataApi handlers 或 IPC 直接调用服务层。

## 概述

`KnowledgeFacade` 是知识库系统对外的统一接口层，为 MCP Tools、HTTP API、Agent 等外部调用者提供简洁、安全的 API。

## 架构层级

```
┌─────────────────────────────────────────────────────────────────────────────┐
│                         External Access Layer                                │
├─────────────────────────────────────────────────────────────────────────────┤
│                                                                              │
│   ┌───────────────┐    ┌───────────────┐    ┌───────────────┐               │
│   │   MCP Tools   │    │   HTTP API    │    │  Agent Tools  │               │
│   └───────┬───────┘    └───────┬───────┘    └───────┬───────┘               │
│           │                    │                    │                        │
│           └────────────────────┼────────────────────┘                        │
│                                ▼                                             │
│   ┌─────────────────────────────────────────────────────────────────────┐   │
│   │                      KnowledgeFacade                                 │   │
│   │                      ═══════════════                                 │   │
│   │   对外提供统一、简洁的 API                                            │   │
│   │                                                                      │   │
│   │   📗 标准模式 (Safe Mode)                                            │   │
│   │   ├── addDocument(baseId, input) → itemId                           │   │
│   │   ├── search(baseId, query) → results                               │   │
│   │   ├── getJobStatus(itemId) → status                                 │   │
│   │   └── removeDocument(baseId, itemId)                                │   │
│   │                                                                      │   │
│   │   ⚡ 上帝模式 (God Mode)                                             │   │
│   │   ├── rawQuery(baseId, sql) → results                               │   │
│   │   ├── updateChunkMetadata(baseId, chunkId, metadata)                │   │
│   │   └── getChunks(baseId, itemId) → chunks                            │   │
│   │                                                                      │   │
│   └─────────────────────────────────────────────────────────────────────┘   │
│                                │                                             │
└────────────────────────────────┼─────────────────────────────────────────────┘
                                 │
┌────────────────────────────────┼─────────────────────────────────────────────┐
│                      DataApi Layer (现有)                                    │
├────────────────────────────────┼─────────────────────────────────────────────┤
│   ┌─────────────────────────────────────────────────────────────────────┐   │
│   │  KnowledgeBaseService          │        KnowledgeItemService        │   │
│   │  (SQLite CRUD - Base)          │        (SQLite CRUD - Items)       │   │
│   └─────────────────────────────────────────────────────────────────────┘   │
│                                │                                             │
└────────────────────────────────┼─────────────────────────────────────────────┘
                                 │
┌────────────────────────────────┼─────────────────────────────────────────────┐
│                     Service Layer (已重构)                                   │
├────────────────────────────────┼─────────────────────────────────────────────┤
│                                ▼                                             │
│   ┌─────────────────────────────────────────────────────────────────────┐   │
│   │                    KnowledgeOrchestrator                             │   │
│   │                    (协调层 - 状态管理)                                │   │
│   └─────────────────────────────────────────────────────────────────────┘   │
│                                │                                             │
│                                ▼                                             │
│   ┌─────────────────────────────────────────────────────────────────────┐   │
│   │                    KnowledgeProcessor                                │   │
│   │                    (处理层 - 业务流程)                                │   │
│   │                    OCR → Read → Embed → Store                        │   │
│   └─────────────────────────────────────────────────────────────────────┘   │
│                                │                                             │
│                                ▼                                             │
│   ┌─────────────────────────────────────────────────────────────────────┐   │
│   │                    KnowledgeQueueManager                             │   │
│   │                    (调度层 - 并发控制)                                │   │
│   └─────────────────────────────────────────────────────────────────────┘   │
│                                │                                             │
│                                ▼                                             │
│   ┌─────────────────────────────────────────────────────────────────────┐   │
│   │                    KnowledgeServiceV2                                │   │
│   │                    (存储层 - 向量操作)                                │   │
│   └─────────────────────────────────────────────────────────────────────┘   │
│                                                                              │
└──────────────────────────────────────────────────────────────────────────────┘
```

## 层级职责

| 层级 | 调用者 | 职责 |
|------|--------|------|
| **KnowledgeFacade** | MCP/API/Agent | 对外简化接口，隐藏内部细节 |
| **DataApi Services** | Renderer UI / Facade | SQLite CRUD + 调用 Orchestrator |
| **Orchestrator** | DataApi Services | 协调处理流程，管理状态回调 |
| **Processor** | Orchestrator | 封装 OCR → Read → Embed 流程 |
| **QueueManager** | Processor | 任务调度，并发控制 |
| **ServiceV2** | Processor | 向量存储，搜索，重排序 |

## Facade vs DataApi 服务的区别

| 维度 | DataApi 服务 | KnowledgeFacade |
|------|-------------|-----------------|
| **面向对象** | 内部 UI (Renderer) | 外部系统 (MCP, API, Agent) |
| **接口风格** | CRUD 风格，暴露 DTO | 业务语义，隐藏细节 |
| **返回值** | 完整实体 (KnowledgeItem) | 简化结果 (只含必要信息) |
| **错误处理** | DataApiError | 业务友好的错误信息 |
| **上帝模式** | 无 | 有 (rawQuery, updateChunkMetadata) |

## API 设计

### 类型定义

```typescript
// ═══════════════════════════════════════════════════════════════════════════
// 输入类型
// ═══════════════════════════════════════════════════════════════════════════

interface AddDocumentInput {
  type: 'file' | 'url' | 'note' | 'sitemap' | 'directory'
  data: KnowledgeItemData
  metadata?: Record<string, unknown>
}

interface SearchOptions {
  topK?: number
  threshold?: number
  rerank?: boolean
  filter?: {
    itemIds?: string[]
    metadata?: Record<string, unknown>
  }
}

// ═══════════════════════════════════════════════════════════════════════════
// 输出类型
// ═══════════════════════════════════════════════════════════════════════════

interface AddDocumentResult {
  itemId: string
  status: 'pending' | 'ocr' | 'read' | 'embed' | 'completed' | 'failed'
}

interface SearchResult {
  content: string
  score: number
  metadata: Record<string, unknown>
  itemId?: string
  chunkId?: string
}

interface JobStatus {
  status: 'idle' | 'pending' | 'ocr' | 'read' | 'embed' | 'completed' | 'failed'
  progress: number
  error?: string
  stage?: 'ocr' | 'read' | 'embed'
}

interface ChunkInfo {
  id: string
  content: string
  metadata: Record<string, unknown>
  embedding?: number[]
}
```

### Facade 类

```typescript
class KnowledgeFacade {
  // ─────────────────────────────────────────────────────────────────────────
  // 📗 标准模式 - 简单安全的操作
  // ─────────────────────────────────────────────────────────────────────────

  /**
   * 添加文档到知识库 (异步)
   * @returns itemId 用于查询处理状态
   */
  async addDocument(baseId: string, input: AddDocumentInput): Promise<AddDocumentResult>

  /**
   * 搜索知识库 (同步)
   */
  async search(baseId: string, query: string, options?: SearchOptions): Promise<SearchResult[]>

  /**
   * 查询处理状态
   */
  async getJobStatus(itemId: string): Promise<JobStatus>

  /**
   * 删除文档 (同步)
   */
  async removeDocument(baseId: string, itemId: string): Promise<void>

  /**
   * 列出知识库中的文档
   */
  async listDocuments(baseId: string, options?: { status?: ItemStatus }): Promise<DocumentInfo[]>

  // ─────────────────────────────────────────────────────────────────────────
  // ⚡ 上帝模式 - 直接操作底层
  // ─────────────────────────────────────────────────────────────────────────

  /**
   * 直接执行 SQL 查询 (只读)
   * 用于: Agent 查询知识库统计信息、调试等
   */
  async rawQuery(baseId: string, sql: string): Promise<unknown[]>

  /**
   * 获取文档的所有 chunks
   * 用于: Agent 检查分块结果、调试
   */
  async getChunks(baseId: string, itemId: string): Promise<ChunkInfo[]>

  /**
   * 更新 chunk 的 metadata (Agent 标注)
   * 用于: Agent 添加标签、评分、标记过时等
   */
  async updateChunkMetadata(
    baseId: string,
    chunkId: string,
    metadata: Record<string, unknown>
  ): Promise<void>

  /**
   * 给 chunk 打分 (影响搜索权重)
   * 用于: Agent 标记高质量/低质量内容
   */
  async scoreChunk(baseId: string, chunkId: string, score: number): Promise<void>
}
```

## 使用示例

### MCP Tool: 添加笔记

```typescript
// Agent 在对话中学习到新信息，写入知识库
const result = await knowledgeFacade.addDocument(baseId, {
  type: 'note',
  data: { content: '用户说项目A改名为凤凰计划', sourceUrl: 'chat://xxx' },
  metadata: { tags: ['项目A', '凤凰计划'], source: 'conversation' }
})
// { itemId: 'xxx', status: 'queued' }
```

### MCP Tool: 搜索

```typescript
// Agent 搜索知识库
const results = await knowledgeFacade.search(baseId, '凤凰计划进度', {
  topK: 5,
  rerank: true
})
```

### MCP Tool: Agent 标注 (上帝模式)

```typescript
// Agent 发现过时信息，标记为过时
await knowledgeFacade.updateChunkMetadata(baseId, chunkId, {
  outdated: true,
  outdatedReason: '项目已暂停',
  annotatedBy: 'agent',
  annotatedAt: Date.now()
})
```

### MCP Tool: 查询统计 (上帝模式)

```typescript
// Agent 查询知识库统计信息
const stats = await knowledgeFacade.rawQuery(baseId,
  'SELECT COUNT(*) as total, type FROM chunks GROUP BY type'
)
```

## 实现要点

### 1. Facade 调用 DataApi 服务

```typescript
class KnowledgeFacade {
  async addDocument(baseId: string, input: AddDocumentInput): Promise<AddDocumentResult> {
    // Facade 调用 ItemService，不直接调用 Orchestrator
    const { items } = await knowledgeItemService.create(baseId, {
      items: [{ type: input.type, data: input.data }]
    })
    return { itemId: items[0].id, status: 'queued' }
  }

  async search(baseId: string, query: string, options?: SearchOptions): Promise<SearchResult[]> {
    // 调用 KnowledgeBaseService.search()
    const base = await knowledgeBaseService.getById(baseId)
    return knowledgeServiceV2.search({ base, search: query })
  }
}
```

### 2. 上帝模式直接访问 VectorStore

```typescript
class KnowledgeFacade {
  async rawQuery(baseId: string, sql: string): Promise<unknown[]> {
    // 安全检查: 只允许 SELECT
    if (!sql.trim().toUpperCase().startsWith('SELECT')) {
      throw new Error('Only SELECT queries are allowed in rawQuery')
    }
    // 直接查询 LibSQLVectorStore
    return knowledgeServiceV2.rawQuery(baseId, sql)
  }

  async getChunks(baseId: string, itemId: string): Promise<ChunkInfo[]> {
    return knowledgeServiceV2.getChunksByItemId(baseId, itemId)
  }
}
```

## 文件位置

```
src/main/services/knowledge/
├── KnowledgeFacade.ts          # 对外统一接口
├── KnowledgeOrchestrator.ts    # 协调层
├── KnowledgeProcessor.ts       # 处理层
├── KnowledgeServiceV2.ts       # 存储层
├── queue/
│   └── KnowledgeQueueManager.ts
└── ...
```

## 与 Agentic RAG 的关系

参见 [knowledge-v2.md](./knowledge-v2.md) 中的 Agentic RAG 愿景。

`KnowledgeFacade` 是实现 Agentic RAG 的基础设施：

| 能力 | Facade API | 说明 |
|------|-----------|------|
| 基础 RAG | `search()` | 混合检索 + 重排序 |
| 知识维护 | `addDocument()` | Agent 对话中直接写入 |
| 智能标注 | `updateChunkMetadata()` | Agent 标记过时、打分 |
| 上帝模式 | `rawQuery()` | Agent 自由探索知识库 |

通过 `KnowledgeFacade`，Agent 可以：
1. 在对话中学习新知识并写入知识库
2. 发现过时信息时主动标注
3. 给高质量内容打分，提升检索权重
4. 自由查询知识库结构和统计信息
