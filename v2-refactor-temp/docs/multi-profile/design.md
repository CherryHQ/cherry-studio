...
落盘于 `~/.cherrystudio/profiles.json`（自管文件而非 BootConfig key：注册表是列表结构，BootConfig 为扁平 KV；预启动读已记忆化、原子写已具备，合并的收益不抵结构错配）。

- 注册表带 `version` 字段（当前 1），缺失按 1 兼容——结构必然演化（avatar / lastUsedAt 等），版本号先行。
- `id`：8 位 base62 `nanoid`（`customAlphabet('0-9A-Za-z', 8)`，无前缀），不可变；`addProfile` 生成时对现有 id 查重、撞则重生成。默认 profile 保留 id `default`（不经生成器）。命名：`name` 允许重名（id 唯一标识）；默认 profile 可改名（`name` 仅为 label，id 恒为 `default`）。
- 变换：`addProfile / renameProfile / setActive : ProfileRegistry -> ProfileRegistry`；`findEntry(reg, id)`。
- 读写：`readProfileRegistry()` 逐条校验 entry——坏条目丢弃并记 error 日志，不连累合法条目；文件缺失 / 不可解析回落仅含默认 profile 的注册表（记 error 日志，不静默）。`writeProfileRegistry(reg)` 经临时文件 + rename 原子写。
...
origin 级存储：所有窗口共享同一 origin，localStorage / IndexedDB 中的 per-profile 内容（散置凭据、Dexie 库等）不随 reload 消失——切换重置须 `session.clearStorageData`（限 localStorage / IndexedDB，白名单排除有意全局的 key）。

池化窗口的挂起（`suspendPool`）须在重置完成后成对恢复，否则池化契约在首次切换后永久失效；retention 保活的隐藏单例窗口须销毁而非隐藏，避免旧 profile 的活 renderer 幸存。
...
| 6 | 裸 `ipcMain.handle` 存量 | `wrapIpcHandler(fn)` 薄封装一次性迁完（存量少），不 monkey-patch |
