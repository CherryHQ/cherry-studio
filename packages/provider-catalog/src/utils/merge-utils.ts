...
export function mergeObjects<T extends Record<string, any>>(
  ...
  const { preserveExisting = true, alwaysOverwrite = [], neverOverwrite = [] } = options
...
/**
 * Deduplicate models array by ID (case-insensitive)
 * Keeps the first occurrence of each ID
 *
 * @param models - Array of models (may contain duplicates)
 * @returns Deduplicated array with lowercase IDs
 */
export function deduplicateModels(models: ModelConfig[]): ModelConfig[] {
  const seen = new Map<string, ModelConfig>()

  for (const model of models) {
    const normalizedId = model.id.toLowerCase()
    if (!seen.has(normalizedId)) {
      // Keep first occurrence, ensure lowercase ID
      seen.set(normalizedId, { ...model, id: normalizedId })
    } else {
      // Merge with existing (fill undefined values)
      const existing = seen.get(normalizedId)!
      seen.set(normalizedId, mergeObjects(existing, model))
    }
  }
...
export const MergeStrategies = {
  ...
  PRESERVE_MANUAL: {
    preserveExisting: true,
    alwaysOverwrite: ['pricing', 'contextWindow', 'maxOutputTokens'],
    neverOverwrite: ['description', 'capabilities', 'inputModalities', 'outputModalities']
  } as MergeOptions
}
