...
describe('buildThemeContractCss', () => {
  it('maps token sources into the public theme contract', async () => {
    ...
    expect(css).toContain('--cs-theme-accent: var(--cs-primary);')
    expect(css).not.toContain('--cs-theme-accent-hover')
    ...
    expect(css).toContain('--color-theme-accent-soft: color-mix(in srgb, var(--color-theme-accent) 60%, transparent);')
    expect(css).toContain('--color-theme-accent-foreground: var(--cs-theme-accent-foreground);')
    expect(css).toContain('--color-control-accent: var(--color-theme-accent);')
    expect(css).toContain('--color-control-accent-foreground: var(--color-theme-accent-foreground);')
    expect(css).toContain('--color-link: var(--color-theme-accent);')
    ...
    expect(css).toContain('/* Component Slots (internal — see tokens/colors/component.css) */')
    expect(css).toContain('Advanced CSS may override them')
    expect(css).toContain('--color-button-elevated-from: var(--cs-button-elevated-from);')
    expect(css).toContain('--color-tabbar-glass-surface: var(--cs-tabbar-glass-surface);')
    expect(css).toContain('--color-switch-thumb: var(--cs-switch-thumb);')
    expect(css).toContain('--color-badge-highlight-border: var(--cs-badge-highlight-border);')
