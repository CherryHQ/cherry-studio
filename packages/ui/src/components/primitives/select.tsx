...
const selectTriggerVariants = cva(
  cn(
    ...
    'border border-input bg-transparent hover:border-border-strong hover:bg-transparent dark:bg-transparent',
  ...
  return (
    <SelectPrimitive.Trigger
      ...
      <SelectPrimitive.Icon asChild>
        <ChevronDownIcon className="size-4 text-foreground-tertiary" />
...
function SelectItem({ className, children, ...props }: React.ComponentProps<typeof SelectPrimitive.Item>) {
  return (
    <SelectPrimitive.Item
      ...
      className={cn(
        "focus:bg-accent focus:text-accent-foreground data-[state=checked]:bg-control-accent/10 data-[state=checked]:text-control-accent [&_svg:not([class*='text-'])]:text-muted-foreground relative flex w-full cursor-default items-center gap-2 rounded-lg py-1 pr-8 pl-2 text-sm outline-hidden select-none data-[disabled]:pointer-events-none data-[disabled]:opacity-50 [&_svg]:pointer-events-none [&_svg]:shrink-0 [&_svg:not([class*='size-'])]:size-4 *:[span]:last:flex *:[span]:last:items-center *:[span]:last:gap-2",
        className
      )}
      {...props}>
      <span className="absolute right-2 flex size-3.5 items-center justify-center">
        <SelectPrimitive.ItemIndicator>
          <CheckIcon className="size-4 text-control-accent" />
        </SelectPrimitive.ItemIndicator>
      </span>
