...
export interface SelectDropdownProps<T extends { id: string }> {
   ...
   * The trigger uses the shared outlined control appearance by default; use `data-[state=open]:*` selectors
   * to customize the open-state surface per instance.
   */
      ...
      return (
        <div
          className={cn(
            ...
            isSelected && 'bg-control-accent/10 text-control-accent'
          )}>
          <button
            type="button"
            onClick={() => {
    ...
    return (
      <button
        ...
        className={cn(
          ...
          isSelected ? 'bg-control-accent/10 text-control-accent' : 'text-foreground hover:bg-muted'
        )}>
        {renderItem(item, isSelected)}
      </button>
    )
    ...
    <Popover open={open} onOpenChange={setOpen}>
      <PopoverTrigger asChild>
        <button
          type="button"
          className={cn(
            'flex h-8 w-full items-center justify-between rounded-lg border border-input bg-transparent px-3 text-sm transition-colors hover:border-border-strong hover:bg-transparent focus-visible:border-ring dark:bg-transparent',
            triggerClassName
          )}>
          ...
          <ChevronDown
            size={12}
            className={cn('ml-2 shrink-0 text-foreground-tertiary transition-transform', open && 'rotate-180')}
          />
        </button>
      </PopoverTrigger>
      <PopoverContent
        align="start"
        sideOffset={4}
        className="w-(--radix-popover-trigger-width) rounded-lg border border-border-subtle bg-popover/70 p-1 shadow-lg backdrop-blur-xl supports-[backdrop-filter]:bg-popover/60">
