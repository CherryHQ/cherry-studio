...
export const ColorPickerSelection = memo(
  ({ className, 'aria-label': ariaLabel, ...props }: ColorPickerSelectionProps) => {
    ...
    const handlePointerMove = useCallback(
      (event: PointerEvent) => {
        if (isDragging) commitFromEvent(event)
      },
...
export const ColorPickerEyeDropper = ({ className, ...props }: ColorPickerEyeDropperProps) => {
  ...
  const handleEyeDropper = async () => {
    try {
      ...
      // EyeDropper throws when the user cancels.
    }
  }
...
export const ColorPickerFormat = ({ className, ...props }: ColorPickerFormatProps) => {
  ...
  if (mode === 'rgb') {
    const rgb = color.rgb().array().slice(0, 3).map(Math.round)
  ...
  if (mode === 'hsl') {
    const hsl = color.hsl().array().slice(0, 3).map(Math.round)
