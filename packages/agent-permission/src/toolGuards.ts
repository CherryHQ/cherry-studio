...
import { normalizeLegacyPermissionMode } from './types'
...
export interface ToolGuardContext {
  ...
  /** Legacy persisted names are accepted until the persistence cutover phase. */
  readonly permissionMode: AgentPermissionMode | string | undefined
  ...
  /** Optional same-process diagnostic sink. A thrown condition is always treated as a non-match. */
  readonly log?: (event: { message: string; ruleId?: string; toolName?: string; error?: unknown }) => void
  /** Opaque context supplied by the adapter for runtime-local conditions. */
  readonly guardContext?: unknown
}
...
export async function evaluateToolGuards(
  ...
  const bypass = normalizeLegacyPermissionMode(ctx.permissionMode) === 'full'
