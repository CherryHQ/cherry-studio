/**
 * Product-conduct detectors shared by every agent runtime.
 *
 * These detectors classify normalized shell/tool facts; they do not authorize a call by themselves.
 * The runtime-neutral evaluator decides how each fact is enforced in the current mode.
 */
...
const DESTRUCTIVE_COMMAND_PATTERNS: readonly DestructiveCommandPattern[] = [
  { reason: 'permanent file deletion', pattern: /\b(?:rm|unlink|shred|srm)\b/i },
  { reason: 'permanent Windows file deletion', pattern: /\b(?:remove-item|clear-content|del|erase|rd|rmdir)\b/i },
  {
    reason: 'scripted permanent file deletion',
    pattern: /\b(?:shutil\.rmtree|os\.(?:remove|unlink|rmdir)|fs\.(?:rm|unlink|rmdir)(?:sync)?)\b/i
  },
  { reason: 'recursive find deletion', pattern: /\bfind\b[\s\S]*\s-delete(?:\s|$)/i },
  {
  ...
  { reason: 'discarding version-control data', pattern: GIT_DESTRUCTIVE_COMMAND_PATTERN },
  {
  ...
  { reason: 'a process-exhaustion payload', pattern: /:\s*\(\s*\)\s*\{\s*:\s*\|\s*:\s*&\s*\}\s*;?\s*:/i }
]
...
export function detectDestructiveAssistantCommand(command: string): string | undefined {
  return DESTRUCTIVE_COMMAND_PATTERNS.find(({ pattern }) => pattern.test(normalizeCommand(command)))?.reason
}

export function isLarkFormSubmissionCommand(command: string): boolean {
  return LARK_FORM_SUBMISSION_PATTERN.test(normalizeCommand(command))
}

export function isGitHubIssueCreationCommand(command: string): boolean {
  return GITHUB_ISSUE_CREATION_PATTERN.test(normalizeCommand(command))
}
