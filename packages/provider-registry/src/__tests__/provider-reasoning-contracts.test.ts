...
describe('provider reasoning contracts', () => {
  ...
  it.each(['kimi-k3', 'kimi-k3-fast'])('gives Moonshot %s an effort wire of its own', (modelId) => {
    ...
    expect(moonshotWire?.effort?.operations).toEqual([
  ...
  // DeepSeek publishes one effort table for every SKU (thinking_mode guide), so the contracts must not
  // drift apart — and none may send `xhigh` verbatim, which DeepSeek degrades to `high`.
