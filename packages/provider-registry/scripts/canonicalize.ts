...
// OpenRouter's `:batch` twin is the same model submitted through the async Batch API — a separate job
// endpoint the app never calls, listed at a discount. Dropped rather than folded onto the base: folding
// would let the discounted twin claim the base row's pricing and `apiModelId`.
