...
export const getLowerBaseModelName = (id: string, delimiter: string = '/'): string => {
  ...
  // for fireworks provider model ids, convert "v3p2" => "v3.2"
  if (id.toLowerCase().includes('accounts/fireworks/models/')) {
    baseModelName = baseModelName.replace(/(\d)p(?=\d)/g, '$1.')
  }
