...
/**
 * Assistant entity ID — either:
 * - The system-reserved literal `DEFAULT_ASSISTANT_ID` ('default'), used by
 *   `DefaultAssistantSeeder` for the built-in row and by `ChatMigrator` as
 *   the orphan-FK fallback target.
 * - A standard UUID v4 for user-created and migrated assistants.
 */
export const AssistantIdSchema = z.union([z.literal(DEFAULT_ASSISTANT_ID), z.uuidv4()])
...
export const AssistantSchema = z.strictObject({
  /** Assistant ID — UUID v4 (DB-generated) or the literal 'default' for the system-reserved row */
...
/**
 * Default assistant row — shared by `DefaultAssistantSeeder` and `AssistantMigrator` backstop.
 *
 * Annotated so a future required field on the entity fails compilation here,
 * forcing the constant to be updated alongside the schema.
 */
export const DEFAULT_ASSISTANT_PAYLOAD = {
  id: DEFAULT_ASSISTANT_ID,
  // TODO: i18n — must stay build-time-stable so seeder version hash doesn't drift on locale switches
  name: 'Default Assistant',
  prompt: '',
  emoji: '🌟',
  description: '',
  settings: DEFAULT_ASSISTANT_SETTINGS
} as const satisfies Pick<Assistant, 'id' | 'name' | 'prompt' | 'emoji' | 'description' | 'settings'>
