...
export function selectExternalText(platform: Platform): void {
  ...
  const script = [
    ...
    `Add-Type -TypeDefinition 'using System; using System.Runtime.InteropServices; public struct NativeRect { public int Left; public int Top; public int Right; public int Bottom; } public static class NativeMouse { [DllImport("user32.dll")] public static extern bool GetWindowRect(IntPtr window, out NativeRect rect); [DllImport("user32.dll")] public static extern bool SetCursorPos(int x, int y); [DllImport("user32.dll")] public static extern void mouse_event(uint flags, uint x, uint y, uint data, UIntPtr extra); }'`,
    ...
    '} else {',
    '  $windowRect = [NativeRect]::new()',
    '  if (-not [NativeMouse]::GetWindowRect($notepad.MainWindowHandle, [ref]$windowRect)) { throw "Notepad bounds could not be read" }',
    '  $rect = [System.Windows.Rect]::new($windowRect.Left, $windowRect.Top + 55, $windowRect.Right - $windowRect.Left, $windowRect.Bottom - $windowRect.Top - 55)',
    '}',
