...
- `runtime.ts` composes caller-owned semantics/scopes with implementation-owned structural parts.

The protocol contains static semantic tokens and optional runtime `scope:*` tokens. There is no exact-node `id:*`
namespace or persistent identity registry.
...
as authored `data-ui` `part:*` tokens. Semantic/scope tokens passed through component props are merged with the
intrinsic node's structural semantics, including through JSX spreads and Radix `asChild` slots.
