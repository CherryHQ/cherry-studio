...
describe('validate accepts translations the catalog already relies on', () => {
  ...
  it('flags nothing in the shipped catalog', () => {
    ...
    for (const dir of ['src/renderer/i18n/locales', 'src/main/i18n/locales']) {
      const base = flatten(JSON.parse(fs.readFileSync(path.join(ROOT, dir, 'en-us.json'), 'utf-8')))
      const localeDirectory = path.join(ROOT, dir)
      for (const file of fs
        .readdirSync(localeDirectory)
        .filter((filename) => filename.endsWith('.json') && filename !== 'en-us.json')) {
        const target = flatten(JSON.parse(fs.readFileSync(path.join(localeDirectory, file), 'utf-8')))
