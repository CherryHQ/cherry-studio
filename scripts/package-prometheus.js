...
const entries = ['skills','scripts','lib','rules','references','agents','templates','hooks','docker','commands','shared','docs','schemas','assets','config','.agents/skills']
...
function inventory(directory, prefix = '') {
  return fs.readdirSync(path.join(directory,prefix),{ withFileTypes: true }).flatMap((entry) => {
    ...
    return [{ path: relative.split(path.sep).join('/'), sha256: crypto.createHash('sha256').update(fs.readFileSync(path.join(directory,relative))).digest('hex') }]
  })
}
...
function packagePrometheus() {
  ...
  const gitlink = execFileSync('git',['ls-tree','HEAD','resources/prometheus-skills-mini'],{ cwd: root, encoding: 'utf8' }).trim().split(/\s+/)[2]
  ...
  for (const entry of entries) {
    ...
    if (fs.existsSync(from)) fs.cpSync(from,path.join(destination,entry),{ recursive: true, dereference: true,
      filter: (filename) => !/(?:^|[\\/])(?:node_modules|__tests__|\.git)(?:[\\/]|$)|\.test\.[cm]?[jt]s$/.test(filename) })
  }
  ...
  if (npm && /npm-cli\.js$/.test(npm)) execFileSync(process.execPath,[npm,'ci','--ignore-scripts','--include=dev','--no-audit','--no-fund'],{ cwd: destination, stdio: 'inherit' })
  else if (process.platform === 'win32') {
    const npmCli = path.join(path.dirname(process.execPath),'node_modules','npm','bin','npm-cli.js')
    execFileSync(process.execPath,[npmCli,'ci','--ignore-scripts','--include=dev','--no-audit','--no-fund'],{ cwd: destination, stdio: 'inherit' })
  } else execFileSync('npm',['ci','--ignore-scripts','--include=dev','--no-audit','--no-fund'],{ cwd: destination, stdio: 'inherit' })
  ...
  if (!artifacts.compassSkills?.sha256 || !artifacts.compassSkills?.url) throw new Error('Compass skill archive is not pinned')
  ...
  fs.rmSync(archive)
  for (const skill of fs.readdirSync(compass)) fs.cpSync(path.join(compass,skill),path.join(destination,'skills',skill),{ recursive: true })
  const skills = fs.readdirSync(path.join(destination,'skills'),{ withFileTypes: true }).filter((entry) => entry.isDirectory() && fs.existsSync(path.join(destination,'skills',entry.name,'SKILL.md'))).map((entry) => entry.name).sort()
  ...
  for (const skill of skills) fs.cpSync(path.join(destination,'skills',skill),path.join(root,'resources','skills',skill),{ recursive: true })
  const manifest = { schema: 1, revision, skills, tools: Object.fromEntries(artifacts.tools.map((tool) => [tool.name,tool.version])), sources: artifacts.sources, images: artifacts.images, files: inventory(destination) }
