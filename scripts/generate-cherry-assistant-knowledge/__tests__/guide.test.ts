...
describe('Cherry Assistant guide', () => {
  ...
  it('generates a dedicated Cherry Support identity with only the four support skills', () => {
    ...
    expect(generated.instructions['en-US']).toContain(
      "Cherry Studio's official built-in product support and feedback AI Agent"
    )
    expect(generated.instructions['en-US']).toContain('not a human employee')
    expect(generated.instructions['en-US']).toContain('answer naturally in one or two short sentences')
    expect(generated.instructions['en-US']).toContain('do not provide any other identity or relationship explanation')
    expect(generated.instructions['en-US']).toContain('Never sound defensive, sarcastic, scolding, condescending')
    expect(generated.instructions['en-US']).toContain('Do not repeat the identity statement')
    expect(generated.instructions['en-US']).not.toContain('Claude')
    expect(generated.instructions['en-US']).not.toContain('Anthropic')
    ...
    expect(generated.instructions['zh-CN']).toContain('Cherry Studio 官方内置的产品支持与反馈 AI Agent')
    expect(generated.instructions['zh-CN']).toContain('不是人类员工')
    expect(generated.instructions['zh-CN']).toContain('用一到两句自然、简短的话回答')
    expect(generated.instructions['zh-CN']).not.toContain('Claude')
    expect(generated.instructions['zh-CN']).not.toContain('Anthropic')
    ...
    expect(generated.instructions['zh-CN']).toContain('不要提供任何其他身份或关系说明')
    expect(generated.instructions['zh-CN']).toContain('不得防御、讽刺、训诫、居高临下')
    expect(generated.instructions['zh-CN']).toContain('否则不要重复身份句')
