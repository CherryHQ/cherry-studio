...
const targets = [
  { runner: 'windows-2025', platform: 'win32', arch: 'x64', script: 'build:win:x64' },
  { runner: 'windows-11-arm', platform: 'win32', arch: 'arm64', script: 'build:win:arm64' },
...
const selected = process.env.RELEASE_PLATFORMS ? process.env.RELEASE_PLATFORMS.split(',').map(value => value.trim()) : available
for (const platform of selected) {
  if (!available.includes(platform) || !targets.some(value => `${value.platform}-${value.arch}` === platform)) throw new Error(`No published native payload for ${platform}`)
}
