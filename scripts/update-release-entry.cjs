...
const retained = previous?.version === version ? previous.artifacts.filter(item => !selected.includes(`${item.platform}-${item.arch}`)).map(item => ({ ...item, source: item.source || previous.source })) : []
const artifactRows = [...retained, ...entries.filter(entry => selected.includes(`${entry.platform}-${entry.arch}`)).flatMap((entry) => entry.artifacts.map((artifact) => ({ ...artifact, platform: entry.platform, arch: entry.arch, source: entry.source })))]
const pendingPlatforms = platforms.filter(target => !artifactRows.some(item => `${item.platform}-${item.arch}` === target))
const manifest = { version, source: process.env.GITHUB_SHA, publishedAt: new Date().toISOString(), pendingPlatforms, artifacts: artifactRows }
fs.writeFileSync(manifestFile, JSON.stringify(manifest, null, 2) + '\n')
const rows = artifactRows.map((item) => `| \`${item.name}\` | ${(item.size / 1048576).toFixed(1)} MB | [Download](${item.url}) | \`${item.sha256}\` | ${item.signing} | [\`${item.source.slice(0,9)}\`](https://github.com/${process.env.GITHUB_REPOSITORY}/commit/${item.source}) |`).join('\n')
...
const updated = start < 0 ? original.replace(marker, `${marker}\n\n${release}`) : original.slice(0, start) + release + (end < 0 ? '' : original.slice(end + 1))
