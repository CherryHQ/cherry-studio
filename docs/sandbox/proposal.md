...
未来插件可以使用 TypeScript 编写 Agent 定义和业务逻辑，并复用 Cherry 的 Agent 执行循环。这是检验沙盒可用性的使用场景，不是本次交付完整插件系统的要求。
...
能力探测结果需区分“支持”“部分支持”“不可用”及原因。只有所需能力均满足时，才能向用户表示该会话满足对应隔离策略；不追求用一个布尔值掩盖平台差异。
...
例如，主进程执行 Agent 时，可以请求插件生成指令、处理工具结果或执行业务判断。插件在处理期间可以调用获准的宿主方法，再将结果返回主进程。通信必须支持这种嵌套调用，不能因主进程正在等待插件结果而阻塞插件对宿主的请求。
