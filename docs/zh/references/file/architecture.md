...
> **FileManager 管理通过显式调用 `createInternalEntry` / `ensureExternalEntry` 引入的文件**——以 `internal`（Cherry 拥有内容）或 `external`（仅记录路径引用）两种 origin 之一存在。调用方选择哪种 origin 属于业务层决策，FileManager 不假设。
...
│
├── index.ts              ← 模块 barrel; 仅导出 FileManager + 公共类型
│                           (internal/* 不导出; 外部 import 自然拿不到)
│
├── FileManager.ts        ← 唯一 lifecycle service + 公开 facade
│     │                     public method 薄委托到 internal/*; 自身持有 versionCache
│     │                     负责 IPC 注册与 FileHandle.kind 分派
...
│     ├── registerIpcHandlers() — 统一 IPC 入口，按 FileHandle.kind 分派
│     └── Electron dialog (showOpenDialog / showSaveDialog)
│
├── internal/             ← 私有实现，不经 index.ts 导出；外部禁止 import
│     │                     每个纯函数显式接收 FileManagerDeps (repo/versionCache/danglingCache)
...
│
├── versionCache.ts       ← LRU 类型定义; 实例由 FileManager 持有为 private field
│
...
│
└── ops/ (纯函数, sole FS owner, 对整个 main 进程开放)
...
- **FileManager** 是 entry 管理系统的**唯一公开入口**——负责 FileEntry 的完整生命周期与内容操作。它的 public API 只认 `FileEntryId` / `FileHandle`。启动时后台执行 orphan sweep（清理 internal 残留 UUID 文件），**不阻塞 ready 信号**
- **FileManager 作为 facade 而非 God class**——业务方法实现为 `internal/*` 下的纯函数 (`(deps, params) => result`)，FileManager class 只做 IPC 注册、`FileHandle.kind` 分派、persist `versionCache`；详见 [FileManager Architecture](./file-manager-architecture.md)
...
| 位置 | 可见性 | 访问方式 |
|---|---|---|
| `FileManager` class + 公共类型 | **Main 全进程** | `import { FileManager } from '@main/file'` 或 `application.get('FileManager')` |
| `ops/*` | **Main 全进程** | `import { atomicWriteFile } from '@main/file/ops'`（BootConfig、MCP oauth 等可直接用） |
| `watcher/`（`createDirectoryWatcher` 工厂） | **Main 全进程** | 业务 service 需要监听外部目录时调用 |
| `danglingCache` | **file-module 内部** | 外部通过 DataApi `includeDangling` 读；不直接 import |
| `internal/*` | **仅 FileManager** | 其他位置（包括 file-module 内的 `ops/` / `watcher/`）都不得 import |

边界强化：`src/main/file/index.ts` barrel 不 re-export `internal/*`；外部 `import from '@main/file'` 自然拿不到。Phase 1b 实现时如发现越界，追加 ESLint `no-restricted-imports` 规则兜底。
...
| 方法 | 说明 |
|---|---|
| `createInternalEntry` / `batchCreateInternalEntries` | 创建新 Cherry 拥有的 FileEntry（写入 `{userData}/files/{id}.{ext}`；每次调用产生独立新 entry，无冲突可能） |
| `ensureExternalEntry` / `batchEnsureExternalEntries` | 按 `externalPath` upsert——入口先 `canonicalizeExternalPath(raw)` 规范化（见 `pathResolver.ts`），同路径 non-trashed entry 复用，已 trashed 则 restore，无则新建。幂等设计，调用方可安全重复调 |
...
- **Cherry 不追踪外部文件的 rename/move**——文件在 Cherry 外部被移动，对应 entry 变 dangling（best-effort 语义）；调用方需主动对新路径再次调用 `ensureExternalEntry` 建立新引用（按 path upsert，命中现有 entry 则复用）
...
- **Staleness 契约（best-effort）**：`dangling` 是 FS 观察的**时间维度可变**值——watcher 可能未覆盖、文件可能在 cache 命中之后被外部删除。消费方**必须**允许 React Query 的默认 `staleTime`（5min）或更短触发自然刷新；**不要**设置 `staleTime: Infinity`——那等于"要 dangling 但拒绝重检"的自相矛盾组合。用户显式刷新某 entry 时走 File IPC `refreshMetadata(id)` + 对 query 调用 `mutate(...)`

**`includePath`**（原始绝对路径）：
- Handler 调 main 侧 `resolvePhysicalPath(entry)` 返回绝对路径字符串
- **Intended uses**：agent context embedding / 拖放到外部 app（`webContents.startDrag` 的 `file` 字段）/ subprocess spawn / "Open in external editor" UX
- **NOT intended**：不要当稳定标识符缓存（storage layout 未来可能变）、不要 string-concat 进 shell 命令而不独立 sanitize、不要用这条路径 bypass FileManager 做写操作、只需要身份引用时用 `entry.id`
- **按 convention 约束**：类型系统无法阻止 renderer 拿到字符串后滥用，code review 逐调用点审"这个 caller 真的需要 path 字符串吗"

**`includeUrl`**（file:// URL with safety）：
- Handler 调 main 侧 `resolveSafeUrl(entry)` 产生 `file://` URL；危险文件（.sh/.bat/.ps1 等）返回 dirname 避免误点执行
- **Scoped 能力**：仅服务 `<img src>` / `<video src>` / `<embed>` 的同步渲染；safety wrap 只防 HTML rendering 上下文的 hover/load 副作用，**不是**通用路径安全原语
- 不要把这个 URL 拼进命令行参数、subprocess args 或其他非 HTML 场景——需要原始路径时用 `includePath`
- 让 renderer 不知晓内部存储布局（id+ext 拼接、userData 路径），存储格式变化不影响 renderer

### 3.1.1.1 关于 "Main is SoT for path resolution"

Main 侧作为 path resolution 的 SoT，指的是 **authority（谁定义解析规则）**——`resolvePhysicalPath` 决定 id + ext 如何拼接、userData 挂在哪、将来是否改成 hash 分桶等。Renderer 消费 Main 产出的字符串结果，但不分享 authority：
- 存储布局在 Main 侧迭代时，renderer 代码零改动
- Renderer **持有**字符串值（locality），不**定义**计算规则（authority）

`includePath` / `includeUrl` 把 path string 带到 renderer 进程，是让 renderer 完成"持有值"这一步——**locality** 的扩散不等于 **authority** 的扩散。前者是自然的消费关系，后者才是真正的 SoT 被撕开。
...
#### (2b) 新增 sourceType 的开发者 checklist

为避免"加了 sourceType 但忘记某个环节"的治理漏洞，新增 variant 时按以下顺序落地（缺一不可）：

| 步骤 | 位置 | 动作 | 强制手段 |
|---|---|---|---|
| 1 | `packages/shared/data/types/file/ref/<name>.ts` | 新建 variant 文件：declare `xxxSourceType` / `xxxRoles` / `xxxRefFields` + `xxxFileRefSchema = createRefSchema(...)` | 代码 review |
| 2 | `packages/shared/data/types/file/ref/index.ts` | 把 variant 加到 `allSourceTypes`（类型聚合）+ `FileRefSchema` 判别联合 | 类型系统 narrow 失败 |
| 3 | `src/main/data/services/orphan/FileRefCheckerRegistry.ts` | 在 `Record<FileRefSourceType, SourceTypeChecker>` 里补 checker | **编译期强制**（Record 缺 key → TS 报错）|
| 4 | 业务 service（delete flow） | 在对象被删除时调用 `fileRefService.cleanupBySource(sourceType, id)` | 代码 review + 单测 + 第 3 步兜底 |

**设计意图**：推拉模型互补——

- **Pull**（第 4 步）：业务 service 自己在 delete flow 里主动清，这是首选路径，避免 ref 堆积
- **Push**（第 3 步）：OrphanRefScanner 作为安全网，定期扫描 `file_ref` 发现 sourceId 不存在的行并删除。**编译期 Record 闭合性**保证不会漏注册 sourceType
- **没有 `onSourceDeleted` per-sourceType 钩子**：`cleanupBySource` 的清理逻辑对所有 sourceType 相同（删 `(sourceType, sourceId)` 匹配的行），business-specific 清理（比如知识库删除时重建向量）属业务 service 自身 delete flow，不应和 ref 系统耦合

参考实现（Phase 1b 提供 tempSession 作为模板）：

```typescript
// src/main/data/services/orphan/FileRefCheckerRegistry.ts
// 编译期强制：每个 FileRefSourceType 都有 checker；缺失则 TypeScript 报错
export const fileRefCheckers: Record<FileRefSourceType, SourceTypeChecker> = {
  temp_session: {
    sourceType: 'temp_session',
    checkExists: async () => new Set()  // temp 没有 persistent source，全部视为"已删"
  },
  // 新增 sourceType 后，这里缺 key 的话 TypeScript 编译报错
}
```
...
- **External entry path unique**：同路径同时最多一个 non-trashed entry（SQLite partial unique index）。`ensureExternalEntry` 由此具备 upsert 语义：存在 non-trashed 则复用，存在 trashed 则 restore，其他情况新建
