...
这些模块管理各自的文件，可直接使用 `node:fs`，不受 file module 的 ops 约束。
...
| 字段       | `local_managed`   | `local_external`              | 结论            |
| ---------- | ----------------- | ----------------------------- | --------------- |
| `name`     | 纯 DB（物理名=ID）| 触发 `fs.rename`（物理名=name）| 必须走 File IPC |
| `ext`      | 纯 DB             | 触发 `fs.rename`              | 必须走 File IPC |
| `parentId` | 纯 DB             | 触发 `fs.rename`（目录移动）  | 必须走 File IPC |
| `sortOrder`| 纯 DB             | 纯 DB                         | 可走 DataApi    |

> 当前 schema 无 `sortOrder` 字段，因此 DataApi 暂无 PATCH 端点。未来新增纯展示字段时再添加。
...
|                                                                         |
| Role: the ONLY module *within file module* that imports `fs` / `shell`  |
...
|             all FS ops v                                          |
|  +-----------------------------------------------------------+    |
|  | ops.ts  *** MODULE-INTERNAL FS OWNER (pure functions) ***            |    |
|  |                                                           |    |
...
**为什么禁止业务 Service 直接操作 file module 管理的文件**：

- **路径不透明**：物理路径由 `mountType` 决定，业务服务不应假设路径格式
- **缓存一致性**：FileManager 维护内存路径缓存，绕过它会导致不一致
- **同步引擎冲突**：对 `local_external` mount 直接写入会被 chokidar 捕获为外部变更

此约束的 scope 是 **file module 管理的文件**（即 fileEntryTable 中的条目对应的物理文件）。其他模块管理的自有文件（Knowledge 向量索引、Agent 工作区、MCP 配置等）不在此约束范围内，可直接使用 `node:fs`。

#### Open Question: `resolvePhysicalPath` 是否公开

业务 Service 需要将文件内容传递给外部库（AI SDK 上传、文档预处理 API 等）。当前存在两种消费模式：

1. **Buffer** — 大部分场景。`FileManager.read(id, { encoding: 'binary' })` 返回 `Uint8Array`，覆盖文件上传、base64 编码等
2. **ReadStream** — 大文件流式上传（如 300MB PDF）。Buffer 会占满内存，需要流式传输

| 方案 | 路径暴露 | 约束强度 | API 面积 | 代价 |
|------|----------|----------|----------|------|
| A. 公开 `resolvePhysicalPath` | 暴露 | Convention（约定级别） | 小 | 调用方可绕过 FileManager 直接操作 FS，破坏缓存一致性和同步引擎不变量 |
| B. 不公开，FileManager 提供 `read` + `createReadStream` | 不暴露 | Structural（结构级别） | 大 | 每遇到新消费模式需扩展 FileManager API |
| C. 公开但命名制造摩擦（如 `unsafeResolvePath`）| 暴露 | Convention + 命名警示 | 小 | 本质同 A，但降低无意误用概率 |

**当前状态**：未决定。`resolvePhysicalPath` 暂保留在 `IFileManager` 接口中，最终方案在实现阶段根据实际消费场景确定。

> **Convention, not structural**：无论选择哪种方案，业务 Service 均可通过 `fileTreeSafe` 获取 FileEntry 元数据。方案 A/C 下还可获取物理路径。此约束无法在类型系统层面完全强制，违反约定会导致缓存不一致和同步冲突。
