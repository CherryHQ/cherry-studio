...
Main 进程内部的 service（FileManager、业务 Service 等）可以直接 import 调用 data repository，不需要经过 DataApi handler。DataApiService 桥接仅用于 Renderer 访问。
...
FileManager 直接注册所有 IPC handler（通过 `this.ipcHandle()`），自身**不直接调用 `fs` 模块**。涉及条目的操作由 FileManager 内部协调 DB（通过 repository）+ FS（通过 fs 纯函数）；纯路径操作直接调用 fs 纯函数。Electron dialog API（`showOpenDialog` / `showSaveDialog`）由 FileManager 直接处理，因为 dialog 返回的是用户选择结果，不是 FS 操作。

#### IPC 方法按操作类型分类

**条目操作**：涉及 FileEntry 的操作（FileManager 协调 DB + fs 纯函数）。
...
| `read` | 解析路径 → 调用 fs 纯函数读文件 | — |
| `write` | 解析路径 → 调用 fs 纯函数写文件 | — |
| `getMetadata` | 查条目 + 调用 fs 纯函数 `fs.stat` | — |
| `open` | 解析路径 → 调用 fs 纯函数 `shell.openPath` | — |
| `showInFolder` | 解析路径 → 调用 fs 纯函数 `shell.showItemInFolder` | — |
| `batch*` | 逐项执行条目操作 | — |

**纯路径操作**：不涉及条目，FileManager 直接调用 fs 纯函数。

| 方法 | FilePath 入参 |
|------|---------------|
| `read` | 直接读文件 |
| `write` | 直接写文件 |
| `getMetadata` | 直接 `fs.stat` |
| `open` | 直接 `shell.openPath` |
| `showInFolder` | 直接 `shell.showItemInFolder` |
| `save` | dialog 选路径 → 写文件 |
| `listDirectory` | 列出外部目录内容 |
| `validateNotesPath` | 校验路径合法性 |

**FileManager 自行处理**：Electron dialog（无 FS 操作）。
...
| FS:   none -- delegates ALL FS operations to fs 纯函数              |
...
+---------------------------------------------------------------------+
| ops.ts (pure functions)  *** SOLE FS OWNER ***                       |
| (NOT a class/service — stateless exported functions)                |
|                                                                     |
...
- **ops.ts 是唯一直接 `import node:fs` 的模块**——所有 FS 操作（读写/stat/移动/删除等）经过它，它只接受文件路径，不感知条目。chokidar 作为第三方事件库不在此约束内，但 FileManager 内部 sync 逻辑需要的 FS 操作仍通过 ops.ts 执行
- **FileManager 是唯一的 lifecycle service**——注册 IPC handler，解析 entryId → filePath，协调 DB 变更（通过 repository）+ FS 操作（通过 ops.ts 纯函数），管理 chokidar watching
- **Renderer 永远不直接操作 FS**，所有 FS 操作通过 IPC 委托给 Main
- **FileTreeService 是纯条目层（data repository）**——只做 DB 读写和树查询，不碰 FS
- **FileRefService 是纯引用层（data repository）**——完全不碰 FS
- **DataApi Handler 完全不碰 FS**

### 6.4 职责边界总结

| 层 | 类型 | 碰 DB | 碰 FS | 碰 Electron API | 暴露给 Renderer |
|----|------|-------|-------|-----------------|----------------|
| **FileManager** | lifecycle | 通过 repository | **否（通过 fs 纯函数）** | dialog + shell | 是（IPC） |
...
|  Reader   Service (coordination → fs 纯函数)                      |
|  (read)   (read+write)                                            |
|                                                                   |
|  . . . . . . . . . . . . . . . . . . . . . . . . . . . . . . .  |
|  Background Services                                              |
...
- **ops.ts 是唯一直接 `import node:fs` 的模块**——所有 FS 操作经过它。chokidar 作为第三方事件库不在此约束内，但 sync 逻辑中需要的 FS 操作仍通过 ops.ts 执行
- **FileManager 是唯一的 lifecycle service**——注册 IPC handler，解析 entryId → filePath，协调 DB（通过 repository）+ FS（通过 ops.ts 纯函数），管理 chokidar watching
- FileTreeService 是纯条目层（data repository），DataApi Handler 直接调用它做只读查询；FileManager 调用它做条目变更

**关键数据流**：

- **Renderer → Main（读取）**：DataApi → Handler → FileTreeService → DB
- **Renderer → Main（写操作）**：IPC → FileManager（协调 DB + fs 纯函数）
- **业务 Service → 文件数据**：纯 DB 操作（条目查询 + 引用管理）直接调 data repository，涉及 FS 的操作（读写文件内容）走 FileManager（内部调用 fs 纯函数）
- **外部文件系统 → Main**：chokidar → FileManager 内部同步引擎 → fs 纯函数（stat）+ FileTreeService（DB）

### 7.2 业务服务的接触点

业务服务通过两个途径与文件管理系统交互：

- **纯 DB 操作**（条目查询 + 引用管理）→ 直接 import data repository（`fileTreeReader` / `fileRefService`）
...
BusinessService
    |
    +-- direct import data repository (pure DB, read + ref write)
    ...
    |
    +-- via FileManager (involves FS, calls fs 纯函数 internally)
...
由于条目数据与 FS 强关联，条目的写操作（create / update / delete）必须经过 FileManager 协调 FS + DB。为在编译期强制这一约束，FileTreeService 导出读写两个类型：
...
// full instance — FileManager imports this
...
src/main/data/                        -- data layer (pure DB)
  services/
    FileTreeService.ts                -- repository: exports fileTreeService + fileTreeReader
...
- **FileManager** import `fileTreeService`（完整实例）— 可以调读写方法，因为它负责协调 DB + FS（通过 fs 纯函数）
- **ops.ts 纯函数** 不接触 DB — 只接受文件路径，执行底层 FS 操作。业务 Service 不应直接调用 fs 纯函数，而是通过 FileManager
  ...
  "rules": {
    "no-restricted-imports": ["error", {
      "paths": [{
        ...
        "message": "Use fileTreeReader for read-only access. Only FileManager may import fileTreeService."
      }]
    }]
  },
  // FileManager.ts exempt via overrides (uses fileTreeService for coordination)
  "overrides": [{
    "files": ["src/main/services/FileManager.ts"],
...
Lifecycle service 和 data repository 的区别参见 [Lifecycle Decision Guide](../../en/references/lifecycle/lifecycle-decision-guide.md)。
...
+-- FileManager                  -- the ONE lifecycle service:
                                    ...
                                    coordinate DB (via repository) + FS (via fs 纯函数)
...
```
     ...
     app.whenReady()
          ...
          v         WhenReady
     FileManager
     (the ONE lifecycle service)
     (1. build path cache)
     (2. register IPC handlers)
     (3. start chokidar watching)
     (calls FileTreeService + fs 纯函数 internally)
          |
     onAllReady()
          |
...
- **FileTreeService / FileRefService** 是 data repository（非 lifecycle），随 DataApiService 初始化就绪，供 FileManager 和 DataApi handler 调用
- **ops.ts 纯函数** 是唯一 `import fs` 的模块，只接受文件路径，不感知条目或 DB。不需要生命周期管理——纯函数，无状态
- **FileManager** 是文件管理系统唯一的 lifecycle service，在 `WhenReady` 阶段完成：(1) 构建内存路径缓存（`Map<entryId, absolutePath>`），(2) 通过 `this.ipcHandle()` 注册所有 IPC handler，(3) 启动 chokidar watching。所有 FS 操作通过 fs 纯函数执行，自身不 `import fs`。IPC handler 由 lifecycle 系统自动管理：`onStop()` 返回后统一移除，无需手动 `removeHandler()`；restart 时重新注册（详见 [Lifecycle Usage - IPC](../../en/references/lifecycle/lifecycle-usage.md#api)）。chokidar watching 作为内部子模块，通过 `OperationLock` 跳过自发操作
- **OrphanRefScanner** 在 `Background` 阶段运行，不阻塞启动。通过 `onAllReady()` 确认所有业务服务就绪后开始扫描

#### 业务服务的依赖声明

业务服务需声明对 `FileManager` 的依赖（FileRefService 是 data repository，无需 `@DependsOn`，直接 import 调用）：
...
业务服务只需依赖 `FileManager` 即可——IPC 注册和 chokidar watching 都是 FileManager 内部职责，对业务服务透明。
