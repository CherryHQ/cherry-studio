...
所有文件系统实体（文件、目录、挂载点）统一表达为**节点（Node）**，存储在单一节点表中。
...
├── parentId: 父节点 ID（邻接表模式）
└── mountId: 所属挂载点 ID（冗余字段，避免递归查询）
```

三种节点类型的不变量（Invariants）：
...
挂载点是节点树的根，每个挂载点通过 `providerConfig` 定义一种存储模式。挂载点本身也是节点（`type='mount'`），与文件/目录共用同一张表。
...
- **`local_managed`**: `{mount.basePath}/{node.id}.{node.ext}` — 平坦存储，目录仅逻辑存在
- **`local_external`**: `{mount.basePath}/{...ancestorNames}/{node.name}.{node.ext}` — 映射 OS 目录树
- **`system`**: 无自有存储，Trash 中的节点使用原 `mountId` 对应的 mount 解析路径

维护内存级路径缓存 `Map<nodeId, absolutePath>`，树变更时重建。
...
远程 API 为 Source of Truth → 本地缓存 + 节点表作为镜像层。按需下载，缓存失效通过 `cachedAt` 与远程 `updatedAt` 比较判断。
...
| ------------ | -------------------------- | ------------------- | -------------------------------- |
| **Trash**    | 移动到 `system_trash` 下   | 仅 DB 操作          | 物理移动到 `.trash/{nodeId}/`    |
| **Restore**  | 移回 `previousParentId`    | 仅 DB 操作          | 物理移回原路径                   |
| **永久删除** | 硬删节点（CASCADE 子节点） | 删物理文件 + 删 DB  | 删 `.trash` 中的物理文件 + 删 DB |

**设计要点**：

- Trash 节点保持原 `mountId` 不变（路径解析仍用原 mount）
- 子节点的 `parentId` 不变，整棵子树随父节点进入/离开 Trash
- 回收站只展示直接子节点（`parentId = system_trash`）
...
```
+-------------------------------------------------------+
| Layer 1: nodeId CASCADE                               |
| file node deleted -> file_ref auto-cascaded           |
...
| 端点                            | 用途                                               |
| ------------------------------- | -------------------------------------------------- |
| `GET /files/nodes`              | 节点列表（支持 mountId / parentId / inTrash 过滤） |
| `GET /files/nodes/:id`          | 单节点查询                                         |
| `GET /files/nodes/:id/children` | 子节点（文件树懒加载，支持排序分页）               |
| `GET /files/nodes/:id/refs`     | 文件的所有引用方                                   |
...
| .fileManager |---IPC---->|   |                                   |
| .createNode()|           |   +-- all ops ------> FileService     |
| .read()      |           |   |                   (sole FS owner) |
| .trash()     |           |   |                                   |
...
**委托 FileService**：所有涉及 FS 的操作，无论入参是 NodeId 还是 FilePath。

| 方法 | NodeId 入参 | FilePath 入参 |
|------|-------------|---------------|
| `createNode` | FS 写入 + 创建节点（按 providerType） | — |
| `trash` / `restore` | Trash 逻辑（按 providerType） | — |
| `permanentDelete` | 删物理文件 + 删节点 | — |
| `move` / `copy` | FS 移动/复制 + 更新节点 | — |
| `read` | 解析路径 → 读文件 | 直接读文件 |
| `write` | 解析路径 → 写文件 | 直接写文件 |
| `getMetadata` | 查节点 + `fs.stat` | 直接 `fs.stat` |
...
| FS:   all FS ops -- node-related (by providerType) and raw path    |
|       read / write / stat / rename / unlink / copy / open / list    |
| DB:   delegates to FileTreeService for all node mutations           |
...
|                                                                     |
| Role: node CRUD, tree queries, path resolution                      |
| FS:   none (pure DB)                                                |
| DB:   nodeTable read/write, tree traversal (children, ancestors)    |
...
- **FileService 是进程中唯一 `import fs` 的模块**——所有 FS 操作（无论 NodeId 还是 FilePath）最终都经过它
- **Renderer 永远不直接操作 FS**，所有 FS 操作通过 IPC 委托给 Main
- **FileTreeService 是纯节点层**——只做 DB 读写和树查询，不碰 FS
...
|    node write resolve node write resolve node write resolve       |
|      |   |   |          |   |   |        |   |     |              |
|      v   v   v          v   v   v        v   v     v              |
...
- FileTreeService 是纯节点层，DataApi Handler 直接调用它做只读查询；FileService 调用它做节点变更

**关键数据流**：

- **Renderer → Main（读取）**：DataApi → Handler → FileTreeService → DB
- **Renderer → Main（写操作）**：IPC → FileIpcService → FileService（FS + Node）
- **业务 Service → 文件数据**：纯 DB 操作（节点查询 + 引用管理）直接调 data service，涉及 FS 的操作（读写文件内容）走 FileService
...
- **纯 DB 操作**（节点查询 + 引用管理）→ 直接 import data service（`fileTreeReader` / `fileRefService`）
...
+-- 'temp_session'    -> (mount_temp nodes with no ref: auto-cleanup)
```
...
BusinessService
    ...
    |   +-- fileTreeReader.getById(nodeId)        -> FileTreeNode
    |   +-- fileTreeReader.getChildren(parentId)  -> FileTreeNode[]
    |   +-- fileRefService.create(dto)            -> FileRef
    |   +-- fileRefService.cleanupBySource(...)   -> void
    |
    +-- via FileService (involves FS)
    |   +-- readFile(nodeId, opts?)               -> string | Uint8Array
    |   +-- writeFile(nodeId, data)               -> void
...
由于节点数据与 FS 强关联，节点的写操作（create / update / delete）必须经过 FileService 协调 FS + DB。为在编译期强制这一约束，FileTreeService 导出读写两个类型：
...
class FileTreeService {
  // read methods
  getById(id: string): Promise<FileTreeNode> { ... }
  getChildren(parentId: string): Promise<FileTreeNode[]> { ... }
  // write methods
  create(dto: CreateNodeDto): Promise<FileTreeNode> { ... }
  update(id: string, dto: UpdateNodeDto): Promise<FileTreeNode> { ... }
...
src/main/data/                        -- data layer (pure DB)
  ...
  db/schemas/
    node.ts                           -- nodeTable + fileRefTable
  db/seeding/
    nodeSeeding.ts                    -- system mount initialization
...
PasteService                          MessageService
    ...
    |                         |            |
    +-- 3'. delete temp ref   |            |
    +-- (scanner later cleans up no-ref nodes)
```
...
+-- NodeSeedingService           -- seed system nodes
      @DependsOn(DbService)
...
+-- FileTreeService              -- node CRUD (pure DB)
...
- **NodeSeedingService** 在 `BeforeReady` 阶段幂等创建系统挂载点
- **FileTreeService / FileRefService** 是 data service（非 lifecycle），随 DataApiService 初始化就绪，供 FileService 和 DataApi handler 调用
- **FileService** 在 `WhenReady` 阶段构建内存路径缓存（`Map<nodeId, absolutePath>`），是唯一 `import fs` 的模块
...
MessageService
  @DependsOn(FileService)
  +-- queries nodes via fileTreeReader (pure DB, read-only)
...
| `ext`/`size` 存放 | **保留在 nodeTable**    | 高频过滤/排序需索引支持，查询性能优先             |
...
- **系统节点不可删除**：`mount_files`、`mount_notes`、`system_trash`、`mount_temp` 受保护
...
| 新增挂载点                               | 创建新 mount 节点 + 配置 `providerConfig`           |
| 笔记体系迁入                             | 通过 `local_external` mount 纳入统一节点表          |
