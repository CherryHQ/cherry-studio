...
FileService --lock--> OperationLock <--check-- SyncEngine (chokidar)
    |                                              |
    |--FsService.rename()--> ... --> unlink event --> "locked, skip"
    |--db.update()                                 |
...
| .fileManager |---IPC---->|   |                                   |
| .createEntry()|          |   +-- entry ops ----> FileService     |
| .read()      |           |   |                   (coordination)  |
| .trash()     |           |   |                                   |
| .select()    |           |   +-- raw path ops -> FsService       |
| .open()  ... |           |   |                   (sole FS owner) |
|              |           |   +-- dialog only --> Electron dialog |
+--------------+           |      (no fs, just showOpenDialog)     |
                           +--------------------------------------+
```

FileIpcService 自身**不调用 `fs` 模块**。涉及条目的操作委托给 FileService（协调 DB + FS），纯路径操作（无需解析 entryId）可直接委托 FsService。FileIpcService 仅保留 Electron dialog API（`showOpenDialog` / `showSaveDialog`），因为 dialog 返回的是用户选择结果，不是 FS 操作。

#### IPC 方法按分发目标分类

**委托 FileService**：涉及 FileEntry 的操作（需协调 DB + FS）。
...
| `read` | 解析路径 → 委托 FsService 读文件 | — |
| `write` | 解析路径 → 委托 FsService 写文件 | — |
| `getMetadata` | 查条目 + 委托 FsService `fs.stat` | — |
| `open` | 解析路径 → 委托 FsService `shell.openPath` | — |
| `showInFolder` | 解析路径 → 委托 FsService `shell.showItemInFolder` | — |
| `batch*` | 逐项委托 FileService | — |

**委托 FsService**：纯路径操作（不涉及条目，无需 FileService 协调）。
...
| FS:   none -- entry ops → FileService, raw path ops → FsService    |
| Own:  Electron dialog API only (showOpenDialog/showSaveDialog)      |
+---------------------------------------------------------------------+
| FileService  (coordination layer, no fs)                            |
| (Lifecycle Service, WhenReady phase)                                |
|                                                                     |
| Role: resolve entryId → filePath, coordinate DB + FS                |
| FS:   none -- delegates ALL FS operations to FsService              |
...
| FS:   none -- delegates to FsService for stat/scan                  |
|       owns chokidar instance, but FS reads go through FsService     |
...
- **FsService 是进程中唯一 `import fs` 的模块**——所有 FS 操作最终都经过它，它只接受文件路径，不感知条目
- **FileService 是协调层**——解析 entryId → filePath，协调 DB 变更（FileTreeService）+ FS 操作（FsService），自身不 `import fs`
- **Renderer 永远不直接操作 FS**，所有 FS 操作通过 IPC 委托给 Main
- **FileTreeService 是纯条目层**——只做 DB 读写和树查询，不碰 FS
- **FileIpcService 是纯 IPC 入口**——不碰 FS，条目操作委托 FileService，纯路径操作可直接委托 FsService
- **ExternalSyncEngine 通过 FsService 做 FS 读取**——自身不直接调用 `fs`
...
|----|------|-------|-------|-----------------|----------------|
| **FileIpcService** | lifecycle | 否（委托） | 否（委托 FileService / FsService） | dialog | 是（IPC） |
| **FileService** | lifecycle | 通过 data service | **否（委托 FsService）** | 否 | 否（内部） |
| **FsService** | lifecycle | 否（纯 FS） | **是（唯一 FS owner）** | shell | 否（内部） |
| **ExternalSyncEngine** | lifecycle | 通过 data service | 通过 FsService | 否 | 否（内部） |
...
|  |                                                           |    |
|  |  pure path-based, no entry/DB awareness                   |    |
|  +-----------------------------------------------------------+    |
|                       |                                           |
...
|  Reader   Service (coordination → FsService)                      |
|  (read)   (read+write)                                            |
|                                                                   |
|  . . . . . . . . . . . . . . . . . . . . . . . . . . . . . . .  |
|  Background Services                                              |
...
- **FsService 是唯一碰 `fs` 的模块**——所有其他模块通过它间接访问文件系统
- **FileService 是协调层**——解析 entryId → filePath，协调 DB（FileTreeService）+ FS（FsService），自身不 `import fs`
- FileIpcService 是纯 IPC 入口，条目操作委托 FileService，纯路径操作可直接委托 FsService
- FileTreeService 是纯条目层，DataApi Handler 直接调用它做只读查询；FileService 调用它做条目变更

**关键数据流**：

- **Renderer → Main（读取）**：DataApi → Handler → FileTreeService → DB
- **Renderer → Main（写操作）**：IPC → FileIpcService → FileService（协调 DB + FsService）或直接 → FsService（纯路径操作）
- **业务 Service → 文件数据**：纯 DB 操作（条目查询 + 引用管理）直接调 data service，涉及 FS 的操作（读写文件内容）走 FileService（内部委托 FsService）
- **外部文件系统 → Main**：chokidar → ExternalSyncEngine → FsService（stat）+ FileTreeService（DB）
...
BusinessService
    ...
    |
    +-- via FileService (involves FS, delegates to FsService internally)
    ...
    x-- FsService (direct)                        -> FORBIDDEN (business services must go through FileService)
...
src/main/services/                    -- lifecycle services
  FsService.ts                        -- sole FS owner, pure path-based ops
  FileService.ts                      -- coordination layer, imports fileTreeService + FsService
  FileIpcService.ts                   -- imports FileService + FsService, registers IPC handlers
  ExternalSyncEngine.ts               -- imports FsService, chokidar watch
```

- **业务 Service** import `fileTreeReader`（`Pick` 类型）— 只有读方法，调写方法编译报错
- **FileService** import `fileTreeService`（完整实例）— 可以调读写方法，因为它负责协调 DB + FS（通过 FsService）
- **FsService** 不接触 DB — 只接受文件路径，执行底层 FS 操作。业务 Service 不应直接调用 FsService，而是通过 FileService
...
// eslint config
{
  ...
  // FsService.ts exempt via overrides (sole FS owner)
  "overrides": [{
    "files": ["src/main/services/FsService.ts"],
...
+-- FsService                    -- sole FS owner, pure path-based ops
+-- FileService                  -- coordination layer, build path cache
|     @DependsOn(FsService)
+-- FileIpcService               -- register IPC handlers (no FS)
|     @DependsOn(FileService, FsService)
...
```
     ...
     app.whenReady()
          ...
          v         WhenReady
     FsService
     (sole FS owner, pure path-based ops)
          |
          v
     FileService
     (build path cache, coordination layer)
     (calls FileTreeService + FsService internally)
          |
+---------+-----------+
|                     |
FileIpcService  ExternalSyncEngine
...
- **FsService** 在 `WhenReady` 阶段初始化，是唯一 `import fs` 的模块，只接受文件路径，不感知条目或 DB
- **FileService** 在 `WhenReady` 阶段构建内存路径缓存（`Map<entryId, absolutePath>`），依赖 FsService 执行所有 FS 操作，自身不 `import fs`
- **FileIpcService** 在 `WhenReady` 阶段通过 `this.ipcHandle()` 注册所有 IPC handler。条目操作委托 FileService，纯路径操作可直接委托 FsService，少数不涉及 FS 的 Electron API（如 `select` → `dialog.showOpenDialog`）由自身直接处理（参见 6.2）。IPC handler 由 lifecycle 系统自动管理：`onStop()` 返回后统一移除，无需手动 `removeHandler()`；restart 时重新注册（详见 [Lifecycle Usage - IPC](../../en/references/lifecycle/lifecycle-usage.md#api)）
- **ExternalSyncEngine** 在 `onInit()` 中启动 chokidar，通过 FsService 做 FS 读取，通过 `FileService.OperationLock` 跳过自发操作
