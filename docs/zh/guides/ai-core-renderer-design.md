...
### Phase 3.1b: 消息持久化（进行中）

> 前置: P2 联调 ✅ + Data API 接口形态确定（已由之前 session 完成基础实现）

**Step 3.1b.1: onFinish 持久化** — 部分完成

`useAiChat.ts` 中已有 `onFinish` 回调基础，但需与 DataApi 完整对齐（消息 ID 策略、user/assistant 关系）。

**Step 3.1b.2: 消息树语义对齐** — 待完成

V1 的消息树用 `askId` 关联一组助手回复。需与 DataApi 设计对齐：parentId 关系、消息创建时机、abort/error 持久化策略。

**Step 3.1b.3: initialMessages 加载** — 待完成

Topic 切换时从 DataApi 加载历史消息，转为 `CherryUIMessage[]` 传入 `useAiChat({ initialMessages })`。需实现 DB parts → UIMessage 的反向转换。
...
**Step 3.2.4: CitationBlock 去 Redux** — 待完成

Citation 数据通过 props 传递而非 Redux 查找。需真实数据验证 citation part 结构。
