...
```
  ...
  ④ ToolRegistry 接入内置 tools (WebSearch, Knowledge, Memory) + MCP tool 动态注册
     MCP tool 按需注册 ✅ 已实现 (registerMcpTools + createMcpTool + callTool)
     ⚠️ MCP tool 动态生命周期（server 连接/断开自动注册/注销）依赖 #14123 MCPService 重构:
       - 需要 MCPService 统一入口 + server 生命周期事件 (server:started / server:stopped)
       - 需要 isServerConnected(id) API 实现 checkAvailable
       - 需要 CallToolArgs 扩展支持 context 传递
       - 当前: 按需注册可正常工作，不需要等 #14123
       - 重构后: ToolRegistry 监听事件自动注册/注销，checkAvailable 检查连接状态
