...
- **A fullscreen window must never be hidden** — on macOS, `hide()` (`orderOut:`) on a fullscreen window orphans its fullscreen Space, leaving a black screen the user cannot dismiss ([electron#20263](https://github.com/electron/electron/issues/20263), [#39572](https://github.com/electron/electron/issues/39572)). WM therefore skips close interception and destroys instead of recycling a fullscreen window on release (standby replenish covers the next `open()`); consumers owning the hide path must not hide one either.
