...
| `behavior.closeToHide` | registry boolean | Declarative close policy for reusable singleton surfaces. `WindowManager.close()` and native close events hide the window instead of destroying it; native close is still allowed during app quit. |
