...
**Cursor (keyset) pagination.** List endpoints that page by a `(sortKey, id)` tuple use the shared codec + ordering helper in `services/utils/keysetCursor.ts`. Existing one-direction endpoints keep `decodeListCursor` / `encodeCursor` and `keysetOrdering(...).where` / `orderBy`. New bidirectional endpoints use `createKeysetCursorCodec` plus `keysetOrdering(...).seek('previous' | 'next')`, which binds an opaque token to a caller-normalized query family and keeps the exclusive WHERE predicate, SQL order, and canonical result restoration together. The request remains `{ cursor?, limit? }`; direction is carried by the token. Do NOT hand-write cursor encode/decode, the keyset WHERE tuple, or the `ORDER BY` in a service. See [services/utils/README.md](../../../src/main/data/services/utils/README.md) for family ownership, decode policies, and boundaries, and the [Pagination Guide](./data-pagination-guide.md) for the end-to-end pagination model.
