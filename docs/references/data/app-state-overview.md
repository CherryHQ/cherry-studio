...
| `agent-session-fork:<operationId>` | `AgentSessionForkService` | `AgentSessionForkJournal` (v1/v2) | Artifact ownership, commit status and workspace retention for [fork recovery](../testing/agent-session-fork.md#recovery-and-cleanup). Committed entries identify sessions requiring native history. |
