...
| `agent-session-fork:<operationId>` | `AgentSessionForkService` | `AgentSessionForkJournal` (v1/v2) | Operation-owned native SDK artifacts, publication/commit status and workspace retention. Cleanup checks ownership and registered workspace references; uncertain ownership preserves files. Other domains query the owner's API, never the key directly. |
