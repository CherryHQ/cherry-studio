...
> **Note:** `bootConfigSchemas.ts` is auto-generated by the V1-to-V2 migration pipeline. Add durable keys through `classification.json` or the generator's `MANUAL_BOOT_CONFIG_ITEMS`, then regenerate (see [V1 to V2 Data Migration](#v1-to-v2-data-migration) below).

`app.user_data_path` is consumed by `resolveUserDataLocation()` during preboot. It is keyed by normalized executable path, including AppImage and Windows portable executable normalization. `temp.user_data_relocation` is transient preboot operation state: `pending` is written by the renderer request path, consumed by the relocation gate before normal bootstrap, then cleared on success or changed to `failed` for the terminal failure UI. `temp.*` keys are not backup/sync material and should default to `null`.
...
`resolveUserDataLocation()` uses the same normalization when reading `app.user_data_path`, so migrated AppImage/portable records match the lookup key.
