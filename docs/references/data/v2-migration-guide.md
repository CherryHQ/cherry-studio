...
- `core/MigrationEngine.ts` coordinates all migrators in order, surfaces progress to the UI, and uses `app_state.key = 'migration_v2_status'` as the durable migration status marker. Edition provenance is recorded separately under `appEdition:v1Migration` by `AppEditionService`. The engine clears new-schema tables before running and aborts on validation or global foreign-key failure.
