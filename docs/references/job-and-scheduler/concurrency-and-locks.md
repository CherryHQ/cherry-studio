...
| --- | --- | --- | --- | --- |
| **0** Synchronous write transaction | `DbService.withWriteTx` | All write transactions across the app (every `withWriteTx` callsite) | µs — one tx | Runs each write as one synchronous `BEGIN IMMEDIATE` transaction on the single better-sqlite3 connection (behind the isReady guard). Synchronous execution on one persistent connection serializes all writes inherently — no mutex or retry needed. Reusable by any service. |
