...
- **`isScheduleOverdue` has three branches** (relevant when picking `catchUpPolicy: 'after-startup'`):
  ...
  - **`interval`** triggers compare the first `createdAt + k × intervalMs` grid point after `lastRun` (or after `createdAt` when never fired) against `now()`. SchedulerService maintains no `nextRun` for interval schedules, so this recomputes the same grid `armSchedule` targets — anchoring on the grid rather than on `lastRun + intervalMs` keeps a manual or late fire from shifting the calendar and hiding the next missed point for a whole period.
