...
Admission is capped at **5,000 entries** and **32 MiB of aggregate input JSON**; exceeding either throws `JOB_BATCH_TOO_LARGE` before anything is persisted. The per-entry 1 MiB `input` cap bounds one row, not the batch, so these ceilings are what keep a high-fan-out producer from parking an unbounded payload in the main process and holding the synchronous write transaction open. Preparation happens entirely outside that transaction — handler lookup, validation, and row ids are all resolved up front, so the write lock covers the idempotency lookup and the INSERTs, not payload validation or id generation. Because the batch is homogeneous, every entry without an explicit `scheduledAt` shares one batch timestamp.
...
After commit, initial snapshots are published in one shared-cache batch when its estimated structured-clone payload stays within 4 MiB. Larger admissions skip that cache publication entirely; renderer observers use the existing DataApi cold-start fallback instead of receiving an unbounded IPC message. Delayed rows are armed individually, and pending rows trigger at most one dispatch kick per affected queue. Cache publication and each delayed timer are best-effort post-commit effects: one failure is logged without blocking queue creation, later timers, or pending dispatch — and because the rows are already committed, none of them can surface as an enqueue error.

The two variants reach those effects differently. `enqueueBatch` owns its transaction, so it runs them **inline before returning**, exactly like `enqueue`: the freshly committed snapshots are the truth, and no caller code can interleave. Deferring them would be unsafe — `cancel` / `cancelMany` finalize rows without yielding, so a same-tick cancel would be overwritten by a stale `pending` snapshot that nothing republishes. `enqueueBatchTx` composes with an existing synchronous `withWriteTx` callback, where the caller's transaction may still roll back or move rows on; it therefore defers by one microtask and **re-reads** the rows, discarding resolvers for any that vanished. Callers must not swallow its database errors inside the transaction callback. Batch enqueue optimizes durable admission only — beyond the admission ceilings above it adds no outstanding-capacity limit and does not change per-queue/global running concurrency.
