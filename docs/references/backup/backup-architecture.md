...
> [!NOTE]
> **This section is the D-model target-state for restore.** Upstream `createSnapshot`/`applyMigrations`/preboot promotion gate (`backupRestoreGate.ts` / `runRestorePromotion`) **landed via #16884 (merged to main)**; the C-import **staging spine** (`ImportOrchestrator`: snapshot → fingerprint → seal → 2nd fingerprint → staged journal + `BackupService.startRestore`/`performRestoreRecovery`) is **landed and tested**. Still **pending (fail-closed stubs)**: the detached merge engine, write-quiesce (#16849/#16850 — `WindowManager.acquireMutationCapableWindowHold` + renderer mutation admission+barrier), and file-resource staging — no staged journal is written without them. Hook context types in `contributorTypes.ts` document D-model semantics in JSDoc (`AfterImportContext` / `RestoreResourceContext`); `restoreResources` is a paper contract (the preboot gate drives blob promotion via `RestoreJournal.fileResources`, not a per-contributor runtime hook). Hence the difference between this section and the current code is **target vs pending-stubs, not an omission** — reviewers should not judge by "spec must describe current state".
...
1. **write quiesce** (bounded, strict subset of the old RESTORE BARRIER): before createSnapshot, quiesce + drain **all** live SQLite writers; missing even one may violate "never delete local data" (@0xfullex #16714):
   ...
   - **drain in-flight renderer-originated writes**: restore SHALL guarantee **no renderer-originated DB write** within the snapshot→relaunch window — this is an **explicit C-import requirement, not an assumption** (current codebase WindowManager only manages window lifecycle, DataApi/Preference IPC handler has no restore gate, C-import must implement). **Mechanism selected (C-import plan, synced with @0xfullex #16850)**: **a1** — WindowManager `acquireMutationCapableWindowHold()` destroys all mutation-capable renderer BrowserWindows (kill renderer process, no renderer can send IPC) + blocks subsequent mutation-capable window opens + pauses pool/replenish + abort reopens the main window; main-process **native progress UI** replaces renderer `RestorePopup`. a2 (main-boundary `activeRestore` flag rejecting at DataApi/`Preference_Set` IPC entry) is **deleted** — the hold must persist (MainWindow can be re-opened via tray/activate, QuickAssistant/Selection have independent open paths, pooled windows have async standby replenish), so a one-shot flag is insufficient. **"Disable window" is not enough** — a disabled BrowserWindow's renderer JS still runs, still sends IPC. Stronger than V1 `RestorePopup` (single-window modal, V1 short file-copy window acceptable), because the D-model SQLite import window is longer. Regardless of a1, in-flight renderer-originated writes already dispatched into main must first be **drained via an independent mutation admission + in-flight barrier at each mutation dispatcher** (ApiServer mutation / Preference IPC / IpcApi / legacy write IPC — BackupService only unifies acquire/drain, does not atomically track in-flight handlers; R2-M1): after a1 takes effect + main-side pause, wait for a "no in-flight DB write" window before createSnapshot; otherwise they land on old live after snapshot and are overwritten by work.sqlite at promotion, i.e. lost.
