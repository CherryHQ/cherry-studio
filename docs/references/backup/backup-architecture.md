...
>
> [!NOTE]
> The "drizzle throughout / no bare-SQL column-name risk" claim scopes to the **export read path** (`ExportOrchestrator` drizzle handle + contributor declarations). The **importer merge path** (`MergeEngine`) uses a detached better-sqlite3 connection with hand-rolled raw SQL by design (sync tx performance): physical column names go through `physicalColumn` (MergeEngine) + `quoteIdent`. Emitting `DbColumnEntry.dbName` from codegen to harden the importer is a later ticket — not a claim that MergeEngine already uses drizzle.
...
> [!NOTE]
> **This section is the D-model target-state for restore.** Upstream `createSnapshot`/`applyMigrations`/preboot promotion gate (`backupRestoreGate.ts` / `runRestorePromotion`) **landed via #16884 (merged to main)**. The C-import **staging spine** (`ImportOrchestrator`: quiesce → fingerprint → snapshot → merge → migrate → seal → stage → 2nd fingerprint → staged journal + `BackupService.startRestore`/`performRestoreRecovery`) and detached merge engine are **in this PR** — `BackupService.startRestore` runs the spine via **partial quiesce** (`BACKUP_IN_PROGRESS` IPC gate + `JobManager.pause` #16925 + best-effort `drainInFlight`); merge = **backfill-when-absent + FIELD_MERGE on natural-key/slot conflict + SKIP on uuid-entity conflict + dangling-ref repair** (absent natural-key aggregates INSERT keeping the backup PK — fresh-install restore keeps preferences/providers/API keys/workspaces; identityKey conflicts run column-level FIELD_MERGE keeping the local row+PK while backup fills SQL NULL / `fieldMergePolicies`-empty columns and absent members INSERT; unresolvable refs degrade: nullable FK → SET NULL, NOT NULL FK → prune; message.data `fileEntryId` blobs missing from `stagedFileEntryIds` are disclosed). **Deterministic PK** natural-key (e.g. `user_model.id = providerId::modelId`) stays FK-clean without remapping; **non-deterministic PK** natural-key (e.g. `agent_workspace`/`tag`) keeps local PK — B1 rewrites backup-side FKs (root owning FK + required JSON entity-id) to the remapped local canonical, only true dangling refs fall to repair + disclose (conflict identity propagation landed via B1); **uuid-entity** remains SKIP. Full quiesce **landed** (a1 WindowManager hold destroys mutation-capable renderer + blocks subsequent mutation-capable window opens + pauses pool/replenish; #17014 AI/channel pause; IPC `BACKUP_IN_PROGRESS` gate; native Notification replaces renderer RestorePopup — sealed restore main-process auto-relaunches) + OVERWRITE/RENAME + file blobs/KB/Notes stagers = **follow-up**; `startRestore` no longer throws `BACKUP_RESTORE_NOT_READY`. SKILLS `dir-add` is covered only by unit tests (`restoreJournal.ts` schema + preboot gate consume path in `restorePromotion.ts`); file blobs, knowledge directories, and Notes remain fail-closed / deferred (write-quiesce is complete via a1). Hook context types in `contributorTypes.ts` document D-model semantics in JSDoc (`AfterImportContext` / `RestoreResourceContext`); `restoreResources` is a paper contract (the preboot gate drives blob promotion via `RestoreJournal.fileResources`, not a per-contributor runtime hook).

Restore runs the **D model** (Detached Merge + Preboot Promotion, aligned with @0xfullex #16714): **never touch the live DB in-process** — at runtime merge on a detached `work.sqlite` copy, preboot atomic rename promotion. The live DB is only rename-replaced during the preboot zero-connection window, structurally eliminating the entire class of risks: half-restored / WAL sidecar replay / runtime rollback. Merge semantics (SKIP / FIELD_MERGE / only-add + aggregate conflict + identity propagation) all remain part of the design contract; only the import target changes from live to detached work.sqlite (**implemented today**: backfill-when-absent + FIELD_MERGE for natural-key/slot + SKIP for uuid-entity + dangling-ref repair; conflict identity propagation for non-deterministic PK still B1 follow-up).
...
1. **write quiesce** (bounded, strict subset of the old RESTORE BARRIER): before createSnapshot, quiesce + drain **all** live SQLite writers; missing even one may violate "never delete local data" (@0xfullex #16714):
   ...
   - **drain in-flight renderer-originated writes**: restore SHALL guarantee **no renderer-originated DB write** within the snapshot→relaunch window — this is an **explicit C-import requirement, not an assumption** (the pre-quiesce codebase had no restore gate; **this PR adds the IPC restore gate** — DataApi/Preference/IpcApi reject mutations via `BACKUP_IN_PROGRESS`, see Mechanism below; the window-level hold a1 landed). **Mechanism — PARTIAL in this PR, full follow-up**: this PR gates IPC mutations via a `BACKUP_IN_PROGRESS` flag (`src/main/data/db/backup/quiesceGate.ts`, module singleton held for the restore window) at each mutation dispatcher — DataApi `IpcAdapter` rejects non-GET requests, `PreferenceService` rejects `Preference_Set`/`SetMultiple`, `IpcApiService` rejects non-`backup.*` routes (read requests pass; merge runs on a detached `work.sqlite`, so live reads stay safe). BackupService additionally `pause`s JobManager (#16925, refcounted hold gates new dispatch + croner) and `drainInFlight`s its in-flight executions (best-effort, bounded timeout) before the snapshot. **Full quiesce (landed)**: a1 — WindowManager `acquireMutationCapableWindowHold()` destroys all mutation-capable renderer BrowserWindows (kill renderer process, no renderer can send IPC) + blocks subsequent mutation-capable window opens + pauses pool/replenish; main-process **native Notification** replaces renderer `RestorePopup` (sealed restore main-process auto-relaunches) ("disable window" is not enough — a disabled BrowserWindow's renderer JS still runs and sends IPC; stronger than V1 `RestorePopup` because the D-model SQLite import window is longer); #17014 pauses AI streams / agent turns / channel messages. **Residual write paths partial quiesce does NOT cover**: legacy `File_`/`Cache_` write IPC, legacy `Backup_*` restore (S3/local/WebDAV via `LegacyBackupManager` — a same-purpose sibling that can race a v2 restore if the user clicks legacy v1 restore mid-window), main-process `DbService` direct writes outside IPC, un-drained AI/channel turns — the preboot promotion gate (#16884 atomic rename) is the correctness backstop for any write that slips past. **Quiesce status**: #16850 (JobManager) **DONE via #16925 MERGED** + `drainInFlight` + IPC `BACKUP_IN_PROGRESS` gate **this PR**; AI/channel = **#17014 LANDED** (writer pause methods + BackupService.quiesceWriters wiring, merged via origin/main); a1 WindowManager hold = LANDED.
...
3. **detached connection** (independent better-sqlite3, not live `DbService.sqlite`): run contributor import pipeline on work.sqlite (handle parameterized, detached drizzle, not `DbService.getDb()`; merge semantics SKIP / FIELD_MERGE / only-add per design contract — **implemented**: backfill-when-absent + FIELD_MERGE for natural-key/slot + SKIP for uuid-entity + dangling-ref repair; non-deterministic conflict identity propagation still B1); `defer_foreign_keys=ON` + upsert / leaf-cascade, see below) + `applyMigrations` (backup already migrate-forwarded in step 0; custom SQL) + **FTS rebuild** (importer responsibility, in work.sqlite) + **offline verification** (integrity_check + foreign_key_check + domain checks + FTS consistency). A work.sqlite that fails verification is never promoted.
...
This plan makes RestoreRecoveryPoint (pre-snapshot + journal + file resource promotion / undo) an in-scope must-deliver item. **Snapshot creation failure SHALL block restore**. **Restore safety status**: #16884 (preboot promotion gate `runRestorePromotion` + journal schema + `createSnapshot`/`checkpointTruncate`/`hashDbFile`/`readAppliedChain` primitives) is **merged to main**; the C-import staging spine and detached merge engine are **in this PR** — `BackupService.startRestore` runs the spine via **partial quiesce** + **FIELD_MERGE (natural-key) / SKIP (uuid-entity) merge with dangling-ref repair** (no longer gated `BACKUP_RESTORE_NOT_READY`; see §9 step 1; non-deterministic conflict identity propagation landed via B1). **DB restore stager is in this PR**; file/KB/Notes stagers are **deferred** (a1 WindowManager hold landed; #17014 AI/channel pause landed); the **export pipeline is live**. Contributor is not responsible for full-database snapshot and promotion.
