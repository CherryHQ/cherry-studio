...
>
> **Upstream prerequisites (gating)**: DbService `createSnapshot(targetPath)` (VACUUM INTO, restore merge base only) and `backupTo(targetPath)` (online export copy) + `applyMigrations(db)` + the preboot promotion gate are landed. **Deprecated** (D model does not need): `restoreDbFromSnapshot` / `verifyLiveDb` / onInit recovery gate; and runtime preference/cache mutation gates (restore promotes before normal startup). Remaining gates are complete write-quiesce and the non-SKILLS resource stagers.
>
> [!NOTE]
