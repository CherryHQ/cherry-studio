...
This plan makes RestoreRecoveryPoint (pre-snapshot + journal + file resource promotion / undo) an in-scope must-deliver item. **Snapshot creation failure SHALL block restore**. **Restore safety status**: #16884 (preboot promotion gate `runRestorePromotion` + journal schema + `createSnapshot`/`checkpointTruncate`/`hashDbFile`/`readAppliedChain` primitives) is **merged to main**; the C-import staging spine and detached merge engine are **in this PR** — `BackupService.startRestore` runs the spine via **partial quiesce** + **backfill/SKIP merge with dangling-ref repair** (no longer gated `BACKUP_RESTORE_NOT_READY`; see §9 step 1; field-level FIELD_MERGE + conflict identity propagation are follow-up). **DB restore stager is in this PR**; file/KB/Notes stagers + full quiesce are **deferred after #17014**; the **export pipeline is live**. Contributor is not responsible for full-database snapshot and promotion.
