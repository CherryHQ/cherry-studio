...
2. **write quiesce** (bounded, a strict subset of the old RESTORE BARRIER): pause three autonomous main-side DB writers — JobManager (cron / GC / overdue) + in-flight AI streams / agent turns + inbound channel messages **+ drain in-flight renderer-originated writes** (DataApi mutation / `Preference_Set` IPC — writes dispatched before the barrier must drain first, otherwise they land on the old live after the snapshot and get overwritten at promotion). Mechanism **a1 selected** (a2 deleted; **unimplemented, C-import TODO — not yet in any PR**): WindowManager `acquireMutationCapableWindowHold()` destroys all mutation-capable renderer windows + blocks re-open + main-process native progress UI; plus an **independent renderer mutation admission + in-flight barrier** at each mutation dispatcher (R2-M1, not the deprecated `@WriteSilenceable` runtime silencing — that "no gate" note referred only to `@WriteSilenceable`-style gates, not the admission+barrier which IS required). cache is naturally fresh-loaded after the restore relaunch; no live writer during apply. The quiesce interfaces for the three autonomous writers are owned by their respective modules; renderer in-flight drain + global window blocking is orchestrated by BackupService (acquiring a write-quiet barrier). **Quiesce status**: #16850 (JobManager) **DONE via #16925 MERGED**; AI/channel = **#17014 OPEN** (fixes #16849); a1 + renderer drain = unimplemented.
