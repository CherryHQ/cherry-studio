...
`scripts/download-binaries.js` fills `resources/binaries/<platform>-<arch>/`, which is what the app extracts from at boot. During packaging (`before-pack.js` passes `--packaging`) it downloads there directly. A dev run instead downloads into `<git-common-dir>/cherry-binaries/`, a cache shared by every worktree of the checkout, and hard-links the result into `resources/binaries/` — so a second worktree costs links rather than a repeat download, and the runtime still reads the one path it always did. Downloads stage under `.staging-<checkout-id>/` inside that cache and are renamed into place only after their checksum passes, which keeps concurrent worktrees from writing each other's files and lets an interrupted transfer resume. Version markers publish the same way rather than being written in place, because an in-place write keeps the file's inode and would push the new version number through every worktree's hard link while its binaries stayed old. Cache-internal directories (`.staging-*`, `.retired-*`) are never mirrored into a worktree. The cache is not reachable by `git clean`; delete the directory to force a fresh download.
