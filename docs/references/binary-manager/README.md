...
`binary.list_tools` returns state-file entries with `managed: true`, including explicitly installed `node` or `python` runtimes. These entries show the Runtime badge but remain updateable and removable through the normal mise path; removal warns that dependent npm/pip tools may break. Its only live-mise supplement is an **unrecorded** `node` or `python` runtime dependency, marked `managed: false`; these auto-discovered entries are display-only and resolve through Cherry-managed mise shims. Other unrecorded `mise ls` entries are intentionally ignored because BinaryManager state remains the authority for manageable tools.
