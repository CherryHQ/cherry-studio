...
Under the mutation lock it validates the canonical intent, rejects an active install/remove for the same name, enforces the same name/spec single-owner invariants as install, proves the exact tool spec is installed, verifies the named executable target is runnable, and only then persists the manifest entry. The installed-spec proof reads only the `mise ls --json <spec>` entry keyed by the queried spec — a `core:` runtime is reported under its bare name (`core:node` → `node`), which is the only key normalization accepted, so an aliased or unexpected mise response is never mistaken for proof and ownership is never inferred from a PATH or shim name. If `requestedVersion` is supplied it must be a concrete semver and equal the observed version exactly; a range or tag such as `11` or `latest` is rejected rather than silently ignored. For a `node`/`python` runtime claim without `requestedVersion` the observed version is pinned into the entry. It returns the observed version. A manifest write failure leaves the tool runnable-but-unowned, and success broadcasts `availability_changed` through the normal manifest invalidation. Like `install_tool`/`remove_tool`, the route refuses a `senderId: null` caller.
