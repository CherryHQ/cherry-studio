...
2. **Unverified premise.** The "most common duplicate trigger for CJK users" justification was AI-authored in a comment, and there is no observed evidence that this app's path sources (`showOpenDialog`, drag-drop, `fs.readdir`) ever surface the same file in divergent normalization forms.
