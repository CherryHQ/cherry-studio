...
最重要的现状结论（2026-06-10 更新）：**9 表 material 模型 + `KnowledgeIndexStore` 已由 PR A 落地**，`search()` 与索引 job 已切新 store，旧单表 `libsql_vectorstores_embedding` + `external_id` API（`replaceByExternalId` / `listByExternalId` / `deleteByIdAndExternalId`）已从 runtime 移除（仅 v1→v2 迁移器仍读旧表作来源）。本文 §4/§5 的 schema 与接口现为「已实现」；仍待做为 PR B + PR C（见 §15.2）。

2026-06-09 重新核实补充（上游已合并）：embedding / rerank 已改走 `AiService` → 用户配置 provider（不再是本地 ONNX，见 §5.5）；BM25 + 向量 + RRF hybrid 检索已可跑（PR A 已切到新可移植 store；BM25-only 降级仍待 v2.x）；删除 / 迁移 / 渲染解耦等大部分已落地。最新 PR 规划见 §15，引擎可移植性设计见 §5.6。
...
> **状态横幅（2026-06-10）：整套 9 表 material 模型已由 PR A 落地（9 张全部创建，`indexStore/schema.ts`）。** runtime 已写入本结构，旧单表 `libsql_vectorstores_embedding` 仅剩 v1→v2 迁移器读取作来源。`usage_event` 等自适应增强结构是更未来的工作（v2.x / PR C），论证见 [调研报告 §4](./knowledge-research-report.md)，本节不展开。
...
不建议创建（及原因）：`knowledge_item`（材料由目录 + `index.sqlite` 管理）、`job`（进度复用 JobManager）、`embedding_target`（`search_text.embedding_text_hash` 已连接文本与向量）、`schema_migration`（用 `index_meta.schema_version` 当版本游标，演进迁移见 §4.1）、`ignore_rule`（规则由代码侧维护，仅记 `ignore_rules_version`）、`material_link`（缺明确维护方）。
...
- `schema_version` 是后续 schema 演进的版本游标：打开时与代码常量 `KNOWLEDGE_INDEX_SCHEMA_VERSION` 比对，落后则跑 forward-only 就地迁移——加/删字段用 `ALTER ADD/DROP COLUMN` 保留数据（libsql 与 better-sqlite3 同为 SQLite，迁移 SQL 可移植，与 §5.6「规范数据可移植」同一原则）；改约束/类型走「建新表 copy 旧行 → rename」，动 FTS 索引列补 rebuild；迁移失败 / 库损坏 / 版本比代码新时从源头重建（`index.sqlite` 是派生库，这是兜底安全网）。完整策略见[表结构设计 §11](./index-sqlite-schema-design.md)。

> as-built: `index_meta` 单行写入 + `base_id` 校验已随 PR A `ensureIndexMeta` 落地；但其 snapshot **契约比对 + 选择性重嵌**的设计仍**保留为未来计划目标**。baseline 过渡手段：embedding 配置**每库不可变**，改模型/维度会触发把整库 restore 进一个新库（`KnowledgeVectorStoreService.ts` + `RagConfigPanel` restore 流程），而非 snapshot 选择性重嵌。restore 仅是过渡，终态仍以本节 snapshot 机制为准。

> as-built: 上述 schema 演进迁移 runner **尚未实现**（无「读版本 → 比对 → 迁移」逻辑，步骤表为空）。`schema_version` 字段随 9 表 store 写入，但其驱动的演进迁移留待 v2.x 首次对已发布用户改 schema 前落地；开发期改 schema 直接删本地 `.cherry/index.sqlite` 重建。
...
> **状态（2026-06-10）：接口已由 PR A 落地。** 运行时已走 `KnowledgeIndexStore` 的 material 级 API（`rebuildMaterial` / `deleteMaterial` / `listMaterialUnits` / `listExistingEmbeddingHashes`）；旧 `replaceByExternalId` / `listByExternalId` / `deleteByIdAndExternalId` 已移除。
...
>
> 这与 [research-2026-06-09 §Q1](./research-2026-06-09-future-knowledge-tools-and-adaptive-search.md)（L31/L45/L70/L137）的调研共识基本一致：该共识本就反对把 `hybridAlpha` 这类「算法名」暴露进 Agent 入参（最多给一个可选软提示 `intent?:'lookup'|'broad'`），把它收回 base 级配置正落在共识一侧；分歧仅在 `threshold`——调研推荐的入参是 `{query,baseIds,topK?,cursor?}`，本项目额外把 `threshold` 也作为逐调用旋钮暴露（召回质量调试），是有意的小幅偏离。「各检索面共用同一组逐调用参数」目前只部分落地：REST `POST /knowledge-bases/search` 的 `top_k` 已接到逐调用 `topK`（上界/默认与 `kb__search` 共享 `KNOWLEDGE_SEARCH_MAX_TOP_K`/`KNOWLEDGE_SEARCH_DEFAULT_TOP_K` 常量），但 `threshold` 尚未暴露到 REST/IPC 面，召回测试面板也仍用默认值——这两处接线属后续 PR。
...
> as-built: 渲染层去 FileEntry 已完成（无 `/files/entries/:id`、add-item DTO 已拆分、`fileProcessing.startJob` 用 `FileHandle`）。`deleteItemChunk` 全链路已由 PR A 移除（preload / IpcChannel / handler 一并删）。仍待执行：note 仍持久化 `data.content`；material preview/attach 未做；**chat 附件按钮被直接删除**（待 chat 管线迁移后再接 material handle，比计划更激进）。
...
## 14. 外部源 Connector 落地（能力声明式 · 飞书 / WebDAV / 企业 Gateway · 未来扩展技术纲要）

> 架构论证、飞书 / WebDAV 能力事实、被否方案、产品决策点见 [调研报告 §6](./knowledge-research-report.md)（消费级 / 外部云源接入）与 [§5](./knowledge-research-report.md)（企业内网 Remote File Mode）。本节为未来扩展（v2.x+）技术设计纲要，非当前 v2 必做。

### 14.1 共性抽象：能力声明式 Connector

把每个外部源建模成一个薄 Connector，公共层（9 表 store、切块、embedding、hybrid 检索、locator 契约、kb__* 工具、同步循环、鉴权刷新框架）只写一次，Connector 不碰。**差异收进能力声明（数据），不收进接口方法（代码）**——源没有的能力（如 WebDAV 无服务端搜索）声明为「无此能力」、对应职责缺席，而非实现一个永远 throw / 撒谎的空壳。

> **形态未定，本节不锁类型。** 一个 Connector 究竟是内部接口实现、还是独立 plugin / 扩展点（带注册、隔离、第三方可插拔），目前都未确定。下面只描述**概念职责**与**需要区分的能力维度**，不给出具体接口签名 / 类型定义——这些待形态拍定后再写。

**概念职责（一个源接入至少要能回答的）：**

- **必备**：① 逐层枚举源里有哪些材料（每条带稳定身份 + 一个「变了没有」的版本标记）；② 凭一个对公共层不透明的 locator 取回某份材料的规范化文本 / 字节，并如实给出「仅片段 / 无权限 / 不支持 / 版本冲突 / 已失效」等状态（绝不伪装成功）；③ 声明自己的能力；④ 把源原生错误码翻译成公共层统一状态。
- **可选（由能力声明 gate，缺了就走公共层兜底）**：原生搜索（仅自带服务端搜索的源）、变更检测（推送 / 轮询 / 无）、鉴权（OAuth / Basic / 无）。

**需要区分的能力维度**（仅列维度，取值与各源映射见[调研报告 §6.3](./knowledge-research-report.md) 能力位表）：原生搜索种类（none / keyword / semantic / hybrid）、搜索粒度（文档级 / chunk 级）、分数语义（相关度 / 排名 / 无）、变更方式（推送 / 轮询 / 无）、内容形态（文件 / 结构化）、鉴权形态（无 / Basic / OAuth）、配额形态（无 / 频控 / 频控+月度）、默认内容落点（本地镜像 / 留远端）。

**公共层按能力声明分派（调用前协商，能力前移到 kb__list）：**

- 检索：无原生搜索 → 本地 hybrid / BM25；关键词 + 文档级 → 委托原生搜索一路召回 + 本地向量召回 RRF（按分数语义处理分数、按粒度决定是否取回后本地切段）；语义 / 混合 → 可直接委托。
- 变更：推送接事件、轮询按配额退避、无则等手动 refresh。
- 鉴权：OAuth 自动刷新（加锁 + 原子覆盖轮换 refresh_token）、Basic 失效弹重填，统一出口 forbidden / needs-reauth。
- locator：沿用不透明契约，内部编码委托给产生它的 Connector，公共层只做与内部格式无关的归属 + 版本 stale 校验（见 §7）。

> **`storageKind` 单轴拆解：** 旧 `storageKind=local|remote` 把「内容放哪 / 检索在哪 / 有没有原生搜索 / 推送还是轮询」压成一维，导致 WebDAV 与飞书同为 remote 却需源特判。应降为「默认内容落点」一个维度（内容位置轴），与搜索 / 变更 / 鉴权 / 配额各维度正交。

### 14.2 飞书知识库 connector（结构化 API 源）

- 能力声明：关键词搜索（文档级、排名分）、轮询变更（`revision_id`）、结构化内容、OAuth、频控 + 月度配额、默认留远端（语义升级后转本地镜像）。
- 枚举 = `GET /wiki/v2/spaces/:id/nodes` 按 `has_child` BFS（≤50/页、约 100/分限速退避）；稳定身份 = `node_token`+`obj_token`、版本标记 = `revision_id`；shortcut 节点只记引用不重复抽取。
- 取内容 = docx `raw_content` 取整篇纯文本（5/秒）；内嵌 sheet / bitable 第一版不下钻、返回「仅片段」并诚实标注「含未索引内嵌内容」；mindnote / 画板无结构化接口 → 仅索引 title、命中返回「仅片段」+ 飞书 url。
- 原生搜索 = 委托 `search/v2/doc_wiki/search`（文档级 + 高亮摘要，snippet 去标签、score 标排名分）。
- 鉴权 = OAuth v2（`user_access_token` 2h、`refresh_token` 7 天单次轮换须加锁 + 原子覆盖、365 天重授权、`offline_access` + 只读 scope `wiki:wiki:readonly`+`docx:document:readonly`+`drive:file:readonly`+`search:docs:read`）。
- 落地分阶段 = 骨架先行（默认、零正文 / 零 embedding、检索委托）→ 知情升级为本地语义镜像（见 [调研报告 §6.4](./knowledge-research-report.md)）。

### 14.3 WebDAV connector（远程文件源）

- 能力声明：无原生搜索（强制本地建索引）、轮询变更（ETag + Last-Modified）、文件内容、Basic 鉴权、频控（无月度上限）、默认小库本地镜像 / 大盘留远端。
- 枚举 = 逐层 `Depth:1 PROPFIND` BFS；稳定身份 = `href`、版本标记 = `ETag`(+`Last-Modified` 兜底)、是否目录 = `resourcetype==collection`。
- 取内容 = HTTP `GET`（探测 `Accept-Ranges` 后才用 Range）；原始文件交既有 path-based fileProcessing 解析抽文本。
- 无原生搜索（职责缺席）→ 公共层强制本地建索引；这是抽象的「地板」验证：不为无搜索能力的源写任何空壳搜索。
- 鉴权 = Basic（地址 + 账号 + 应用专用密码，按 base 加密存储；撤销 / 改主密码 → 401 触发重填表单）。

### 14.4 企业内网 Gateway（既有 Remote File Mode，作为一个能力位组合）

> 在共性抽象下，企业内网 Knowledge Gateway = 一组能力（语义 / 混合检索〔服务端有索引〕、轮询变更、OAuth/SSO、无月度配额、默认留远端）+ 服务端 ACL；不是独立子系统。Gateway 架构论证、不推荐方案评估、MVP1-4 路线见 [调研报告 §5](./knowledge-research-report.md)。
...
### 14.5 material 身份改造对远端源的影响（PR A 前置评估）

当前 9 表 schema 为本地文件模型设计，远端源（飞书 node、WebDAV href）接入需在 PR A 设计阶段评估是否一次性纳入——v2 schema 是 throwaway（见仓库根 `CLAUDE.md` 的「v2 Refactoring」一节），PR A 冻结前改零成本、冻结后改是破坏性变更：

| 现状（本地文件承重墙） | 远端源需求 | 改造方向 |
| --- | --- | --- |
| `material.relative_path` NOT NULL UNIQUE + CHECK（非空 / 不以 `/` 开头 / 非 `.cherry`） | 远端源无合法本地路径；飞书轻骨架要求无路径也能存在 | 加源身份列（如 `source_locator_json`）或放宽 `relative_path` 约束 |
| `origin` 枚举 = user / processor / agent / captured / discovered | 需标识 connector 来源 | 扩 `origin` 或加 `connector_id` |
| `search_unit.unit_type` 无 `document` 值 | 飞书文档级命中 | 评估加 `document` / 或委托命中不入 `search_unit`、仅经 RetrievalRouter 透传 |
| read 一致性靠 `current_content_hash`（可空） | 远端无本地 content_hash | read 校验分两路：mirror 走 `content_hash`、remote 走 `versionTag`（见 §7） |

> 当前 PR A（9 表 store）的落地状态与决策见 §15.2 / §15.3；本节为 v2.x+ 方向，**不阻塞当前 v2 与 PR A 的本地路径**，仅提示「若要支持远端源，material 身份模型须在 store 契约冻结前预留」这一决策点（亦列入 §17）。
...
- 检索栈 BM25(FTS5)+向量(`vector_distance_cos`)+RRF hybrid，默认 hybrid——原建在旧单表（`LibSQLVectorStore.query`），**PR A 已切到新 9 表 store**（`KnowledgeIndexStore.search`）；BM25-only 降级仍属 v2.x、未做。
...
### 15.2 PR 路线（PR A 已落地，仍待 PR B + PR C）

> 状态(2026-06-10): **PR A 已落地**——9 表 material 模型 + `KnowledgeIndexStore` 已建（`indexStore/schema.ts`、`KnowledgeIndexStore.ts`），`search()` 与索引 job 已切到新 store，`deleteItemChunk` 全链路已移除；旧单表 `libsql_vectorstores_embedding` 仅剩 v1→v2 迁移器作迁移来源读取。核心在 commit `029709d201`，C1 / I1–I8 review 修订在本分支。下表 PR A 行视为完成，**仍待做为 PR B + PR C**。

| PR | 目标 | 主要范围 | 依赖 | 规模 / 风险 |
| --- | --- | --- | --- | --- |
| **✅ PR A · 可移植 IndexStore + 9 表 + 检索切换（已落地）** | 用引擎可移植 schema 落地新 store，并把索引 / 检索切过去 | `SqliteDriver` 接口 + libsql 适配 + `VectorIndex` 适配（首版暴力扫描、不建 vec0/ANN 索引，§5.6）；建 9 表 + FTS5（trigram，照搬 `message.ts` external-content + trigger）；`rebuildMaterial` 事务原子替换（旧 unit 清理、embedding 以**纯 BLOB** 写入并按「文字指纹+模型+维度」复用、FTS rowid 对齐）；`search()` 切新 store（BM25 / 向量 / RRF，保留旧 `KnowledgeSearchResult` shape、**embedding-必需契约不变**）；索引 job 由 `replaceByExternalId` 切到 `rebuildMaterial`；**随切换移除 `deleteItemChunk` 全链路**（其后端 `deleteByIdAndExternalId` 此时消失） | 无 | 生产 ~2.5–4k LOC / 20–30 文件；测试 ~3–5k LOC。**风险最高**：事务原子性、FTS rowid、向量 BLOB 跨引擎可移植 |
...
**外部源 Connector 相关待拍板（v2.x+，见 §14 与[调研报告 §6.5](./knowledge-research-report.md)）：**

- **检索地板**：本地索引永远为地板（WebDAV 唯一可行、score 统一）vs 允许飞书 `fidelity=skeleton-only` 的纯委托轻骨架作合法常驻态——须产品拍板，工程定不了。
- **material 身份改造时机**：远端源所需的 `source_locator_json` / `origin` 扩枚举 / `relative_path` 放宽 / read 双路校验是否在 PR A 冻结前一并纳入（见 §14.5）。
- **能力位静态 vs 运行时可变**：与「能力协商前移到 kb__list 一次性快照」相关；倾向静态 + 注册期探测，运行态变化走 forbidden / unsupported / stale 兜底。
- **飞书个人版可行性（前置阻断）**：个人免费用户能否自助授权第三方应用并大规模索引官方未明确，须真实账号 / 自建测试企业实测 OAuth + doc_wiki/search，作为发布前阻断项。
- **跨设备同步**：含向量的 `index.sqlite` 与 OAuth 凭据（`refresh_token` 单次轮换的多端竞态）如何跨设备——判为与 Connector 正交，先只同步「接了哪些源 + 接入配置」。
