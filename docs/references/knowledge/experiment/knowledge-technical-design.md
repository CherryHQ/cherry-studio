...
Migrated v1 folders keep their vectors: a v1-indexed `directory` item migrates into a `completed` container `directory` plus one `completed` `file` child per embedded file, re-using the folder's already-embedded vectors verbatim (no re-embed) by reading each chunk's path from the legacy `vectors.source` column. Only when those legacy vectors are unreadable does the folder fall back to a tombstone — `warning` + the `directory_not_migrated` code, which the UI renders as an amber re-embed warning ("needs re-embed") instead of a red error; re-indexing then re-expands from `data.path` (copy files into the base, create child items, embed). See §7 for the decision record.
...
- **v1 directory migration — re-attribute vectors, tombstone as fallback (decided 2026-06-11 as tombstone-only, reversed to re-attribution 2026-06-13)**: a v1-indexed folder migrates by re-using its already-embedded vectors instead of dropping them. `KnowledgeMigrator` expands the `directory` item into a `completed` container `directory` plus one `completed` `file` child per embedded file (`expandLegacyDirectoryItem`), reading each file's path from the legacy `vectors.source` column (`loadLoaderSourceMap`) and publishing a loader-id → child-item-id remap over `sharedData`; `KnowledgeVectorMigrator` consumes that remap so the folder's chunks land on the per-file children rather than being skipped as `non_indexable_container`. The v1 vectors are reused verbatim — no re-embedding. Children carry the external `source` path and a **virtual** `relativePath` (their own id): the originals are **not** copied into the base (v1 never stored the folder inside Cherry), so search uses the migrated vectors directly and there is no `raw/` file. Synthesized children are inserted after their container (FK `(baseId, groupId) → (baseId, id)`) and are skipped by the file-copy pass (they have no storage name to back-fill). **Fallback:** when the legacy vectors are unreadable/empty or the folder has no loader ids, `expandLegacyDirectoryItem` returns null and the item keeps the original tombstone — `warning` + `directory_not_migrated` (UI: amber re-embed warning), re-indexing re-expands from `data.path`; because the tombstone now appears only when vectors genuinely could not migrate, the "needs re-embed" message is accurate (it no longer fires for folders whose data did migrate). Background: v1 embedded each folder file individually but booked every loader id on the one directory item (no per-file items), so without this re-attribution the vectors are dropped as `non_indexable_container` and the v2 index comes up empty. **Known v2.0 limitation (MVP):** re-indexing a *single* migrated child cannot rebuild from a `raw/` file (there is none) and falls back to its `source`; re-adding the whole folder via `data.path` always works. The earlier-rejected placeholder-file alternative stays rejected (synthetic content would bake in as truth on later reindex; binary extensions cannot host text). The legacy embedjs DB stays in place (rollback insurance).
- **Watcher preconditions (surfaced by the directory-migration review)** — constraints for the future file-watcher PR:
  - Index cleanup must be driven by user intent (explicit delete/reindex), never inferred from a missing file: url/note materials use the item id as a virtual `relative_path` with no on-disk file, migrated v1 directory children likewise carry a virtual `relative_path` (their `source` points at the file's own original external path — the folder path lives on the container — never copied into the base), and migrated urls have no snapshot until their first reindex. (The knowledge_item exit-path decision below pulls url materialization into PR B; once landed, notes and migrated directory children keep a virtual path.)
...
- **knowledge_item exit path (decided 2026-06-11)**: `knowledge_item` does not survive into v2.x — the base folder becomes self-describing, `index.sqlite` stays purely derived, each remaining duty of the table gets a new home, then the table drops. Shape validated against QMD (MinerU-Document-Explorer): collection config outside the DB, path-addressed `documents` + hash-addressed `content`/`content_vectors`, scan-sync with an `active` soft-delete — and no item table at all. Sub-decisions:
  ...
  - **One-shot exit migration (v2.x)**: notes materialize to md + frontmatter (hash round-trip, zero re-embeds); directory tombstones and file-less failed rows become manifest entries (the amber state survives; repair = the standard add-directory copy-into-base flow — directories are one-shot copies, not connection-type sources); v2-native file/url/directory rows need nothing; then `DROP TABLE knowledge_item`. No dual-read window, no lazy per-base migration. Caveat from the directory re-attribution reversal (§ above): a migrated v1 directory's `file` children are `completed` but file-less (vectors only, no `raw/` file), so the derivation invariant cannot rebuild them from the folder — at exit they are treated like the file-less rows (manifest/repair entry keyed to the folder's `data.path`), not as v2-native file rows.
