...
KnowledgeBase/{baseId}/
  ...
  raw/                           # the single material root — all material bytes live here
    file/paper.pdf               # user-uploaded source file
    file/paper.md                # processor output (same partition as its source)
    directory/MyDocs/report.md   # a directory import keeps its whole tree under directory/
    url/example-page.md          # captured URL snapshot
    note/my-note.md              # captured note snapshot
```

- `raw/` is the **single material root**; every material's bytes live under it, partitioned by *import-action* type (`file` / `directory` / `url` / `note`). A directory import places its whole tree — including its `type=file` leaves — under `raw/directory/<name>/…`, so the partition is keyed by the top-level add, not by each leaf's type.
- `.cherry/**` is a reserved prefix, a **sibling** of `raw/`, and never enters the `material` table.
- `material.relative_path` is relative to `raw/` and **includes** the import-type segment (e.g. `url/example-page.md`); byte resolution is `{baseDir}/raw/{relative_path}` and never consults the type column. The `<type>/` segment is **decorative** — no code parses the path to infer type/origin; those always come from `knowledge_item` (the derived `material` table no longer carries an `origin` column, see §4.2). Path safety is enforced in the main process by `assertSafeKnowledgeRelativePath` rooted at `raw/` (zod only validates shape).
- Processor output shares its source's partition (`raw/file/paper.md` beside `raw/file/paper.pdf`); the source-vs-derived distinction is read from `knowledge_item` (`indexedRelativePath`), not from the directory or a `material` column.
