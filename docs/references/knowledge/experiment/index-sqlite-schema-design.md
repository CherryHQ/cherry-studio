...
> 状态(2026-06-08): 本篇整体仍为**设计稿，尚未实现，但仍是要执行的计划**。文中描述的 9 张表（`index_meta` / `material` / `material_relation` / `content` / `search_unit` / `content_index_entry` / `search_text` / `embedding` / `search_text_fts`）、material 模型、`KnowledgeIndexStore`（`rebuildMaterial` / `deleteMaterial` / `listMaterialUnits`）、`unit_id` 稳定生成、chunk offset、embedding GC 等都**尚未落地**：当前运行时仍是旧的单表 `libsql_vectorstores_embedding` + `external_id` API（`replaceByExternalId` / `listByExternalId` / `deleteByIdAndExternalId`，见 `packages/vectorstores/libsql`）。下文逐节会标注「已具备地基 / 仍待执行 / baseline 现状」。baseline 已经先行落地的地基：`index.sqlite` 已位于 `{baseId}/.cherry/index.sqlite`；用户文件已拷入 base 目录并以 `relativePath` 记录；path-based 文件处理已就绪。这些不改变本篇作为未来终态设计的性质。
...
> 状态(2026-06-08): 下表 9 张表**全部仍待实现，已建 0 张**。运行时仍是旧单表 `libsql_vectorstores_embedding`（`packages/vectorstores/libsql`），尚未创建任何 `index.sqlite` 业务表。本节为终态设计，保留为要执行的计划。
...
> 状态(2026-06-08): 此 `index_meta` snapshot + 契约比对 + 选择性重嵌的设计仍是**未来计划目标**，保留不变。baseline 当前作为过渡手段：embedding 配置每库不可变，改模型 / 维度会触发把整库 restore 进一个新库（见 `KnowledgeVectorStoreService.ts:37-39` 注释 + `RagConfigPanel` restore 流程），而不是 snapshot 选择性重嵌。restore 仅是过渡，未来仍以本节 snapshot 机制为终态。
...
索引库已位于 `{baseId}/.cherry/index.sqlite`，v2 -> v2.x 切换时无需再移动目录或索引文件。仍需保证的是 v2 阶段使用本文表结构，而不是先写临时向量表（当前 baseline 尚未实现本文表结构，见顶部状态注）。
