...
v2.x 切换到文件夹型知识库后，最终位置为：

```text
KnowledgeBase/{baseId}/.cherry/index.sqlite
```

如果用户在 v2 已完成知识库迁移，升级到 v2.x 时应尽量只移动 `index.sqlite` 到 `.cherry/`，不重嵌入、不重切 chunk。

当前 v2 就应把用户上传文件通过 filesystem copy 写入 `KnowledgeBase/{baseId}/` 可见目录，而不是只通过 FileManager external entry 指向原始外部文件。这样 v2.x 切换到文件夹型知识库时不需要再次复制用户文件。

当前 v2 的 `index.sqlite` 可以先位于：
...
用户文件已经在 `KnowledgeBase/{baseId}/` 下，因此切换时只需要移动索引库和隐藏系统文件，不需要搬迁用户材料。
...
当前 v2 仍可保留 `knowledge_item.type = directory | sitemap` 作为 UI 和 Job 编排容器，但它们不创建 `material`，也不直接进入索引。directory 上传时先把目录树复制到 `KnowledgeBase/{baseId}/`，展开出的 leaf file items 记录各自的 `relativePath` 并创建 material。sitemap 展开出的 URL snapshots 落成 Markdown leaf files 后再创建 material。v2.x 中目录由真实文件夹表达，sitemap 更接近一次导入任务或 captured source group，而不是长期 material。

当前 v2 写入知识库目录时遇到同路径冲突，先默认使用 keep-both 策略，自动生成 `_2`、`_3` 等后缀，避免覆盖用户已有文件。`knowledge_item.data.relativePath` 和 `material.relative_path` 都记录最终落盘路径。v2.x 再提供 replace / keep both / skip 的完整文件管理器式冲突对话框。
...
当前 v2 知识库调用 file processing 时使用 `file.kind = path` 和 `output.kind = path`。其他仍需要 FileManager 托管产物的业务可以继续使用 `file.kind = entry` 和 `output.kind = managed_artifact`。processor 内部只接收 resolved `FileInfo`，不需要关心调用方选择 entry 还是 path。
...
| `schema_migration` | 第一版使用 `index_meta.schema_version` 即可 |
...
1. 当前 v2 创建 `KnowledgeBase/{baseId}/` 和 `index.sqlite`；v2.x 切换时再移动到 `.cherry/index.sqlite`。
...
如果用户从开发中的 v2 升级到 v2.x，应尽量只移动目录和索引文件：
...
9. 实现 v2 到 v2.x 的 `index.sqlite` 位置移动。
