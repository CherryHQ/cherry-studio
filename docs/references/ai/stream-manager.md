...
| Clean `success` with no renderable parts | `onExecutionError` (`NoResponseError`) | Unless `request.allowEmptySuccess` (agent sessions), an empty-but-successful turn is folded as a `no_response` error instead of persisting a blank bubble |
