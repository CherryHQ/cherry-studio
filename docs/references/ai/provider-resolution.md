...
(`@ai-sdk/open-responses`, spec-neutral Responses dialect for third-party
providers) is deliberately NOT named `openai-responses`, or this suffix
probe would silently reroute every `adapterFamily: 'openai'` responses
endpoint onto it. `azure-anthropic` is not reached through the suffix
