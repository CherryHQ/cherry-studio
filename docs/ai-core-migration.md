...
| 操作 | 数量 | 说明 |
|------|------|------|
| **新建/重写** | ~9 个 | AiService (已存在,更新)、AiCompletionService (已存在,重写)、IpcChatTransport、shared schemas、`AiStreamBroker` + 三个 Sink 实现 + `BrokerStreamTarget` + `ClaudeCodeStreamAdapter`(全部在 Phase 2 完成;**不**造 `UIMessageAccumulator`,**不**造 `MessagePersistenceService` —— 前者复用 AI SDK 工具,后者直接用 Main 端已有的 `messageService` singleton) |
 ...
 * 构造 PersistenceSink。从源头避开 `activeNodeId` 多窗口竞态(Codex Finding 2 根治)。
 */
/**
...
export const aiStreamRequestSchema = z.object({
   ...
   *
   * **控制平面身份**:Broker 主表 key、`Ai_Stream_Abort` / `Ai_Stream_Detach` 精确路由键、
   ...
   * Phase 1(Broker 未落地)阶段的老路径做过渡;Phase 2 Step 2.3 之后应该不再
  ...
  //    Phase 2 的 Broker 模型下,regenerate 退化成"Renderer 生成新 requestId +
...
- Main 端建立 `AiStreamBroker` 作为"活跃流注册表",**按 `requestId` 索引**(一次生成尝试的身份),配合 `topicId → 当前活跃 requestId` 的辅助索引(给 steering 和多窗口观察者 attach 用);支持多订阅者、reconnect、grace period
- 持久化下沉到 Main 端 —— `PersistenceSink` 直接调 Main 端已有的 `messageService` singleton(`src/main/data/services/MessageService.ts`),不新增中间 Service。**注意**:Main 端**没有** Renderer 那个 HTTP-like `dataApiService` —— Renderer 客户端不能在 Main 里用,Main 直接用 service singleton
- Channel push 流和 Renderer user 流走同一套 Broker 机制,不再分两条路径

**负责人**: Person A (Main Broker + 持久化) + Person B (Renderer Transport + preload) + Person C (IPC channel 契约)

> **重要**:本阶段一次到位地把 Broker 架构建起来。之前版本的文档把这部分拆在 Phase 7 作为"事后补丁",踩过坑之后再补;现在前置到 Phase 2,避免 Phase 3/4 在错误的 Transport 基础上构建 Renderer 侧代码。架构详细设计见下方各 Step。
...
1. 添加以下 channel 常量:
   ```typescript
   // === Broker 拥有的 4 个请求通道 ===
   Ai_Stream_Open   = 'ai:stream:open'     // Renderer → Main: 发送消息(Broker 按 topicId 判断开新 request 或 steer)
...
- **两个 id,分工明确**:
  ...
  - `requestId` = Renderer 在 `sendMessages` 里 `crypto.randomUUID()` 生成,控制平面身份(Broker 主表 key、abort/detach 路由、in-memory 去重)
- 详见 [requestId / topicId 命名空间与并发约束](#requestid--topicid-命名空间与并发约束) 小节
- 订阅者身份由 `(event.sender, requestId)` 联合确定 —— 同一 WebContents 对同一 request 只挂一个 Sink
...
**注意**:
- **不暴露** `steerMessage` 接口 —— 用户追加消息直接调 `streamOpen`,Broker 内部根据当前 topic 是否有活跃流自动路由到 `steer` 或 `startStream`,Renderer 无感
- **`abort` 按 `requestId` 精确路由**(而不是 topicId)—— 这样 regenerate / stop+retry 场景下,迟到的 abort 不会误杀下一轮新流;同样地 `detach` 也按 `requestId`

### Step 2.3 及后续:AiStreamBroker 架构详细设计
...
> **历史注释**:在更早的文档版本里,上述三个问题被描述为"Phase 3 落地后才发现的 retrospective 问题",Broker 作为"Phase 7 补丁"事后追加。现在把它前置到 Phase 2,是吸取这个教训 —— 把正确的架构一次建对,不要给错的 Transport 留插入点。

### 现状:Renderer 侧已经有两条 transport 了

Broker 的价值不止是"让 Phase 3 不走弯路"—— 它还**同时修复了 v1 / 过渡期 v2 里已经存在的两条并存 transport**。这个现状很多人意识不到,所以单独列出来。
...
| IPC push (`setupChannelStream`) | 删除,被 `WebContentsSink` 通用实现取代 |
| `addChannelUserMessage`(~50 行) | 删除,用户消息由 Main 端 `PersistenceSink` 落库 + DataApi 广播 |
| `ChannelStreamController` 类型 | 删除,Broker 的 `StreamSink` 接口取代 |
| API server 的 `/v1/agents/:id/sessions/:id/messages` SSE endpoint | Renderer 停止调用。保留为公共 API(给外部客户端)还是删除,由产品决定 |

估算 `messageThunk.ts` 里 **~500 行 Renderer 侧 transport + streaming 胶水代码**可以随 Broker 落地一起删掉;API server 侧的 SSE route 也可以简化或停用。这部分节省在"为什么值得做"小节里被列为独立收益。
...
Phase 2 的核心设计就是把 Main 端从"一次性管线"升级成"按 `requestId` 索引的活跃流注册表"(即 `AiStreamBroker`,`requestId` 是一次生成尝试的身份,`topicId` 保留作为会话维度 + steering/多窗口 attach 的反查键),让 Transport 的 `reconnectToStream` 从一开始就是真的。一旦可用,上面三个 Renderer 侧的问题全部自然消失。
...
└────────────────────────────────────────────────┘
                     ↓ 流完成
        PersistenceSink.onDone → SQLite (DataApi)
```
...
| id | 平面 | 用途 |
|---|---|---|
| `requestId` | 控制平面 | Broker `activeStreams` 主表 key、`abort/detach/attach(byRequestId)` 精确路由、in-memory 去重键、一次生成尝试的身份 |
| `topicId` | 数据平面 | Sink.id 构造(`wc:X:topicId`, `persistence:topicId`,配合 steering 的 upsert 语义)、push payload 的过滤键、useChat `{ id }` 状态复用 key、DataApi 关联键、`attach(byTopicId)` 观察者模式 |

详见 [`requestId / topicId 命名空间与并发约束`](#requestid--topicid-命名空间与并发约束) 小节。

**核心抽象**:
- **`StreamSink`**:任何想消费 chunks 的东西都实现这个极窄接口。`WebContentsSink`(推给 Renderer)、`PersistenceSink`(写 SQLite)、`ChannelAdapterSink`(推回 Discord/飞书等)是三个内置实现
- **`BrokerStreamTarget`**:伪装成 `WebContents` 的虚拟 target,实现 `send` + `isDestroyed` 两个方法,加上可选的 `setFinalMessage`。传给 `AiService.executeStream`,让它误以为在推给一个真 WebContents;内部把每个 chunk 记入 buffer、多播给所有 sinks;上游(agentLoop / ClaudeCodeStreamAdapter)在流结束前通过 `setFinalMessage` 塞进 AI SDK 工具产出的完整 UIMessage。`AiService` 主体方法对这些近乎无感,只在 `hooks.afterIteration` 里多一次 `target.setFinalMessage?.()` 调用

### 核心数据结构

```ts
// src/main/ai/stream-broker/types.ts

/**
 * Broker 传给 Sink 的终态 —— 区分"正常完成"vs"被用户中止"的半成品,
 * 保留 v1 ChatSession.handleFinish 的 success / paused 语义。
 *
 * 定义在 types.ts 而不是 PersistenceSink.ts,是因为它是 `StreamSink.onDone`
 * 的签名组成部分,所有 sink 实现都要引用它,不是 PersistenceSink 专属。
 */
 ...
 * `setFinalMessage` 是 Broker 专用的可选扩展 —— 上游(agentLoop 或
 * ClaudeCodeStreamAdapter)在流结束前把 AI SDK 工具函数产出的完整 UIMessage
 * 通过这个 setter 传下来,避免 Broker 自己 rebuild。
 *
 * 定义在 types.ts 是因为:
 *   1. `AiService.executeStream` 的签名就用它作为参数类型(见"对 AiService
 *      的最小侵入清单"小节),是 Phase 2 分层的关键接口,不是 BrokerStreamTarget
 *      私有
 *   2. 理论上任何"流的出口"(比如 unit test 里的 MockStreamTarget)都可以实现它,
 *      放在类型文件里让测试代码不用 import BrokerStreamTarget
 */
...
/**
 * 流的消费者抽象。Broker 不区分 sink 的种类,通过四个方法统一调度。
 * sink 的任何内部状态/分类都封装在实现内部,不向外暴露。
 */
export interface StreamSink {
  /**
   * 稳定唯一标识,用于:
   *   - sinks 集合去重(同一个订阅者不重复加入)
   *   - detach 时精确定位
   *   - 日志 trace
   * 实现者负责保证唯一。Broker 不解析它的内容。
   */
  readonly id: string

  onChunk(chunk: UIMessageChunk): void
  /**
   * 流结束(**包含 success 和 paused 两种终态**)。Broker 传入一个终态对象:
   *
   *   - `result.finalMessage` 由上游(agentLoop.afterIteration hook 或
   *     ClaudeCodeStreamAdapter 的 complete / abort 分支)通过
   *     `BrokerStreamTarget.setFinalMessage` 在 onDone 之前塞进 `stream.finalMessage`,
   *     由 Broker 原样转发给每个 sink。可能为 `undefined`(上游未提供或异常路径);
   *     sink 自己决定 undefined 时如何降级。
   *   - `result.status` 区分"自然结束"('success')和"用户中止时的半成品"('paused'),
   *     对应 v1 `ChatSession.handleFinish:114-127` 的 success/paused 分支。
   *     Broker 根据"终止路径是自然 agentLoop 兑现 还是 abortController 触发"决定;
   *     Sink 可以用这个字段决定是否执行 post-done 副作用(rename / metering 等)。
   *
   * 为什么是对象而不是多参数:新增终态字段时不用改所有 sink 的签名,只加字段即可。
   */
  onDone(result: StreamDoneResult): void | Promise<void>
  onError(error: SerializedError): void | Promise<void>

  /**
   * Sink 的**存活检查**(liveness),不是"生命周期阶段"。
   *
   * Broker 在多播 chunks 时调用 isAlive() 判断该 sink 是否还应该收事件:
   *   - `false` → Broker 立即从 sinks map 里剔除它,不再收到任何后续回调
   *   - `true`  → 继续接收 onChunk/onDone/onError
   *
   * 各 sink 的存活条件:
   *   - `WebContentsSink`  = `!wc.isDestroyed()`(窗口未销毁)
   *   - `ChannelAdapterSink` = `adapter.connected`(IM adapter 未断线)
   *   - `PersistenceSink`  = `true`(永远,直到流结束被 Broker 显式删除)
   *
   * ⚠️ 这和文档里其他两个"生命周期"是**不同层次**的概念,不要混淆:
   *   1. **BaseService 生命周期**(`@Injectable` + `onInit/onStop`)是 DI 容器
   *      管理的**服务**级别生存期,per app 一次。AiStreamBroker 本身是一个
   *      BaseService,但 Broker 里的单个 Sink 不是。
   ...
   *      在 agentLoop 执行到某一步时做事。Sink 完全不参与这个时序 ——
   *      Sink 只接收流 **事件**(chunk/done/error),不接收执行 **阶段**。
   *
   * 简言之:
   *   - BaseService 生命周期 = "这个服务实例什么时候活着"(coarse,per app)
   *   - agentLoop 执行钩子 = "这次流执行到哪一步了"(fine,per iteration)
   *   - Sink `isAlive()` = "这个 observer 还有效吗"(瞬时 boolean,per check)
   */
  isAlive(): boolean
}
...
export interface ActiveStream {
  /**
   * Broker 控制平面的**主身份** —— 一次生成尝试的唯一 id,由 Renderer 在用户点
   ...
   *   - Sink 的生命周期被压进"整个 topic"的时间轴,per-turn 的副作用会跨轮串台
   * 所以控制平面(`abort`/`attach`/`done`/Sink id)必须按 `requestId` 路由,
  ...
  /** Broker 拥有的 AbortController,独立于 Renderer 生命周期。
   *  Broker 直接持有,通过 abortController.abort() 掐流。 */
  abortController: AbortController

  /** 所有消费者。key = sink.id。Broker 不关心 value 的具体类型。 */
   ...
   * 流完成后的完整 UIMessage。
   *
   * **由上游(agentLoop / ClaudeCodeStreamAdapter)通过 `BrokerStreamTarget.setFinalMessage()`
   * 在流结束前塞进来**,Broker 自己不 rebuild —— 直接复用 AI SDK 的
   * `readUIMessageStream` 等工具函数产出,避免重造 chunk 状态机。
   *
   * 可能为 `undefined`(上游没提供 / 流异常中断 / 旧路径直接调 executeStream
   * 不走 agentLoop 的 afterIteration hook)。sink 自己决定 undefined 时怎么降级。
   */
  finalMessage?: CherryUIMessage
   ...
   *
   * ⚠️ 这是 backend-owned metadata,不是"流的类型 discriminator"。Broker
...
**关键约束(写进 `AiStreamBroker` 注释作为不变量)**:

1. Broker 不得通过 `instanceof`、`id` 前缀解析、或任何其他方式判别 sink 的具体类型。所有分派都通过 `StreamSink` 的四个方法完成
2. 任何针对特定种类 sink 的行为都必须封装在该 sink 的实现内部。若 Broker 需要新的协调能力,扩展 `StreamSink` 接口,不要绕过去
3. Sinks 相互独立:单个 sink 抛异常或死亡不得影响其他 sink 或流本身
4. `sink.id` 唯一性由实现者负责,Broker 把重复 id 当作 upsert(新替换旧)
5. `PersistenceSink` 是否挂载是 `startStream` 的前置条件,由调用方决定;Broker 不做"没挂就兜底挂一个"这种隐式行为 —— 有些场景(playground / debug 试跑)就是故意不落库

### BrokerStreamTarget:伪装的 WebContents

`StreamTarget` interface 已经在 `types.ts` 里定义(见上方"核心数据结构"小节)。这里是它的 Broker 专用实现 —— 一个"伪装成 WebContents 的出口",让 `AiService.executeStream` 不用知道它在跟 Broker 说话。

```ts
// src/main/ai/stream-broker/BrokerStreamTarget.ts
...
export class BrokerStreamTarget implements StreamTarget {
   ...
   * 的流"的身份就是 Broker 控制平面的 `requestId`。上游 agentLoop / ClaudeCodeStream
   * 产生的 chunk / done / error 事件都要精确回到这条流,不能按 topicId 路由
   * (否则在同一 topic 的前一轮流刚结束、下一轮立刻开始的场景下,迟到的事件会
   * 挂错流)。
   */
  constructor(
    private readonly broker: AiStreamBroker,
   ...
   *   4. AiService 调 send(Ai_StreamDone) → broker.onDone → 分发到 sinks(此时 stream.finalMessage 已经就位)
   *
   ...
   * 然后 setFinalMessage → broker.onDone。
   */
  setFinalMessage(message: CherryUIMessage): void {
    // @ts-expect-error 访问 Broker 内部 map,或在 Broker 上暴露 setter 方法
    const stream = this.broker.activeStreams.get(this.requestId)
    if (stream) stream.finalMessage = message
  }
}
```

> **BrokerStreamTarget 的两个职责**:
> 1. **收 chunks**(通过 `send` 的 `.chunk` / `.error` 字段)—— 实时多播给 sinks
> 2. **收最终 UIMessage**(通过 `setFinalMessage`)—— 由上游从 AI SDK 工具拿现成的,Broker 自己不造轮子
>
...
```ts
// src/main/ai/stream-broker/sinks/WebContentsSink.ts
...
export class WebContentsSink implements StreamSink {
   ...
   * 同一个 topic 的 steering 路径下,Renderer 连续发 msg1/msg2 时,Broker 会把
   * msg2 的 sinks 追加到 msg1 的 ActiveStream.sinks Map 里。如果 WebContentsSink
   * 的 id 按 requestId 构造,msg1 的 `wc:X:R1` 和 msg2 的 `wc:X:R2` 会**同时存在**,
   * onChunk 会往同一个 WebContents **双发**每条 chunk,UI 上重影。用 topicId
   * 作为 id 可以让第二个 sink 通过 upsert **替换**第一个,只保留一条订阅。
   *
   * 跨轮次隔离不需要 sink id 保证 —— 两轮对应两个不同的 ActiveStream 实例,
   * 各自拥有独立的 sinks Map,从来不会同时出现在同一个 Map 里。
   */
  readonly id: string
  ...
  onDone(result: StreamDoneResult): void {
    // WebContentsSink 不关心 finalMessage —— Renderer 侧的 useChat 自己会从 chunks 重建 UIMessage。
...
// 所以 PersistenceSink 直接调用 Main 端真正的 `MessageService` singleton。
import { messageService } from '@main/data/services/MessageService'
import type { StreamSink, StreamDoneResult } from '../types'

export interface PersistenceSinkOptions {
  /** 一次生成尝试的身份 —— **仅用于日志/trace**,不参与 sink.id 构造。
   *  sink.id 按 topicId 构造(见 constructor 注释),这里留一个 requestId 字段是
   ...
   *  也保证 PersistenceSink 拿到的 parent 是**真实存在于 SQLite 的** message id。 */
   ...
   * 按场景构造闭包传入。PersistenceSink 本身不 care 里面做什么。
   *
   * 失败不影响落库结果 —— PersistenceSink 内部会 try/catch 包一层,只 warn 不 throw。
   */
...
export class PersistenceSink implements StreamSink {
  ...
  constructor(private readonly ctx: PersistenceSinkOptions) {
    ...
    // 追加",新来的 PersistenceSink 必须 **upsert 替换**旧的(更新 parentUserMessageId
    // 为最新一条 steered user message),才能保证 onDone 时只写一条 assistant 且
    // parent 正确。如果用 requestId 作为 id,两次的 PersistenceSink 会**共存**在
    // 同一个 ActiveStream.sinks Map 里,onDone 触发两次,导致同一次 AI 回复被
    // 复制成两条 DB 记录(parent 一条指 U1 一条指 U2),破坏 v1 的"steering = 线性链"
    // 语义(U1 → U2 → A,只有一条 A)。
    //
    // 跨轮次隔离不需要靠 sink id 保证 —— 两轮对应两个不同的 ActiveStream 实例,
    // 各自拥有独立的 sinks Map,从来不会同时出现在同一个 Map 里。
  ...
  async onDone(result: StreamDoneResult): Promise<void> {
    ...
    //    注意 try/catch —— hook 的失败不影响落库结果,也不阻断 Broker.onDone 对
    //    其他 sink 的分发(Broker 的 sink loop 本身也有 try/catch,这里是双保险)。
...
> **为什么 PersistenceSink 不新建一个 `MessagePersistenceService`**:Main 端真正的持久化入口就是 `messageService`(`src/main/data/services/MessageService.ts` 的 named export singleton)—— 它已经是 DataApi 后面的 SQLite 写入实现,自带事务 + parent 校验 + activeNodeId 维护。PersistenceSink 直接调它即可,再造一个 `MessagePersistenceService.saveExchange()` 只是无意义的间接(v1 `ChatSession.handleFinish` 走的是 Renderer 端 `dataApiService.post` HTTP 客户端,Phase 2 之后持久化改在 Main 端,自然用 Main 端的同一个 service singleton)。整个 PersistenceSink 文件 < 60 行。
>
> **user message 为什么不是 PersistenceSink 的职责(但也不能让 Renderer 自己落)**:Renderer 端 `streamingService.createUserMessage`(`messageThunk.ts:1068`)有一个已知竞态:它不传 `parentId`,依赖 `topic.activeNodeId` 做自动解析,多窗口同时操作同一 topic 时容易挂错分支。Phase 2 之后的策略是:
>
...
> - **Main 端 `handleStreamRequest` 在一个事务内完成"落 user message → 起流 → 构造 PersistenceSink(parentUserMessageId = 刚落的 user.id)"**,整个过程显式 `parentId`,完全不碰 `topic.activeNodeId` 自动解析
> - PersistenceSink 仍然只管 assistant message,但它拿到的 `parentUserMessageId` 是 Main 端刚刚原子落库完的真实 id,不再信任 Renderer 传来的任何外键
>
> ChannelMessageHandler / Agent scheduler 路径同理:由它们自己在 Main 端先调 `messageService.create` 落 user message 再构造 PersistenceSink。
>
...
```ts
// src/main/ai/stream-broker/sinks/ChannelAdapterSink.ts
...
```ts
// src/main/ai/stream-broker/AiStreamBroker.ts
...
export class AiStreamBroker extends BaseService {
  /**
   * 主注册表 —— **按 requestId 索引**。这是 Broker 控制平面的权威 Map。
   ...
   *
   * 用途:
   *   1. **Steering**:用户在流进行中继续发消息,Broker 需要找到"这个 topic
  ...
  protected async onInit(): Promise<void> {
    ...
    // WebContents 销毁时自动清理它挂载的所有 sinks
  ...
  /**
   * 启动一条新流。调用方负责构造所有初始 sinks —— 持久化不是隐式兜底。
   *
   * 语义:
  ...
  startStream(input: {
    ...
    if (existingByRequest) {
      // 把本次调用想挂的新 sinks 追加上去(重试场景下 Renderer 的 WebContents
      // 可能换了,需要让新 WebContentsSink 接上,但 PersistenceSink 由第一次
      // 调用已挂,这里 sink.id 去重会自动吞掉重复)
      for (const sink of input.sinks) this.addSinkByRequestId(input.requestId, sink)
    ...
    const stream: ActiveStream = {
      ...
      sinks: new Map(input.sinks.map((s) => [s.id, s])),
    ...
    const target = new BrokerStreamTarget(this, input.requestId)
    const aiService = application.get('AiService')

    // 把 Broker 拥有的 signal + pendingMessages 传进去 ——
   ...
   *   - 该 topic 已有 streaming 流 → `steer()` + 把发起者的 sinks 追加到**已有那条流**上
  ...
  send(input: {
    ...
    sinks: StreamSink[]           // 本次发起者的 sinks(通常是 WebContentsSink + PersistenceSink)
    ...
    if (activeRequestId) {
      ...
      if (existing?.status === 'streaming') {
        // Steering 路径:把消息入队,把发起者的 sinks 挂到已有流上
        existing.pendingMessages.push(input.userMessage)
        for (const sink of input.sinks) this.addSinkByRequestId(activeRequestId, sink)
  ...
  /** 按 requestId 加入一个新的 sink。返回 true 表示已加入;false 表示流不存在。 */
  addSinkByRequestId(requestId: string, sink: StreamSink): boolean {
    const stream = this.activeStreams.get(requestId)
    if (!stream) return false
    stream.sinks.set(sink.id, sink)  // upsert:同 id 覆盖
    // 回放 buffer 给这个新 sink
    for (const chunk of stream.buffer) sink.onChunk(chunk)
    return true
  }

  removeSinkByRequestId(requestId: string, sinkId: string): void {
    const stream = this.activeStreams.get(requestId)
  ...
  // === BrokerStreamTarget 的回调入口(全部按 requestId) ===
   ...
   *   - 被中止:Broker 自己的 `abort(requestId)` 内部,在 abort 之后给 sinks 发一次
  ...
  async onDone(requestId: string, status: 'success' | 'paused' = 'success'): Promise<void> {
    ...
    // 注意:stream.finalMessage 应该已经被上游通过 BrokerStreamTarget.setFinalMessage
    // 在 onDone 之前塞好了(agentLoop.afterIteration hook / ClaudeCodeStreamAdapter 的
    // 'complete'/'cancelled' 回调)。Broker 自己不做 rebuild。
    // 如果 finalMessage 仍是 undefined,说明上游没有提供 —— sink 自己按 undefined 处理。
   ...
   * `PersistenceSink.onDone` 已经把消息落库,数据不会丢,只是走 DB 路径一次。
   *
  ...
  /** BrokerStreamTarget 用它决定 isDestroyed 返回什么 */
   ...
   *   2. 构造 WebContentsSink(event.sender, requestId) + PersistenceSink(parentUserMessageId = 步骤 1 的 id)
   *   3. 调 `this.send({ requestId, topicId, request, userMessage, sinks })`
   ...
   *   - **步骤 1 必须同步 await 完成**,因为步骤 2 的 PersistenceSink 依赖它的返回 id
   *   - 步骤 1 失败要把错误直接返回给 Renderer,不继续起流
   *   - ChannelMessageHandler / Agent scheduler 路径走自己的入口,在各自的 handler 里
   *     同样先调 `messageService.create` 落 user-side 消息再构造 PersistenceSink,并自己
  ...
  private async handleStreamRequest(
    ...
    if (this.activeStreams.has(req.requestId)) {
      ...
      // 把本次发起者的 WebContentsSink 挂上去(可能是 Renderer 重连场景)。
      // 注意 sink id 按 topicId 构造,addSink 的 upsert 语义会吞掉重复订阅。
      this.addSinkByRequestId(req.requestId, new WebContentsSink(sender, req.topicId))
    ...
    // 步骤 2:构造 sinks —— id 都按 topicId 构造(见 sink 实现的注释)。
    // requestId 只在 PersistenceSinkOptions 里作为日志/trace 字段传入,不参与 id。
    ...
    const webContentsSink = new WebContentsSink(sender, req.topicId)

    // 步骤 3:路由给 Broker(内部会判断是 startStream 还是 steer)
    const result = this.send({
      requestId: req.requestId,
      topicId: req.topicId,
      request: req,
      userMessage,
      sinks: [webContentsSink, persistenceSink]
    })
   ...
   *     topicId(useQuery 读出来的树),不知道原始 requestId。Broker 用
   *     topicToActiveRequest 反查当前活跃 request,挂 WebContentsSink 上去
   *
  ...
  private handleAttach(
    ...
    // streaming 中:挂 sink + 回放 buffer。sink id 按 topicId 构造,不按 requestId。
    this.addSinkByRequestId(requestId, new WebContentsSink(sender, stream.topicId))
  ...
  private handleDetach(sender: WebContents, req: AiStreamDetachRequest): void {
    // Detach:按 requestId 找到 ActiveStream,按 (wc.id, topicId) 构造 sink id 移除。
    // requestId 是控制平面路由键(告诉 Broker 去哪个 Map 里找),topicId 是 sink 身份键。
    const stream = this.activeStreams.get(req.requestId)
    if (!stream) return
    this.removeSinkByRequestId(req.requestId, `wc:${sender.id}:${stream.topicId}`)
  }

  private cleanupByWebContents(wc: WebContents): void {
    // 遍历 activeStreams,删掉 id 前缀为 `wc:${wc.id}:` 的所有 sink
    const prefix = `wc:${wc.id}:`
    for (const stream of this.activeStreams.values()) {
      for (const sinkId of stream.sinks.keys()) {
        if (sinkId.startsWith(prefix)) stream.sinks.delete(sinkId)
      }
    }
  }
}
...
3. **`executeStream` 注册 `afterIteration` hook 捕获 UIMessage**:这是 Phase 2 新增的责任 —— 在 `runAgentLoop` 的 hooks 里挂一个 `afterIteration(ctx, result)` 回调,从 `result.uiMessage` 拿到 AI SDK 工具产出的完整 UIMessage,调 `target.setFinalMessage?.(result.uiMessage)` 塞给 Broker。见下方 "`agentLoop` 的配套修改" 小节
4. **把 AiCompletionService 的两个 per-requestId 注册表上移到 Broker 层**(注意:requestId 这个**概念**没删,只是管理方从 AiCompletionService 换成 Broker):
   - `AiCompletionService.registerRequest / removeRequest / abort(requestId)` —— 删掉,Broker 在 `activeStreams: Map<requestId, ActiveStream>` 主表里直接持有每条流的 `AbortController`,abort 走 `Broker.abort(requestId)`(`Ai_Stream_Abort` IPC 直接路由到这里)
   - `AiCompletionService.pendingMessageQueues: Map<requestId, PendingMessageQueue>` 和 `getPendingMessageQueue / steer(requestId, message)` —— 删掉,队列的归属从"按 requestId 索引的全局 Map"改为"`ActiveStream.pendingMessages` 字段",随流 GC(steering 消息由 Broker 根据 topicId 反查当前活跃 requestId,再推进对应的队列)
   - 对应的 `ipcOn(Ai_Abort)` 和 `ipcOn(Ai_SteerMessage)` 挂接都删掉,Renderer 的 abort 走 `Ai_Stream_Abort({ requestId })`,steer 走 `Ai_Stream_Open`(Broker 按 topicId 发现当前有活跃流则路由到 steer,忽略 Renderer 传的 requestId)
   - **executeStream 本身不再认识 requestId** —— 它只收 `target: StreamTarget` + `signal: AbortSignal`,`target` 实际上是 `BrokerStreamTarget(broker, requestId)`,requestId 被**封装在 target 闭包里**,executeStream 完全看不到。这是关键的分层:AiCompletionService 是"一次 AI 调用的库函数",不关心"这次调用是谁的哪一轮";Broker 才是那个管"谁的哪一轮"的控制平面
5. **删除 `Ai_StreamRequest` / `Ai_Abort` / `Ai_SteerMessage` 三个 `ipcHandle` / `ipcOn` 注册**。Renderer 全部走 `Ai_Stream_*`,由 AiStreamBroker 接管
...
将来如果把"流执行"从 `AiService` 里剥离出来(比如合并到 Broker 内部),这个 `StreamTarget` 接口也是清晰的切分点。

**顺带收益**:`AiService.ts` 会从 ~140 行瘦到 ~100 行左右,因为一次性管线管理 + per-call registry 是它内部最啰嗦的一部分。瘦身之后 `AiService` 的职责变得非常纯粹:**一个 lifecycle-managed 服务,封装 AiCompletionService 的各种非流 API,流由 Broker 调用 `executeStream` 作为库函数使用**。
...
export interface IterationResult {
  ...
  uiMessage: CherryUIMessage       // ← 新增:给持久化/UI/Broker 用,由 AI SDK 工具产出
  usage: LanguageModelUsage
  finishReason: string
...
**实现提示**:AI SDK v5 里获取完整 UIMessage 的最直接方式是对 `result.toUIMessageStream()` 的输出做一次 `tee()`,一路给 Broker 实时流转,另一路喂给 `readUIMessageStream`(或同名 utility)消费到最后一条 message。tee 是 Web Streams 原生 API,零额外状态机代价。具体 import 看 `@ai-sdk/ui-utils` 或 `ai` 包当前版本,实现时查一下即可。
...
const stream = runAgentLoop(params, request.messages, options?.signal, {
  hooks: {
    ...
    afterIteration: async (ctx, result) => {
      // 关键:把 AI SDK 产出的 UIMessage 通过 target 传给 Broker
...
**Adapter 的职责**:订阅 `'data'` 事件,把 chunks 累积成 UIMessage,在 `complete` 触发前用 AI SDK 工具(同一个 `readUIMessageStream` 或等价的)rebuild,塞给 Broker:

```ts
// src/main/ai/stream-broker/adapters/ClaudeCodeStreamAdapter.ts

export async function startClaudeCodeStreamViaBroker(params: {
  topicId: string
  ...
  // 1. Claude SDK 启动(用 Broker 传进来的 AbortController)
  ...
  // 3. 分两路消费
  const [forBroker, forReconstruct] = chunkStream.readable.tee()

  //    3a. 一路: 逐 chunk 送给 Broker 做实时多播
  void (async () => {
    const reader = forBroker.getReader()
    while (true) {
      const { done, value } = await reader.read()
      if (done) break
...
**两条路径调用的 AI SDK 工具是同一个**,产出的 UIMessage 格式也一致 —— Broker 对两边完全透明,不需要知道上游是 agentLoop 还是 ClaudeCodeStreamAdapter。
...
enum IpcChannel {
  ...
  // === Phase 2 新增(Broker 拥有) ===
  Ai_Stream_Open   = 'ai:stream:open',   // Renderer → Main:发送消息(Broker 按 topicId 判断开新流 or steer)
  Ai_Stream_Attach = 'ai:stream:attach', // Renderer → Main:reconnect(支持 byRequestId / byTopicId 两种模式)
 ...
 *     topicId,Broker 用 `topicToActiveRequest` 反查当前活跃 request
 */
...
interface AiStreamDetachRequest {
  ...
  requestId: string
  // 不需要 subscriber id —— event.sender 就是身份,Broker 按
  // `wc:${event.sender.id}:${requestId}` 精确定位这个窗口对该 request 的 sink
}
...
// === Push payloads (Main → Renderer, 由 WebContentsSink 发出) ===
// 所有 push **只带 topicId,不带 requestId** —— 数据平面按 topicId 聚合,
// 原因见 WebContentsSink 注释。控制平面的 requestId 只存在于 request/response
...
export class IpcChatTransport implements ChatTransport<UIMessage> {
  async sendMessages(options) {
    ...
    // 路径下同一 topic 的多次发送会被 Broker 合并到一条 ActiveStream,chunks 属于
    // 同一条流,只能按 topicId 聚合。
   ...
   *     让 Broker 反查当前活跃 request 并 attach
   *
...
- **数据平面(chunk 推送 + listener 过滤)** = `topicId`。理由见上方 steering 解释:msg1 和 msg2 在 Broker 里合并成一条流,chunks 属于同一个 WebContentsSink(upsert 去重后只有一个),只能按 topicId 聚合,过滤不能按 requestId
...
Cherry Studio 有至少三种流发起场景,Broker 都要统一支持 —— 但 **Broker 本身不区分它们**,所有差异通过"挂什么 sinks"表达:

| 发起场景 | Sinks 组合 | 备注 |
|---|---|---|
| Renderer user 发起 | `WebContentsSink` + `PersistenceSink` | 标准对话 |
| Channel incoming(纯 bot 回复) | `ChannelAdapterSink` + `PersistenceSink` | Discord/Slack/飞书 |
| Channel incoming + 用户同时在看 | `ChannelAdapterSink` + `PersistenceSink` + `WebContentsSink` | bot 回复且 debug 面板开着 |
| Agent scheduler(定时任务) | `PersistenceSink`(+ 可选 `WebContentsSink`) | 后台自主 agent |
...
async handleIncoming(adapter: ChannelAdapter, msg: IncomingMessage) {
  ...
  // 作为 Broker 主表 key + in-memory 去重键。
  const requestId = crypto.randomUUID()

  // 在这之前先调 `messageService.create` 落 user-side 消息(用 channel 平台消息
  // 转成 CherryUIMessage 的 parts),拿到真实的 id 后才能构造 PersistenceSink。
  ...
  aiStreamBroker.startStream({
    requestId,
    topicId,
    request,
    sinks: [
      new ChannelAdapterSink(adapter, msg.chatId),   // adapter + platformChatId 在 sink 里固化
      new PersistenceSink({
        requestId,
        topicId,
        assistantId,
...
> **发起源信息在哪**:注意上面的调用**没有** `source` / `kind` 字段。Broker 从 sinks 列表就能完整地知道它要多播给谁、向谁落库、向谁推回 bot 回复。"谁发起的"这个问题对 Broker 没有意义 —— 它只关心"chunks 往哪送"。调用方如果想打日志说明是 channel 触发的,自己在调用前 `logger.info('starting stream', { topicId, from: 'channel', channelId: adapter.channelId })` 就行,不污染 Broker 数据结构。

> **注意命名空间**:`msg.chatId` 是 Discord/Slack/飞书等 IM 平台的会话 id(例如 Discord channel id),**不是** Broker 的 key。Broker 的 key 始终是 Cherry Studio 的 `topicId`,由 `resolveTopicId(adapter, msg)` 从 `(channelId, platformChatId)` 映射得到。两者同名容易混淆,留意类型和来源。

**顺带获得的能力**:Renderer 用户打开对应 topic 时,`Ai_Stream_Attach` 会把新的 `WebContentsSink` 加入这条由 channel 发起的流 —— **用户可以实时看到 bot 正在回复什么**。这是 v1/v2 现状都没有的能力,在新架构下零成本。

### requestId / topicId 命名空间与并发约束

Broker 里有**两个 id**,分工明确,不要混用:
...
| `requestId` | Renderer 在用户点发送时生成 UUID | **控制平面身份**:Broker 的 `activeStreams` 主表 key、`abort/attach/detach` 路由 key、Sink.id 前缀、in-memory 去重键、IPC chunks listener 过滤键 | 一次生成尝试(start → done/error/aborted + grace period) |
...
**并发约束**:同一 `topicId` 在 Broker 里同时只能有一条活跃 request(由 `topicToActiveRequest: Map<topicId, requestId>` 反查索引强制)。但"一条流"不等于"一次发送" —— 用户在流跑的时候继续发消息是合法的,Broker 的 `send()` 路由会检测到当前 topic 已有活跃 request,把新消息的 `requestId` **忽略**,把消息 push 到现有 `ActiveStream.pendingMessages` 队列,由 `runAgentLoop.prepareStep` 在内层步间 drain 拼进 context(Phase 1 设计原则里的 "Pending Messages Steering 双层保障")。

所以:
- **同一 topic 同时一条活跃 request** —— 这是对 Broker 的约束,对用户透明
...
`startStream` 遇到 `topicToActiveRequest` 指向 grace period 内(done/error/aborted)的旧 request 时,会**提前驱逐**旧 request,让新 request 立即开始 —— 这修复了"停止后立即重试"/"出错后立即重发"/"完成后秒速连发"三种用户动作的顺滑度。迟到 reconnect 的代价:按旧 requestId attach 会拿到 'not-found',Renderer 退化到 `useQuery('/topics/:id/messages')` 从 DB 读,由 `PersistenceSink` 已落库的数据兜底。
...
| ClaudeCodeStream 事件 | Broker 调用 |
|---|---|
| `{ type: 'chunk', chunk }` (`index.ts:926-932`) | `broker.onChunk(topicId, toUIMessageChunk(chunk))` |
| `{ type: 'complete' }` (`:952`) | `broker.onDone(topicId)` |
| `{ type: 'error', error }` (`:983`) | `broker.onError(topicId, serializeError(error))` |
| `{ type: 'cancelled', error }` (`:968`) | `broker.onError(topicId, ...)` 或 `broker.abort(topicId)` |
...
export async function startClaudeCodeStreamViaBroker(params: {
  ...
  // 1. 让 Broker 先注册 ActiveStream(带上 sinks + 自己的 AbortController)
  const stream = broker.registerEmpty({
  ...
  // 3. 调 ClaudeCodeService,传入 Broker 的 AbortController
  const claudeStream = await claudeCodeService.invoke(
    params.prompt,
    params.session,
  ...
  // 4. 事件转发 —— Broker 负责多播给所有 sinks
  claudeStream.on('data', (evt) => {
    if (evt.type === 'chunk') {
      broker.onChunk(params.topicId, toUIMessageChunk(evt.chunk))
    } else if (evt.type === 'complete') {
      // init 消息里捕获的 sdkSessionId 已经写进了 stream.sourceSessionId
      void broker.onDone(params.topicId)
    } else if (evt.type === 'error' || evt.type === 'cancelled') {
      void broker.onError(params.topicId, serializeError(evt.error ?? new Error('unknown')))
    }
  })
...
`ClaudeCodeStream` 的 `processSDKQuery` 在收到 SDK init 消息时已经把 `session_id` 存到 `stream.sdkSessionId`(`index.ts:864`)。Adapter 需要监听这个点,同步写到 `ActiveStream.sourceSessionId`。可以用一个轻量的 `once('sdk-init', ...)` 事件,或在 Broker 的 `registerEmpty` 返回对象上暴露 setter。

**(b) Steering 转发到 `userInputStream.enqueue`**
Broker 的 `steer(topicId, message)` 只是把消息推进 `stream.pendingMessages` 队列。Claude Code 路径的消费方是 `userInputStream`(见 `createUserMessageStream`,`index.ts:647`),需要把 pendingMessages 的新增事件转换成 `SDKUserMessage` push 进去:
...
#### `renameAgentSessionIfNeeded` 作为 `PersistenceSink.afterPersist` hook 注入

`messageThunk.ts:220-279` 的"流完成后用摘要给 session 自动重命名"逻辑,**Phase 2 之后作为 `PersistenceSink` 的 post-persist 钩子注入**,而不是新写一个独立的 Sink 类。

**`PersistenceSink` 加一个 `afterPersist` 可选参数**:
...
export interface PersistenceSinkOptions {
   ...
   * 用途:agent session 自动重命名、usage 上报、知识库回填、telemetry 收尾……
   * 由调用方(handleStreamRequest / ChannelMessageHandler / Agent scheduler)
   * 按场景注入。PersistenceSink 本身不 care hook 里做什么,只负责在落库成功之后
   * try/catch 包一层调用,让 hook 的失败不污染落库结果、不阻断 Broker.onDone 的
   * 其他 sink 分发。
   */
  afterPersist?: (finalMessage: CherryUIMessage) => Promise<void>
}
...
**`handleStreamRequest` 按场景注入 hook**(承接上一节 Broker 里 `handleStreamRequest` 的步骤 2,展开 `buildAfterPersistHook` 的细节):

```ts
private buildAfterPersistHook(
  req: AiStreamOpenRequest,
  userMessage: Message  // 步骤 1 里落库完的真实 user message,带真实 id
): PersistenceSinkOptions['afterPersist'] {
...
**为什么是 hook 参数而不是独立 Sink 类**:

`SessionAutoRenameSink` 如果升级成独立 sink,`StreamSink` 接口的四个方法里只有 `onDone` 是"真的用"的,`onChunk / onError` 是 no-op,`isAlive` 永远 true。**用一个完整的观察者类装一个纯粹的后置副作用,是杀鸡用牛刀**。区分"真正的 Sink"和"post-persist hook"的经验法则(详见 `StreamSink.isAlive` 注释):

| 如果这个消费者…… | 用 Sink 类 | 用 `afterPersist` hook |
|---|---|---|
| 需要 `onChunk` 实时做事 | ✅ | ❌ |
...
**三个内置 Sink(WebContents / Persistence / ChannelAdapter)**每个都在左列勾了**至少一项**,它们是真正的观察者。**Agent 重命名、usage 上报、知识库回填、自动标题生成**这类 post-done 副作用全部落在右列,统一用 `afterPersist` hook。

**多个 post-persist 动作怎么组合**:如果将来 hook 里要做的事 ≥ 3 件,用 `Promise.allSettled` 并发跑多个 helper 函数(**不是**拆成多个 Sink):
...
`Promise.allSettled` 天然提供 error isolation —— 每个 helper 自己的失败不影响其他。PersistenceSink 的 try/catch 只需要在最外层包一层 warn,就覆盖所有情况。
...
>
> **正确分层**是把"所有 assistant 消息都应该跑的副作用"从 Sink 层抽出来,走 outbox + worker 模式:
> - `PersistenceSink.onDone` 在同事务里往 outbox 表插一条 `message_persisted` domain event
...
2. **`BrokerStreamTarget` 是两条后端路径唯一的汇合点**。任何"后端无关的 post-done 副作用"只能挂在 Sink 层才能被两条路径都触发。塞进 agentLoop hook 会错过 ClaudeCodeStreamAdapter 路径
...
4. **次序被破坏**。agentLoop 的 `afterIteration` 在**每次 outer iteration 结束时**触发,一次流的 multi-iteration steering 场景下会触发**多次**;而 rename 是"整条流结束一次"语义。如果硬塞进 afterIteration,rename 会被触发 N 次,每次摘要源都是"当前这一轮的 assistant message",结果就是 session 名字一直变。更糟:`afterIteration` 发生在 **persistence 之前**(persistence 发生在 `Broker.onDone` 里,由 `Ai_StreamDone` 信号触发,时序在 agentLoop 兑现之后),rename 想依赖"消息已落库"这个状态就拿不到,回到"必须 persistence-first"的隐式约束 —— 而 Sink 层的 `afterPersist` 天然就是"落库成功后"语义,次序自然正确

**所以不论这四个理由哪一条,都足以说明"把 post-done 副作用推回 agentLoop"是错的**。正确的分层是:

- **`agentLoop.hooks`** = 执行引擎内部的控制面(AiService 是读者)
- **`PersistenceSink.afterPersist`** = 流完成后的业务副作用(handleStreamRequest 是读者)
- **两个 hook 的时序关系**:agentLoop hooks 全部跑完 → agentLoop 兑现 result → AiService 调 `Broker.onDone(topicId, 'success')` → PersistenceSink.onDone → `messageService.create(…)` → `afterPersist`(只在 success 路径下跑)

一个是"流怎么跑",一个是"流跑完后业务上还要做什么",这两个层次永远不要糅在一起。

#### Phase 6 落地后 adapter 怎么处理

当 Phase 6 (`ClaudeCodeService → 统一 ToolLoopAgent`) 完成后,agent 流和普通 chat 都走 `AiCompletionService.streamText` 一条路径,`ClaudeCodeStreamAdapter` 整个文件变成死代码,一并删除。Broker 主体零改动 —— 这就是 adapter 模式的意义,把 backend 迁移的影响隔离在一个文件里。
...
**Step 2.3 · 基础设施 (1-2 天)**
- [ ] 新建 `src/main/ai/stream-broker/` 目录
- [ ] 定义 `StreamSink` / `StreamTarget` / `ActiveStream` / `AiStreamBrokerConfig` 类型(**不定义 `StreamSource`** —— Broker 不区分流的发起源)
- [ ] `ActiveStream.sourceSessionId` 字段预留给 ClaudeCodeService 路径使用
- [ ] 实现 `BrokerStreamTarget`
- [ ] **不写** `UIMessageAccumulator` —— 最终 UIMessage 由上游 agentLoop / ClaudeCodeStreamAdapter 调 AI SDK 的 `readUIMessageStream`(或等价工具)产出,Broker 只负责存 `stream.finalMessage` 字段
- [ ] `AiStreamBroker` 空壳 + lifecycle 注册,`onInit` 只注册 4 个 channel 但暂不处理
- [ ] `AiService.executeStream` 参数类型放宽为 `StreamTarget`(确保既有调用方零破坏)
- [ ] **不新建** `MessagePersistenceService` —— `PersistenceSink` 直接 `import { messageService } from '@main/data/services/MessageService'` 调 `messageService.create(topicId, { parentId: parentUserMessageId, ... })`。如果 reasoning parts 的 `thinking_millsec → cherry.thinkingMs` 规范化还没在 Phase 1 stream plugin 里完成,抽一个 `normalizeReasoningParts(parts)` util 函数放在 sink 同目录,10 行以内
- [ ] `PendingMessageQueue` 加 `Emitter<'push'>` 事件订阅能力(给 Step 2.4b 的 ClaudeCodeStreamAdapter 用)
- [ ] 单元测试:FakeSink 验证 Broker 的分发逻辑、buffer 回放、isAlive 清理、异常隔离

**Step 2.4 · Renderer 流接入 (2-3 天)**
- [ ] `AiStreamBroker.startStream` / `send` / `steer` / `onChunk` / `onDone` / `onError` / `abort` 完整实现
- [ ] `handleStreamRequest` IPC handler 实现(构造 WebContentsSink + PersistenceSink)
...
- [ ] 新建 `src/main/ai/stream-broker/adapters/ClaudeCodeStreamAdapter.ts`(~30 行,见"ClaudeCodeService 集成"小节)
...
- [ ] `handleAttach` + `addSink` + buffer 回放
- [ ] `IpcChatTransport.reconnectToStream` 真正实现
- [ ] 测试:切 topic → 切回来 → 流继续 → 完成
- [ ] 测试:多窗口同看 topic → chunks 两边同步
- [ ] 测试:WebContents destroyed 时自动从 sinks 剔除

**Step 2.6 · 持久化下沉 & 旧 Renderer 侧路径清理 (2 天)**
- [ ] `AiStreamBroker.onDone(topicId, status)` 通过 `PersistenceSink` 完成落库(`PersistenceSink` 直接调 Main 端 `messageService.create`,无中间 Service),status 由 Broker 根据自然完成 vs abort 路径决定
- [ ] `handleStreamRequest` 在起流之前**原子**落 user message —— 调 `messageService.create(topicId, { role: 'user', parentId: req.parentAnchorId, data })` 拿真实 id,再用这个 id 作为 PersistenceSink 的 `parentUserMessageId`。从源头避开 Renderer `streamingService.createUserMessage` 依赖 `topic.activeNodeId` 的多窗口竞态
- [ ] 搬迁 `messageThunk.ts:220-279` 的 `renameAgentSessionIfNeeded` 逻辑 —— **作为 `PersistenceSink.afterPersist` hook 注入**,`handleStreamRequest` 在识别到 agent session 时构造一个闭包传入。用 `[req.userMessage, finalMessage]` 内存数据生成摘要,不走 DB 读
- [ ] 删除 v1 `messageThunk.ts` 里 `createAgentMessageStream` / `createSSEReadableStream` / `withAbortStreamPart` / `fetchAndProcessAgentResponseImpl` / `setupChannelStream` / `addChannelUserMessage` / `renameAgentSessionIfNeeded` 等函数(~500 行),它们都被 Broker + 三个内置 sink + `afterPersist` hook 统一取代
- [ ] Renderer 侧改用官方 `useChat({ id: topicId, transport, messages: historyFromUseQuery })`
- [ ] 历史消息改走 `useQuery('/topics/:id/messages')`(DataApi)

> **"PersistenceSink 不做业务逻辑磁铁" 原则**:`PersistenceSink.onDone` 的主体方法只管"把消息写 SQLite"一件事,**业务副作用通过 `afterPersist` hook 参数注入**,不直接写进 PersistenceSink 的代码里。将来增加新副作用时:
> - 在 `handleStreamRequest` 构造 PersistenceSink 时,把新动作加到 `afterPersist` 闭包里
> - 如果 hook 里要做的事 ≥ 3 件,用 `Promise.allSettled([maybeRename, maybeMeter, maybeIndex, ...])` 并发跑多个 helper 函数
> - **不改** Broker,**不改** PersistenceSink 主体,**不改** 任何已有 sink
>
> **为什么是 hook 参数而不是独立 Sink 类**:Sink 是"对流有结构性交互"的观察者(需要 `onChunk` / 有独立 `isAlive` / 有独立错误处理),它有四个方法要实现。post-persist 副作用只用 `onDone` 的 finalMessage,其他三个方法都是 no-op —— 用一个完整的观察者类装一个纯粹的后置回调,是杀鸡用牛刀。详见前面 "`renameAgentSessionIfNeeded` 作为 `PersistenceSink.afterPersist` hook 注入" 小节里"Sink vs post-persist hook 的选择经验法则"。
>
...
- [ ] 实现 `ChannelAdapterSink`
- [ ] 改造 `ChannelMessageHandler.handleIncoming` 走 `aiStreamBroker.startStream`
...
| 情况 | 策略 |
|---|---|
| 同一 topicId 流进行中再次 `Ai_Stream_Open`(同一用户追加消息) | **不是冲突**。Broker `send()` 路由到 `steer()` —— 把新消息推进 `stream.pendingMessages`,由 `runAgentLoop.prepareStep` 在下一步边界 drain 并拼进 context。同时把发起者的 sinks 挂上去,继续观察同一条流 |
| 流完成/出错/abort 后进入 grace period,用户立即重试 | **不是冲突**。`startStream` 发现 existing.status ≠ 'streaming' 时调 `evictStream` 提前驱逐旧流,放行新流。代价:迟到的 reconnect 拿不到 finalMessage,退化到 `useQuery` 从 DB 读(`PersistenceSink` 已落库,数据不丢) |
...
| WebContents 意外 crash | `onWebContentsDestroyed` 自动遍历 `activeStreams`,删掉 id 前缀为 `wc:${wc.id}:` 的所有 sink,走 detach 逻辑 |
| 所有 Renderer 全关 + `backgroundMode='continue'` | Broker 继续跑,完成后 `PersistenceSink` 落库,下次启动从 DB 读到结果 |
| 所有 Renderer 全关 + `backgroundMode='abort'` | `BrokerStreamTarget.isDestroyed()` 返回 true,`AiService.executeStream` 循环退出,流被 abort,不落库 |
...
| 多窗口同看同一 topic | 两个窗口各有一个 `WebContentsSink`(id 分别是 `wc:1:topicX` / `wc:2:topicX`),都收到实时 chunks;持久化只在 Main 端做一次 |
| abort 时机:有些 sink 想停、有些不想 | `Ai_Stream_Abort` 是全局 abort(停整条流);单个窗口只退订用 `Ai_Stream_Detach` |
| abort 时的半成品要不要落库 | 取决于上游在 abort 路径下是否调了 `setFinalMessage`。agentLoop 可以在 `onError` / abort 分支里也调一次 AI SDK 工具拿当前部分结果,塞进 stream.finalMessage;PersistenceSink 看到有值就落 `status='paused'`,没值就按当前 `ChatSession.handleFinish:114-127` 的判断降级(有内容落库 / 无内容跳过) |
| Buffer 溢出(超长流) | 超过 `maxBufferChunks` 停止 buffer 但继续多播;迟到 reconnect 只能拿到部分回放 → UI 提示"历史不完整,请从 DB 重新加载" |
| 同一 WebContents 对同一 topicId 重复 Attach | sink id = `wc:${wc.id}:${topicId}` 稳定,`addSink` 自动 upsert(替换旧的),不会重复分发。调用方可以无脑调 Attach,Broker 幂等 |
| Main 自启动时有从 DB 恢复的"未完成"流 | 不做。Broker 只管内存中的活跃流,Main 重启 = 流丢失。Renderer 重连拿不到 → 走 DB fallback |

### AiStreamBroker 测试策略

Broker 的核心逻辑可以用一个最小 FakeSink 覆盖,不需要 mock `WebContents` / `ChannelAdapter` / `DataApi`:
...
- chunk 按顺序分发给所有 sink
- `alive = false` 后会被自动清理
- `onChunk` 抛异常不影响其他 sink 和流本身
- buffer 回放到新加入的 sink(通过 `addSink`)
- `abort` 后不再分发 chunk,`abortController.signal` 被正确触发
- `startStream` 遇到已存在 topicId 抛 `ConflictError`
- grace period 后 `activeStreams` 被清理
- `backgroundMode='abort'` 且 sinks 全清空时 `shouldStopStream` 返回 true
- 重复 `addSink` 同一 sink id 时 upsert 生效(不重复分发)

这正是 `StreamSink` 接口统一的直接收益:**Broker 的测试不需要知道生产里真实的 sink 长什么样**,也不需要 Electron 环境。
...
| 2.3 | `src/main/ai/stream-broker/types.ts` | 新建(`StreamSink` / `StreamTarget` / `ActiveStream` / `AiStreamBrokerConfig` 等接口) |
| 2.3 | `src/main/ai/stream-broker/BrokerStreamTarget.ts` | 新建 |
| 2.3 | `src/main/ai/agentLoop.ts` | 修改(`IterationResult` 加 `uiMessage` 字段 + 在 Promise.all 之后调 AI SDK 工具产出 UIMessage) |
| 2.3 | `src/main/ai/stream-broker/AiStreamBroker.ts` | 新建(lifecycle 服务 + 4 个 IPC handler 骨架) |
| 2.3 | `src/main/ai/stream-broker/sinks/WebContentsSink.ts` | 新建 |
| 2.3 | `src/main/ai/stream-broker/sinks/PersistenceSink.ts` | 新建 |
| 2.3 | `src/main/ai/AiService.ts` | 修改(`executeStream` target 类型放宽 + 新增 options 参数) |
| 2.3 | `src/main/core/application/serviceRegistry.ts` | 修改(注册 `AiStreamBroker`)|
| 2.4 | `src/renderer/src/transport/IpcChatTransport.ts` | 新建(sendMessages 走 `Ai_Stream_Open`) |
| 2.4 | `src/main/ai/AiStreamBroker.ts` | `startStream` / `send` / `steer` / `onChunk` / `onDone` / `onError` / `abort` 完整实现 |
| 2.4b | `src/main/ai/stream-broker/adapters/ClaudeCodeStreamAdapter.ts` | 新建(~30 行,订阅 `ClaudeCodeStream` 事件转发到 Broker) |
...
| 2.6 | `handleStreamRequest` 构造 `PersistenceSink` 的地方 | 修改(agent session 时注入 `afterPersist` hook,搬迁 `renameAgentSessionIfNeeded` 逻辑作为闭包 —— 不新建独立文件) |
...
| 2.7 | `src/main/ai/stream-broker/sinks/ChannelAdapterSink.ts` | 新建 |
| 2.7 | `src/main/services/agents/services/channels/ChannelMessageHandler.ts` | 修改(走 `aiStreamBroker.startStream`) |
...
- **双写期(Step 2.6)**:在 Step 2.6 切换的过渡一两天里,如果旧 `messageThunk.ts` 里的某些函数还没来得及全部删除,Renderer 侧旧持久化逻辑和 Main 侧 `PersistenceSink` 可能同时写。**风险**:如果去重逻辑有 bug,可能出现重复消息。**缓解**:过渡期不超过一周;E2E 测试覆盖"切 topic + 关窗口 + 重开"的组合;准备 SQL 清理脚本
- ~~UIMessageAccumulator 的 chunk state machine 风险~~ —— **这条风险不存在**。Broker 不自己 rebuild,直接让上游 agentLoop / ClaudeCodeStreamAdapter 调 AI SDK 的 `readUIMessageStream`(或等价工具)产出 UIMessage。"chunks → UIMessage" 状态机是 AI SDK 官方维护的,我们只是消费者
...
- **ClaudeCodeService 集成**:Phase 6 完成前,ClaudeCodeService 通过一层薄 adapter(~30 行)接入 Broker —— 订阅它的 `'data'` 事件,转发到 `broker.onChunk/onDone/onError`。Phase 6 完成后这个 adapter 自动变成死代码,顺手删除

### 为什么值得做(给决策者)

- **避免走弯路**:如果 Phase 2 先写"朴素 Transport" + Phase 3 再追 `ChatSessionManager` + 事后再 Phase 7 重构,这条路线会产生 ~400 行中间死代码 + 一轮额外的 revert 风险。直接在 Phase 2 上 Broker,省 2-3 周往返
- **直接解锁一个产品能力**:Discord bot 回复期间用户打开 debug 面板,能看到 bot 正在说什么。当前 v1/v2 都做不到(channel 流和 user 流是两条独立管线)
- **持久化的可靠性提升**:当前关闭窗口时还在跑的流会丢消息,Broker 之后流在 Main 端跑完并落库,和 Renderer 生死无关
...
- **持久化**:下沉到 Main 端 `PersistenceSink`(已在 Phase 2 Step 2.3 建立),Renderer **不** 使用 `onFinish` 写库
- **Reconnect**:Transport 的 `reconnectToStream` 在 Phase 2 Step 2.5 已经实现,切回 topic 能自动接上正在跑的流

原方案(`useChat` + Renderer 侧 `onFinish` 持久化 + `initialMessages` 灌历史)有三个根本性问题 —— 组件 unmount 即丢消息、被迫造 440 行的 `ChatSessionManager`、历史和活跃流混装于 `chat.messages` —— 详见 [Phase 2 架构详述: AiStreamBroker](#phase-2-架构详述-aistreambroker) 章节。
...
8. **不要** 在 `useChat` 里写 `onFinish` 做持久化 —— 持久化由 Main 端 `PersistenceSink` 完成

> 本步骤依赖 Phase 2 Step 2.3-2.5 已经就绪的 Transport 和 `PersistenceSink`。
...
原计划在 `useAiChat` 里做 chatId 前缀判断(`agent-session:*`)切换 Agent 模式。由于 `useAiChat` 本身在 Phase 2 Broker 架构下不再需要,Agent 模式改为在 `useChat` 调用处直接处理:
...
| 3 | 3.5 | `src/renderer/src/pages/home/Chat.tsx` | 修改(依赖 Phase 2 Broker + Transport 已就绪) |
...
| 7 | 7.1 | `src/main/ai/stream-broker/` | 新建(Broker + 三个内置 Sink + BrokerStreamTarget) |
...
| 7 | 7.5 | `src/main/services/agents/services/channels/ChannelMessageHandler.ts` | 修改(走 Broker) |
...
```
  ...
  ⑯ StreamSink 接口 + BrokerStreamTarget(带 setFinalMessage)
  ⑯ AiStreamBroker (activeStreams / buffer / 多播 / grace period / steering)
  ⑯ PersistenceSink 直接调 Main 端 messageService(不新建 MessagePersistenceService)
  ...
  ⑯ ChannelAdapterSink + ChannelMessageHandler 迁移到 Broker
...
```
  ...
  ℹ️ 直接使用官方 useChat({ id: topicId, transport }) —— 依赖 Phase 2 Broker + Transport 已就绪
```
...
> **注**:更早版本的文档把 AiStreamBroker 作为"Phase 7 补丁"放在最后 —— 那是基于"Phase 3 落地后才发现问题"的 retrospective 框架。现在已经前置到 Phase 2,正文章节 [Phase 2 架构详述: AiStreamBroker](#phase-2-架构详述-aistreambroker) 保留了设计完整内容。Phase 6(ClaudeCodeService → ToolLoopAgent)和 Phase 2 Broker 仍然相互独立,可并行推进。
```
