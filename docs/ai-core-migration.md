...
| 操作 | 数量 | 说明 |
|------|------|------|
| **新建/重写** | ~10 个 | AiService (已存在,更新)、AiCompletionService (已存在,重写)、IpcChatTransport、shared schemas、`AiStreamBroker` + `MessagePersistenceService` + 三个 Sink 实现 + `BrokerStreamTarget` + `ClaudeCodeStreamAdapter`(全部在 Phase 2 完成;不自造 UIMessageAccumulator,复用 AI SDK 工具) |
...
- Main 端建立 `AiStreamBroker` 作为"活跃流注册表",按 `topicId` 索引、支持多订阅者、reconnect、grace period
- 持久化下沉到 Main 端 `MessagePersistenceService`,由 `PersistenceSink` 挂载
...
1. 添加以下 channel 常量:
   ```typescript
   // === Broker 拥有的 4 个请求通道 ===
   Ai_Stream_Open   = 'ai:stream:open'     // Renderer → Main: 发送消息(新流或 steer 已有流)
   Ai_Stream_Attach = 'ai:stream:attach'   // Renderer → Main: reconnect 到 topicId(只观察,不发消息)
   Ai_Stream_Detach = 'ai:stream:detach'   // Renderer → Main: 主动退订(不 abort,流继续跑)
   Ai_Stream_Abort  = 'ai:stream:abort'    // Renderer → Main: 按 topicId abort 整条流

   // === Main → Renderer 推送通道(按 topicId 路由) ===
...
**关键约束**:
- **只有一个 id** —— `topicId`,同时是 Broker 活跃流的路由 key、IPC chunk 的过滤 key、AI SDK `useChat({ id })` 的状态复用 key。不引入 `requestId`
- 订阅者身份由 `event.sender`(WebContents)提供,Renderer 不生成 UUID
...
**注意**:不暴露 `steerMessage` / `abort(requestId)` 等按 requestId 操作的接口 —— 用户追加消息直接调 `streamOpen`,Broker 内部路由到 `steer` 或 `startStream`。abort 按 topicId 而不是 requestId。
...
Phase 2 的核心设计就是把 Main 端从"一次性管线"升级成"按 `topicId` 索引的活跃流注册表"(即 `AiStreamBroker`),让 Transport 的 `reconnectToStream` 从一开始就是真的。一旦可用,上面三个 Renderer 侧的问题全部自然消失。
...
**单 id 原则**:整个 Phase 2 架构全程只使用一个 id —— `topicId`。它既是 Broker 活跃流的路由 key,也是 IPC chunk 的过滤 key,也是 AI SDK `useChat({ id })` 的状态复用 key。不再引入 `requestId`(现有 AiService 那套一次性 UUID 的机制在 Phase 2 Step 2.8 一并清理)。订阅者身份由 `event.sender`(WebContents)提供,不需要 Renderer 生成额外的 UUID。
...
export interface StreamSink {
  ...
  /**
   * 流正常结束。`finalMessage` 由上游(agentLoop.afterIteration hook 或
   * ClaudeCodeStreamAdapter 的 complete 回调)通过
   * `BrokerStreamTarget.setFinalMessage` 在此之前塞进 `stream.finalMessage`,
   * 由 Broker 原样转发给每个 sink。可能为 `undefined`(上游未提供或异常路径);
   * sink 自己决定 undefined 时如何降级。
   */
  onDone(finalMessage?: CherryUIMessage): void | Promise<void>
  onError(error: SerializedError): void | Promise<void>

  /** Broker 定期调用以清理失效的 sink。 */
  isAlive(): boolean
}

export interface ActiveStream {
  /** 唯一稳定 key —— Cherry Studio 对话的 topicId */
  topicId: string

  /** Broker 拥有的 AbortController,独立于 Renderer 生命周期。
   *  Broker 直接持有,通过 abortController.abort() 掐流;
   *  不再需要 per-call requestId 注册表。 */
  ...
  /**
   * Steering 队列(搬自 Phase 1 的 `PendingMessageQueue`,改为按 topicId 归属)。
   *
   * 当用户在流进行中继续发消息时,这些消息入本队列,不创建新流。
   * `runAgentLoop` 的 `prepareStep`(内层步间)和外层循环边界都会 drain
   * 这个队列,把新消息拼进下一轮 context。流 reap 时随 ActiveStream 一起 GC,
   * 无需独立的 `Map<requestId, PendingMessageQueue>` 注册表。
   */
...
export class WebContentsSink implements StreamSink {
  /** 订阅者身份 = (topicId, wc.id) —— 同一窗口对同一 topic 只订阅一次 */
  readonly id: string
  ...
  onDone(_finalMessage?: CherryUIMessage): void {
...
> **为什么 id 是 `wc:${wc.id}:${topicId}` 而不仅是 `wc:${wc.id}`**:同一个 WebContents 可能同时看多个 topic(比如多标签)。sink 身份要区分"哪个窗口 + 哪个 topic";但**同一窗口对同一 topic 只有一个订阅者**(React 渲染一个 Chat 组件),不需要更细的粒度。这和"去掉 requestId"一致 —— 没有"同窗口多订阅"的现实场景。
...
export class PersistenceSink implements StreamSink {
  ...
  onChunk(_chunk: UIMessageChunk): void {
    ...
    // 若将来做 streaming persistence,在此增量写 ChunkStore
  }

  async onDone(finalMessage?: CherryUIMessage): Promise<void> {
    // finalMessage 由上游(agentLoop 的 afterIteration hook 或
    // ClaudeCodeStreamAdapter 的 'complete' 回调)通过 BrokerStreamTarget.setFinalMessage
    // 塞进 stream 再转发过来。正常情况下一定有值。
    if (!finalMessage) {
      // undefined 说明上游没提供 —— 可能是旧路径、异常中断、或实现 bug。
      // 策略:不落库,打 warn 让运维能看到,避免悄悄丢数据
    ...
    await this.persistence.saveExchange({
      topicId: this.ctx.topicId,
...
export class ChannelAdapterSink implements StreamSink {
  ...
  async onDone(finalMessage?: CherryUIMessage): Promise<void> {
    ...
    const text = finalMessage ? extractPlainText(finalMessage) : this.fallbackText
    if (text) await this.adapter.sendMessage(this.platformChatId, text)
  }
...
@ServicePhase(Phase.WhenReady)
@DependsOn(['AiService', 'MessagePersistenceService'])
export class AiStreamBroker extends BaseService {
  ...
  protected async onInit(): Promise<void> {
    ...
    this.ipcHandle(IpcChannel.Ai_Stream_Attach, (event, req: AiStreamAttachRequest) =>
      this.handleAttach(event.sender, req.topicId)
    )
    this.ipcHandle(IpcChannel.Ai_Stream_Detach, (event, req: AiStreamDetachRequest) =>
      this.handleDetach(event.sender, req.topicId)
    )
   ...
   *
   * 语义:
   *   - 如果 topicId 没有活跃流 → 创建新 ActiveStream
   *   - 如果 topicId 有 done/error/aborted 状态的流(grace period 内) → 提前驱逐旧流,创建新的
   *   - 如果 topicId 有 streaming 状态的流 → 抛 ConflictError
   *
   * **注意**:这个方法是"开新流"语义。如果调用方的意图是"同一 topic 继续发消息",
   * 应该走 `send()` 路由(它会在有活跃流时自动改走 `steer()`,而不是抛错)。
   */
  startStream(input: {
    topicId: string
    ...
    if (existing) {
      if (existing.status === 'streaming') {
        throw new ConflictError(`Stream for topic ${input.topicId} is already active`)
      }
      // grace period 内的 done/error/aborted —— 提前驱逐,放行新流
      // 注意:evict 之前抄下 sourceSessionId,这样 Claude Code 路径
      //       可以跨 evict 继续 resume 上一轮的 SDK session
      inheritedSessionId = existing.sourceSessionId
      this.evictStream(input.topicId, existing)
    }
    ...
    const target = new BrokerStreamTarget(this, input.topicId)
   ...
   *
   * 调用者同时想订阅这条流的 chunks 的话,还要单独调 `addSink`(或走 `send()`
   * 路由一次性完成)。
   */
   ...
   *
   * 路由逻辑:
   *   - 已有 streaming 流 → `steer()` + `addSink()`(附带把本次发起者订阅上去)
   *   - 没有流 / 流已结束 → `startStream()`
   *
   * Renderer 侧的 `Ai_Stream_Send` IPC handler 就是一层薄包装,把 `event.sender`
   * 转成 `WebContentsSink`,把 DTO 转成 request 后调这个方法。
   */
  send(input: {
    topicId: string
    request: AiStreamRequest
    userMessage: CherryUIMessage   // 本次用户发的消息(供 steering 或落库)
    ...
    if (existing?.status === 'streaming') {
      ...
      for (const sink of input.sinks) this.addSink(input.topicId, sink)
      return 'steered'
    }
    // 新流路径
  ...
  /** 按 topicId 加入一个新的 sink。返回 true 表示已加入;false 表示流不存在。 */
  addSink(topicId: string, sink: StreamSink): boolean {
    const stream = this.activeStreams.get(topicId)
  ...
  removeSink(topicId: string, sinkId: string): void {
    const stream = this.activeStreams.get(topicId)
  ...
  onChunk(topicId: string, chunk: UIMessageChunk): void {
    ...
    for (const [id, sink] of stream.sinks) {
      ...
      try {
        ...
        logger.warn('Sink onChunk threw', { topicId, sinkId: id, err })
      }
    }
    for (const id of dead) stream.sinks.delete(id)
  }

  async onDone(topicId: string): Promise<void> {
    const stream = this.activeStreams.get(topicId)
    if (!stream) return

    stream.status = 'done'
    // 注意:stream.finalMessage 应该已经被上游通过 BrokerStreamTarget.setFinalMessage
    // 在流结束前塞好了(agentLoop.afterIteration hook 或 ClaudeCodeStreamAdapter 的
    // 'complete' 回调)。Broker 自己不做 rebuild。
    // 如果 finalMessage 仍是 undefined,说明上游没有提供 —— sink 自己按 undefined 处理。

    for (const [id, sink] of stream.sinks) {
      try {
        await sink.onDone(stream.finalMessage)
      } catch (err) {
        logger.warn('Sink onDone threw', { topicId, sinkId: id, err })
      }
    }
  ...
  async onError(topicId: string, error: SerializedError): Promise<void> {
    ...
    for (const [id, sink] of stream.sinks) {
      try {
        ...
        logger.warn('Sink onError threw', { topicId, sinkId: id, err })
      }
    }
  ...
  /**
   * 提前驱逐一个 grace period 内的 done/error/aborted 流,让出 topicId slot。
   *
   * 代价:迟到的 reconnect 会拿到 'not-found' 而不是 'done + finalMessage',
  ...
  // === IPC handler 具体实现略 ===
  //     handleStreamRequest: 构造 WebContentsSink(sender, topicId) + PersistenceSink,调 send()
  //                          (send 内部路由到 startStream 或 steer,Renderer 无感)
  //     handleAttach:        构造新的 WebContentsSink(sender, topicId),调 addSink(自带 buffer 回放)
  //     handleDetach:        用 sink.id = `wc:${sender.id}:${topicId}` 精确移除
  //     cleanupByWebContents:遍历 activeStreams,删掉 id 前缀为 `wc:${wc.id}:` 的所有 sink
}
```
...
/**
 * AiService.executeStream 实际使用的 WebContents 子集。
 *
 * `send` / `isDestroyed` 是原本 WebContents 就有的两个方法。
 * `setFinalMessage` 是 Broker 专用的可选扩展 —— 上游(agentLoop 或
 * ClaudeCodeStreamAdapter)在流结束前把 AI SDK 工具函数产出的完整
 * UIMessage 通过这个 setter 传下来,避免 Broker 自己 rebuild。
 */
...
export class BrokerStreamTarget implements StreamTarget {
  ...
  send(channel: string, payload: { chunk?: UIMessageChunk; error?: SerializedError }): void {
    switch (channel) {
      case IpcChannel.Ai_StreamChunk:
        if (payload.chunk) this.broker.onChunk(this.topicId, payload.chunk)
        break
      case IpcChannel.Ai_StreamDone:
        void this.broker.onDone(this.topicId)
        break
      case IpcChannel.Ai_StreamError:
        if (payload.error) void this.broker.onError(this.topicId, payload.error)
        break
    }
  }

  isDestroyed(): boolean {
    return this.broker.shouldStopStream(this.topicId)
  }
  ...
  setFinalMessage(message: CherryUIMessage): void {
    ...
    const stream = this.broker.activeStreams.get(this.topicId)
...
> 1. **收 chunks**(通过 `send` 的 `.chunk` / `.error` 字段)—— 实时多播给 sinks,不 care payload 里还有什么其他字段(比如旧版遗留的 `requestId`)
...
4. **删除 per-requestId 注册表(两个)**:
   - `AiCompletionService.registerRequest / removeRequest / abort(requestId)` —— Broker 直接持有每条流的 `AbortController`,用 `abortController.abort()` 掐流,不需要通过 requestId 反查
   - `AiCompletionService.pendingMessageQueues: Map<requestId, PendingMessageQueue>` 和 `getPendingMessageQueue / steer(requestId, message)` —— 队列的归属从"按 requestId 索引的全局 Map"改为"`ActiveStream.pendingMessages` 字段",随流 GC
   - 对应的 `ipcOn(Ai_Abort)` 和 `ipcOn(Ai_SteerMessage)` 挂接都删掉,Renderer 的 abort/steer 走 `Ai_Stream_Abort` / `Ai_Stream_Open`(新消息路由到 steer)
...
enum IpcChannel {
  // === 保留用于流 chunks 的推送(payload 形状变了:requestId → topicId) ===
  ...
  Ai_Stream_Open   = 'ai:stream:open',   // Renderer → Main:发送消息(新流或 steer 已有流)
  Ai_Stream_Attach = 'ai:stream:attach', // Renderer → Main:reconnect 到 topicId(只观察,不发消息)
  Ai_Stream_Detach = 'ai:stream:detach', // Renderer → Main:主动退订(不 abort)
  Ai_Stream_Abort  = 'ai:stream:abort',  // Renderer → Main:按 topicId abort(整条流停止)
...
Payload 类型(注意:全部只有一个 id —— `topicId`):
...
interface AiStreamOpenRequest {
  topicId: string
  assistantId: string
  userMessage: CherryUIMessage   // 触发本轮的用户消息(供持久化)
  // ...其余原 AiStreamRequest 字段(messages, model, tools, ...)
}

interface AiStreamAttachRequest {
  topicId: string
}
...
interface AiStreamDetachRequest {
  ...
  // `wc:${event.sender.id}:${topicId}` 精确定位这个窗口对该 topic 的 sink
}
...
Renderer 侧 chunk listener 按 `topicId` 过滤(而不是按 `requestId`):
...
export class IpcChatTransport implements ChatTransport<UIMessage> {
  async sendMessages(options) {
    ...
    // abort 改为 detach —— 仅退订,流在 Main 端继续跑完并落库
  ...
  async reconnectToStream({ chatId: topicId }): Promise<ReadableStream<UIMessageChunk> | null> {
    ...
    return this.buildListenerStream(topicId)  // 带 buffer 回放
  }
...
**相比 Phase 2 旧版的 delta**:
- 删除 `const requestId = crypto.randomUUID()`
- 删除 `requestId` 在闭包内的传递和在 chunk listener 里的过滤
- chunk 过滤键从 `data.requestId === requestId` 变为 `data.topicId === topicId`
- `cancel`/`abort` 改为 `streamDetach`,不再调用 `window.api.ai.abort(...)`
- `reconnectToStream` 从返回 `null` 变为真实现
...
async handleIncoming(adapter: ChannelAdapter, msg: IncomingMessage) {
  ...
  aiStreamBroker.startStream({
    ...
    sinks: [
      ...
      new PersistenceSink(persistenceService, {
        topicId,
        assistantId,
        userMessage: convertIncomingToCherryMessage(msg)
      })
    ]
  })
}
...
**唯一 id**:Broker 全程只认 `topicId`(= Cherry Studio SQLite `topics` 表的主键),它同时是 Broker 的路由 key、IPC 推送的过滤 key、AI SDK `useChat({ id })` 的状态复用 key。

三种发起源都映射到同一个 `topicId`:

- **Renderer 发起**:UI 传入当前打开的 `topicId`
- **Channel 发起**:`ChannelMessageHandler` 根据 `(channelId, platformChatId)` → `topicId`(映射逻辑沿用现有实现)
- **Agent 发起**:调度器用自己的 agent task 元数据解析出对应的 `topicId`

**并发约束**:同一 topicId 在 Broker 里同时只能有一条活跃流。但"一条流"不等于"一次发送" —— 用户在流跑的时候继续发消息是合法的,消息会入 `stream.pendingMessages` 队列,由 `runAgentLoop.prepareStep` 在内层步间 drain 拼进 context(这就是 Phase 1 设计原则里的 "Pending Messages Steering 双层保障"。Phase 2 Broker 把队列的归属从 `Map<requestId, PendingMessageQueue>` 搬到 `ActiveStream.pendingMessages`,按 topicId 随流 GC,更干净)。

所以:
- **同一 topic 同时一条流** —— 这是对 Broker 的约束,对用户透明
- **用户感知的"多次发送"** —— 都合法,通过 steering 队列聚合,不报错
...
`startStream` 遇到 grace period 内(done/error/aborted)的旧流时会**提前驱逐**,让新流立即开始 —— 这修复了"停止后立即重试"/"出错后立即重发"/"完成后秒速连发"三种用户动作的顺滑度。迟到 reconnect 的代价:拿不到 finalMessage,退化到 DataApi 从 SQLite 读,由 `PersistenceSink` 已落库的数据兜底。
...
#### `renameAgentSessionIfNeeded` 改为事件订阅者

`messageThunk.ts:220-279` 的"流完成后用摘要给 session 自动重命名"逻辑,**Phase 2 之后不塞进 `PersistenceSink.onDone`**。正确做法:

- `PersistenceSink.onDone` 完成后,Broker(或 `MessagePersistenceService`)通过事件总线发一个 `ExchangeSaved` 事件
- 一个独立的 `SessionAutoRenameService`(或类似命名)订阅这个事件,跑重命名逻辑
- 其他"流完成后的副作用"(标题生成、usage 上报、error 分类重试、知识库引用回填……)都走同一个订阅模型

这是前面讨论过的 **"handleFinish 不应该成为业务逻辑磁铁"** 原则的直接落地 —— 业务副作用永远是事件的**订阅者**,不是核心流程的内联分支。
...
- [ ] 新建 `MessagePersistenceService`,先搬 v1 `ChatSession.handleFinish` / `messageThunk` 的持久化逻辑进 `saveExchange` 方法(Topic lazy-create、reasoning 规范化、parentId 链接)
...
- [ ] `AiStreamBroker.onDone` 通过 `PersistenceSink` 完成落库
- [ ] `MessagePersistenceService.saveExchange` 完成后发送 `ExchangeSaved` 事件(通过 `Emitter` 或 lifecycle 服务事件总线)
- [ ] 新建 `SessionAutoRenameService`,订阅 `ExchangeSaved` 事件,跑重命名逻辑(搬自 `messageThunk.ts:220-279` 的 `renameAgentSessionIfNeeded`)
- [ ] 删除 v1 `messageThunk.ts` 里 `createAgentMessageStream` / `createSSEReadableStream` / `withAbortStreamPart` / `fetchAndProcessAgentResponseImpl` / `setupChannelStream` / `addChannelUserMessage` / `renameAgentSessionIfNeeded` 等函数(~500 行),它们都被 Broker + WebContentsSink + PersistenceSink + 事件订阅者统一取代
- [ ] Renderer 侧改用官方 `useChat({ id: topicId, transport, messages: historyFromUseQuery })`
- [ ] 历史消息改走 `useQuery('/topics/:id/messages')`(DataApi)

> **"handleFinish 不做业务逻辑磁铁" 原则**:`PersistenceSink.onDone` 只管"把消息写 SQLite",不做标题生成 / usage 上报 / error 分类 / 知识库回填等副作用。这些都走事件订阅 —— 每一种都是一个独立服务订阅 `ExchangeSaved` 事件。将来增加新副作用时,新写一个订阅者,**不改** Broker 或 PersistenceSink。
...
| 2.3 | `src/main/ai/persistence/MessagePersistenceService.ts` | 新建(搬迁 v1 持久化逻辑) |
| 2.3 | `src/main/ai/AiService.ts` | 修改(`executeStream` target 类型放宽 + 新增 options 参数) |
| 2.3 | `src/main/core/application/serviceRegistry.ts` | 修改(注册 `AiStreamBroker` / `MessagePersistenceService`) |
...
| 2.6 | `src/main/ai/persistence/MessagePersistenceService.ts` | 修改(`saveExchange` 完成后发 `ExchangeSaved` 事件) |
| 2.6 | `src/main/ai/SessionAutoRenameService.ts`(或类似) | 新建(订阅 `ExchangeSaved`,搬迁 `renameAgentSessionIfNeeded`) |
...
| 7 | 7.1 | `src/main/ai/stream-broker/` | 新建(Broker + Sinks + Accumulator) |
| 7 | 7.1 | `src/main/ai/persistence/MessagePersistenceService.ts` | 新建 |
| 7 | 7.1 | `src/main/ai/AiService.ts` | 放宽 target 类型 |
...
```
  ...
  ⑯ MessagePersistenceService (v1 持久化逻辑统一下沉到 Main)
