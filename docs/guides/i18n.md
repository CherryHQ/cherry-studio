...
This script fills in texts marked as `[to be translated]` using the Claude Agent SDK.
...
Two switches change how a run is answered, and they are independent so their effects can be measured separately:

- `--translator endpoint|claude` — who translates. `endpoint` (default) uses `TRANSLATION_*`; `claude` uses the Claude Agent SDK and needs `ANTHROPIC_API_KEY`.
- `--research` — prepend a tool-enabled pass that greps the codebase for what renders each key and translates from that brief. Needs `ANTHROPIC_API_KEY`.

On a 141-string benchmark (the keys one round of feature PRs added, across ja-jp, de-de and ru-ru), blind pairwise judging of the strings `--research` changed, holding the translator fixed:

| translator | research verdict | run cost |
| --- | --- | --- |
| endpoint (deepseek-v4-flash) | net **−6** strings (21 better, 27 worse, 29 tie) | ~$0.02 → $1.63 with research |
| claude (sonnet) | net **+13** strings (26 better, 13 worse, 27 tie) | $0.50 → $1.68 with research |

So research is worth roughly a 9% improvement on a strong translator and is actively harmful on a small one, for about $1.20 a run. CI therefore runs neither: plain `endpoint`, which style examples and the key path already make competitive.
