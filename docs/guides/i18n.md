...
There is also an opt-in `--engine research` mode that first spends one tool-enabled Claude Agent SDK pass reading the codebase to learn which UI element renders each key, then translates from that brief. It needs `ANTHROPIC_API_KEY`. On a 141-string benchmark it produced no measurable quality gain over the default once style examples were in place, at roughly 50× the cost, so it is not what CI runs.
