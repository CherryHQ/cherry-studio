...
CI runs neither switch: plain `endpoint`, no research. `--research` adds about $1.20 and roughly two minutes to a run, and on the one benchmark it has been measured against it did not show a quality gain worth that. See [PR #18510](https://github.com/CherryHQ/cherry-studio/pull/18510) for the measurement and its caveats — it is a single 141-string sample scored by an LLM judge with no human calibration, so treat it as a reason to leave the switch off by default rather than as a settled result.
