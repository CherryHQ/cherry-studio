...
| `pnpm i18n:check` | Check nested structure, key alignment, sorting, duplicates, and main-process literal `t()` key coverage | Sorts the base locale when needed |
| `pnpm i18n:validate` | Validate translated values, placeholders, interpolation, component tags, nested references, and protected terms | No |
...
4. Use Claude Code to replace every `[to be translated]` value in both catalogs.
5. Review the locale diff, then run `pnpm i18n:check` and `pnpm i18n:validate`.

The following prompt can be given directly to Claude Code:
...
- Run pnpm i18n:sync before translating, then run pnpm i18n:check and pnpm i18n:validate.
```

Pull request CI runs `i18n:sync` and fails when it produces an uncommitted diff. It then runs the deterministic validation gate:

```bash
pnpm i18n:validate
```

The validator rejects remaining placeholders, empty translations, missing interpolation variables, changed `<Trans>` component tags, changed `$t()` references, bracketed model notes, implausibly long explanations, and dropped protected terms. CI also runs the strict hardcoded-string check.

These deterministic checks cannot prove that Claude Code produced a translation or determine whether a fluent translation preserves the intended meaning. Review semantic quality in the originating pull request.
...
2. **Run Validation Before Commit**: Use `pnpm i18n:check` and `pnpm i18n:validate` to catch i18n issues early.
