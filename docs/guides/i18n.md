...
### `i18n:unused` - Find and Clean Unused Keys
...
By default, it only prints a report and does not modify any files:
...
Cleaning is opt-in. The command never deletes keys unless `--clean` is passed.

Run interactive cleanup:

```bash
pnpm i18n:unused --clean
```
...
```bash
pnpm i18n:unused --clean --groups common,settings
```

Run non-interactive cleanup for all unused keys:

```bash
pnpm i18n:unused --clean --all
```
...
- Known label maps in `src/renderer/src/i18n/label.ts`
...
3. Clean only a small namespace at a time with `--clean --groups <namespace>`, or use `--clean --all` when the full report has already been reviewed.
