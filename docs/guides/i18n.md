...
1. **Research** — an agent greps the codebase for each pending key to learn which UI element renders it, what the user is doing at that moment, and how long the string may be. The resulting brief is language-independent and shared by every locale.
2. **Translate** — one tool-less request per locale, given the brief, the English source, the `zh-cn` reference and the glossary.
3. **Validate** — deterministic checks. A translation that loses an interpolation variable, drops a `<0>` tag, carries a bracketed note or echoes the placeholder marker is discarded and its `[to be translated]` placeholder kept, so the next run retries it. The script exits non-zero when anything was discarded.

Set `ANTHROPIC_API_KEY` in your environment or `.env`. `I18N_RESEARCH_MODEL` and `I18N_TRANSLATE_MODEL` both default to `claude-sonnet-5`.

```bash
pnpm i18n:translate                            # every locale
pnpm i18n:translate --locale ja-jp --dry-run   # one locale, print briefs and results without writing
```
