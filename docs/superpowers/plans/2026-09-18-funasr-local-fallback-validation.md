...
- `packages/system-speech/validation/routePolicy.ts`: retain Apple 26+ / FunASR pre-26 routing tests.
...
- [x] **Step 1: Add a validation runner that reports metadata only**
...
- [x] **Step 2: Use the existing Settings card to download explicitly**
...
- [x] **Step 3: Run normal real-model validation**

Use the Apple TTS WAV already generated by the system-speech validation as input. Run `pnpm validate:funasr -- --input <wav> --user-data <Cherry userData>`. Expected: `transcriptNonEmpty: true` and at least one segment.

- [x] **Step 4: Run network-denied validation**
...
- The later stacked PR2 review uses `git diff 3ac951ed5f...HEAD` to exclude PR1's Apple system-speech changes.
