...
Use `PageSidePanel` for page-owned management surfaces such as mini-app display settings, translate settings, and translate history. It prefers the page-level dialog portal container, then falls back to the nearest `PortalContainerProvider` or `document.body`. In app shell route tabs, this keeps the panel attached to its owning tab so a still-open surface stays hidden instead of surfacing over the active tab. Keep the backdrop and panel in that same portal: the backdrop uses viewport positioning, while the panel uses the narrow platform policy provided by `TabRouter`.

- Backdrop: fixed `inset-0`, `z-60`, `bg-black/50`, fades over `0.15s`; it covers the full window, including the app sidebar and top tab bar
- Panel: `top-3 bottom-3`, `right-3` or `left-3`, `z-70`; Windows/Linux use absolute positioning within the tab content root (leaving the top tab bar clear), while macOS uses fixed viewport positioning (no reserved tab-bar height)
...
- "Build a page-owned settings side panel with `PageSidePanel`: prefer the page-level dialog portal container, then the nearest overlay container or body; keep the backdrop fixed across the full window, and use the app shell's narrow panel positioning policy (Windows/Linux container-positioned, macOS viewport-positioned); `top-3 bottom-3 right-3`, `w-100`, `bg-card`, `rounded-3xl`, `shadow-xl`, `title` for the shared `text-base` heading, body `px-6 py-4`, `PageSidePanelSection` groups separated by `gap-8`, and `PageSidePanelItem` rows separated by `gap-5` inside each group. Use only `PageSidePanel` for non-settings history/list/detail drawers, with a task-specific body layout."
