...
- **Section-title-as-page-title exception**: when the page-level label is itself a *group name* that should match the in-list group labels (e.g. the root `/settings` route, whose page label "Models" sits next to in-list labels "Plugins / App Settings / Productivity / System"), render a header slot that copies `PageHeader`'s outer geometry (`mt-3.5 mb-2 flex h-8 shrink-0 items-center pl-5`) but uses section-title typography (`text-xs text-foreground-muted`) instead of the PageHeader heading style. The slot preserves vertical alignment with the right column's `PageHeader` baseline while keeping the visual hierarchy "group label → items" consistent across the whole sidebar.
