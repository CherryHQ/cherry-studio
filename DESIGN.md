...
- **Yes** → keep the default ghost color (`text-foreground`). The icon *is* the action.
...
| Case | Color | Example |
|---|---|---|
| Page-primary action in chrome | `text-foreground` (ghost default) | Mini-apps page top-right `+` and menu — the page exists to launch apps; these icons *are* the action. |
...
| Toggle while active | keep `text-foreground` while active; otherwise muted | Panel-toggle icon while its panel is open. |
| Destructive row action | `text-foreground-muted hover:text-destructive` | Delete X next to a custom language row. |

**Rule of thumb:** if an area shows 3+ icon buttons, at most one should sit at `text-foreground`. The rest are utilities — mute them. Otherwise the eye has no anchor.

**Do not:**
- Apply `text-foreground` to every icon button by reflex — the default is for one action per cluster, not all of them.
- Use `text-primary` as a "more emphasis" replacement for `text-foreground`; `text-primary` is reserved for selected / branded states, not for raising icon weight.
...
Source: `PageSidePanel` from `@cherrystudio/ui` (`packages/ui/src/components/composites/PageSidePanel/index.tsx`).
...
- Section: `flex flex-col gap-3`, title `font-semibold text-sm`, optional right-aligned low-emphasis actions.
- Item: title/description stack with a trailing `action`, `gap-5` between preference rows inside a section.
- Related settings groups should be separated by `gap-8`.
