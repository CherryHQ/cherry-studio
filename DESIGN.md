...
- **Section-title-as-page-title exception**: when the page-level label is itself a *group name* that should match the in-list group labels (e.g. the root `/settings` route, whose page label "Models" sits next to in-list labels "Plugins / App Settings / Productivity / System"), omit the `PageHeader` entirely and render the page-level label as the **first MenuList section title**. To keep that first label's baseline aligned with the right column's `PageHeader` heading (~35px from the column top), apply `pt-6` to the `MenuList` className (e.g. `cn(settingsSubmenuListClassName, 'pt-6')`). This gives the in-list group labels and the page-level label identical typography (`settingsSubmenuSectionTitleClassName`) and identical bottom spacing (`pb-1 + gap-1` = 8px), while the `pt-6` keeps left and right columns visually aligned at the top.
