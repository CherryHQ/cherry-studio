...
- **Section-title-as-page-title exception**: when the page-level label is itself a *group name* that should match the in-list group labels (e.g. the root `/settings` route, whose page label "Models" sits next to in-list labels "Plugins / App Settings / Productivity / System"), render a header slot that copies `PageHeader`'s outer geometry (`mt-3.5 mb-2 flex h-8 shrink-0 pl-5`) but uses section-title typography (`text-xs text-foreground-muted`) instead of the PageHeader heading style. Use `items-end` + `leading-none` on the label so the bottom gap matches the in-list group labels (both end up with `mb-2` = 8px of breathing room before the first menu item). The slot preserves the outer 54px footprint so both columns stay vertically aligned, while keeping the visual hierarchy "group label → items" consistent across the whole sidebar.
