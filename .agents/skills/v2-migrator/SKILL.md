...
**Dexie Settings source:** The Dexie `settings` table is a generic KV store (`{ id: string, value: any }`) that also contains user preferences. These are classified under `dexieSettings` in `classification.json` (separate from the `dexie` section which covers business data tables). The `dexieSettings` entries are **manually maintained** — they are not auto-extracted by the inventory script. For the full key inventory, see [PR #10162 comment](https://github.com/CherryHQ/cherry-studio/pull/10162#issuecomment-4010796619). Note that `dexieSettings` entries currently only support `category: "preferences"`, and dynamic keys (containing `${}` patterns) require complex mapping logic — they cannot use simple 1:1 `DEXIE_SETTINGS_MAPPINGS`.
