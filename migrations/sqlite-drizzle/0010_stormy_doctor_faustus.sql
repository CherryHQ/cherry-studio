...
CREATE TABLE `__new_knowledge_item` (
	...
	CONSTRAINT "knowledge_item_status_check" CHECK("__new_knowledge_item"."status" IN ('idle', 'preparing', 'processing', 'reading', 'embedding', 'completed', 'failed', 'warning', 'deleting')),
	CONSTRAINT "knowledge_item_type_status_check" CHECK(
        ...
        OR ("__new_knowledge_item"."type" = 'directory' AND "__new_knowledge_item"."status" IN ('idle', 'preparing', 'processing', 'completed', 'failed', 'warning', 'deleting'))
      ),
	CONSTRAINT "knowledge_item_status_error_check" CHECK(
        ...
        OR (
          "__new_knowledge_item"."status" IN ('failed', 'warning')
